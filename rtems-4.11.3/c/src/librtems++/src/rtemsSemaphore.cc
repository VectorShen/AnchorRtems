...
rtemsSemaphore::rtemsSemaphore (const char *sname, const Scope scope, const uint32_t counter, const WaitMode wait_mode, const Type type, const Priority priority, const Ceiling ceiling, const rtems_task_priority priority_ceiling):name (0),
owner (true),
id (0)
{
	...
	create (sname,
			scope,
			counter, wait_mode, type, priority, ceiling, priority_ceiling);
}

rtemsSemaphore::rtemsSemaphore (const char *sname,
								const uint32_t node):name (0), owner (false),
id (0)
{
	strcpy (name_str, "NOID");
	connect (sname, node);
...
rtemsSemaphore::rtemsSemaphore (const rtemsSemaphore & semaphore):name (0),
owner (false), id (0)
{
	name = semaphore.name;
...
rtemsSemaphore::rtemsSemaphore ():name (0), owner (false), id (0)
{
	strcpy (name_str, "NOID");
}
												...
												priority_ceiling)
{
	...
	set_status_code (rtems_semaphore_create (name,
											 counter,
											 scope | wait_mode | type | priority
											 | ceiling, priority_ceiling, &id));
...
const rtemsSemaphore & rtemsSemaphore::
operator= (const rtemsSemaphore & semaphore)
{
	if (!owner)
	{
		name = semaphore.name;
