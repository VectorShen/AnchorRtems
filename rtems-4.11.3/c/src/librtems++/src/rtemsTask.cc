...
rtemsTask::rtemsTask (const char *tname, const rtems_task_priority initial_priority, const uint32_t stack_size, const rtems_mode preemption, const rtems_mode timeslice, const rtems_mode asr, const rtems_interrupt_level interrupt_level, const FloatingPoint floating_point, const Scope scope):name (rtems_build_name ('S', 'E', 'L', 'F')),
owner (true),
id (RTEMS_SELF), argument (0)
{
	strcpy (name_str, "SELF");
	create (tname,
			initial_priority,
			stack_size,
			preemption, timeslice, asr, interrupt_level, floating_point, scope);
}

rtemsTask::rtemsTask (const char *tname,
					uint32_t node):name (rtems_build_name ('S', 'E', 'L',
															 'F')),
owner (false), id (RTEMS_SELF), argument (0)
{
	strcpy (name_str, "SELF");
	connect (tname, node);
}
...
rtemsTask::
rtemsTask (const rtemsTask & task):name (rtems_build_name ('S', 'E', 'L', 'F')),
owner (false), id (RTEMS_SELF), argument (0)
{
	name = task.name;
...
rtemsTask::rtemsTask ():name (rtems_build_name ('S', 'E', 'L', 'F')),
owner (false), id (RTEMS_SELF), argument (0)
{
	strcpy (name_str, "SELF");
}
										 ...
										 const Scope scope)
{
	...
	name = rtems_build_name (name_str[0],
							 name_str[1], name_str[2], name_str[3]);
...
const rtems_status_code rtemsTask::wake_after (const rtems_interval micro_secs)
{
	...
	return
		set_status_code (rtems_task_wake_after
...
const rtems_status_code rtemsTask::
set_priority (const rtems_task_priority priority)
{
