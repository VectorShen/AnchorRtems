...
class rtemsTask:public rtemsStatusCode
{
  public:
	enum FloatingPoint
	{
		fpoff = RTEMS_NO_FLOATING_POINT,
	...
	// connects to a task
	rtemsTask (const char *name, const uint32_t node =
	...
	virtual const rtems_status_code restart (const rtems_task_argument
											 argument);
	...
	virtual const rtems_status_code wake_after (const rtems_interval
												micro_secs);
