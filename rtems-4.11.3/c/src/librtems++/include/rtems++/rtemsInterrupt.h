...
class rtemsInterrupt:public rtemsStatusCode
{
  public:
	rtemsInterrupt ();
	...
	// catch the interrupt
	virtual const rtems_status_code isr_catch (const rtems_vector_number
											 vector);
	...
	// the old handler
	const rtems_isr_entry old_isr_handler () const
	{
		return old_handler;
	}
...
void rtemsInterrupt::chain () const const
{
	if (old_interrupt)
