...
class rtemsTimer:public rtemsStatusCode
{
  public:
	...
	inline const rtems_status_code repeat_fire_at (const rtems_interval
												 micro_secs);
...
const rtems_status_code rtemsTimer::fire_after (const rtems_interval micro_secs)
{
	...
	return set_status_code (rtems_timer_fire_after (id,
													...
													(usecs), common_handler,
													this));
}

const rtems_status_code rtemsTimer::
repeat_fire_at (const rtems_interval micro_secs)
{
	repeat = true;
	rtems_interval usecs = micro_secs &&
		(micro_secs < rtems_configuration_get_microseconds_per_tick ())?
