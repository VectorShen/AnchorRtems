...
class rtemsMessageQueue:public rtemsStatusCode
{
  public:
	// attribute a message queue can have
	enum WaitMode
	{
		wait_by_fifo = RTEMS_FIFO,
	...
	rtemsMessageQueue (const char *name, const uint32_t node =
	...
	inline const rtems_status_code send (const void *buffer, const size_t size);
...
{
	return
		set_status_code (rtems_message_queue_send (id, (void *)buffer, size));
}
...
{
	return
		set_status_code (rtems_message_queue_urgent (id, (void *)buffer, size));
}
