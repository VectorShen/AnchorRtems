...
class rtemsSemaphore:public rtemsStatusCode
{
  public:
	...
	enum Type
	{
		binary = RTEMS_BINARY_SEMAPHORE,
		counting = RTEMS_COUNTING_SEMAPHORE
	};
	enum Priority
	{
		no_priority_inherit = RTEMS_NO_INHERIT_PRIORITY,
		inherit_priority = RTEMS_INHERIT_PRIORITY
	};
	enum Ceiling
	{
		no_priority_ceiling = RTEMS_NO_PRIORITY_CEILING,
	...
	virtual const rtems_status_code create (const char *name,
											...
											wait_by_fifo, const Type type =
											binary, const Priority priority =
											no_priority_inherit,
