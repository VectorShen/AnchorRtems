...
void arm_cp15_set_exception_handler (Arm_symbolic_exception_name exception,
									 void (*handler) (void))
{
	if ((unsigned)exception < MAX_EXCEPTIONS)
	  {
		  uint32_t *cpu_table = (uint32_t *) 0 + MAX_EXCEPTIONS;
		  uint32_t *mirror_table =
			  (uint32_t *) bsp_vector_table_begin + MAX_EXCEPTIONS;
		  ...
		  if (current_handler != (uint32_t) handler)
			{
				...
				rtems_cache_flush_multiple_data_lines (mirror_table,
													   table_size);
				 ...
				 */
				rtems_cache_invalidate_multiple_instruction_lines (mirror_table,
																   table_size);

				rtems_cache_invalidate_multiple_instruction_lines (cpu_table,
																   table_size);
