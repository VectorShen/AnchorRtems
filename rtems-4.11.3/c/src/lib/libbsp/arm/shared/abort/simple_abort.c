...
void _print_full_context (uint32_t spsr)
{
	...
	__asm__ volatile (ARM_SWITCH_TO_ARM
					...
					"mov %[prev_lr], lr\n"
					"msr cpsr_c, %[cpsr]\n"
					ARM_SWITCH_BACK:[prev_sp] "=&r" (prev_sp),
					[prev_lr] "=&r" (prev_lr),[cpsr] "=&r" (cpsr),
					[arm_switch_reg] "=&r" (arm_switch_reg):[spsr]
					"r" (spsr):"cc");
...
void do_data_abort (uint32_t insn, uint32_t spsr, Context_Control * ctx)
{
