...
#ifdef UARTDEBUG
static void uartError (int uart)
{
...
void BSP_uart_init (int uart, int baud, int hwFlow)
{
	...
	/* Enable FIFO */
	uwrite (uart, FCR,
			FIFO_EN | XMIT_RESET | RCV_RESET | RECEIVE_FIFO_TRIGGER12);
...
void BSP_uart_intr_ctrl (int uart, int cmd)
{
	...
	switch (cmd)
	{
		...
		case BSP_UART_INTR_CTRL_ENABLE:
			uwrite (uart, IER,
					(RECEIVE_ENABLE |
					 TRANSMIT_ENABLE | RECEIVER_LINE_ST_ENABLE));
			break;
		case BSP_UART_INTR_CTRL_TERMIOS:
			uwrite (uart, IER, (RECEIVE_ENABLE | RECEIVER_LINE_ST_ENABLE));
			break;
...
unsigned BSP_poll_char_via_serial ()
{
...
int BSP_uart_termios_write_com1 (int minor, const char *buf, int len)
{
	...
	if (!termios_tx_active_com1)
	{
		...
		uwrite (BSP_UART_COM1, IER,
				(RECEIVE_ENABLE | TRANSMIT_ENABLE | RECEIVER_LINE_ST_ENABLE));
	}
	...
	for (;;)
	{
		...
		switch (vect)
		{
			case NO_MORE_INTR:
				...
				if (off != 0)
				{
					...
					rtems_termios_enqueue_raw_characters
						(termios_ttyp_com1, (char *)buf, off);
				}
				return;
			case TRANSMITTER_HODING_REGISTER_EMPTY:
				...
				ret =
					rtems_termios_dequeue_characters (termios_ttyp_com1, 1);

				/* If nothing else to send disable interrupts */
				if (ret == 0)
				{
					uwrite (BSP_UART_COM1, IER,
							(RECEIVE_ENABLE | RECEIVER_LINE_ST_ENABLE));
					termios_tx_active_com1 = 0;
				}
				break;
		...
		switch (vect)
		{
			case NO_MORE_INTR:
				...
				if (off != 0)
				{
					/* Update rx buffer */
					rtems_termios_enqueue_raw_characters
						(termios_ttyp_com2, (char *)buf, off);
				}
				return;
			case TRANSMITTER_HODING_REGISTER_EMPTY:
				/*
				...
				ret =
					rtems_termios_dequeue_characters (termios_ttyp_com2, 1);
