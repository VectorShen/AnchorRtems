	...
	if (ctx->frame_buffer != NULL)
	  {
		  ...
		  regs->lcd.control = cfg->control | PL111_LCD_CONTROL_LCD_EN;

		  pl111_fb_power_delay (cfg);

		  regs->lcd.control = cfg->control
			  | PL111_LCD_CONTROL_LCD_EN | PL111_LCD_CONTROL_LCD_PWR;
	  }
	else
	  {
		  sc = RTEMS_NO_MEMORY;
											 ...
											 void *arg)
{
	...
	sc = rtems_semaphore_create (rtems_build_name ('F', 'B', ' ', ' '),
								 1,
								 RTEMS_COUNTING_SEMAPHORE, 0, &ctx->semaphore);
	if (sc != RTEMS_SUCCESSFUL)
	  {
									   ...
									   void *arg)
{
	...
	sc = rtems_semaphore_obtain (ctx->semaphore, RTEMS_WAIT, RTEMS_NO_TIMEOUT);
	if (sc == RTEMS_SUCCESSFUL)
	  {
