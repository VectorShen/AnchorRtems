...
static rtems_status_code set_system_clk (uint32_t sys_clk,
										 uint32_t hse_clk, uint32_t hse_flag);
...
static void init_main_osc (void)
{
	...
	status = set_system_clk (STM32F4_SYSCLK / 1000000L,
							 STM32F4_HSE_OSCILLATOR / 1000000L, 1);
										 ...
										 uint32_t hse_clk, uint32_t hse_flag)
{
	...
	rcc->pllcfgr |= RCC_PLLCFGR_PLLM (pll_m) |	/* input divider */
		...
		/* HSE v HSI */
		(hse_flag ? RCC_PLLCFGR_PLLSRC_HSE : RCC_PLLCFGR_PLLSRC_HSI) | RCC_PLLCFGR_PLLQ (pll_q);	/* PLLQ divider */

	/* set prescalers for the internal busses */
	rcc->cfgr |= apbpre1 | apbpre2 | ahbpre;
