...
void console_register_devices (console_tbl * new_ports, size_t number_of_ports)
{
	...
	Console_Port_Tbl = realloc (Console_Port_Tbl,
								...
								rtems_device_minor_number minor, void *arg)
{
	...
	if ((args->iop->flags & LIBIO_FLAGS_READ) &&
		cptr->pDeviceFlow && cptr->pDeviceFlow->deviceStartRemoteTx)
	{
								 ...
								 rtems_device_minor_number minor, void *arg)
{
	...
	{
		if ((args->iop->flags & LIBIO_FLAGS_READ) &&
			cptr->pDeviceFlow && cptr->pDeviceFlow->deviceStopRemoteTx)
		{
										...
										void *arg)
{
	...
	if (!Console_Port_Tbl)
	{
		console_initialize_pointers ();
		Console_Port_Data =
			calloc (Console_Port_Count, sizeof (console_data));
		if (Console_Port_Data == NULL)
		{
	...
	for (minor = 0; minor < Console_Port_Count; minor++)
	{
			...
			port->pDeviceFns->deviceProbe (minor))
		{
			...
			{
				status =
					rtems_io_register_name (port->sDeviceName, major,
											minor);
			...
			{
				status =
					rtems_io_register_name (CONSOLE_DEVICE_NAME, major,
											minor);
				if (status != RTEMS_SUCCESSFUL)
				{
