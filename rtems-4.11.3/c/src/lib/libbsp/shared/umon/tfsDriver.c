...
static bool rtems_tfs_is_directory (const char *path, size_t pathlen)
{
	return path[pathlen - 1] == '/';
}

static int rtems_tfs_mount_me (rtems_filesystem_mount_table_entry_t * mt_entry,
							 const void *data)
{
...
int rtems_initialize_tfs_filesystem (const char *path)
{
	...
	status =
		mount ("umon", TFS_PATHNAME_PREFIX, "tfs", RTEMS_FILESYSTEM_READ_WRITE,
			 NULL);
 ...
 */
static int rtems_tfs_open_worker (rtems_libio_t * iop,
 ...
 */
static int rtems_tfs_open (rtems_libio_t * iop,
 ...
 */
static ssize_t rtems_tfs_read (rtems_libio_t * iop, void *buffer, size_t count)
{
	int ret, fd;
 ...
 */
static int rtems_tfs_close (rtems_libio_t * iop)
{
	int fd;
	char *info;
	struct tfdinfo *tip;
...
static ssize_t rtems_tfs_write (rtems_libio_t * iop,
...
static off_t rtems_tfs_lseek (rtems_libio_t * iop, off_t offset, int whence)
{
	int ret, fd;
 ...
 *
 */
static int rtems_tfs_ftruncate (rtems_libio_t * iop, off_t count)
{
	int ret, fd;
...
static int rtems_tfs_ioctl (rtems_libio_t * iop, uint32_t cmd, void *buf)
{
	int ret;
...
static int rtems_tfs_fstat (const rtems_filesystem_location_info_t * loc,
							struct stat *buf)
{
...
static int rtems_tfs_clone_node_info (rtems_filesystem_location_info_t * loc)
{
	int rv = 0;
...
static void rtems_tfs_free_node_info (const rtems_filesystem_location_info_t *
									loc)
{
	free (loc->node_access);
}

static bool rtems_tfs_are_nodes_equal (const rtems_filesystem_location_info_t *
									 a,
