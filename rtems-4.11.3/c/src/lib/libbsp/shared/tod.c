									...
									void *arg)
{
	...
	for (minor = 0; minor < RTC_Count; minor++)
	{
		...
		if (RTC_Table[minor].deviceProbe
			&& RTC_Table[minor].deviceProbe (minor))
		{
			/*
	...
	for (minor++; minor < RTC_Count; minor++)
	{
		...
		if (RTC_Table[minor].deviceProbe
			&& RTC_Table[minor].deviceProbe (minor))
		{
			status = rtems_io_register_name (RTC_Table[minor].sDeviceName,
											 major, minor);
			if (status != RTEMS_SUCCESSFUL)
			{
				rtems_fatal_error_occurred (status);
							...
							rtems_device_minor_number minor, void *arg)
{
	...
	rv = RTC_Table[RTC_Minor].pDeviceFns->deviceGetTime (RTC_Minor, tod);
	if (rv != 0)
	{
							 ...
							 rtems_device_minor_number minor, void *arg)
{
	...
	rv = RTC_Table[RTC_Minor].pDeviceFns->deviceSetTime (RTC_Minor, tod);
	if (rv != 0)
	{
		return RTEMS_IO_ERROR;
	}
 ...
 */
int setRealTime (const rtems_time_of_day * tod)
{
	if (!RTC_Present)
		return -1;
