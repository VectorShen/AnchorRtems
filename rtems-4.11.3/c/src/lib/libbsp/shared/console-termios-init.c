...
RTEMS_INTERRUPT_LOCK_DEFINE (static, console_lock, "console")
	 ...
	 rtems_status_code console_device_install (const char *device_file,
											 const
											 ...
											 context)
{
	...
	return rtems_termios_device_install (device_file,
										 console_major,
										 minor, handler, flow, context);
}
										...
										void *arg)
{
	...
	for (minor = 0; minor < console_device_count; ++minor)
	{
		...
		{
			sc = rtems_termios_device_install (ctx->device_file,
											 major,
											 minor,
											 ctx->handler,
											 ctx->flow, ctx->context);
			...
			if (!console_device_done)
			{
				...
				sc = rtems_io_register_name (CONSOLE_DEVICE_NAME, major,
											 minor);
				if (sc != RTEMS_SUCCESSFUL)
				{
