...
static inline void bsp_interrupt_set_handler_unique (rtems_vector_number index,
													 bool unique)
{
	rtems_vector_number i = index / 8;
	rtems_vector_number s = index % 8;
...
static inline bool bsp_interrupt_is_empty_handler_entry (const
...
static inline void
bsp_interrupt_clear_handler_entry (bsp_interrupt_handler_entry * e,
...
static inline bool bsp_interrupt_allocate_handler_index (rtems_vector_number
														 vector,
														 rtems_vector_number *
														 index)
{
 ...
 */
static rtems_status_code bsp_interrupt_handler_install (rtems_vector_number
														vector,
														const char *info,
														...
														handler, void *arg)
{
	...
	bsp_interrupt_set_handler_unique (index,
									RTEMS_INTERRUPT_IS_UNIQUE (options));
 ...
 */
static rtems_status_code bsp_interrupt_handler_remove (rtems_vector_number
													 vector,
													 rtems_interrupt_handler
													 handler, void *arg)
{
 ...
 */
static rtems_status_code bsp_interrupt_handler_iterate (rtems_vector_number
														vector,
														...
														routine, void *arg)
{
	...
	if (!bsp_interrupt_is_empty_handler_entry (current))
	{
		do
		{
			...
			routine (arg, current->info, options, current->handler,
					 current->arg);
			current = current->next;
		}
		while (current != NULL);
