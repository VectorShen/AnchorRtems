...
RTEMS_INTERRUPT_LOCK_DEFINE (static,
						 bsp_interrupt_server_lock, "Interrupt Server")
...
static void bsp_interrupt_server_trigger (void *arg)
{
	...
	if (e->node.next == NULL)
	{
		...
		rtems_interrupt_lock_acquire (&bsp_interrupt_server_lock,
										&lock_context);
		rtems_chain_append_unprotected (&bsp_interrupt_server_chain,
										&e->node);
		rtems_interrupt_lock_release (&bsp_interrupt_server_lock,
										&lock_context);
	}
	else
	{
...
static void bsp_interrupt_server_task (rtems_task_argument arg)
{
	...
	while (true)
	{
		...
		sc = rtems_event_receive (BSP_INTERRUPT_EVENT,
...
static void bsp_interrupt_server_per_handler_routine (void *iterate_arg,
													const char *info,
														 ...
														 handler, void *arg)
{
	...
	sc = rtems_interrupt_handler_iterate (vector,
...
rtems_status_code rtems_interrupt_server_initialize (rtems_task_priority
													 priority,
													 size_t stack_size,
													 rtems_mode modes,
													 ...
													 rtems_id * server)
{
	...
	sc = rtems_task_create (rtems_build_name ('I', 'R', 'Q', 'S'),
							priority,
							stack_size,
							modes, attributes, &bsp_interrupt_server_id);
	if (sc != RTEMS_SUCCESSFUL)
	{
		return RTEMS_TOO_MANY;
	}

	sc = rtems_task_start (bsp_interrupt_server_id,
						 bsp_interrupt_server_task, 0);
	if (sc != RTEMS_SUCCESSFUL)
	{
