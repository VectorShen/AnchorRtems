...
void rtems_gdb_stub_get_registers_from_context (unsigned int *registers,
												Thread_Control * th);
...
static int rtems_gdb_stub_id_to_index (Objects_Id thread_obj_id)
{
	int gdb_index = 0;
	int first = 1;
	size_t api_index;
	...
	for (api_index = 1;
		 gdb_index == 0 && api_index <= OBJECTS_APIS_LAST; ++api_index)
	{
...
Thread_Control *rtems_gdb_index_to_stub_id (int gdb_index)
{
	...
	for (api_index = 1;
		 th == NULL && api_index <= OBJECTS_APIS_LAST; ++api_index)
	{
		if (_Objects_Information_table[api_index] != NULL)
		{
			const Objects_Information *info =
...
int rtems_gdb_stub_get_next_thread (int gdb_index)
{
	...
	for (api_index = 1;
		 next_gdb_index == 0 && api_index <= OBJECTS_APIS_LAST; ++api_index)
	{
		if (_Objects_Information_table[api_index] != NULL)
		{
			const Objects_Information *info =
			...
			if (gdb_index <= last)
			{
				...
				for (potential_next = start;
					 next_gdb_index == 0 && potential_next <= last;
					 ++potential_next)
				{
					if (info->local_table[potential_next - first + 1] !=
						NULL)
					{
...
/* Get thread information, return 0 if thread does not
   exist and 1 otherwise */
static int rtems_gdb_stub_get_thread_info (int gdb_index,
		...
		if (_Objects_Information_table[api_index] != NULL)
		{
			...
			if (gdb_index <= last)
			{
				...
				if (th != NULL)
				{
					...
					rtems_object_get_name (th->Object.id, 5,
										 info->name);
					info->more_display[0] = 0;	/* Nothing */
...
static int parse_qp (const char *in, int *mask, int *thread)
{
	const char *ptr;
...
static void
pack_qq (char *out, int mask, int thread,
...
static int parse_ql (const char *in, int *first, int *max_count, int *athread)
{
	const char *ptr;
...
static char *reserve_qm_header (char *out)
{
	ASSERT (out != NULL);
...
/* Format: qM<count-02x><done-01x><arg_thread_id>[<found_thread_id-ft>...] */
static char *pack_qm_thread (char *out, int thread)
{
	ASSERT (out != 0);
...
/* Format: qM<count-02x><done-01x><arg_thread_id>[<found_thread_id-ft>...] */
static void pack_qm_header (char *out, int count, int done, int athread)
{
	ASSERT (out != 0);
...
void rtems_gdb_process_query (char *inbuffer,
							char *outbuffer, int do_threads, int thread)
{
	char *optr;
...
unsigned char get_byte (const unsigned char *addr)
{
	return *addr;
}

void set_byte (unsigned char *addr, int val)
{
	*addr = val;
}
...
int rtems_gdb_stub_get_offsets (unsigned char **text_addr,
