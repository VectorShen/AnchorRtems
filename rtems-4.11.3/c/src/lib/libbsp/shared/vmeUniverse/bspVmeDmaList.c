...
#ifdef DEBUG
static void lprint (VMEDmaListNode d)
{
...
static VMEDmaListNode lalloc (VMEDmaListClass pc)
{
	VMEDmaListNode rval;
	int i;
	...
	if (!pc->freeList)
	{
		...
		else
		{
			...
			if (!
				(memptr = malloc (blksize * (LCHUNK) + pc->desc_align - 1)))
			{
				free (pc->freeList);
				pc->freeList = 0;
				return 0;
...
static int lfree (VMEDmaListNode d)
{
	if (d->p || d->n)
		return -1;
...
static int lenq (VMEDmaListNode a, VMEDmaListNode d)
{
	if (a)
	{
		/* enqueue */
...
int
BSP_VMEDmaListDescriptorStartTool (volatile void *controller, int channel,
								 VMEDmaListNode n)
{
	if (!n)
		return -1;
...
VMEDmaListNode
BSP_VMEDmaListDescriptorSetupTool (VMEDmaListNode n,
								 uint32_t attr_mask,
								 uint32_t xfer_mode,
								 uint32_t pci_addr,
								 uint32_t vme_addr, uint32_t n_bytes)
{
	...
	if (n->class->
		desc_setup (n->d, attr_mask, xfer_mode, pci_addr, vme_addr, n_bytes))
	{
		return 0;
	}
...
VMEDmaListNode
BSP_VMEDmaListDescriptorNewTool (VMEDmaListClass pc,
								 uint32_t attr_mask,
								 uint32_t xfer_mode,
								 uint32_t pci_addr,
								 uint32_t vme_addr, uint32_t n_bytes)
...
int BSP_VMEDmaListDescriptorDestroy (BSP_VMEDmaListDescriptor p)
{
	VMEDmaListNode d = p;
	return lfree (d);
}

int BSP_VMEDmaListDestroy (BSP_VMEDmaListDescriptor p)
{
	VMEDmaListNode d = p;
	VMEDmaListNode n;
	while (d)
...
int
BSP_VMEDmaListDescriptorEnq (BSP_VMEDmaListDescriptor p,
							 BSP_VMEDmaListDescriptor q)
{
...
void BSP_VMEDmaListDescriptorSetUsr (BSP_VMEDmaListDescriptor p, void *usrData)
{
	VMEDmaListNode d = p;
	d->usrData = usrData;
}

void *BSP_VMEDmaListDescriptorGetUsr (BSP_VMEDmaListDescriptor p)
{
	VMEDmaListNode d = p;
	return d->usrData;
}

int BSP_VMEDmaListRefresh (BSP_VMEDmaListDescriptor p)
{
	VMEDmaListNode anchor = p;
...
#ifdef DEBUG
void BSP_VMEDmaListDump (BSP_VMEDmaListDescriptor p)
{
	VMEDmaListNode d = p;
	while (d)
	{
