...
volatile LERegister *vmeUniverseBaseAddr (void)
{
	if (!vmeUniverse0BaseAddr)
		vmeUniverseInit ();
	return vmeUniverse0BaseAddr;
}

int vmeUniversePciIrqLine (void)
{
...
static inline void
WRITE_LE (unsigned long val, volatile LERegister * adrs, unsigned long off)
{
	...
	*(volatile unsigned long *)(((unsigned long)adrs) + off) = val;
	...
	__asm__ __volatile__ ("stwbrx %1, %0, %2"::"b" (off), "r" (val),
						"r" (adrs));
...
static inline unsigned long READ_LE0 (volatile LERegister * adrs)
{
#if (__LITTLE_ENDIAN__ == 1)
...
READ_LE (volatile LERegister * adrs, unsigned long off)
{
	...
	register unsigned long rval;
	/* offset is in bytes and MUST not end up in r0 */
	__asm__ __volatile__ ("lwbrx %0, %2, %1":"=r" (rval):"r" (adrs), "b" (off));
#if 0
...
static int uprintk (char *fmt, va_list ap)
{
	int rval;
...
static void uprintf (FILE * f, char *fmt, ...)
{
	va_list ap;
	va_start (ap, fmt);
#ifdef __rtems__
...
static int vmeUniverseFindPciBase (int instance, volatile LERegister ** pbase)
{
	int bus, dev, fun;
	unsigned short wrd;
	pci_ulong busaddr;
	...
	if (BSP_PCI_FIND_DEVICE (PCI_VENDOR_TUNDRA,
							 PCI_DEVICE_UNIVERSEII, instance, &bus, &dev, &fun))
		return -1;
	...
	if ((unsigned long)(busaddr) & 1)
	{
		...
		if (BSP_PCI_CONFIG_IN_LONG
			(bus, dev, fun, PCI_UNIVERSE_BASE1, &busaddr)
	...
	*pbase = (volatile LERegister *)BSP_PCI2LOCAL_ADDR (busaddr);
	...
	BSP_PCI_CONFIG_OUT_SHORT (bus, dev, fun, PCI_COMMAND,
							wrd | PCI_COMMAND_MEMORY | PCI_COMMAND_MASTER);
...
static int
disableUniversePort (int ismaster, int portno, volatile unsigned long *preg,
					 void *param)
{
	unsigned long cntrl;
	cntrl = READ_LE0 (preg);
...
static int
cfgUniversePort (volatile LERegister * base,
				 unsigned long ismaster,
				 unsigned long port,
				 unsigned long address_space,
				 unsigned long vme_address,
				 unsigned long local_address, unsigned long length)
{
	...
	{
		preg +=
			(ismaster ? UNIV_REGOFF_PCITGT0_CTRL : UNIV_REGOFF_VMESLV0_CTRL) /
			sizeof (LERegister);
	}
	else
	{
		preg +=
			(ismaster ? UNIV_REGOFF_PCITGT4_CTRL : UNIV_REGOFF_VMESLV4_CTRL) /
			sizeof (LERegister);
		p -= 4;
	}
	preg += 5 * p;
...
static int
showUniversePort (int ismaster,
				int portno, volatile LERegister * preg, void *parm)
{
...
static int
hasPWENWindow (int ismaster, int portno, volatile LERegister * preg, void *parm)
{
	unsigned long cntrl = READ_LE0 (preg);
	unsigned long mask =
		ismaster ? (UNIV_MCTL_EN | UNIV_MCTL_PWEN) : (UNIV_SCTL_EN |
													UNIV_SCTL_PWEN);
	return (cntrl & mask) == mask ? -1 : 0;
}

static int
mapOverAll (volatile LERegister * base, int ismaster,
			int (*func) (int, int, volatile LERegister *, void *), void *arg)
{
...
#undef TSILL
	...
	{
		if ((rval = func (ismaster, port, rptr, arg)))
			return rval;
...
static int xlateFindPort (volatile LERegister * base,	/* Universe base address */
...
int vmeUniverseXlateAddrXX (volatile LERegister * base,	/* Universe base address */
							int master,	/* look in the master windows */
							int reverse,	/* reverse mapping; for masters: map local to VME */
							unsigned long as,	/* address space */
							unsigned long aIn,	/* address to look up */
...
int vmeUniverseXlateAddr (int master,	/* look in the master windows */
						int reverse,	/* reverse mapping; for masters: map local to VME */
						unsigned long as,	/* address space */
						unsigned long aIn,	/* address to look up */
						unsigned long *paOut	/* where to put result */
...
void vmeUniverseReset (void)
{
	...
	if (2 == UNIV_REV (vmeUniverse0BaseAddr))
		vmeUniverseWriteReg (UNIV_U2SPEC_DTKFLTR |
							 ...
							 UNIV_U2SPEC_PREt28_FAST, UNIV_REGOFF_U2SPEC);
	...
	vmeUniverseWriteReg (vmeUniverseReadReg (UNIV_REGOFF_PCI_CSR) |
						 UNIV_PCI_CSR_D_PE | UNIV_PCI_CSR_S_SERR |
						 UNIV_PCI_CSR_R_MA | UNIV_PCI_CSR_R_TA |
						 UNIV_PCI_CSR_S_TA, UNIV_REGOFF_PCI_CSR);

	vmeUniverseWriteReg (UNIV_L_CMDERR_L_STAT, UNIV_REGOFF_L_CMDERR);

	vmeUniverseWriteReg (UNIV_DGCS_STOP | UNIV_DGCS_HALT | UNIV_DGCS_DONE |
...
int vmeUniverseInit (void)
{
	...
	else
	{
		...
		rval = 0;
		uprintf (stderr,
				 "Universe II PCI-VME bridge detected at 0x%08x, IRQ %d\n",
				 (unsigned int)vmeUniverse0BaseAddr, vmeUniverse0PciIrqLine);
	}
	return rval;
}

void vmeUniverseMasterPortsShowXX (volatile LERegister * base, FILE * f)
{
...
void vmeUniverseSlavePortsShowXX (volatile LERegister * base, FILE * f)
{
...
int
vmeUniverseMasterPortCfgXX (volatile LERegister * base,
							unsigned long port,
							unsigned long address_space,
							unsigned long vme_address,
							unsigned long local_address, unsigned long length)
{
	return cfgUniversePort (base, 1, port, address_space, vme_address,
							local_address, length);
}

int
vmeUniverseMasterPortCfg (unsigned long port,
						unsigned long address_space,
						unsigned long vme_address,
						unsigned long local_address, unsigned long length)
{
...
int
vmeUniverseSlavePortCfgXX (volatile LERegister * base,
						 unsigned long port,
						 unsigned long address_space,
						 unsigned long vme_address,
						 unsigned long local_address, unsigned long length)
{
	return cfgUniversePort (base, 0, port, address_space, vme_address,
							local_address, length);
}

int
vmeUniverseSlavePortCfg (unsigned long port,
						 unsigned long address_space,
						 unsigned long vme_address,
						 unsigned long local_address, unsigned long length)
{
...
void vmeUniverseDisableAllSlavesXX (volatile LERegister * base)
{
...
void vmeUniverseDisableAllMastersXX (volatile LERegister * base)
{
...
void
vmeUniverseWriteRegXX (volatile LERegister * base, unsigned long value,
...
{
	vmeUniverseWriteReg (vmeUniverseReadReg (UNIV_REGOFF_MISC_CTL) |
						 UNIV_MISC_CTL_SW_SYSRST, UNIV_REGOFF_MISC_CTL);
}
...
void vmeUniverseCvtToLE (unsigned long *ptr, unsigned long num)
{
	...
	while (p > ptr)
	{
		...
		__asm__ __volatile__ ("lwzu 0, -4(%0)\n"
								"stwbrx 0, 0, %0\n":"=r" (p):"0" (p):"r0");
#elif defined(__rtems__)
		p--;
		st_le32 (p, *p);
...
vmeUniverseIntRaiseXX (volatile LERegister * base, int level, unsigned vector)
{
	...
	if (vmeUniverseReadRegXX (base, UNIV_REGOFF_VINT_STAT) &
		UNIV_VINT_STAT_SWINT (level))
	{
		return -2;			/* already asserted */
	}

	/* Write Vector */
	vmeUniverseWriteRegXX (base, UNIV_VINT_STATID (vector),
...
int
vmeUniverseMapCRGXX (volatile LERegister * base, unsigned long vme_base,
					 unsigned long as)
{
	uint32_t mode;
	...
	{
		uprintf (stderr,
				 "vmeUniverse: CRG already mapped and in use by interrupt manager\n");
		return -1;
	}
#endif
	...
	mode =
		UNIV_VRAI_CTL_EN | UNIV_VRAI_CTL_PGM | UNIV_VRAI_CTL_DATA |
		UNIV_VRAI_CTL_SUPER | UNIV_VRAI_CTL_USER;

	if (VME_AM_IS_SHORT (as))
	{
		mode |= UNIV_VRAI_CTL_VAS_A16;
	}
	else if (VME_AM_IS_STD (as))
	{
		mode |= UNIV_VRAI_CTL_VAS_A24;
	}
	else if (VME_AM_IS_EXT (as))
	{
...
int vmeUniverseMapCRG (unsigned long vme_base, unsigned long as)
{
...
static uint32_t xfer_mode2dctl (uint32_t xfer_mode)
{
	uint32_t dctl;
	...
	if (!VME_AM_IS_SHORT (xfer_mode) && !VME_AM_IS_STD (xfer_mode)
		&& !VME_AM_IS_EXT (xfer_mode))
		return BSP_VMEDMA_STATUS_UNSUP;
 ...
 */
int
vmeUniverseDmaSetupXX (volatile LERegister * base, int channel, uint32_t mode,
					 uint32_t xfer_mode, void *custom)
{
	uint32_t dctl, dgcs;
...
int
vmeUniverseDmaSetup (int channel, uint32_t mode, uint32_t xfer_mode,
					 void *custom)
{
	DFLT_BASE;
...
int
vmeUniverseDmaStartXX (volatile LERegister * base, int channel,
					 uint32_t pci_addr, uint32_t vme_addr, uint32_t n_bytes)
{
	if (channel != 0)
		return BSP_VMEDMA_STATUS_UNSUP;
...

	{
		...
		WRITE_LE (dgcs, b, dgcsoff);
		WRITE_LE (pci_addr, b, UNIV_REGOFF_DLA);
		WRITE_LE (vme_addr, b, UNIV_REGOFF_DVA);
		WRITE_LE (n_bytes, b, UNIV_REGOFF_DTBC);
		dgcs |= UNIV_DGCS_GO;
...
int
vmeUniverseStartDMAXX (volatile LERegister * base,
					 unsigned long local_addr,
					 unsigned long vme_addr, unsigned long count)
{
	return vmeUniverseDmaStartXX (base, 0, local_addr, vme_addr, count);
}

int
vmeUniverseDmaStart (int channel, uint32_t pci_addr, uint32_t vme_addr,
					 uint32_t n_bytes)
{
...
/* This entry point is deprecated */
int
vmeUniverseStartDMA (unsigned long local_addr,
					 unsigned long vme_addr, unsigned long count)
{
...
uint32_t vmeUniverseDmaStatusXX (volatile LERegister * base, int channel)
{
	uint32_t dgcs;
	if (channel != 0)
		return BSP_VMEDMA_STATUS_UNSUP;
...
uint32_t vmeUniverseDmaStatus (int channel)
{
	DFLT_BASE;
...
typedef struct VmeUniverseDmaListDescRec_
{
	...
	LERegister1 dcpp;
	LERegister1 dummy3;
} __attribute__ ((aligned (32), __may_alias__)) VmeUniverseDmaListDescRec;

typedef VmeUniverseDmaListDescRec *VmeUniverseDmaListDesc;

static void uni_desc_init (DmaDescriptor);
static int uni_desc_setup (DmaDescriptor, uint32_t, uint32_t, uint32_t,
						 uint32_t, uint32_t);
static void uni_desc_setnxt (DmaDescriptor, DmaDescriptor);
static void uni_desc_dump (DmaDescriptor);
static int uni_desc_start (volatile void *controller_addr, int channel,
						 DmaDescriptor p);
...
static int
uni_desc_setup (DmaDescriptor p,
				uint32_t attr_mask,
				uint32_t xfer_mode,
				uint32_t pci_addr, uint32_t vme_addr, uint32_t n_bytes)
{
	...
	(volatile void *controller_addr, int channel, DmaDescriptor p)
{
	...
	vmeUniverseWriteRegXX (base, BSP_LOCAL2PCI_ADDR ((uint32_t) p),
						 UNIV_REGOFF_DCPP);
...
static void uni_desc_dump (DmaDescriptor p)
{
	VmeUniverseDmaListDesc d = p;
...
typedef struct UniverseIRQEntryRec_
{
	VmeUniverseISR isr;
	void *usrData;
...
static int lvl2bit (unsigned int level)
{
	int shift = -1;
...
int vmeUniverseIntRoute (unsigned int level, unsigned int pin)
{
	int i, shift;
	unsigned long mask, mapreg, flags, wire;

	if (pin >= UNIV_NUM_WIRES || !universe_wire[pin]
		|| !vmeUniverseIrqMgrInstalled)
		return -1;
...
VmeUniverseISR vmeUniverseISRGet (unsigned long vector, void **parg)
{
	unsigned long flags;
...
static void universeSpecialISR (unsigned long status)
{
...
static void universeVMEISR (rtems_irq_hdl_param arg)
{
	int pin = (int)arg;
	UniverseIRQEntry ip;
	...
	msk = 1 << lvl;
#else
	for (msk = UNIV_LINT_STAT_VIRQ7, lvl = 7; lvl > 0; lvl--, msk >>= 1)
	{
		if (lintstat & msk)
			break;
	}
	...
	vmeUniverseWriteReg (linten &
						 ~(((msk << 1) -
							UNIV_LINT_STAT_VIRQ1) & wire_mask[pin]),
						 UNIV_REGOFF_LINT_EN);
...
#endif
		printk
			("vmeUniverse ISR: error read from STATID register; (level: %i) STATID: 0x%08x -- DISABLING\n",
			 lvl, status);
	}
	else if (!(ip = universeHdlTbl[status & UNIV_VIRQ_STATID_MASK]))
	{
		...
		/* TODO: log error message - RTEMS has no logger :-( */
		printk
			("vmeUniverse ISR: no handler installed for this vector; (level: %i) STATID: 0x%08x -- DISABLING\n",
			 lvl, status);
	}
	else
	{
...
/* STUPID API */
static void my_no_op (const rtems_irq_connect_data * arg)
{
}

static int my_isOn (const rtems_irq_connect_data * arg)
{
...
mappedAndProbed (unsigned long vme_addr, unsigned as, unsigned long *pcpu_addr)
{
	...
	/* try to find mapping */
	if (0 > (j = xlateFindPort (vmeUniverse0BaseAddr,
								1, 0,
								as | VME_MODE_AS_MATCH, vme_addr, pcpu_addr)))
	{
		uprintf (stderr,
				 "vmeUniverse - Unable to find mapping for %s VME base (0x%08x)\n",
				 regtype, vme_addr);
	...
	else
	{
		...
		*pcpu_addr = BSP_PCI2LOCAL_ADDR (*pcpu_addr);
		...
		{
			uprintf (stderr,
					 "vmeUniverse - IRQ manager using VME %s to flush FIFO\n",
					 regtype);
			return j;
		}
		else
		{
			uprintf (stderr,
					 "vmeUniverse - Found slot info but detection of universe in VME %s space failed\n",
					 regtype);
		}
	}
	return -1;
...
int vmeUniverseInstallIrqMgrAlt (int flags, int uni_pin0, int pic_pin0, ...)
{
	int rval;
	va_list ap;
...
vmeUniverseInstallIrqMgrVa (int flags, int uni_pin0, int pic_pin0, va_list ap)
{
	...
	for (i = 0; uni_pin[i] >= 0; i++)
	{
		...
		{
			if (uni_pin[j] == uni_pin[i])
				return -6;
			if (pic_pin[j] == pic_pin[i])
				return -7;
		}
	}
	...
	if (flags & VMEUNIVERSE_IRQ_MGR_FLAG_PW_WORKAROUND)
	{
		 ...
		 */
		if ((i =
			 ((READ_LE (vmeUniverse0BaseAddr, UNIV_REGOFF_VCSR_BS) >> 27) &
				0x1f)) > 0)
		{
		...
		if (-1 == i)
		{
			...
			if (UNIV_VRAI_CTL_EN &
				(j = READ_LE (vmeUniverse0BaseAddr, UNIV_REGOFF_VRAI_CTL)))
			{
				switch (j & UNIV_VRAI_CTL_VAS_MSK)
				{
					case UNIV_VRAI_CTL_VAS_A16:
						i = VME_AM_SUP_SHORT_IO;
						break;
					case UNIV_VRAI_CTL_VAS_A24:
						i = VME_AM_STD_SUP_DATA;
						break;
					case UNIV_VRAI_CTL_VAS_A32:
						i = VME_AM_EXT_SUP_DATA;
						break;
					default:
						break;
				}
				vme_reg_base =
					READ_LE (vmeUniverse0BaseAddr,
							 UNIV_REGOFF_VRAI_BS) & ~(UNIV_CRG_SIZE - 1);
			}
			...
			{
				i = mappedAndProbed (vme_reg_base, (i & VME_AM_MASK),
									 &cpu_base);
			}
		}
		...
		if (i < 0)
		{
			...
			{
				uprintf (stderr,
						 "vmeUniverse IRQ manager - BSP configuration error: registers not found on VME\n");
				uprintf (stderr,
						 "(should open outbound window to CSR space or map CRG [vmeUniverseMapCRG()])\n");
				uprintf (stderr,
						 "Falling back to PCI but you might experience spurious VME interrupts; read a register\n");
				uprintf (stderr,
						 "back from user ISR to flush universe FIFO as a work-around or\n");
				uprintf (stderr,
						 "make sure ISR accesses device using a window with posted-writes disabled\n");
			}
			else
			{
				uprintf (stderr,
	...
	for (i = 0; uni_pin[i] >= 0; i++)
	{
		...
		connectIsr ((flags & VMEUNIVERSE_IRQ_MGR_FLAG_SHARED), universeVMEISR,
					pic_pin[i], i);
	}
...
vmeUniverseInstallIrqMgr (int vmeIrqUnivOut,
						int vmeIrqPicLine,
						int specialIrqUnivOut, int specialIrqPicLine)
{
	return vmeUniverseInstallIrqMgrAlt (0,	/* bwds compat. */
										vmeIrqUnivOut, vmeIrqPicLine,
										specialIrqUnivOut, specialIrqPicLine,
										-1);
}

int vmeUniverseInstallISR (unsigned long vector, VmeUniverseISR hdl, void *arg)
{
	UniverseIRQEntry ip;
	unsigned long flags;
	volatile UniverseIRQEntry *pe;

	if (vector > sizeof (universeHdlTbl) / sizeof (universeHdlTbl[0])
		|| !vmeUniverseIrqMgrInstalled)
		return -1;
...
int vmeUniverseRemoveISR (unsigned long vector, VmeUniverseISR hdl, void *arg)
{
	UniverseIRQEntry ip;
	unsigned long flags;
	volatile UniverseIRQEntry *pe;
...
static int intDoEnDis (unsigned int level, int dis)
{
	...
	rtems_interrupt_disable (flags);
	if (dis < 0)
		vmeUniverseWriteReg (vmeUniverseReadReg (UNIV_REGOFF_LINT_EN) & ~v,
							 UNIV_REGOFF_LINT_EN);
	else
	{
		vmeUniverseWriteReg (vmeUniverseReadReg (UNIV_REGOFF_LINT_EN) | v,
...
int vmeUniverseIntEnable (unsigned int level)
{
	return intDoEnDis (level, 1);
}

int vmeUniverseIntDisable (unsigned int level)
{
	return intDoEnDis (level, -1);
}

int vmeUniverseIntIsEnabled (unsigned int level)
{
...
static void loopbackTstIsr (void *arg, unsigned long vector)
{
	LoopbackTstArgs *pa = arg;
	if (RTEMS_SUCCESSFUL !=
		rtems_message_queue_send (pa->q, (void *)&vector, sizeof (vector)))
	{
		/* Overrun ? */
		printk
			("vmeUniverseIntLoopbackTst: (ISR) message queue full / overrun ? disabling IRQ level %i\n",
			 pa->l);
		vmeUniverseIntDisable (pa->l);
	}
}

int vmeUniverseIntLoopbackTst (int level, unsigned vector)
{
	DFLT_BASE;
	...
	{
		fprintf (stderr,
				 "Software interrupts can only use even-numbered vectors, sorry.\n");
		return -1;
	}
	...
	if (RTEMS_SUCCESSFUL != (sc = rtems_message_queue_create (rtems_build_name ('t', 'U', 'I', 'I'), 4, sizeof (unsigned long), 0,	/* default attributes: fifo, local */
															&q)))
	{
		rtems_error (sc,
					 "vmeUniverseIntLoopbackTst: Unable to create message queue");
		goto bail;
	}
	...
	installed++;
	if (vmeUniverseInstallISR
		(UNIV_VME_SW_IACK_INT_VEC, loopbackTstIsr, (void *)&a))
	{
		fprintf (stderr,
				 "Unable to install VME ISR to IACK special vector %i\n",
	...
	for (i = 0; i < 3; i++)
	{
		sc = rtems_message_queue_receive (q, &msg, &size, RTEMS_WAIT, 20);
		if (sc)
		{
  ...
  bail:
	...
	if (installed > 1)
		vmeUniverseRemoveISR (UNIV_VME_SW_IACK_INT_VEC, loopbackTstIsr,
							(void *)&a);
	if (q)
