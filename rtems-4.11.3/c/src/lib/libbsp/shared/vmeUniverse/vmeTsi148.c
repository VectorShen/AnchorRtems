...
int vmeTsi148FindPciBase (int instance, BERegister ** pbase)
{
	...
	if (BSP_PCI_FIND_DEVICE (PCI_VENDOR_TUNDRA,
							 PCI_DEVICE_TSI148, instance, &bus, &dev, &fun))
		return -1;
	...
	*pbase =
		(BERegister *) (((pci_ulong) BSP_PCI2LOCAL_ADDR (busaddr)) & ~0xff);
...
int vmeTsi148InitInstance (unsigned instance)
{
	...
	{
		uprintf (stderr,
				 "Tundra Tsi148 PCI-VME bridge detected at 0x%08x, IRQ %d\n",
...
#define TSI_VSTAT_BDFAIL		(1<<14)
	TSI_WR (base, TSI_VSTAT_REG,
			TSI_RD (base, TSI_VSTAT_REG) & ~TSI_VSTAT_BDFAIL);
...
static int am2omode (unsigned long address_space, unsigned long *pmode)
{
	...
	switch (address_space & VME_AM_MASK)
	{
		...
		case VME_AM_2eVME_3U:
			...
			if (VME_AM_IS_2eSST (address_space))
			{
				...
				{
					default:
					case VME_AM_2eSST_LO:
						mode |= TSI_OTAT_2eSSTM_160;
						break;
					case VME_AM_2eSST_MID:
						mode |= TSI_OTAT_2eSSTM_267;
						break;
					case VME_AM_2eSST_HI:
						mode |= TSI_OTAT_2eSSTM_320;
						break;
				}
			}
			else
...
static int am2imode (unsigned long address_space, unsigned long *pmode)
{
	unsigned long mode = 0;
	unsigned long pgm = 0;
	...
	if (VME_AM_IS_2eSST (address_space))
	{
		...
		switch (VME_AM_IS_2eSST (address_space))
		{
			default:
			case VME_AM_2eSST_LO:
				mode |= TSI_ITAT_2eSSTM_160;
				break;
			case VME_AM_2eSST_MID:
				mode |= TSI_ITAT_2eSSTM_267;
				break;
			case VME_AM_2eSST_HI:
				mode |= TSI_ITAT_2eSSTM_320;
				break;
		}
		address_space = ck2esst (address_space);
	}
...
static void
readTriple (BERegister * base,
			unsigned reg,
			unsigned long long *ps,
			unsigned long long *pl, unsigned long long *po)
{
	*ps = TSI_RD (base, reg);
...
static unsigned long inboundGranularity (unsigned long itat)
{
	switch (itat & TSI_ITAT_AS (-1))
	{
		case TSI_ITAT_ADMODE_A16:
			return 0xf;
		case TSI_ITAT_ADMODE_A24:
			return 0xfff;
		default:
			break;
	}
...
static int
configTsiPort (BERegister * base,
			 int isout,
			 unsigned long port,
			 unsigned long address_space,
			 unsigned long vme_address,
			 unsigned long pci_address, unsigned long length)
{
	...
	{
		uprintf (stderr,
				 "Tsi148 %s Port Cfg: invalid port; reserved by the interrupt manager for CRG\n",
				 name);
		return -1;
	}

	if (length
		&& (isout ? am2omode (address_space, &mode) :
			am2imode (address_space, &mode)))
	{
		uprintf (stderr,
				 "Tsi148 %s Port Cfg: invalid address space / mode flags\n",
				 name);
		return -1;
	}
	...
	if (isout)
	{
		...
		for (i = 0; i < TSI148_NUM_OPORTS; i++)
		{
			/* ignore 'this' port */
			if (i == port
				|| !(TSI_OTAT_EN & TSI_RD (base, TSI_OTAT_REG (i))))
				continue;
			...
			{
				uprintf (stderr,
						 "Tsi148 Outbound Port Cfg: PCI address range overlaps with port %i (0x%08x..0x%08x)\n",
						 i, s, l);
				return -1;
			}
		}
	...
	else
	{
		...
		for (i = 0; i < TSI148_NUM_IPORTS; i++)
		{
			/* ignore 'this' port */
			if (i == port
				|| !(TSI_ITAT_EN & (s = TSI_RD (base, TSI_ITAT_REG (i)))))
				continue;
			...
			if (!
				(mode & s &
				 (TSI_ITAT_SUP | TSI_ITAT_USR | TSI_ITAT_PGM |
				TSI_ITAT_DATA)))
			{
			...
			{
				uprintf (stderr,
						 "Tsi148 Inbound Port Cfg: VME address range overlaps with port %i (0x%08x..0x%08x)\n",
						 i, s, l);
				return -1;
			}
		}
	...
	if (0 == length)
	{
		TSI_WR (base, tat_reg,
				TSI_RD (base,
						tat_reg) & ~(isout ? TSI_OTAT_EN : TSI_ITAT_EN));
		return 0;
	}

	if ((vme_address & mask) || (pci_address & mask) || (length & mask))
	{
		uprintf (stderr,
				 "Tsi148 %s Port Cfg: invalid address/length; must be multiple of 0x%x\n",
				 name, mask + 1);
		return -1;
	}
	...
	{
		uprintf (stderr,
				 "Tsi148 %s Port Cfg: invalid address/length; must be < 1<<32\n",
				 name);
		return -1;
	}
...
static int disableTsiPort (BERegister * base, int isout, unsigned long port)
{
	return configTsiPort (base, isout, port, 0, 0, 0, 0);
}

int
vmeTsi148InboundPortCfgXX (BERegister * base,
						 unsigned long port,
						 unsigned long address_space,
						 unsigned long vme_address,
						 unsigned long pci_address, unsigned long length)
{
	return configTsiPort (base, 0, port, address_space, vme_address,
						pci_address, length);
}

int
vmeTsi148InboundPortCfg (unsigned long port,
						 unsigned long address_space,
						 unsigned long vme_address,
						 unsigned long pci_address, unsigned long length)
{
	return configTsiPort (THEBASE, 0, port, address_space, vme_address,
						pci_address, length);
}

int
vmeTsi148OutboundPortCfgXX (BERegister * base,
							unsigned long port,
							unsigned long address_space,
							unsigned long vme_address,
							unsigned long pci_address, unsigned long length)
{
	return configTsiPort (base, 1, port, address_space, vme_address,
						pci_address, length);
}

int
vmeTsi148OutboundPortCfg (unsigned long port,
						unsigned long address_space,
						unsigned long vme_address,
						unsigned long pci_address, unsigned long length)
{
	return configTsiPort (THEBASE, 1, port, address_space, vme_address,
						pci_address, length);
}

static int xlateFindPort (BERegister * base,	/* TSI 148 base address */
	...
	)
{
	...
	switch (as & VME_MODE_MATCH_MASK)
	{
		...
		default:
			if (outbound)
				mode_msk =
					TSI_OTAT_PGM | TSI_OTAT_SUP | TSI_OTAT_ADMODE (-1) |
					TSI_OTAT_EN;
			else
				mode_msk =
					TSI_ITAT_PGM | TSI_ITAT_DATA | TSI_ITAT_SUP | TSI_ITAT_USR
					| TSI_ITAT_AS (-1) | TSI_ITAT_EN;
			break;
	}
	...
	{
		uprintf (stderr,
				 "vmeTsi148XlateAddr: invalid address space/mode argument");
		return -2;
	}
	...
	for (port = 0; port < TSI148_NUM_OPORTS;
		 port++, tsau_reg += skip, tat_reg += skip)
	{
...
int vmeTsi148XlateAddrXX (BERegister * base,	/* TSI 148 base address */
						int outbound,	/* look in the outbound windows */
						int reverse,	/* reverse mapping; for outbound ports: map local to VME */
						unsigned long as,	/* address space */
						unsigned long aIn,	/* address to look up */
...
int vmeTsi148XlateAddr (int outbound,	/* look in the outbound windows */
						int reverse,	/* reverse mapping; for outbound ports: map local to VME */
						unsigned long as,	/* address space */
						unsigned long aIn,	/* address to look up */
						unsigned long *paOut	/* where to put result */
...
static void uprintfllx (FILE * f, unsigned long long v)
{
	if (v >= ((unsigned long long)1) << 32)
		uprintf (f, "0x%lx%08lx ", (unsigned long)(v >> 32),
				 (unsigned long)(v & 0xffffffff));
	else
		uprintf (f, "0x%08lx ", (unsigned long)(v & 0xffffffff));
}

void vmeTsi148OutboundPortsShowXX (BERegister * base, FILE * f)
{
	int port;
	unsigned long mode;
	char tit = 0;
	...
	for (port = 0; port < TSI148_NUM_OPORTS; port++)
	{
		...
		{
			case TSI_OTAT_ADMODE_A16:
				uprintf (f, "A16");
				break;
			case TSI_OTAT_ADMODE_A24:
				uprintf (f, "A24");
				break;
			case TSI_OTAT_ADMODE_A32:
				uprintf (f, "A32");
				break;
			case TSI_OTAT_ADMODE_A64:
				uprintf (f, "A64");
				break;
			case TSI_OTAT_ADMODE_CSR:
				uprintf (f, "CSR");
				break;
			default:
				uprintf (f, "A??");
		...
		if (mode & TSI_OTAT_PGM)
			uprintf (f, ", PGM");
		if (mode & TSI_OTAT_SUP)
			uprintf (f, ", SUP");
		if (!(TSI_OTAT_MRPFD & mode))
			uprintf (f, ", PREFETCH");
...
void vmeTsi148OutboundPortsShow (FILE * f)
{
	vmeTsi148OutboundPortsShowXX (THEBASE, f);
}

void vmeTsi148InboundPortsShowXX (BERegister * base, FILE * f)
{
	int port;
	unsigned long mode;
	char tit = 0;
	...
	for (port = 0; port < TSI148_NUM_IPORTS; port++)
	{
		...
		switch (mode & TSI_ITAT_AS (-1))
		{
			...
			case TSI_ITAT_ADMODE_A24:
				uprintf (f, "A24");
				break;
			case TSI_ITAT_ADMODE_A32:
				uprintf (f, "A32");
				break;
			case TSI_ITAT_ADMODE_A64:
				uprintf (f, "A64");
				break;
			default:
				uprintf (f, "A??");
		...
		if (mode & TSI_ITAT_PGM)
			uprintf (f, ", PGM");
		if (mode & TSI_ITAT_DATA)
			uprintf (f, ", DAT");
		if (mode & TSI_ITAT_SUP)
			uprintf (f, ", SUP");
		if (mode & TSI_ITAT_USR)
			uprintf (f, ", USR");

		if (mode & TSI_ITAT_2eSSTB)
			uprintf (f, ", 2eSSTB");
		if (mode & TSI_ITAT_2eSST)
			uprintf (f, ", 2eSST");
		if (mode & TSI_ITAT_2eVME)
			uprintf (f, ", 2eVME");
		if (mode & TSI_ITAT_MBLT)
			uprintf (f, ", MBLT");
		if (mode & TSI_ITAT_BLT)
			uprintf (f, ", BLT");
...
void vmeTsi148InboundPortsShow (FILE * f)
{
...
int vmeTsi148MapCRGXX (BERegister * b, uint32_t vme_base, uint32_t as)
{
	uint32_t mode;
	...
	{
		uprintf (stderr,
				 "vmeTsi148: CRG already mapped and in use by interrupt manager\n");
		return -1;
	}

	/* enable all, SUP/USR/PGM/DATA accesses */
	mode =
		TSI_CRGAT_EN | TSI_CRGAT_SUP | TSI_CRGAT_USR | TSI_CRGAT_PGM |
		TSI_CRGAT_DATA;
...
int vmeTsi148MapCRG (uint32_t vme_base, uint32_t as)
{
...
static inline int uni2tsivec (int v)
{
...
static int lvl2bitno (unsigned int level)
{
	if (level >= 256)
...
static void tsiVMEISR (rtems_irq_hdl_param arg)
{
	int pin = (int)arg;
	 ...
	 */
	while ((lintstat =
			(TSI_RD (vmeTsi148RegBase, TSI_INTS_REG) & wire_mask[pin])))
	{
		...
		do
		{
...
#else
			{
				...
				{
					0xffff0000, 0xff00ff00, 0xf0f0f0f0, 0xcccccccc,
					0xaaaaaaaa
				};
				int i;
				unsigned tmp;
			...
			if (!(ip = irqHdlTbl[vector]))
			{
				...
				printk
					("vmeTsi148 ISR: ERROR: no handler registered (level %i) IACK 0x%08x -- DISABLING level %i\n",
					 lvl, vector, lvl);
			}
			else
			{
...
{
	return (int)(TSI_RD (THEBASE, TSI_INTEO_REG) &
				 TSI_RD (THEBASE, TSI_INTEN_REG));
}

static void connectIsr (int shared, rtems_irq_hdl isr, int pic_line, int slot)
{
...
int vmeTsi148InstallIrqMgrAlt (int shared, int tsi_pin0, int pic_pin0, ...)
{
	int rval;
	va_list ap;
...
mappedAndProbed (unsigned long vme_addr, unsigned as, unsigned long *pcpu_addr)
{
	...
	/* try to find mapping */
	if (0 > (j = xlateFindPort (THEBASE,
								1, 0,
								as | VME_MODE_AS_MATCH, vme_addr, pcpu_addr)))
	{
		uprintf (stderr,
				 "vmeTsi148 - Unable to find mapping for %s VME base (0x%08x)\n",
				 regtype, vme_addr);
	...
	else
	{
		...
		*pcpu_addr = BSP_PCI2LOCAL_ADDR (*pcpu_addr);
		...
		{
			uprintf (stderr,
					 "vmeTsi148 - IRQ manager using VME %s to flush FIFO\n",
					 regtype);
			return j;
		}
		else
		{
			uprintf (stderr,
					 "vmeTsi148 - Found slot info but detection of tsi148 in VME %s space failed\n",
					 regtype);
		}
	}
	return -1;
...
vmeTsi148InstallIrqMgrVa (int shared, int tsi_pin0, int pic_pin0, va_list ap)
{
	...
	if (-1 == i)
	{
		...
		if ((TSI_CRGAT_EN & (j = TSI_RD (THEBASE, TSI_CRGAT_REG))))
		{
			...
			vme_reg_base =
				TSI_RD (THEBASE, TSI_CBAL_REG) & ~(TSI_CRG_SIZE - 1);
		}
	...
	{
		uprintf (stderr,
				 "vmeTsi148 IRQ manager - BSP configuration error: registers not found on VME\n");
		uprintf (stderr,
				 "(should open outbound window to CSR space or map CRG [vmeTsi148MapCRG()])\n");
		uprintf (stderr,
				 "Falling back to PCI but you might experience spurious VME interrupts; read a register\n");
		uprintf (stderr,
				 "back from user ISR to flush posted-write FIFO as a work-around\n");
...
int vmeTsi148InstallISR (unsigned long vector, VmeTsi148ISR hdl, void *arg)
{
	IRQEntry ip;
	int v;
	unsigned long flags;
...
int vmeTsi148RemoveISR (unsigned long vector, VmeTsi148ISR hdl, void *arg)
{
	int v;
	IRQEntry ip;
	unsigned long flags;
...
static int intDoEnDis (unsigned int level, int dis)
{
	...
	if (!dis)
		return (int)(v & TSI_RD (b, TSI_INTEO_REG) & TSI_RD (b, TSI_INTEN_REG))
			? 1 : 0;
...
static void loopbackTstIsr (void *arg, unsigned long vector)
{
		...
		rtems_message_queue_send (pa->q, (void *)&vector, sizeof (vector)))
	{
		/* Overrun ? */
		printk
			("vmeTsi148IntLoopbackTst: (ISR) message queue full / overrun ? disabling IRQ level %i\n",
			 pa->l);
...
int vmeTsi148IntLoopbackTst (int level, unsigned vector)
{
	...
	{
		rtems_error (sc,
					 "vmeTsi148IntLoopbackTst: Unable to create message queue");
		goto bail;
	}
	...
	installed++;
	if (vmeTsi148InstallISR
		(TSI_VME_SW_IACK_INT_VEC, loopbackTstIsr, (void *)&a))
	{
		uprintf (stderr,
...
bail:
	...
	if (installed > 1)
		vmeTsi148RemoveISR (TSI_VME_SW_IACK_INT_VEC, loopbackTstIsr,
							(void *)&a);
	if (q)
...
unsigned long vmeTsi148ClearVMEBusErrorsXX (BERegister * base, uint32_t * paddr)
{
	...
	if (rval & TSI_VEAT_VES)
	{
		if (paddr)
		{
			...
			*paddr =
				((unsigned long long)TSI_RD (base, TSI_VEAU_REG)) << 32;
...
static int tsi_desc_setup (DmaDescriptor, uint32_t, uint32_t, uint32_t,
						 uint32_t, uint32_t);
static void tsi_desc_setnxt (DmaDescriptor, DmaDescriptor);
static void tsi_desc_dump (DmaDescriptor);
static int tsi_desc_start (volatile void *controller_addr, int channel,
						 DmaDescriptor p);
...
static uint32_t vme_attr (uint32_t xfer_mode)
{
	uint32_t vme_mode;
...
static uint32_t pci_attr (uint32_t xfer_mode)
{
	uint32_t pci_mode = 0;
...
static void tsi_desc_setnxt (DmaDescriptor p, DmaDescriptor n)
{
	VmeTsi148DmaListDescriptor d = p;
	if (0 == n)
	{
...
static void tsi_desc_dump (DmaDescriptor p)
{
	VmeTsi148DmaListDescriptor d = p;
...
int
vmeTsi148DmaSetupXX (BERegister * base, int channel, uint32_t mode,
					 uint32_t xfer_mode, void *custom)
{
	...
	ctl |=
		(BSP_VMEDMA_OPT_THROUGHPUT ==
		 mode ? TSI_DCTL_PBOT_0us : TSI_DCTL_PBOT_1us);
...
int
vmeTsi148DmaListStartXX (BERegister * base, int channel,
						 VmeTsi148DmaListDescriptor d)
{
	uint32_t ctl;
	...
	if (d)
	{
		...
		TSI_WR (base, TSI_DNLAL_REG (channel),
...
int vmeTsi148DmaListStart (int channel, VmeTsi148DmaListDescriptor d)
{
	BERegister *base = THEBASE;
...
int
vmeTsi148DmaStartXX (BERegister * base, int channel, uint32_t pci_addr,
					 uint32_t vme_addr, uint32_t n_bytes)
{
...
int
vmeTsi148DmaStart (int channel, uint32_t pci_addr, uint32_t vme_addr,
				 uint32_t n_bytes)
{
	BERegister *base = THEBASE;
...
uint32_t vmeTsi148DmaStatusXX (BERegister * base, int channel)
{
...
uint32_t vmeTsi148DmaStatus (int channel)
{
	BERegister *base = THEBASE;
...
static int
tsi_desc_setup (DmaDescriptor p,
				uint32_t attr_mask,
				uint32_t xfer_mode,
...
#ifdef DEBUG_MODULAR
void _cexpModuleInitialize (void *unused)
{
	vmeTsi148Init ();
	vmeTsi148Reset ();
}

int _cexpModuleFinalize (void *unused)
{
	int i;
	int rval = 1;
	void (*isrs[TSI_NUM_WIRES]) () =
	{
	isr_pin0, isr_pin1, isr_pin2, isr_pin3,};
