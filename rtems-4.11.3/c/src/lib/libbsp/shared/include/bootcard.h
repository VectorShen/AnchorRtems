...
static inline void bsp_work_area_initialize_default (void *area_begin,
												 uintptr_t area_size)
{
	Heap_Area area =
	{
...
#ifdef CONFIGURE_MALLOC_BSP_SUPPORTS_SBRK
	{
		...
		ptrdiff_t sbrk_amount = bsp_sbrk_init (&area,
											   work_space_size
											   + overhead
											   +
	...
	printk ("heap_start = 0x%p\n"
			"heap_size = %lu\n", area.begin, (unsigned long)area.size);
#endif

	RTEMS_Malloc_Initialize (&area, 1, NULL);
}

static inline void bsp_work_area_initialize_with_table (Heap_Area * areas,
														size_t area_count)
{
