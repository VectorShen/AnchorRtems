...
static void rtems_ata_lock (void)
{
...
static void rtems_ata_unlock (void)
{
...
static void ata_process_request (rtems_device_minor_number ctrl_minor);
...
#if ATA_DEBUG
				ata_printf
					("ata_io_data_request: type: WRITE: %lu, %lu cmd:%02x\n",
					 req->bufs[0].block, req->bufnum,
					 areq->regs.regs[IDE_REGISTER_COMMAND]);
#endif
			}
							 ...
							 ata_req_t * areq)
{
	...
	if (rtems_chain_has_only_one_node (&ata_ide_ctrls[ctrl_minor].reqs))
	{
		 ...
		 */
		ide_controller_read_register (ctrl_minor,
										IDE_REGISTER_ALTERNATE_STATUS, &val);
		...
		if (1)
			{
				...
				ide_controller_read_register (msg.ctrl_minor,
											IDE_REGISTER_STATUS, &byte);
...
static void ata_interrupt_on (const rtems_irq_connect_data * ptr)
{
	...
	ide_controller_write_register (0, IDE_REGISTER_DEVICE_CONTROL_OFFSET, 0x00);
}
...
static int ata_interrupt_isOn (const rtems_irq_connect_data * ptr)
{
	...
	ide_controller_read_register (0, IDE_REGISTER_DEVICE_CONTROL_OFFSET, &byte);
...
ata_pio_in_protocol (rtems_device_minor_number ctrl_minor, ata_req_t * areq)
{
	...
	if (areq->cnt)
	{
		...
		ide_controller_read_data_block (ctrl_minor,
										areq->breq->bufs[0].length *
										areq->cnt, areq->breq->bufs,
										&areq->cbuf, &areq->pos);
		ccbuf = areq->cbuf - ccbuf;
		areq->cnt -= ccbuf;
	}
 ...
 */
static rtems_task ata_queue_task (rtems_task_argument arg)
{
	ata_queue_msg_t msg;
	size_t size;
	ata_req_t *areq;
	...
	while (1)
	{
		...
		rc = rtems_message_queue_receive (ata_queue_id, &msg, &size,
		...
		_ISR_Disable (level);
		areq =
			(ata_req_t *) rtems_chain_first (&ata_ide_ctrls[ctrl_minor].reqs);
		_ISR_Enable (level);
 ...
 *     depend on 'cmd'
 */
static int ata_ioctl (rtems_disk_device * dd, uint32_t cmd, void *argp)
{
	...
	switch (cmd)
	{
		case RTEMS_BLKIO_REQUEST:
			status =
				ata_io_data_request (device, (rtems_blkdev_request *) argp);
			break;
...
static void ata_execute_device_diagnostic (rtems_device_minor_number ctrl_minor,
										 uint16_t * sector_buffer)
{
	...
	if (breq.req.status == RTEMS_SUCCESSFUL)
	{
		...
		if (ATA_DEV_INFO (ctrl_minor, 1).present)
			{
				uint16_t ec = 0;
				ide_controller_read_register (ctrl_minor, IDE_REGISTER_ERROR,
											&ec);
...
rtems_ata_initialize (rtems_device_major_number major,
					rtems_device_minor_number minor_arg, void *args)
{
	uint32_t ctrl_minor;
	rtems_status_code status;
	uint16_t *buffer;
	...
	status = rtems_semaphore_create (rtems_build_name ('A', 'T', 'A', 'L'),
									 1, RTEMS_ATA_LOCK_ATTRIBS, 0, &ata_lock);
	if (status != RTEMS_SUCCESSFUL)
		return status;

	/* create queue for asynchronous requests handling */
	status = rtems_message_queue_create (rtems_build_name ('A', 'T', 'A', 'Q'),
	 ...
	 * details
	 */
	status = rtems_task_create (rtems_build_name ('A', 'T', 'A', 'T'),
		...
		if (IDE_Controller_Table[ctrl_minor].status == IDE_CTRL_INITIALIZED)
	{
		...
		if (IDE_Controller_Table[ctrl_minor].int_driven == true)
		{
...
#if CPU_SIMPLE_VECTORED_INTERRUPTS == TRUE
			status = rtems_interrupt_catch (ata_interrupt_handler,
			...
			if (ata_irq_chain_use < 0)
			{
				...
				if (ata_irq_chain_cnt < ATA_IRQ_CHAIN_MAX_CNT)
				{
					...
					ata_irq_data.handle =
						(rtems_irq_hdl_param) ctrl_minor;

					status =
						((0 ==
						BSP_install_rtems_irq_handler
						(&ata_irq_data)) ? RTEMS_INVALID_NUMBER :
						 RTEMS_SUCCESSFUL);
				}
				else
				{
...
#if CPU_SIMPLE_VECTORED_INTERRUPTS == TRUE
			rtems_chain_append (&ata_int_vec
								[IDE_Controller_Table[ctrl_minor].
								 int_vec], &int_st->link);
#else
			rtems_chain_append (&
								(ata_irq_chain[ata_irq_chain_use].
								 irq_chain), &int_st->link);
#endif
		...
		for (dev = 0; dev < 2; dev++)
			...
			{
				status = ata_identify_device (ctrl_minor,
											dev,
											buffer,
											&ATA_DEV_INFO (ctrl_minor,
															 dev));
				if (status != RTEMS_SUCCESSFUL)
					continue;
				 ...
				 * by the controller
				 */
				status = ide_controller_config_io_speed (ctrl_minor,
														 ATA_DEV_INFO
														 (ctrl_minor,
														dev).
														 modes_available);
				...
				device = rtems_filesystem_make_dev_t (major,
													(ata_devs_number *
													 ATA_MINOR_NUM_RESERVED_PER_ATA_DEVICE));
				status =
					rtems_disk_create_phys (device, ATA_SECTOR_SIZE,
											ATA_DEV_INFO (ctrl_minor,
														dev).
											lba_avaible ?
											ATA_DEV_INFO (ctrl_minor,
				...
				if (status != RTEMS_SUCCESSFUL)
				{
					ata_devs[ata_devs_number].device =
						ATA_UNDEFINED_VALUE;
					continue;
				}
				ata_devs_number++;
