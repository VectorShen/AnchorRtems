 ...
 */
static rtems_isr dec21140Enet_interrupt_handler (struct dec21140_softc *sc)
{
...
static rtems_isr dec21140Enet_interrupt_handler_entry (void)
{
	int i;
...
static void dec21140Enet_initialize_hardware (struct dec21140_softc *sc)
{
...
#ifdef DEC_DEBUG
	printk
		("dec2114x : %02x:%02x:%02x:%02x:%02x:%02x   name '%s%d', io %x, mem %x, int %d\n",
		 sc->arpcom.ac_enaddr[0], sc->arpcom.ac_enaddr[1],
		 sc->arpcom.ac_enaddr[2], sc->arpcom.ac_enaddr[3],
		 sc->arpcom.ac_enaddr[4], sc->arpcom.ac_enaddr[5],
		 sc->arpcom.ac_if.if_name, sc->arpcom.ac_if.if_unit, sc->port,
		 (unsigned)sc->base, sc->irqInfo.name);
#endif
	...
	cp = (volatile unsigned char *)
		malloc (((sc->numRxbuffers + sc->numTxbuffers) * sizeof (struct MD)) +
...
#else
				pci_write_config_dword (pbus, pdev, pfun, 0x40,
...
#endif
#if defined(__PPC__)
	(void)pci_read_config_dword (pbus, pdev, pfun, PCI_BASE_ADDRESS_0, &lvalue);

	sc->port = lvalue & (unsigned int)(~IO_MASK);

	(void)pci_read_config_dword (pbus, pdev, pfun, PCI_BASE_ADDRESS_1, &lvalue);
