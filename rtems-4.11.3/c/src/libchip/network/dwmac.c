...
static int dwmac_if_mdio_busy_wait (const volatile uint32_t * gmii_address)
{
	...
	while ((*gmii_address & MACGRP_GMII_ADDRESS_GMII_BUSY) != 0 && i < timeout)
	  {
...
static int dwmac_if_mdio_read (int phy,
							   void *arg, unsigned phy_reg, uint32_t * val)
{
	int eno = 0;
	...
	if (phy == -1)
	  {
		  reg_value = MACGRP_GMII_ADDRESS_PHYSICAL_LAYER_ADDRESS_SET (reg_value,
																	  self->
																	  MDIO_BUS_ADDR);
	  }
	else
	...
	reg_value = MACGRP_GMII_ADDRESS_GMII_REGISTER_SET (reg_value, phy_reg);
	...
	reg_value = MACGRP_GMII_ADDRESS_CSR_CLOCK_RANGE_SET (reg_value,
														 self->csr_clock);
...
static int dwmac_if_mdio_write (int phy,
								void *arg, unsigned phy_reg, uint32_t val)
{
	int eno = 0;
	dwmac_common_context *self = (dwmac_common_context *) arg;
...
static int dwmac_get_phy_info (dwmac_common_context * self,
							   const uint8_t mdio_bus_addr,
							   uint32_t * oui,
							   uint8_t * model, uint8_t * revision)
{
	int eno = 0;
	uint32_t read;

	eno = dwmac_if_mdio_read (mdio_bus_addr, self, 2, &read);

	if (eno == 0)
	  {
		  *oui = 0 | (read << 6);
		  eno = dwmac_if_mdio_read (mdio_bus_addr, self, 3, &read);
...
static inline void dwmac_disable_irq_tx_transmitted (dwmac_common_context *
													 self)
{
...
static int dwmac_control_request (dwmac_common_context * self,
								  rtems_id task, rtems_event_set event)
{
	int eno = 0;
...
static bool dwmac_if_media_status (dwmac_common_context * self,
								   int *media, uint8_t phy_address)
{
...
static int dwmac_if_interface_stats (void *arg)
{
	...
	int media = 0;
	bool media_ok = dwmac_if_media_status (self, &media, self->MDIO_BUS_ADDR);
	uint32_t oui;
	uint8_t model;
	uint8_t revision;
	...
	if ((self->dmagrp->hw_feature & DMAGRP_HW_FEATURE_MMCSEL) != 0)
	  {
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes transmitted, exclusive of preamble and retried bytes, "
							   "in good and bad frames\n",
							   macgrp->txoctetcount_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames transmitted, exclusive of retried "
		  ...
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames transmitted with length 64 bytes, "
							   "exclusive of preamble and retried frames\n",
							   macgrp->tx64octets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames transmitted with length between 65 and "
							   "127 (inclusive) bytes, exclusive of preamble and retried frames\n",
							   macgrp->tx65to127octets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames transmitted with length between 128 and "
							   "255 (inclusive) bytes, exclusive of preamble and retried frames\n",
							   macgrp->tx128to255octets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames transmitted with length between 256 and "
							   "511 (inclusive) bytes, exclusive of preamble and retried frames\n",
							   macgrp->tx256to511octets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames transmitted with length between 512 and "
							   "1,023 (inclusive) bytes, exclusive of preamble and retried frames\n",
							   macgrp->tx512to1023octets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames transmitted with length between 1,024 and"
							   " maxsize (inclusive) bytes, exclusive of preamble and retried frames\n",
							   macgrp->tx1024tomaxoctets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad unicast frames transmitted\n",
							   macgrp->txunicastframes_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad multicast frames transmitted\n",
							   macgrp->txmulticastframes_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad broadcast frames transmitted\n",
							   macgrp->txbroadcastframes_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames aborted due to frame underflow error\n",
							   macgrp->txunderflowerror);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " successfully transmitted frames after a single collision in "
							   "Half-duplex mode\n", macgrp->txsinglecol_g);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " successfully transmitted frames after more than a single"
							   " collision in Half-duplex mode\n",
							   macgrp->txmulticol_g);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " successfully transmitted frames after a deferral in "
							   "Halfduplex mode\n", macgrp->txdeferred);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames aborted due to late collision error\n",
							   macgrp->txlatecol);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames aborted due to excessive (16) collision errors\n",
							   macgrp->txexesscol);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames aborted due to carrier sense error (no carrier or loss"
							   " of carrier)\n", macgrp->txcarriererr);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes transmitted, exclusive of preamble, in good frames "
							   "only\n", macgrp->txoctetcnt);
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " good frames transmitted\n",
							   macgrp->txframecount_g);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames aborted due to excessive deferral error (deferred for"
							   " more than two max-sized frame times)\n",
							   macgrp->txexcessdef);
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " good PAUSE frames transmitted\n",
							   macgrp->txpauseframes);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good VLAN frames transmitted, exclusive of retried frames\n",
							   macgrp->txvlanframes_g);
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " good and bad frames received\n",
							   macgrp->txoversize_g);
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " good and bad frames received\n",
							   macgrp->rxframecount_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received, exclusive of preamble, in good and bad "
							   "frames\n", macgrp->rxoctetcount_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received, exclusive of preamble, only in good frames\n",
							   macgrp->rxoctetcount_g);
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " good broadcast frames received\n",
							   macgrp->rxbroadcastframes_g);
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " good multicast frames received\n",
							   macgrp->rxmulticastframes_g);
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " frames received with CRC error\n",
							   macgrp->rxcrcerror);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames received with alignment (dribble) error. Valid only"
							   " in 10/100 mode\n", macgrp->rxalignmenterror);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames received with runt (<64 bytes and CRC error) error\n",
							   macgrp->rxrunterror);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " giant frames received with length (including CRC) greater "
							   "than %" PRIu16 " bytes (%" PRIu16
							   " bytes for VLAN tagged) and with CRC"
							   " error\n", macgrp->rxjabbererror,
							   giant_frame_size, giant_frame_size + 4);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames received with length less than 64 bytes, without any"
							   " errors\n", macgrp->rxundersize_g);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames received with length greater than the maxsize (1,518"
							   " or 1,522 for VLAN tagged frames), without errors\n",
							   macgrp->rxoversize_g);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames received with length 64 bytes, exclusive"
							   " of preamble\n", macgrp->rx64octets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames received with length between 65 and 127"
							   " (inclusive) bytes, exclusive of preamble\n",
							   macgrp->rx65to127octets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames received with length between 128 and 255"
							   " (inclusive) bytes, exclusive of preamble\n",
							   macgrp->rx128to255octets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames received with length between 256 and 511"
							   " (inclusive) bytes, exclusive of preamble\n",
							   macgrp->rx256to511octets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames received with length between 512 and "
							   "1,023 (inclusive) bytes, exclusive of preamble\n",
							   macgrp->rx512to1023octets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad frames received with length between 1,024 and"
							   " maxsize (inclusive) bytes, exclusive of preamble and retried frames\n",
							   macgrp->rx1024tomaxoctets_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " good unicast frames received\n",
							   macgrp->rxunicastframes_g);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames received with length error (length type field not "
							   "equal to frame size), for all frames with valid length field\n",
							   macgrp->rxlengtherror);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames received with length field not equal to the valid "
							   "frame size (greater than 1,500 but less than 1,536)\n",
							   macgrp->rxoutofrangetype);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and valid PAUSE frames received\n",
							   macgrp->rxpauseframes);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " missed received frames due to FIFO overflow\n",
							   macgrp->rxfifooverflow);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good and bad VLAN frames received\n",
							   macgrp->rxvlanframes_gb);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames received with error due to watchdog timeout error "
							   "(frames with a data load larger than 2,048 bytes)\n",
							   macgrp->rxwatchdogerror);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames received with Receive error or Frame Extension error"
							   " on the GMII or MII interface\n",
							   macgrp->rxrcverror);
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " received good control frames\n",
							   macgrp->rxctrlframes_g);

		  printf ("\n");

		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good IPv4 datagrams received with the TCP, UDP, or ICMP "
							   "payload\n", macgrp->rxipv4_gd_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " IPv4 datagrams received with header (checksum, length, or "
							   "version mismatch) errors\n",
							   macgrp->rxipv4_hdrerr_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " IPv4 datagram frames received that did not have a TCP, UDP, "
		  ...
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good IPv4 datagrams received that had a UDP payload with "
							   "checksum disabled\n",
							   macgrp->rxipv4_udsbl_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good IPv6 datagrams received with TCP, UDP, or ICMP "
							   "payloads\n", macgrp->rxipv6_gd_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " IPv6 datagrams received with header errors (length or "
							   "version mismatch)\n",
							   macgrp->rxipv6_hdrerr_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " IPv6 datagram frames received that did not have a TCP, UDP,"
							   " or ICMP payload. This includes all IPv6 datagrams with fragmentation"
							   " or security extension headers\n",
							   macgrp->rxipv6_nopay_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good IP datagrams with a good UDP payload. This counter is "
							   "not updated when the counter is incremented\n",
							   macgrp->rxudp_gd_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good IP datagrams whose UDP payload has a checksum error\n",
							   macgrp->rxudp_err_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good IP datagrams with a good TCP payload\n",
							   macgrp->rxtcp_gd_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good IP datagrams whose TCP payload has a checksum error\n",
							   macgrp->rxtcp_err_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good IP datagrams with a good ICMP payload\n",
							   macgrp->rxicmp_gd_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " good IP datagrams whose ICMP payload has a checksum error\n",
							   macgrp->rxicmp_err_frms);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in good IPv4 datagrams encapsulating TCP, UDP,"
							   " or ICMP data\n", macgrp->rxipv4_gd_octets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in IPv4 datagrams with header errors (checksum,"
		  ...
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in IPv4 datagrams that did not have a TCP, UDP"
							   ", or ICMP payload. The value in the IPv4 headers Length field is used"
							   " to update this counter\n",
							   macgrp->rxipv4_nopay_octets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in fragmented IPv4 datagrams. The value in the"
							   " IPv4 headers Length field is used to update this counter\n",
							   macgrp->rxipv4_frag_octets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in a UDP segment that had the UDP checksum "
							   "disabled. This counter does not count IP Header bytes\n",
							   macgrp->rxipv4_udsbl_octets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in good IPv6 datagrams encapsulating TCP, UDP "
							   "or ICMPv6 data\n", macgrp->rxipv6_gd_octets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in IPv6 datagrams with header errors (length, "
							   "version mismatch). The value in the IPv6 headers Length field is used "
							   "to update this counter\n",
							   macgrp->rxipv6_hdrerr_octets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in IPv6 datagrams that did not have a TCP, UDP"
							   ", or ICMP payload. The value in the IPv6 headers Length field is used "
							   "to update this counter\n",
							   macgrp->rxipv6_nopay_octets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in a good UDP segment. This counter does not "
							   "count IP header bytes\n",
							   macgrp->rxudp_gd_octets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in a UDP segment that had checksum errors\n",
							   macgrp->rxudp_err_octets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in a good TCP segment\n",
							   macgrp->rxtcp_gd_octets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in a TCP segment with checksum errors\n",
							   macgrp->rxtcperroctets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in a good ICMP segment\n",
							   macgrp->rxicmp_gd_octets);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " bytes received in an ICMP segment with checksum errors\n",
	...
	if (eno == 0)
	  {
		  ...
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " late collision when receing in half duplex mode\n",
							   self->stats.desc_status_counts_rx.
							   late_collision);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " giant frame or timestamp or checksum error\n",
		  ...
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " ethernet frames\n",
							   ...
							   ethernet_frames);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " destination address filter failures\n",
							   self->stats.desc_status_counts_rx.
							   dest_addr_fail);
		  ...
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " payload error\n",
							   ...
							   source_addr_fail);
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " MAC defers before transmission because of the presence of carrier\n",
		  ...
		  printf ("\nTX frame counts:\n");
		  DWMAC_PRINT_COUNTER ("%" PRIu32
							   " frames received from network stack\n",
		  ...
		  DWMAC_PRINT_COUNTER ("%" PRIu32 " packet buffers regained from DMA\n",
							   self->stats.frame_counts_tx.
							   packets_tranmitted_by_DMA);
...
#ifdef RTEMS_DEBUG
		  {
			  ...
			  (DESC_OPS->print_rx_desc) (self->dma_rx,
										 (unsigned int)self->bsd_config->
										 xbuf_count);

			  (DESC_OPS->print_tx_desc) (self->dma_tx,
										 (unsigned int)self->bsd_config->
										 xbuf_count);
		  }
#endif /* RTEMS_DEBUG */
...
static inline unsigned int dwmac_increment (const unsigned int value,
	...
	while (true)
	  {
		  ...
		  rtems_status_code sc =
			  rtems_bsdnet_event_receive (DWMAC_COMMON_EVENT_TASK_INIT |
		  ...
		  if ((events & DWMAC_COMMON_EVENT_TASK_INIT) != 0)
			{
				...
				for (index = 0; index <= INDEX_MAX; ++index)
				  {
					  self->mbuf_addr_rx[index] =
						  (DESC_OPS->alloc_data_buf) (self);
				...
				while ((DESC_OPS->am_i_rx_owner) (self, idx))
				  {
					  ...
					  unsigned int idx_next =
						  dwmac_increment (self->idx_rx, INDEX_MAX);
					  ...
					  if (status == DWMAC_COMMON_RX_FRAME_STATUS_DISCARD
						...
						{
							DWMAC_PRINT_DBG ("rx discarded: %02u\n", idx);
							++counts->errors;
						...
						{
							DWMAC_PRINT_DBG ("rx dropped llc snap: %02u\n",
											 idx);
							++counts->dropped;
					  ...
					  else
						{
							...
							if (p_m_new != NULL)
								  ...
								  if (p_m != NULL)
									{
										...
										p_m->m_pkthdr.len = sz;
										p_m->m_data =
											mtod (p_m, char *) + ETHER_HDR_LEN;

										DWMAC_COMMON_DSB ();

										DWMAC_PRINT_DBG ("rx: %02u: %u %s%s\n",
														 idx,
														 segment_size,
														 (is_first_seg) ? ("F")
														 : (""),
										...
										ether_input (&self->arpcom.ac_if, eh,
													 p_m);
							...
							else
							  {
								  ...
								  (DESC_OPS->init_rx_desc) (self, idx);
								  DWMAC_PRINT_DBG ("rx dropped: %02u\n", idx);
							  }
						}
					  ...
					  if ((self->dmagrp->status & DMAGRP_STATUS_RU) != 0)
						{
							...
							DWMAC_PRINT_DBG ("rx DMA restart: %02u\n", idx);
...
static struct mbuf *dwmac_next_fragment (struct ifnet *ifp,
										 struct mbuf *m,
										 bool * is_first,
										 bool * is_last, size_t * size)
{
	struct mbuf *n = NULL;
	int sz = 0;
				...
				if ((events & DWMAC_COMMON_EVENT_TX_PHY_STATUS_CHANGE) != 0)
				  {
					  ...
					  eno = CALLBACK->phy_events_get (self->arg, &phy_events);
							...
							if ((phy_events & PHY_EVENT_LINK_UP) != 0)
							  {
								  ...
								  (void)
									  dwmac_update_autonegotiation_params
									  (self);
							  }
						}
				...
				if ((events & DWMAC_COMMON_EVENT_TX_FRAME_TRANSMITTED) != 0)
				  {
					  ...
					  dwmac_common_tx_frame_counts *counts =
						  &self->stats.frame_counts_tx;
					  dwmac_disable_irq_tx_transmitted (self);

					  /* Next index to be transmitted */
					  unsigned int idx_transmitted_next =
						  dwmac_increment (idx_transmitted, INDEX_MAX);
						  ...
						  && (DESC_OPS->am_i_tx_owner) (self, idx_release))
						{
							while (idx_release != idx_transmitted_next
																 ...
																 idx_release))
							  {
								  ...
								  DWMAC_PRINT_DBG ("tx: release %u\n",
												   idx_release);

								  /* Release the DMA descriptor */
								  (DESC_OPS->release_tx_desc) (self,
															   idx_release);
								  ...
								  /* Next release index */
								  idx_release =
									  dwmac_increment (idx_release, INDEX_MAX);
							  }
							if ((self->arpcom.ac_if.if_flags & IFF_OACTIVE) !=
								0)
							  {
				...
				if ((events & DWMAC_COMMON_EVENT_TX_TRANSMIT_FRAME) != 0)
				  {
					  ...
					  while (true)
						{
							unsigned int idx =
								dwmac_increment (idx_transmit, INDEX_MAX);

							p_m = dwmac_next_fragment (&self->arpcom.ac_if,
													   p_m,
													   &is_first,
													   &is_last, &size);
							...
							if (p_m != NULL)
							  {
								  ...
								  rtems_cache_flush_multiple_data_lines (mtod
																		 (p_m,
																		  const
																		  void
																		  *),
								  ...
								  if (is_first)
								  ...
								  else
									{
										...
										(DESC_OPS->release_tx_ownership) (self,
																		  idx_transmit);
									}
...
static void dwmac_if_set_promiscous_mode (dwmac_common_context * self,
										  const bool enabled)
{
	if (enabled)
	  {
...
{
	uint32_t pbl = DWMAC_DMA_BUS_MODE_PBL_DEFAULT;
	uint32_t fixed_burst = DWMAC_DMA_BUS_MODE_FB_DEFAULT;
	uint32_t mixed_burst = DWMAC_DMA_BUS_MODE_MIXED_BURSTS_DEFAULT;
	uint32_t burst_len_4_support = DWMAC_DMA_AXI_BURST_LENGTH_4_DEFAULT;
	uint32_t burst_len_8_support = DWMAC_DMA_AXI_BURST_LENGTH_8_DEFAULT;
	uint32_t burst_len_16_support = DWMAC_DMA_AXI_BURST_LENGTH_16_DEFAULT;
	uint32_t burst_boundary = DWMAC_DMA_AXI_BURST_BOUNDARY_DEFAULT;
	const dwmac_common_dma_ops *DMA_OPS =
		(const dwmac_common_dma_ops *)self->CFG->MAC_OPS->dma;
	const dwmac_common_desc_ops *DESC_OPS =
		(const dwmac_common_desc_ops *)self->CFG->DESC_OPS->ops;
	...
	return (DMA_OPS->init) (self,
							...
							burst_boundary, &self->dma_tx[0], &self->dma_rx[0]);
}
...
static int dwmac_if_up_or_down (dwmac_common_context * self, const bool up)
{
	int eno = 0;
	rtems_status_code sc = RTEMS_SUCCESSFUL;
	struct ifnet *ifp = &self->arpcom.ac_if;
	...
	if (up && self->state == DWMAC_COMMON_STATE_DOWN)
	  {
		  ...
		  if (eno == 0)
			{
				eno = CALLBACK->phy_event_enable (self->arg,
												  PHY_EVENT_LINK_DOWN
												  | PHY_EVENT_LINK_UP);
			}
		  ...
		  if (eno == 0)
			{
				...
				sc = rtems_interrupt_handler_install (self->CFG->IRQ_EMAC,
													  "Ethernet",
		  ...
		  if (eno == 0)
			{
				...
				eno =
					dwmac_control_request (self, self->task_id_tx,
		  ...
		  if (eno == 0)
			{
				eno =
					dwmac_control_request (self, self->task_id_rx,
										   DWMAC_COMMON_EVENT_TASK_INIT);
			}
		  ...
		  if (eno == 0)
			...
			{
				ifp->if_flags &= ~IFF_UP;
				(void)dwmac_control_request (self, self->task_id_tx,
											 DWMAC_COMMON_EVENT_TASK_STOP);
				(void)dwmac_control_request (self, self->task_id_rx,
											 DWMAC_COMMON_EVENT_TASK_STOP);
#ifdef BSP_FEATURE_IRQ_EXTENSION
				(void)rtems_interrupt_handler_remove (self->CFG->IRQ_EMAC,
													  dwmac_core_dma_interrupt,
													  self);
#endif
	...
	else if (!up && self->state == DWMAC_COMMON_STATE_UP)
	  {
		  ...
		  if (eno == 0)
			{
#ifdef BSP_FEATURE_IRQ_EXTENSION
				sc = rtems_interrupt_handler_remove (self->CFG->IRQ_EMAC,
													 dwmac_core_dma_interrupt,
													 self);
#else
				sc = RTEMS_NOT_IMPLEMENTED;
...
static int dwmac_if_interface_ioctl (struct ifnet *ifp,
									 ioctl_command_t cmd, caddr_t data)
{
	...
	switch (cmd)
	  {
		  ...
		  case SIOCSIFFLAGS:
			  ...
			  if (eno == 0 && (ifp->if_flags & IFF_UP) != 0)
				{
					dwmac_if_set_promiscous_mode (self,
												  (ifp->
...
static int dwmac_if_clk_csr_set (const uint32_t gmii_clk_rate,
...
static int dwmac_fixup_unit_count (const dwmac_common_context * self,
								   const int count,
								   const int default_value, const int max)
{
	int ret = count;
...
static int dwmac_if_attach (dwmac_common_context * self,
							struct rtems_bsdnet_ifconfig *bsd_config,
							const dwmac_cfg * driver_config, void *arg)
{
	int eno = 0;
	struct ifnet *ifp = &self->arpcom.ac_if;
	char *unit_name = NULL;
	int unit_number = rtems_bsdnet_parse_driver_name (bsd_config,
													  &unit_name);
	...
	if (self == NULL
		|| bsd_config == NULL || driver_config == NULL || CALLBACK == NULL)
	  {
		  eno = EINVAL;
	  }
	else
	...
	if (eno == 0)
	  {
		  eno = dwmac_if_clk_csr_set (driver_config->GMII_CLK_RATE, self);
	  }
		  ...
		  if (eno == 0)
			{
				...
				/* Receive unit count */
				bsd_config->rbuf_count = dwmac_fixup_unit_count (self,
																 bsd_config->
																 rbuf_count,
				...
				/* Transmit unit count */
				bsd_config->xbuf_count = dwmac_fixup_unit_count (self,
																 bsd_config->
																 xbuf_count,
																 DWMAC_CONFIG_TX_UNIT_COUNT_DEFAULT,
																 DWMAC_CONFIG_TX_UNIT_COUNT_MAX);

				if (DWMAC_GLOBAL_MBUF_CNT / 4 < bsd_config->rbuf_count
				...
				/* Copy MAC address */
				memcpy (self->arpcom.ac_enaddr,
						bsd_config->hardware_address, ETHER_ADDR_LEN);
		  ...
		  if (eno == 0)
			{
				...
				self->task_id_rx = rtems_bsdnet_newproc ("ntrx",
														 4096,
														 dwmac_task_rx, self);
				assert (self->task_id_rx != RTEMS_ID_NONE);
		  ...
		  if (eno == 0)
			{
				...
				self->task_id_tx = rtems_bsdnet_newproc ("nttx",
														 4096,
														 dwmac_task_tx, self);
				assert (self->task_id_tx != RTEMS_ID_NONE);
...
void *dwmac_network_if_attach_detach (struct rtems_bsdnet_ifconfig *bsd_config,
									  const dwmac_cfg * driver_config,
									  void *arg, int attaching)
{
	int eno = 0;
	...
	if (attaching)
	  {
		  ...
		  if (context != NULL)
			{
				eno = dwmac_if_attach (context, bsd_config, driver_config, arg);
...
int dwmac_if_read_from_phy (void *arg, const unsigned phy_reg, uint16_t * val)
{
	int eno = EINVAL;
	dwmac_common_context *self = (dwmac_common_context *) arg;
	uint32_t value;
	...
	if (arg != NULL)
	  {
		  eno = dwmac_if_mdio_read (self->MDIO_BUS_ADDR, self, phy_reg, &value);
	  }
...
int dwmac_if_write_to_phy (void *arg,
						   const unsigned phy_reg, const uint16_t val)
{
	int eno = EINVAL;
	dwmac_common_context *self = (dwmac_common_context *) arg;
	...
	if (arg != NULL)
	  {
		  eno = dwmac_if_mdio_write (self->MDIO_BUS_ADDR, self, phy_reg, val);
	  }
