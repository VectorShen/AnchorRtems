...
{
	unsigned short val;
	debug_regs_pair *dbg = debug_regs;
	int c;
	uint32_t Irql;
		  ...
		  if (c == regno)
			{
				db_printf ("%swrite reg [%d:%x] <- 0x%04x (%-20s)\n",
						   dbg_prefix, regno >> 3, (regno & 0x07) * 2, val,
						   dbg->name);
				break;
			}
		  dbg++;
...
put_data8 (struct lan91cxx_priv_data *cpd, unsigned char val)
{
	...
	HAL_WRITE_UINT8 (((unsigned char *)(cpd->base + ((LAN91CXX_DATA & 0x7)))) +
					 1, val);
...
/* Assumes bank2 has been selected*/
static __inline__ rxd_t get_data (struct lan91cxx_priv_data *cpd)
{
	rxd_t val;
...
static __inline__ unsigned short get_banksel (struct lan91cxx_priv_data *cpd)
{
	unsigned short val;
