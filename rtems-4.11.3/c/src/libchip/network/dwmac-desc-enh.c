...
static void dwmac_desc_enh_rx_set_on_ring_chain (volatile dwmac_desc_ext * p,
												 int end)
{
	/* For simplicity reasons we will not use the second buffer.
	 * If we would use it we would have to set the size to MCLBYTES -1 */
	p->erx.des0_3.des1 =
		DWMAC_DESC_ERX_DES1_RECEIVE_BUFF_2_SIZE_SET (p->erx.des0_3.des1, 0);
...
static void dwmac_desc_enh_tx_set_on_ring_chain (volatile dwmac_desc_ext * p,
												 const bool end)
{
	if (end)
...
static void dwmac_desc_enh_set_tx_desc_len (volatile dwmac_desc_ext * p_enh,
											size_t len)
{
	p_enh->etx.des0_3.des1 =
		DWMAC_DESC_ETX_DES1_TRANSMIT_BUFFER_1_SIZE_SET (p_enh->etx.des0_3.des1,
														len);
}
...
{
	return (DWMAC_DESC_EXT_ERX_DES4_IP_PAYLOAD_TYPE_GET (des4)
			== DWMAC_IP_PAYLOAD_TYPE_UDP);
}
...
static dwmac_common_rx_frame_status dwmac_desc_enh_coe_status (volatile
															   dwmac_desc_ext *
															   p_enh)
{
	...
	if ((DES0 & DWMAC_DESC_ERX_DES0_EXT_STATUS_AVAIL_OR_RX_MAC_ADDR_STATUS)
		  ...
		  if (!dwmac_desc_enh_is_giant_frame (DES0)
			  ...
			  && (DES4 & DWMAC_DESC_EXT_ERX_DES4_IP_HEADER_ERROR) == 0)
			{
				...
				ret = DWMAC_COMMON_RX_FRAME_STATUS_LLC_SNAP;
					...
					!= 0) && dwmac_desc_enh_is_giant_frame (DES0))
			{
				DWMAC_DESC_ENH_PRINT_DBG
	...
	else
	  {
		  uint32_t status = ((uint32_t)
							 (dwmac_desc_enh_is_giant_frame (DES0) << 2U) |
							 (uint32_t) (((DES0 &
...
static int dwmac_desc_enh_get_tx_status (dwmac_common_context * self,
										 const unsigned int idx_tx)
{
	int ret = 0;
	volatile dwmac_desc_ext *dma_tx = (volatile dwmac_desc_ext *)self->dma_tx;
	...
	if ((p_desc->etx.des0_3.des0 & DWMAC_DESC_ETX_DES0_ERROR_SUMMARY) != 0)
	  {
		  ...
		  if ((p_desc->etx.des0_3.des0 & DWMAC_DESC_ETX_DES0_NO_CARRIER) != 0)
			{
				DWMAC_DESC_ENH_PRINT_DBG ("\tno_carrier error\n");
				++counts->no_carrier;
			}

		  if ((p_desc->etx.des0_3.
			   des0 & DWMAC_DESC_ETX_DES0_EXCESSIVE_COLLISION) != 0)
			{
...
static dwmac_common_rx_frame_status
dwmac_desc_enh_get_rx_status (dwmac_common_context * self,
							  const unsigned int desc_idx)
{
	dwmac_common_desc_status_counts_rx *counts =
		&self->stats.desc_status_counts_rx;
	dwmac_common_rx_frame_status ret = DWMAC_COMMON_RX_FRAME_STATUS_GOOD;
	volatile dwmac_desc_ext *dma_rx = (volatile dwmac_desc_ext *)self->dma_rx;
	...
	if ((DES0 & DWMAC_DESC_ERX_DES0_ERROR_SUMMARY) != 0)
	  {
		  ...
		  if ((DES0 & DWMAC_DESC_ERX_DES0_RECEIVE_WATCHDOG_TIMEOUT) != 0)
			{
...
static void dwmac_desc_enh_print_tx_desc (volatile dwmac_desc * p,
		  ...
		  for (index = 0; index < count; ++index)
			{
				...
				printf (" %u own bit\n"
						...
						" %lu Collision Count\n"
						" %u Deferred Bit\n",
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_OWN_BIT) != 0,
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_IRQ_ON_COMPLETION) != 0,
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_LAST_SEGMENT) != 0,
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_FIRST_SEGMENT) != 0,
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_DISABLE_CRC) != 0,
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_DISABLE_PAD) != 0,
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_TRANSMIT_TIMESTAMP_ENABLE)
						!= 0,
						DWMAC_DESC_ETX_DES0_CHECKSUM_INSERTION_CONTROL_GET
						(p_enh[index].etx.des0_3.des0),
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_TRANSMIT_END_OF_RING) != 0,
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_SECOND_ADDR_CHAINED) != 0,
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_TRANSMIT_TIMESTAMP_STATUS)
						!= 0,
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_IP_HEADER_ERROR) != 0,
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_VLAN_FRAME) != 0,
						DWMAC_DESC_ETX_DES0_COLLISION_COUNT_GET (p_enh[index].
																 etx.des0_3.
																 des0),
						(p_enh[index].etx.des0_3.
						 des0 & DWMAC_DESC_ETX_DES0_DEFERRED_BIT) != 0);
				...
				printf (" %lu Transmit Buffer 2 Size\n"
						...
						DWMAC_DESC_ETX_DES1_TRANSMIT_BUFFER_2_SIZE_GET (p_enh
																		[index].
																		etx.
																		des0_3.des1),
						DWMAC_DESC_ETX_DES1_TRANSMIT_BUFFER_1_SIZE_GET (p_enh
																		[index].
																		etx.
																		des0_3.des1));
				printf ("des2\n");
...
static void dwmac_desc_enh_print_rx_desc (volatile dwmac_desc * p,
										  const unsigned int count)
{
	volatile dwmac_desc_ext *p_enh = (volatile dwmac_desc_ext *)p;
	unsigned int index;
		  ...
		  for (index = 0; index < count; ++index)
			{
				...
				printf (" %u Own Bit\n"
						...
						(p_enh[index].erx.des0_3.
						 des0 & DWMAC_DESC_ERX_DES0_OWN_BIT) != 0,
						(p_enh[index].erx.des0_3.
						 des0 & DWMAC_DESC_ERX_DES0_DEST_ADDR_FILTER_FAIL) != 0,
						DWMAC_DESC_ERX_DES0_FRAME_LENGTH_GET (p_enh[index].erx.
															  des0_3.des0),
						(p_enh[index].erx.des0_3.
						 des0 & DWMAC_DESC_ERX_DES0_SRC_ADDR_FILTER_FAIL) != 0,
						(p_enh[index].erx.des0_3.
						 des0 & DWMAC_DESC_ERX_DES0_LENGTH_ERROR) != 0,
						(p_enh[index].erx.des0_3.
						 des0 & DWMAC_DESC_ERX_DES0_VLAN_TAG) != 0,
						(p_enh[index].erx.des0_3.
						 des0 & DWMAC_DESC_ERX_DES0_FIRST_DESCRIPTOR) != 0,
						(p_enh[index].erx.des0_3.
						 des0 & DWMAC_DESC_ERX_DES0_LAST_DESCRIPTOR) != 0,
						(p_enh[index].erx.des0_3.
						 des0 & DWMAC_DESC_ERX_DES0_FREAME_TYPE) != 0,
						(p_enh[index].erx.des0_3.
						 des0 & DWMAC_DESC_ERX_DES0_DRIBBLE_BIT_ERROR) != 0,
						(p_enh[index].erx.des0_3.
						 des0 &
				...
				if ((p_enh[index].erx.des0_3.des0
					 & DWMAC_DESC_ERX_DES0_ERROR_SUMMARY) != 0)
				  {
					  ...
					  if ((p_enh[index].erx.des0_3.
						   des0 & DWMAC_DESC_ERX_DES0_CRC_ERROR) != 0)
						{
							printf ("  CRC Error\n");
						}
				  }
				...
				printf (" %u Disable Interrupt on Completion\n"
						...
						DWMAC_DESC_ERX_DES1_RECEIVE_BUFF_2_SIZE_GET (p_enh
																	 [index].
																	 erx.
																	 des0_3.des1),
						(p_enh[index].erx.des0_3.
						 des1 & DWMAC_DESC_ERX_DES1_RECEIVE_END_OF_RING) != 0,
						(p_enh[index].erx.des0_3.
						 des1 & DWMAC_DESC_ERX_DES1_SECOND_ADDR_CHAINED) != 0,
						DWMAC_DESC_ERX_DES1_RECEIVE_BUFF_1_SIZE_GET (p_enh
																	 [index].
																	 erx.
																	 des0_3.des1));
				printf ("des2\n");
...
static int dwmac_desc_enh_create_rx_desc (dwmac_common_context * self)
{
	...
	if (eno == 0)
	  {
		  eno = (self->CFG->CALLBACK.mem_alloc_nocache) (self->arg,
														 &desc_mem, SIZE_DESCS);
	  }
...
static void dwmac_desc_enh_init_rx_desc (dwmac_common_context * self,
										 const unsigned int index)
{
	volatile dwmac_desc_ext *p_enh = (volatile dwmac_desc_ext *)self->dma_rx;
	const size_t NUM_DESCS = (size_t) self->bsd_config->rbuf_count;
	char *clust_start = mtod (self->mbuf_addr_rx[index], char *);

	assert (NULL != p_enh);

	DWMAC_COMMON_DSB ();

	rtems_cache_invalidate_multiple_data_lines (clust_start,
	...
	if (self->mbuf_addr_rx[index] != NULL)
	  {
		  p_enh[index].erx.des0_3.des1 =
			  DWMAC_DESC_ERX_DES1_RECEIVE_BUFF_1_SIZE_SET (p_enh->erx.des0_3.
														   des1,
														   DWMAC_DESC_COM_BUF_SIZE);
	  }
	else
...
static int dwmac_desc_enh_destroy_rx_desc (dwmac_common_context * self)
{
	...
	if (dma_rx != NULL)
	  {
		  eno = self->CFG->CALLBACK.mem_free_nocache (self->arg,
													  RTEMS_DEVOLATILE (void *,
																		dma_rx));
		  self->dma_rx = NULL;
	  }
		  ...
		  if (p_enh[i].erx.des0_3.des2 != 0)
			{
				...
				(void)dummy;
				memset (RTEMS_DEVOLATILE (void *, &p_enh[i].erx),
						0, sizeof (dwmac_desc_ext));
			}
	  }
...
static inline size_t dwmac_desc_enh_get_rx_frame_len (dwmac_common_context *
													  self,
													  const unsigned int
													  desc_idx)
{
	...
	if (self->dmagrp->hw_feature & DMAGRP_HW_FEATURE_RXTYP1COE)
	  {
		  return DWMAC_DESC_ERX_DES0_FRAME_LENGTH_GET (p_enh[desc_idx].erx.
													   des0_3.des0) - 2U;
	  }
	else
	  {
...
static bool dwmac_desc_enh_am_i_rx_owner (dwmac_common_context * self,
										  const unsigned int desc_idx)
{
	volatile dwmac_desc_ext *p_enh = (volatile dwmac_desc_ext *)self->dma_rx;
	bool am_i_owner;
...
static bool dwmac_desc_enh_am_i_tx_owner (dwmac_common_context * self,
										  const unsigned int idx_tx)
{
	volatile dwmac_desc_ext *p_enh = (volatile dwmac_desc_ext *)self->dma_tx;
	bool am_i_owner;
...
static void dwmac_desc_enh_release_tx_ownership (dwmac_common_context * self,
												 const unsigned int idx_tx)
{
	volatile dwmac_desc_ext *p_enh = (volatile dwmac_desc_ext *)self->dma_tx;
...
static int dwmac_desc_enh_get_tx_ls (dwmac_common_context * self,
									 const unsigned int idx_tx)
{
	volatile dwmac_desc_ext *p_enh = (volatile dwmac_desc_ext *)self->dma_tx;
...
static void dwmac_desc_enh_release_tx_desc (dwmac_common_context * self,
											const unsigned int idx_tx)
{
	volatile dwmac_desc_ext *p_enh = (volatile dwmac_desc_ext *)self->dma_tx;
...
static void dwmac_desc_enh_prepare_tx_desc (dwmac_common_context * self,
											const unsigned int idx,
											const bool is_first,
											const size_t len, const void *pdata)
{
	volatile dwmac_desc_ext *p_enh = (volatile dwmac_desc_ext *)self->dma_tx;
...
static void dwmac_desc_enh_close_tx_desc (dwmac_common_context * self,
										  const unsigned int idx_tx)
{
	volatile dwmac_desc_ext *p_enh = (volatile dwmac_desc_ext *)self->dma_tx;
...
static bool dwmac_desc_enh_is_first_rx_segment (dwmac_common_context * self,
												const unsigned int
												descriptor_index)
{
...
static bool dwmac_desc_enh_is_last_rx_segment (dwmac_common_context * self,
											   const unsigned int
											   descriptor_index)
{
	volatile dwmac_desc_ext *p_descs = (volatile dwmac_desc_ext *)self->dma_rx;
...
DWMAC_DESCRIPTOR_OPS_INITIALIZER (&dwmac_desc_ops_enhanced);
