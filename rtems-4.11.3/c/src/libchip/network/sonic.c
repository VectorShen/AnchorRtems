...
void sonic_print_tx_descriptor (TransmitDescriptorPointer_t tdp)
{
	printf ("TXD ==> %p", tdp);
...
void sonic_print_rx_descriptor (ReceiveDescriptorPointer_t rdp)
{
	printf ("RXD ==> %p\n", rdp);
...
static void sonic_enable_interrupts (struct sonic_softc *sc, uint32_t mask)
{
	void *rp = sc->sonic;
	rtems_interrupt_level level;

	rtems_interrupt_disable (level);
	(*sc->write_register) (rp,
						   SONIC_REG_IMR,
						   (*sc->read_register) (rp, SONIC_REG_IMR) | mask);
	rtems_interrupt_enable (level);
}

static void sonic_disable_interrupts (struct sonic_softc *sc, uint32_t mask)
{
	void *rp = sc->sonic;
	rtems_interrupt_level level;
...
static void sonic_clear_interrupts (struct sonic_softc *sc, uint32_t mask)
{
	void *rp = sc->sonic;
	rtems_interrupt_level level;
...
static void sonic_command (struct sonic_softc *sc, uint32_t mask)
{
	void *rp = sc->sonic;
	rtems_interrupt_level level;
	...
	if (imr & isr & (IMR_PINTEN | IMR_TXEREN))
	  {
		  ...
		  (*sc->write_register) (rp, SONIC_REG_ISR,
								 ISR_PINT | ISR_TXDN | ISR_TXER);
	  }
	...
	while ((status = sc->tdaTail->status) != 0)
	  {
		  ...
		  if ((status & ~(TDA_STATUS_COLLISION_MASK | TDA_STATUS_DEF)) !=
			  0x0001)
		  ...
		  if (status & (TDA_STATUS_EXD |
						TDA_STATUS_EXC | TDA_STATUS_FU | TDA_STATUS_BCM))
			{
				/*
		   ...
		   */
		  collisions =
			  (status & TDA_STATUS_COLLISION_MASK) >>
		  ...
		  if (m->m_len)
			{
				...
				printf ("fp %p 0x%04x%04x %d=%d .. %d\n",
						fp, fp->frag_msw, fp->frag_lsw, fp->frag_size, m->m_len,
						packetSize);
#endif
#if (SONIC_DEBUG & SONIC_DEBUG_DUMP_TX_MBUFS)
				rtems_print_buffer (p,
									(fp->frag_size >
...
SONIC_STATIC void sonic_rda_wait (struct sonic_softc *sc,
				...
				for (i = 0; i < 2; i++)
				  {
					  if ((*sc->read_register) (rp, SONIC_REG_RRP) ==
						  ...
						  (*sc->write_register) (rp,
												 ...
												 (*sc->read_register) (rp,
																	   SONIC_REG_REA));
					  (*sc->write_register) (rp, SONIC_REG_RRP,
											 (*sc->read_register) (rp,
																   SONIC_REG_RRP)
...
*/
	...
	sc->rda = sonic_allocate ((sc->rdaCount * sizeof (ReceiveDescriptor_t)) +
							  sizeof (CamDescriptor_t));
	sc->cdp = (CamDescriptorPointer_t) ((unsigned char *)sc->rda +
										(sc->rdaCount *
										 sizeof (ReceiveDescriptor_t)));
#if (SONIC_DEBUG & SONIC_DEBUG_MEMORY)
	printf ("rda area = %p\n", sc->rda);
	printf ("cdp area = %p\n", sc->cdp);
#endif
	...
	sc->rsa =
		sonic_allocate ((sc->rdaCount + RRA_EXTRA_COUNT) * sizeof *sc->rsa);
#if (SONIC_DEBUG & SONIC_DEBUG_MEMORY)
	printf ("rsa area = %p\n", sc->rsa);
#endif
	 ...
	 */
	(*sc->write_register) (rp, SONIC_REG_CR,
						   CR_RST | CR_STP | CR_RXDIS | CR_HTX);
	...
	if (sc->txDaemonTid == 0)
	  {
		   ...
		   */
		  sc->rxDaemonTid =
			  rtems_bsdnet_newproc ("SNrx", 4096, sonic_rxDaemon, sc);
		  sc->txDaemonTid =
			  rtems_bsdnet_newproc ("SNtx", 4096, sonic_txDaemon, sc);
	  }
