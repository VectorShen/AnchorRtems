	...
	for (n = m; n; n = n->m_next)
	  {
		  ...
		  if (data)
			...
			{					/* must actively discard ie. read it from the chip anyway. */
				while (mlen >= sizeof (*data))
				  {
					  (void)get_data (cpd);
					  mlen -= sizeof (*data);
...
static void sendpacket (struct ifnet *ifp, struct mbuf *m)
{
	...
	put_reg (cpd, LAN91CXX_INTERRUPT, ints | LAN91CXX_INTERRUPT_TX_EMPTY_INT);
...
static void lan91cxx_finish_sent (struct lan91cxx_priv_data *cpd)
{
...

	{
		...
		cpd->stats.tx_total_collisions =
			...
			cpd->stats.tx_mult_collisions + cpd->stats.tx_single_collisions;
 ...
 */
static int smc91111_ioctl (struct ifnet *ifp, ioctl_command_t command,
						   caddr_t data)
{
	struct lan91cxx_priv_data *cpd = ifp->if_softc;
	int error = 0;
...
int lan91cxx_hardware_init (struct lan91cxx_priv_data *cpd)
{
...
#ifdef BSP_FEATURE_IRQ_EXTENSION
	{
		...
		sc = rtems_interrupt_handler_install (cpd->config.vector,
											  cpd->config.info,
											  cpd->config.options,
	...
	rc = rtems_interrupt_handler_install (cpd->config.vector, "smc91cxx",
										  ...
										  lan91cxx_interrupt_handler, cpd);
...
static void lan91cxx_start (struct ifnet *ifp)
{
	...
	put_reg (cpd, LAN91CXX_RCR,
			 LAN91CXX_RCR_STRIP_CRC | LAN91CXX_RCR_RXEN | LAN91CXX_RCR_ALMUL);
...
static int smc_probe (struct lan91cxx_priv_data *cpd)
{
...
#if SMC_DEBUG > 3
	{
		...
		db_printf ("smc_probe:Bank read as a 16 bit value:0x%04x\n", bank16);
		db_printf ("smc_probe:Bank read as an 8 bit value:0x%02x\n", bank16_0);
		db_printf
...
static void lan91cxx_phy_configure (struct lan91cxx_priv_data *cpd)
{
	...
	while (delay--)
	  {
		  if (!(lan91cxx_read_phy (cpd, 0, PHY_CNTL_REG) & PHY_CNTL_RST))
			{
				break;
			}
		  HAL_DELAY_US (100000);
	...
	/* give it ~5 seconds before giving up (no cable?) */
	delay = 50;
	while (!((status = lan91cxx_read_phy (cpd, 0, LAN91CXX_PHY_STAT)) & 0x20))
	  {
		  if (--delay <= 0)
			{
				printf ("Timeout autonegotiation\n");
	...
	for (i = 0; i < sizeof (bits); ++i)
	  {
		  ...
		  /* Clock Hi - input data */
		  put_reg (cpd, LAN91CXX_MGMT, mii_reg | bits[i] | LAN91CXX_MGMT_MCLK);
		  HAL_DELAY_US (50);
