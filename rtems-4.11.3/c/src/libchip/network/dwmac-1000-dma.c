...
static int dwmac1000_dma_init (dwmac_common_context * self,
							   const uint32_t pbl,
							   const uint32_t fb,
							   const uint32_t mb,
	...
	if (limit < 0)
	...
	else
	  {
		  ...
		  if (pbl >= DWMAC_DMA_CFG_BUS_MODE_BURST_LENGTH_8)
			{
				value =
					DMAGRP_BUS_MODE_PBL ((pbl + 1) / 8) | DMAGRP_BUS_MODE_RPBL ((pbl + 1) / 8) | DMAGRP_BUS_MODE_EIGHTXPBL;
			}
		  else
			{
				value =
					DMAGRP_BUS_MODE_PBL (pbl + 1) | DMAGRP_BUS_MODE_RPBL (pbl +
																		  1);
			}
...
static void dwmac1000_dma_operation_mode (dwmac_common_context * self,
										  ...
										  const unsigned int rxmode)
{
	...
	if (txmode == DWMAC_COMMON_DMA_MODE_STORE_AND_FORWARD)
	...
	else
	  {
		  ...
		  if (txmode <= 32)
			{
				value |= DMAGRP_OPERATION_MODE_TTC_SET (value,
	...
	if (rxmode == DWMAC_COMMON_DMA_MODE_STORE_AND_FORWARD)
	...
	else
	  {
		  ...
		  if (rxmode <= 32)
			{
				value = DMAGRP_OPERATION_MODE_RTC_SET (value,
