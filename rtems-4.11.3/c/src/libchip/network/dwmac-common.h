...
/* Foreward declarations */
	...
	struct dwmac_common_core_ops
	{
		...
		void (*set_hash_filter) (dwmac_common_context * self,
								 const bool add, struct ifreq * ifr);

		/* Set/Get Unicast MAC addresses */
		void (*set_umac_addr) (dwmac_common_context * ioaddr,
	...
	struct dwmac_common_dma_ops
	{
		...
		int (*init) (dwmac_common_context * self,
					 const uint32_t pbl,
					 const uint32_t fb,
					 const uint32_t mb,
		...
		void (*dma_mode) (dwmac_common_context * self,
						  const unsigned int txmode, const unsigned int rxmode);
	};
	...
	struct dwmac_common_desc_ops
	{
		...
		void (*init_rx_desc) (dwmac_common_context * self,
		...
		void (*prepare_tx_desc) (dwmac_common_context * self,
								 const unsigned int idx,
								 const bool is_first,
								 const size_t len, const void *pdata);

		/* Set/get the owner of the descriptor */
		void (*release_tx_ownership) (dwmac_common_context * self,
									  const unsigned int idx_tx);
		  bool (*am_i_tx_owner) (dwmac_common_context * self,
								 const unsigned int idx_tx);

		/* Invoked by the xmit function to close the tx descriptor */
		void (*close_tx_desc) (dwmac_common_context * self,
							   const unsigned int idx_tx);

		/* Clean the tx descriptor as soon as the tx irq is received */
		void (*release_tx_desc) (dwmac_common_context * self,
								 const unsigned int idx_tx);

		/* Last tx segment reports the transmit status */
		int (*get_tx_ls) (dwmac_common_context * self,
						  const unsigned int idx_tx);

		/* Return the transmit status looking at the TDES1 */
		int (*tx_status) (dwmac_common_context * self,
						  const unsigned int idx_tx);

		/* Handle extra events on specific interrupts hw dependent */
		  bool (*am_i_rx_owner) (dwmac_common_context * self,
								 const unsigned int desc_idx);

		/* Get the receive frame size */
		  size_t (*get_rx_frame_len) (dwmac_common_context * self,
									  const unsigned int desc_idx);

		/* Return the reception status looking at the RDES1 */
		  dwmac_common_rx_frame_status (*rx_status) (dwmac_common_context *
													 self,
													 const unsigned int
													 desc_idx);
		  bool (*is_first_rx_segment) (dwmac_common_context * self,
									   const unsigned int descriptor_index);
		  bool (*is_last_rx_segment) (dwmac_common_context * self,
									  const unsigned int descriptor_index);
		void (*print_tx_desc) (volatile dwmac_desc * p,
							   const unsigned int count);
		void (*print_rx_desc) (volatile dwmac_desc * p,
							   const unsigned int count);
	};
