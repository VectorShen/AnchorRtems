...
static volatile uint32_t *dwmac_1000_core_get_mac_addr_low (dwmac_common_context
															* self,
															const unsigned int
															mac_addr_index)
{
...
static volatile uint32_t
	*dwmac_1000_core_get_mac_addr_high (dwmac_common_context * self,
...
static void dwmac_1000_core_init (dwmac_common_context * self)
{
	...
	self->macgrp->interrupt_mask =
		...
		| MACGRP_INTERRUPT_MASK_PCSANCIM | MACGRP_INTERRUPT_MASK_TSIM;
...
static void dwmac_1000_core_set_umac_addr (dwmac_common_context * self,
										   const uint8_t * addr,
										   unsigned int reg_n)
{
	dwmac_core_set_mac_addr (addr,
							 dwmac_1000_core_get_mac_addr_high (self, reg_n),
							 dwmac_1000_core_get_mac_addr_low (self, reg_n));
}

static void dwmac_1000_core_set_hash_filter (dwmac_common_context * self,
	...
	if (eno == ENETRESET)
	  {
		  ...
		  DWMAC_1000_CORE_PRINT_DBG ("Frame Filter reg: 0x%08x\n",
									 self->macgrp->mac_frame_filter);
		  DWMAC_1000_CORE_PRINT_DBG ("Hash regs:\n"
									 "0x%08x\n"
									 "0x%08x\n"
									 "0x%08x\n"
									 "0x%08x\n"
