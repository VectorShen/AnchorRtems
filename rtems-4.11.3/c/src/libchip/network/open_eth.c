...
static rtems_isr open_eth_interrupt_handler (rtems_vector_number v)
{
	uint32_t status;
 ...
 */
static void open_eth_initialize_hardware (struct open_eth_softc *sc)
{
	struct mbuf *m;
	int i;
	int mii_cr = 0;
...
#ifdef OETH_SUSPEND_NOTXBUF
	regs->int_mask |=
		OETH_INT_MASK_TXB | OETH_INT_MASK_TXC | OETH_INT_MASK_TXE |
		OETH_INT_BUSY;
...
static void open_eth_rxDaemon (void *arg)
{
	...
	for (;;)
	  {
		  ...
		  while (!
				 ((len_status =
				   dp->regs->xd[dp->rx_ptr +
								dp->txbufs].len_status) & OETH_RX_BD_EMPTY))
			{
				bad = 0;
	...
	while (dp->regs->xd[dp->tx_ptr].len_status & OETH_TX_BD_READY)
	  {
		  ...
		  rtems_bsdnet_event_receive (OPEN_ETH_TX_WAIT_EVENT,
									  RTEMS_WAIT | RTEMS_EVENT_ANY,
									  RTEMS_MILLISECONDS_TO_TICKS (500),
									  &events);
#endif
	  }
 ...
 */
static void open_eth_txDaemon (void *arg)
{
...
static void open_eth_start (struct ifnet *ifp)
{
 ...
 */
static void open_eth_init (void *arg)
{
 ...
 * Stop the device
 */
static void open_eth_stop (struct open_eth_softc *sc)
{
	struct ifnet *ifp = &sc->arpcom.ac_if;
 ...
 */
static void open_eth_stats (struct open_eth_softc *sc)
{
