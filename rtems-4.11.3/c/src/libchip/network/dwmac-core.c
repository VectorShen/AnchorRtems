...
void dwmac_core_dma_interrupt (void *arg)
{
	...
	if (irq_status & (DMAGRP_STATUS_GMI | DMAGRP_STATUS_GLI))
	  {
...
void dwmac_core_set_mac_addr (const uint8_t addr[6],
							  ...
							  volatile uint32_t * reg_low)
{
	...
	*reg_high = data;
	...
	data =
		((uint32_t) addr[3] << 24)
		| ((uint32_t) addr[2] << 16) | ((uint32_t) addr[1] << 8) | addr[0];
	*reg_low = data;
}

/* Enable disable MAC RX/TX */
void dwmac_core_set_mac (dwmac_common_context * self, const bool enable)
{
