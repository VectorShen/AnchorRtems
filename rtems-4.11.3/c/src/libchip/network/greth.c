 ...
 */
static void greth_initialize_hardware (struct greth_softc *sc)
{
	struct mbuf *m;
	int i;
	int phyaddr;
	...
	if ((!sc->gbit_mac) && (phystatus & 0x100))
		write_mii (phyaddr, 9, 0);
...
/* 	  printf("RXBUF: %08x\n", (int) sc->rxdesc[i].addr); */
	...
	regs->mac_addr_msb = sc->arpcom.ac_enaddr[0] << 8 | sc->arpcom.ac_enaddr[1];
	...
	rtems_interrupt_handler_install (sc->vector, "greth",
									 RTEMS_INTERRUPT_SHARED,
									 greth_interrupt_handler, sc);

	regs->ctrl |=
		GRETH_CTRL_RXEN | (sc->fd << 4) | GRETH_CTRL_RXIRQ | (sc->
															  sp << 7) | (sc->
																		  gb <<
																		  8);
...
static void greth_Daemon (void *arg)
{
	...
	for (;;)
	  {
		...
		again:
		  while (!((len_status =
					GRETH_MEM_LOAD (&dp->rxdesc[dp->rx_ptr].
									ctrl)) & GRETH_RXD_ENABLE))
			{
				bad = 0;
...
static int inside = 0;
static int sendpacket (struct ifnet *ifp, struct mbuf *m)
{
	...
	if (inside)
		printf ("error: sendpacket re-entered!!\n");
	inside = 1;
...
static int sendpacket_gbit (struct ifnet *ifp, struct mbuf *m)
{
	struct greth_softc *dp = ifp->if_softc;
	unsigned int len;
	...
	for (;;)
	  {
		  ...
		  if ((m->m_next) == NULL)
			...
			{
				dp->txdesc[dp->tx_ptr].ctrl =
					GRETH_TXD_MORE | ctrl | int_en | m->m_len;
			}
...
static void greth_start (struct ifnet *ifp)
{
 ...
 */
static void greth_init (void *arg)
{
 ...
 * Stop the device
 */
static void greth_stop (struct greth_softc *sc)
{
	struct ifnet *ifp = &sc->arpcom.ac_if;
 ...
 */
static void greth_stats (struct greth_softc *sc)
{
