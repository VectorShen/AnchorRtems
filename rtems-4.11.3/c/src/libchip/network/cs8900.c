...
static inline uint32_t io_pp_get_reg_32 (cs8900_device * cs, uint16_t reg)
{
	rtems_interrupt_level level;
	uint32_t data;
	rtems_interrupt_disable (level);
...
static inline void
io_pp_bit_set_reg_16 (cs8900_device * cs, unsigned short reg,
					  unsigned short mask)
{
	rtems_interrupt_level level;
	rtems_interrupt_disable (level);
...
static inline void
io_pp_bit_clear_reg_16 (cs8900_device * cs, unsigned short reg,
						unsigned short mask)
{
	rtems_interrupt_level level;
	rtems_interrupt_disable (level);
...
static inline void
mem_pp_bit_clear_reg (cs8900_device * cs, unsigned short reg,
					  unsigned short mask)
{
	if (!cs->mem_base)
...
static void cs8900_hardware_init (cs8900_device * cs)
{
	...
	if (cs->mem_base)
	  {
		  ...
		  io_pp_set_reg_16 (cs, CS8900_PP_MEM_BASE + 2,
							(cs->mem_base >> 16) & 0xf);
...
#define MACO(o) cs->arpcom.ac_enaddr[o]
	...
	mem_pp_set_reg (cs, CS8900_PP_IA,
					(((unsigned int)MACO (1)) << 8) | ((unsigned int)MACO (0)));
	mem_pp_set_reg (cs, CS8900_PP_IA + 2,
					(((unsigned int)MACO (3)) << 8) | ((unsigned int)MACO (2)));
	mem_pp_set_reg (cs, CS8900_PP_IA + 4,
					(((unsigned int)MACO (5)) << 8) | ((unsigned int)MACO (4)));
	...
	mem_pp_set_reg (cs, CS8900_PP_RxCFG,
					...
					CS8900_RX_CONFIG_RUNT | CS8900_RX_CONFIG_EXTRA_DATA);
	...
	mem_pp_set_reg (cs, CS8900_PP_RxCTL,
					...
					CS8900_RX_CTRL_INDIVIDUAL | CS8900_RX_CTRL_BROADCAST);
	...
	mem_pp_set_reg (cs, CS8900_PP_TxCFG,
					...
					CS8900_TX_CONFIG_JABBER | CS8900_TX_CONFIG_16_COLLISION);
	...
	mem_pp_bit_set_reg (cs, CS8900_PP_BusCTL, CS8900_BUS_CTRL_ENABLE_INT);
}

rtems_isr cs8900_interrupt (rtems_vector_number v, void *csp)
{
						  ...
						  if (m)
							{
								...
								if (cs->rx_loaded_len == 1)
								  {
									  cs8900_trace (cs, CS8900_T_RX_OK,
													cs->rx_loaded_len);
									  rtems_bsdnet_event_send (cs->rx_task,
															   CS8900_RX_OK_EVENT);
								  }
							}
						  else
							{
								...
								cs8900_trace (cs, CS8900_T_RX_DROPPED,
											  cs->rx_loaded_len);
					...
					if (cs->tx_active)
					  {
						  ...
						  rtems_bsdnet_event_send (cs->tx_task,
												   CS8900_TX_OK_EVENT);
					  }
					break;
...
static inline void cs8900_rx_refill_queue (cs8900_device * cs)
{
		  ...
		  if (!m->m_ext.ext_buf)
			{
				...
				cs8900_trace (cs, CS8900_T_NO_CLUSTERS,
							  cs->eth_stats.rx_no_clusters);
				m_free (m);
				return;
			}
...
static void cs8900_rx_task (void *arg)
{
	...
	mem_pp_bit_set_reg (cs, CS8900_PP_LineCFG,
						CS8900_LINE_CTRL_RX_ON | CS8900_LINE_CTRL_TX_ON);

	/*
	 * Start the software interrupt watchdog.
	 */

	mem_pp_bit_set_reg (cs, CS8900_PP_BufCFG, CS8900_BUFFER_CONFIG_SW_INT);
		  ...
		  if (sc == RTEMS_TIMEOUT)
			{
				...
				if ((cs->eth_stats.int_swint_req -
					 cs->eth_stats.int_swint_res) > 1)
				  {
...
static void cs8900_tx_task (void *arg)
{
	cs8900_device *cs = arg;
	struct ifnet *ifp = &cs->arpcom.ac_if;
	rtems_event_set events;
					  ...
					  do
						{
							...
							mem_pp_set_reg (cs, CS8900_PP_TxLength,
											m->m_pkthdr.len);
							...
							if (buf_status & CS8900_BUS_STATUS_TX_BID_ERROR)
							...
							else
							  {
								  ...
								  if ((buf_status &
									   CS8900_BUS_STATUS_RDY_FOR_TX_NOW) == 0)
									{
...
static void cs8900_start (struct ifnet *ifp)
{
...
static void cs8900_stop (cs8900_device * cs)
{
	mem_pp_bit_clear_reg (cs, CS8900_PP_LineCFG,
						  CS8900_LINE_CTRL_RX_ON | CS8900_LINE_CTRL_TX_ON);

	mem_pp_bit_clear_reg (cs, CS8900_PP_BusCTL, CS8900_BUS_CTRL_ENABLE_INT);
}
...
static void cs8900_stats (cs8900_device * cs)
{
	int i;
	int max_label = 0;
	int len;
	...
	cs->eth_stats.tx_collisions += mem_pp_get_reg (cs, CS8900_PP_TxCol) >> 6;

	printf ("Network Driver Stats for CS8900 :\n");

	for (i = 0; i < (sizeof (eth_statistics_labels) / sizeof (const char *));
		 i++)
	  {
	...
	for (i = 0;
		 i < (sizeof (eth_statistics_labels) / sizeof (const char *)); i++)
	  {
		  printf ("%*s - %10lu", max_label, eth_statistics_labels[i], value[i]);
	...
	for (i = 0; i < cs->trace_in; i++)
	  {
		  printf ("%8ld.%03ld ", cs->trace_time[i] / 1000,
				  cs->trace_time[i] % 1000);
...
static void cs8900_init (void *arg)
{
	cs8900_device *cs = arg;
	struct ifnet *ifp = &cs->arpcom.ac_if;
...
static int cs8900_ioctl (struct ifnet *ifp, ioctl_command_t cmd, caddr_t data)
{
	cs8900_device *cs = ifp->if_softc;
	int error = 0;
	...
	if (attaching)
	  {
		  ...
		  if (config->hardware_address)
			  memcpy (cs->arpcom.ac_enaddr, config->hardware_address,
					  ETHER_ADDR_LEN);
		  else
