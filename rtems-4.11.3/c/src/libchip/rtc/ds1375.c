...
STATIC int rd_bytes (int fd, uint32_t off, uint8_t * buf, int len)
{
	uint8_t ptr = off;
...
STATIC int wr_bytes (int fd, uint32_t off, uint8_t * buf, int len)
{
	uint8_t d[len + 1];
...
STATIC void ds1375_initialize (int minor)
{
	int fd;
	uint8_t cr;
...
STATIC int ds1375_get_time (int minor, rtems_time_of_day * time)
{
	int rval = -1;
	int fd;
...
STATIC int ds1375_set_time (int minor, const rtems_time_of_day * time)
{
	int rval = -1;
	int fd = -1;
	time_t secs;
...
int ds1375_set_time_tst (const char *datstr, rtems_time_of_day * prt)
{
	struct tm tm;
	time_t secs;
...
uint32_t rtc_ds1375_get_register (uintptr_t port, uint8_t reg)
{
	int fd;
	uint8_t v;
	uint32_t rval = -1;
...
void rtc_ds1375_set_register (uintptr_t port, uint8_t reg, uint32_t value)
{
	int fd;
	uint8_t v = value;
...
bool rtc_ds1375_device_probe (int minor)
{
	int fd;
