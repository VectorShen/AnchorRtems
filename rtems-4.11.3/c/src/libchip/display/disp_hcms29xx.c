...
\*=========================================================================*/
{
	 ...
	 * check parameters
	 */
	if ((rc == RTEMS_SUCCESSFUL) && (src == NULL))
	{
		rc = RTEMS_INVALID_ADDRESS;
	}
	if (rc == RTEMS_SUCCESSFUL)
	{
		font_size = sizeof (*src);	/* font_base structure       */
	}
	glyph_idx = 0;
...
\*=========================================================================*/
{
	...
	int smsk, dmsk;
	for (smsk = 0x01, dmsk = 0x80; smsk < 0x100; smsk <<= 1, dmsk >>= 1)
	{
		if ((byte & smsk) != 0)
		{
...
\*=========================================================================*/
{
	 ...
	 * check parameters
	 */
	if ((rc == RTEMS_SUCCESSFUL) && ((src == NULL) || (dst == NULL)))
	{
		rc = RTEMS_INVALID_ADDRESS;
	}
	/*
		 ...
		 glyph_idx < (sizeof (src->latin1) / sizeof (src->latin1[0])))
	{
		if (src->latin1[glyph_idx] != NULL)
		{
			 ...
			 */
			dst->latin1[glyph_idx]->bitmap =
				(const unsigned char *)alloc_next;
			...
			for (bcnt = 0; bcnt < glyph_size; bcnt++)
			{
				...
				{
					byte =
						src->latin1[glyph_idx]->bitmap[glyph_size - 1 -
													 bcnt];
				...
				((unsigned char *)(dst->latin1[glyph_idx]->
									 bitmap))[bcnt] = byte;
			}
		}
		glyph_idx++;
...
| Input Parameters:                                                         |
\*-------------------------------------------------------------------------*/
														 disp_hcms29xx_drv_t * softc_ptr, const volatile char *disp_buffer	/* start of chars to display (4 chars or 'til \0) */
	)
/*-------------------------------------------------------------------------*\
| Return Value:                                                             |
|    rtems_status_code                                                      |
\*=========================================================================*/
{
	...
	if (rc == RTEMS_SUCCESSFUL)
	{
		curr_font =
			...
			? disp_hcms29xx_font_rotate : disp_hcms29xx_font_normal;
		 ...
		 */
		for (i = 0;
			 ((rc == RTEMS_SUCCESSFUL) && (i < DISP_HCMS29XX_DIGIT_CNT)); i++)
		{
			...
			glyph_ptr = (char_avail ? curr_font->latin1[c] : NULL);
			if (glyph_ptr == NULL)
			{
			...
			{
				ret_cnt =
					rtems_libi2c_write_bytes (softc_ptr->disp_param.minor,
												glyph_ptr->bitmap, 5);
				if (ret_cnt < 0)
				{
					rc = -ret_cnt;
...
| Input Parameters:                                                         |
\*-------------------------------------------------------------------------*/
														 disp_hcms29xx_drv_t * softc_ptr, int pwm,	/* value for pwm of LEDs, 0..15 */
...
\*=========================================================================*/
{
	...
	for (run = 0; ((rc == RTEMS_SUCCESSFUL) && (run <= 1)); run++)
	{
		if (rc == RTEMS_SUCCESSFUL)
		{
			if (run == 0)
			{
				ctrl_buffer =
					(0 << 7) |
					((sleep & 0x01) << 6) |
					((peak & 0x03) << 4) | ((pwm & 0x0f) << 0);
			}
			else
			{
				ctrl_buffer =
					(1 << 7) |
					((div & 0x01) << 1) | ((chain & 0x01) << 0);
			}
			/*
			 * select device, set transfer mode, address device
			 */
			if (rc == RTEMS_SUCCESSFUL)
			{
				rc = rtems_libi2c_send_start (softc_ptr->disp_param.
												minor);
			}
			/*
			 * set transfer mode
			...
			if (rc == RTEMS_SUCCESSFUL)
			{
				rc = rtems_libi2c_send_addr (softc_ptr->disp_param.minor,
											 true);
			}
...
\*=========================================================================*/
{
	...
	if (rc == RTEMS_SUCCESSFUL)
	{
		rc = disp_hcms29xx_send_to_control (softc_ptr, 14, 3, 1, 0, 0);	/* pwm/peak/nosleep/div/chain */
	}
	...
	while (rc == RTEMS_SUCCESSFUL)
	{
		...
		if (my_events & DISP_HCMS29XX_EVENT_NEWSTR)
		{
			...
			if (rc == RTEMS_SUCCESSFUL)
			{
				rc = rtems_semaphore_obtain (softc_ptr->disp_param.
											 trns_sema_id, RTEMS_WAIT,
											 RTEMS_NO_TIMEOUT);
			}
			if (rc == RTEMS_SUCCESSFUL)
			{
				...
				softc_ptr->disp_param.
					disp_buffer[sizeof (softc_ptr->disp_param.disp_buffer)
								- 1] = '\0';
			...
			if (rc == RTEMS_SUCCESSFUL)
			{
				rc = rtems_semaphore_release (softc_ptr->disp_param.
												trns_sema_id);
			}
			/*
		...
		if (rc == RTEMS_SUCCESSFUL)
		{
			if (softc_ptr->disp_param.disp_buf_cnt >
				DISP_HCMS29XX_DIGIT_CNT)
			{
...
\*=========================================================================*/
{
	...
	softc_ptr->disp_param.
		trns_buffer[sizeof (softc_ptr->disp_param.trns_buffer) - 1] = '\0';
 ...
 */
{
	...
	if (rc == RTEMS_SUCCESSFUL)
	{
		rc = disp_hcms29xx_alloc_copy_font (&FONT_BASE, &disp_hcms29xx_font_normal, FONT_BASE.descent,	/* shift to visibility... */
											FALSE);	/* do not rotate */
	}
	/* FIXME: translate font for rotation */
	if (rc == RTEMS_SUCCESSFUL)
	{
		rc = disp_hcms29xx_alloc_copy_font (&FONT_BASE, &disp_hcms29xx_font_rotate, 0,	/* do not shift */
											true);	/* rotate font */
	}
	/*
	...
	if (rc == RTEMS_SUCCESSFUL)
	{
		rc = rtems_semaphore_create (DISP_HCMS29XX_TRNS_SEMA_NAME, 1,
									 ...
									 | RTEMS_LOCAL,
									 0, &softc_ptr->disp_param.trns_sema_id);
	}
...
\*=========================================================================*/
{
	...
	for (cnt = 0; cnt < args->count; cnt++)
	{
		...
		{
			softc_ptr->disp_param.dev_buffer[softc_ptr->disp_param.
											 dev_buf_cnt] = '\0';
			/*
			 * transfer string to display string, redisplay it...
			 */
			disp_hcms29xx_update (softc_ptr,
								softc_ptr->disp_param.dev_buffer);
			softc_ptr->disp_param.dev_buf_cnt = 0;
		}
		/*
