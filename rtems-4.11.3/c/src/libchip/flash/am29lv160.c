...
static int rtems_am29lv160_toggle_wait_8 (volatile uint8_t * status)
{
	while (1)
	{
...
static int rtems_am29lv160_toggle_wait_16 (volatile uint16_t * status)
{
	while (1)
	{
...
rtems_am29lv160_write_data_16 (volatile uint16_t * base,
							 uint32_t offset,
							 const uint16_t * data, uint32_t size)
{
...
{
	int ret =
		rtems_am29lv160_verify (sd, device, segment, offset, buffer, size);

	if (ret != 0)
	{
		const rtems_am29lv160_config *ac =
			&rtems_am29lv160_configuration[device];
		uint32_t soffset;
		...
		if (ac->bus_8bit)
			ret =
				rtems_am29lv160_write_data_8 (ac->base, soffset, buffer,
												size);
		else
			ret =
				rtems_am29lv160_write_data_16 (ac->base, soffset, buffer,
												 size);
		...
		{
			ret =
				rtems_am29lv160_verify (sd, device, segment, offset, buffer,
										size);
#if AM26LV160_ERROR_TRACE
			if (ret)
							...
							uint32_t device)
{
	...
	for (segment = 0; segment < dd->segment_count; segment++)
	{
		...
		for (seg_segment = 0;
			 seg_segment < dd->segments[segment].count; seg_segment++)
		{
