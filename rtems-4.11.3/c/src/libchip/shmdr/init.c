...
rtems_mpci_entry Shm_Initialization (void)
{
	...
	Shm_Interrupt_table =
		(Shm_Interrupt_information *) malloc (sizeof (Shm_Interrupt_information)
											* (SHM_MAXIMUM_NODES + 1));
	...
	Shm_Active_node = Shm_Convert (rtems_build_name ('A', 'C', 'T', 'V'));
	...
	if (Shm_Is_master_node ())
	{
		...
		(void)memset ((void *)Shm_Configuration->base,
						0, Shm_Configuration->length);
	...
	for (i = SHM_FIRST_NODE; i <= SHM_MAXIMUM_NODES; i++)
	{
		...
		Shm_Interrupt_table[i].address = Shm_Convert_address ((void *)
																Shm_Convert (((vol_u32) nscb->int_address)));
	...
	(void)rtems_extension_create (rtems_build_name ('M', 'P', 'E', 'X'),
								&MPCI_Shm_extensions, &extension_id);
}
