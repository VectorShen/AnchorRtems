...
{
	return ((uint32_t) s[0] << 24) | ((uint32_t) s[1] << 16) | ((uint32_t) s[2]
																<< 8) |
		(uint32_t) s[3];
}
...
static inline uint32_t sd_card_max_access_time (const uint8_t * csd,
...
static int sd_card_send_command (sd_card_driver_entry * e, uint32_t command,
								 uint32_t argument)
{
	int rv = 0;
  ...
  sd_card_send_command_error:
	...
	for (r = 0; r < SD_CARD_COMMAND_SIZE; ++r)
	{
		...
		{
			RTEMS_SYSLOG_PRINT (" %02" PRIx8 ":[%02" PRIx8 "]",
								e->command[r], e->response[r]);
		}
		else
		{
			RTEMS_SYSLOG_PRINT (" %02" PRIx8 ":%02" PRIx8 "", e->command[r],
								e->response[r]);
		}
	}
	RTEMS_SYSLOG_PRINT ("\n");
...
static int sd_card_send_register_command (sd_card_driver_entry * e,
...
{
	int rv = 0;
	uint8_t stop_transfer[3] =
		{ SD_CARD_IDLE_TOKEN, SD_CARD_STOP_TRANSFER_MULTIPLE_BLOCK_WRITE,
SD_CARD_IDLE_TOKEN };
...
static int sd_card_read (sd_card_driver_entry * e, uint8_t start_token,
						 uint8_t * in, int n)
{
	int rv = 0;
...
static int sd_card_write (sd_card_driver_entry * e, uint8_t start_token,
						uint8_t * out, int n)
{
	int rv = 0;
...
static inline rtems_status_code sd_card_start (sd_card_driver_entry * e)
{
	...
	rv = rtems_libi2c_ioctl (e->bus, RTEMS_LIBI2C_IOCTL_SET_TFRMODE,
							 &e->transfer_mode);
...
static rtems_status_code sd_card_init (sd_card_driver_entry * e)
{
	...
	bool do_cmd58 = true;
	uint32_t cmd_arg = 0;
	uint32_t if_cond_test =
		SD_CARD_FLAG_VHS_2_7_TO_3_3 | SD_CARD_FLAG_CHECK_PATTERN;
	...
	RTEMS_CLEANUP_RV_SC (rv, sc, sd_card_driver_init_cleanup,
	 ...
	 * HCS (> 2G capacity).
	 */
	rv = sd_card_send_register_command (e, SD_CARD_CMD_SEND_IF_COND,
										if_cond_reg, &if_cond_reg);
	...
	while (true)
	{
		if (assume_sd)
		{
			 ...
			 * fails if MMC.
			 */
			rv = sd_card_send_command (e, SD_CARD_ACMD_SD_SEND_OP_COND,
									 cmd_arg);
			if (rv < 0)
			{
				/*
				 ...
				 */
				RTEMS_SYSLOG
					("ACMD41 failed.  Assume MMC and do CMD58 (once) then CMD1\n");
				assume_sd = false;
				cmd_arg = SD_CARD_FLAG_HCS;
				do_cmd58 = true;
				continue;
		...
		else
		{
			...
			if (do_cmd58)
			{
				rv = sd_card_send_command (e, SD_CARD_CMD_READ_OCR,
											 cmd_arg);
				RTEMS_CLEANUP_RV_SC (rv, sc, sd_card_driver_init_cleanup,
	...
	else
	{
		...
		if (high_capacity)
		{
			 ...
			 * set to 0
			 */
			rv = sd_card_send_register_command (e, SD_CARD_CMD_READ_OCR,
												cmd_arg, &reg);
			RTEMS_CLEANUP_RV_SC (rv, sc, sd_card_driver_init_cleanup,
	...
	if (e->verbose)
	{
		...
		rv = sd_card_read (e, SD_CARD_START_BLOCK_SINGLE_BLOCK_READ, block,
							 SD_CARD_CID_SIZE);
		RTEMS_CLEANUP_RV_SC (rv, sc, sd_card_driver_init_cleanup,
		...
		RTEMS_SYSLOG ("Product name             : %c%c%c%c%c%c\n",
	...
	if (csd_structure == 0 || !assume_sd)
	{
		...
		{
			RTEMS_SYSLOG_ERROR
				("Read block size smaller than write block size\n");
			return -RTEMS_IO_ERROR;
		}
	...
	{
		RTEMS_DO_CLEANUP_SC (RTEMS_IO_ERROR, sc, sd_card_driver_init_cleanup,
							 "Unexpected CSD Structure number");
	}
...
static int sd_card_disk_block_read (sd_card_driver_entry * e,
									rtems_blkdev_request * r)
{
	rtems_status_code sc = RTEMS_SUCCESSFUL;
	int rv = 0;
	uint32_t start_address =
		RTEMS_BLKDEV_START_BLOCK (r) << e->block_size_shift;
	uint32_t i = 0;
	...
	if (r->bufnum == 1)
	{
		...
		{
			RTEMS_DO_CLEANUP_RV (-RTEMS_INTERNAL_ERROR, rv,
								 sd_card_disk_block_read_cleanup,
								 "Buffer and disk block size are not equal");
		}
		...
		/* Single block read */
		rv = sd_card_send_command (e, SD_CARD_CMD_READ_SINGLE_BLOCK,
									 start_address);
		RTEMS_CLEANUP_RV (rv, sd_card_disk_block_read_cleanup,
							"Send: SD_CARD_CMD_READ_SINGLE_BLOCK");
		rv = sd_card_read (e, SD_CARD_START_BLOCK_SINGLE_BLOCK_READ,
							 (uint8_t *) r->bufs[0].buffer, (int)e->block_size);
		RTEMS_CLEANUP_RV (rv, sd_card_disk_block_read_cleanup,
	...
	else
	{
		...
		rv = sd_card_send_command (e, SD_CARD_CMD_READ_MULTIPLE_BLOCK,
									 start_address);
		RTEMS_CLEANUP_RV (rv, sd_card_disk_block_read_stop_cleanup,
		...
		for (i = 0; i < r->bufnum; ++i)
		{
			...
			RTEMS_DEBUG_PRINT ("[%02u:%02u]: buffer = 0x%08x, size = %u\n",
							 i + 1, r->bufnum, r->bufs[i].buffer,
							 r->bufs[i].length);
#endif /* DEBUG */

			rv = sd_card_read (e, SD_CARD_START_BLOCK_MULTIPLE_BLOCK_READ,
							 (uint8_t *) r->bufs[i].buffer,
							 (int)e->block_size);
			RTEMS_CLEANUP_RV (rv, sd_card_disk_block_read_stop_cleanup,
							"Read block");
		}
...
static int sd_card_disk_block_write (sd_card_driver_entry * e,
									 rtems_blkdev_request * r)
{
	rtems_status_code sc = RTEMS_SUCCESSFUL;
	int rv = 0;
	...
	if (r->bufnum == 1)
	{
		...
		RTEMS_CLEANUP_RV (rv, sd_card_disk_block_write_cleanup,
							"Send: SD_CARD_CMD_WRITE_BLOCK");
		rv = sd_card_write (e, SD_CARD_START_BLOCK_SINGLE_BLOCK_WRITE,
							(uint8_t *) r->bufs[0].buffer,
							(int)e->block_size);
		RTEMS_CLEANUP_RV (rv, sd_card_disk_block_write_cleanup,
	...
	else
	{
		...
		rv = sd_card_send_command (e, SD_CARD_CMD_WRITE_MULTIPLE_BLOCK,
									 start_address);
		RTEMS_CLEANUP_RV (rv, sd_card_disk_block_write_stop_cleanup,
		...
		for (i = 0; i < r->bufnum; ++i)
		{
			...
			rv = sd_card_write (e, SD_CARD_START_BLOCK_MULTIPLE_BLOCK_WRITE,
								(uint8_t *) r->bufs[i].buffer,
								(int)e->block_size);
			RTEMS_CLEANUP_RV (rv, sd_card_disk_block_write_stop_cleanup,
							"Write block");
											...
											void *arg)
{
	...
	for (minor = 0; minor < sd_card_driver_table_size; ++minor)
	{
		...
		/* Create disk device */
		sc = rtems_disk_create_phys (dev, e->block_size, e->block_number,
