...
\*=========================================================================*/
{
	...
	if (rc == RTEMS_SUCCESSFUL)
	{
		rc = -rtems_libi2c_ioctl (minor,
									RTEMS_LIBI2C_IOCTL_GET_DRV_T, &drv_ptr);
	}
	if ((rc == RTEMS_SUCCESSFUL) &&
...
\*=========================================================================*/
{
	...
	while ((rc == RTEMS_SUCCESSFUL) && (cnt > bytes_sent))
	{
		...
		{
			curr_cnt =
				mem_param_ptr->page_size - (off % mem_param_ptr->page_size);
		}
		/*
...
\*=========================================================================*/
{
	...
	if (rc == RTEMS_SUCCESSFUL)
	{
		tfr_mode.baudrate = mem_param_ptr->baudrate;
		rc = -rtems_libi2c_ioctl (minor,
									RTEMS_LIBI2C_IOCTL_SET_TFRMODE, &tfr_mode);
	}
	/*
	 * address device
	 */
