...
static rtems_status_code i2c_sc620_write (rtems_device_major_number major,
										...
										void *arg)
{
	...
	if (rw->count == 2 && buf[0] < SC620_REG_COUNT)
	{
		...
		rv = rtems_libi2c_start_write_bytes (minor, buf, 2);
		if (rv == 2)
		{
...
static rtems_status_code i2c_sc620_read (rtems_device_major_number major,
										 rtems_device_minor_number minor,
										 void *arg)
{
	rtems_status_code sc = RTEMS_IO_ERROR;
