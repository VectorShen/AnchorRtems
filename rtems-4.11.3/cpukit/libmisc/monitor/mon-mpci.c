...
void
rtems_monitor_mpci_canonical (rtems_monitor_mpci_t * canonical_mpci,
...
const void *rtems_monitor_mpci_next (void *object_info,
...
void rtems_monitor_mpci_dump (rtems_monitor_mpci_t * monitor_mpci, bool verbose)
{
	uint32_t length = 0;
	...
	length +=
		rtems_monitor_dump_decimal ((uint32_t) monitor_mpci->
									maximum_packet_size);
	...
	length +=
		rtems_monitor_symbol_dump (&monitor_mpci->initialization, verbose);
	...
	length +=
		rtems_monitor_symbol_dump (&monitor_mpci->receive_packet, verbose);
