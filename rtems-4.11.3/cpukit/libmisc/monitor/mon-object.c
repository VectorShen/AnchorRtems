...
rtems_id
rtems_monitor_id_fixup (rtems_id id,
						uint32_t default_node, rtems_monitor_object_type_t type)
{
	uint32_t node;
	...
	if (node == 0)
	  {
		  ...
		  id = rtems_build_id (OBJECTS_CLASSIC_API,
							   type,
							   default_node, rtems_object_id_get_index (id));
	  }
	return id;
}
...
#if defined(RTEMS_MULTIPROCESSING)
static rtems_id
rtems_monitor_object_canonical_next_remote (rtems_monitor_object_type_t type,
											rtems_id id, void *canonical)
{
	rtems_id next_id;
	...
	status =
		rtems_monitor_server_request (rtems_object_id_get_node (id), &request,
									  &response);
...
rtems_id
rtems_monitor_object_canonical_next (const rtems_monitor_object_info_t * info,
									 rtems_id id, void *canonical)
{
	rtems_id next_id;
	const void *raw_item;
...
rtems_id
rtems_monitor_object_canonical_get (rtems_monitor_object_type_t type,
									rtems_id id,
									void *canonical, size_t * size_p)
{
...
static void
rtems_monitor_object_dump_1 (const rtems_monitor_object_info_t * info,
							 rtems_id id, bool verbose)
{
	rtems_id next_id;
	rtems_monitor_union_t canonical;

	if ((next_id = rtems_monitor_object_canonical_next (info,
														id,
														&canonical)) !=
...
static void
rtems_monitor_object_dump_all (const rtems_monitor_object_info_t * info,
							   bool verbose)
{
	rtems_id next_id;
	rtems_monitor_union_t canonical;
	...
	next_id =
		RTEMS_OBJECT_ID_INITIAL (OBJECTS_CLASSIC_API, info->type,
								 rtems_monitor_default_node);

	while ((next_id = rtems_monitor_object_canonical_next (info,
														   next_id,
														   &canonical)) !=
		   RTEMS_OBJECT_ID_FINAL)
	  {
...
void
rtems_monitor_object_cmd (int argc,
						  char **argv,
