...
void
rtems_monitor_queue_canonical (rtems_monitor_queue_t * canonical_queue,
							   const void *queue_void)
{
	const Message_queue_Control *rtems_queue =
		(const Message_queue_Control *)queue_void;
...
void rtems_monitor_queue_dump_header (bool verbose __attribute__ ((unused)))
{
	fprintf (stdout, "\
...
void
rtems_monitor_queue_dump (rtems_monitor_queue_t * monitor_queue,
						  bool verbose __attribute__ ((unused)))
{
	...
	length +=
		rtems_monitor_dump_decimal (monitor_queue->number_of_pending_messages);
	length += rtems_monitor_pad (39, length);
	length +=
		rtems_monitor_dump_decimal (monitor_queue->maximum_pending_messages);
