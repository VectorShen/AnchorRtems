							...
							if (translation[index].expecting == c)
							  {
								  ...
								  if (translation[index].branch == 0)
									  return KEYS_EXTENDED | translation[index].
										  key;
								  else
									{
					  ...
					  switch (c)
						{
							...
							case KEYS_RARROW:
								if ((pos < RTEMS_COMMAND_BUFFER_SIZE)
									&& (buffer[pos] != '\0'))
								  {
									  putchar (buffer[pos]);
									  pos++;
					  ...
					  switch (c)
						{
							...
							case '\n':
								...
								if (!repeating)
								  {
									  memcpy (history_buffer[history_next],
											  buffer,
											  RTEMS_COMMAND_BUFFER_SIZE);
									  history_pos[history_next] = pos;
									  if (history_next <
										  (RTEMS_COMMAND_HISTORIES - 1))
										  history_next++;
									  else
										{
											memmove (history_buffer[0],
													 history_buffer[1],
													 RTEMS_COMMAND_BUFFER_SIZE *
													 (RTEMS_COMMAND_HISTORIES -
													  1));
											memmove (&history_pos[0],
													 &history_pos[1],
													 sizeof (history_pos[0]) *
													 (RTEMS_COMMAND_HISTORIES -
													  1));
										}
								  }
	...
	if (!(monitor_flags & RTEMS_MONITOR_NOTASK))
	  {
		   ...
		   */
		  status =
			  rtems_task_start (rtems_monitor_task_id, rtems_monitor_task,
								monitor_flags);
		  if (status != RTEMS_SUCCESSFUL)
			{
