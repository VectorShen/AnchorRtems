...
void
rtems_monitor_driver_canonical (rtems_monitor_driver_t * canonical_driver,
								const void *driver_void)
{
	const rtems_driver_address_table *d =
		(const rtems_driver_address_table *)driver_void;
...
void rtems_monitor_driver_dump_header (bool verbose __attribute__ ((unused)))
{
	fprintf (stdout, "\
  Major      Entry points\n");
						   ...
						   bool verbose)
{
	...
	length +=
		rtems_monitor_symbol_dump (&monitor_driver->initialization, verbose);
