...
rtems_status_code
rtems_monitor_server_request (uint32_t server_node,
	...
	if (response)
	  {
		  status =
			  ...
			  (rtems_monitor_server_response_queue_id, response, &size,
			   RTEMS_WAIT, 100);
		  if (status != RTEMS_SUCCESSFUL)
			{
...
void
rtems_monitor_server_task (rtems_task_argument monitor_flags
	...
	for (;;)
	  {
		  status =
			  rtems_message_queue_receive
			  (rtems_monitor_server_request_queue_id, &request, &size,
			   RTEMS_WAIT, (rtems_interval) 0);
				...
				case RTEMS_MONITOR_SERVER_CANONICAL:
					{
						...
						object_type =
							(rtems_monitor_object_type_t) request.argument0;
						id = (rtems_id) request.argument1;
						next_id =
							rtems_monitor_object_canonical_get (object_type, id,
																&response.
																payload, &size);
...
#define SERVER_OVERHEAD  (offsetof(rtems_monitor_server_response_t, \
				...
				default:
					{
						rtems_error (0, "invalid command to monitor server: %d",
									 request.command);
						goto failed;
					}
			}
...
void rtems_monitor_server_init (uint32_t monitor_flags __attribute__ ((unused)))
{
	...
	if (_System_state_Is_multiprocessing &&
		(_Configuration_MP_table->maximum_nodes > 1))
	  {
		  ...
		  status = rtems_message_queue_create (RTEMS_MONITOR_QUEUE_NAME,
											   maximum_nodes,
											   sizeof
		  ...
		  if (status != RTEMS_SUCCESSFUL)
			{
				rtems_error (status,
							 "could not create monitor server message queue");
				goto done;
			}
		  ...
		  status = rtems_message_queue_create (RTEMS_MONITOR_RESPONSE_QUEUE_NAME, 1,	/* depth */
											   sizeof
		  ...
		  if (status != RTEMS_SUCCESSFUL)
			{
				rtems_error (status,
							 "could not create monitor response message queue");
				goto done;
			}
		  ...
		  (void)memset (rtems_monitor_server_request_queue_ids,
						0, (maximum_nodes + 1) * sizeof (rtems_id));
		  ...
		  status = rtems_task_start (rtems_monitor_server_task_id,
									 rtems_monitor_server_task, monitor_flags);
		  if (status != RTEMS_SUCCESSFUL)
			{
