...
void rtems_monitor_extension_dump_header (bool verbose __attribute__ ((unused)))
{
	fprintf (stdout, "\
  ID       NAME\n");
							...
							bool verbose)
{
	...
	length +=
		rtems_monitor_symbol_dump (&monitor_extension->e_restart, verbose);
	...
	length +=
		rtems_monitor_symbol_dump (&monitor_extension->e_tswitch, verbose);
	...
	length +=
		rtems_monitor_symbol_dump (&monitor_extension->e_exitted, verbose);
