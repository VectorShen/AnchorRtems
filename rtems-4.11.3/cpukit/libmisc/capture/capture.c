...
rtems_capture_match_name_id (rtems_name lhs_name,
							 rtems_id lhs_id,
							 rtems_name rhs_name, rtems_id rhs_id)
{
	/*
...
rtems_capture_by_in_to (uint32_t events,
						rtems_tcb * by, rtems_capture_control_t * to)
{
 ...
 */
static void rtems_capture_initialize_control (rtems_tcb * tcb)
{
	rtems_name name;
	rtems_capture_control_t *control;
 ...
 */
static inline rtems_capture_control_t *rtems_capture_create_control (rtems_name
																	 name,
																	 rtems_id
																	 id)
{
...
void rtems_capture_record_task (rtems_tcb * tcb)
{
	...
	rec.start_priority =
		_RTEMS_tasks_Priority_from_Core (tcb->Start.initial_priority);
 ...
 */
void *rtems_capture_record_open (rtems_tcb * tcb,
								 uint32_t events,
								 size_t size,
								 rtems_interrupt_lock_context * lock_context)
{
 ...
 */
static void rtems_capture_flush_tcb (rtems_tcb * tcb)
{
	tcb->Capture.flags &= ~RTEMS_CAPTURE_TRACED;
}
...
rtems_status_code rtems_capture_flush (bool prime)
{
	...
	if (prime)
		capture_flags_global &=
			~(RTEMS_CAPTURE_TRIGGERED | RTEMS_CAPTURE_OVERFLOW);
	else
 ...
 */
static uint32_t rtems_capture_map_trigger (rtems_capture_trigger_t trigger)
{
	/*
...
rtems_capture_read (uint32_t cpu,
					uint32_t * read, rtems_capture_record_t ** recs)
{
	rtems_interrupt_lock_context lock_context;
	rtems_status_code sc = RTEMS_SUCCESSFUL;
	size_t recs_size = 0;
...
{
	if ((event < RTEMS_CAPTURE_EVENT_START)
