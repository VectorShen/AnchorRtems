...
rtems_capture_cli_open (int argc,
						...
						command_arg RC_UNUSED, bool verbose RC_UNUSED)
{
	uint32_t size = 0;
	bool enable = false;
...
static void rtems_capture_cli_print_task (rtems_tcb * tcb)
{
...
static void rtems_capture_cli_count_tasks (rtems_tcb * tcb)
{
		  ...
		  if (argv[arg][0] == '-')
		  ...
		  else
			{
				if (!rtems_capture_cli_get_name_id
					(argv[arg], &valid_name, &valid_id, &name, &id))
					return;
			}
	  }
...
static char const *watch_control_usage =
...
static void rtems_capture_cli_trigger_worker (int set, int argc, char **argv)
{
				...
				if (!trigger_set)
				  {
					  ...
					  for (t = 0; t < RTEMS_CAPTURE_CLI_TRIGGERS_NUM; t++)
						  if (strcmp
							  (argv[arg],
							   rtems_capture_cli_triggers[t].name) == 0)
							{
								trigger = t;
								found = true;
								break;
							...
							if (!from_valid_name && !from_valid_id)
							  {
								...
								fprintf (stdout,
										 "warning: extra name arguments ignored\n");
						}
							...
							if (!from_valid_name && !from_valid_id)
							  {
								...
								fprintf (stdout,
										 "warning: extra id arguments ignored\n");
						}
					  else if (!to_valid_name && !to_valid_id)
						{
							to_valid_id = true;
	...
	if (set)
		sc = rtems_capture_set_trigger (from_name, from_id, to_name, to_id,
										trigger_mode,
										rtems_capture_cli_triggers[trigger].
										type);
	else
...
rtems_status_code rtems_capture_cli_init (rtems_capture_timestamp timestamp)
{
	...
	for (cmd = 0;
		 cmd <
		 sizeof (rtems_capture_cli_cmds) /
		 sizeof (rtems_monitor_command_entry_t); cmd++)
