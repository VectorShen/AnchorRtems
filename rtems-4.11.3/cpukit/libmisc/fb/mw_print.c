...
void uid_print_message_with_plugin (void *context,
									...
									struct MW_UID_MESSAGE *uid)
{
	...
	switch (uid->type)
	{
		case MV_UID_INVALID:
			...
			break;
		case MV_UID_REL_POS:
			(*handler) (context, "MV_UID_REL_POS - %s x=%d y=%d z=%d\n", uid_buttons (uid->m.pos.btns, buttons, sizeof (buttons)), uid->m.pos.x,	/* x location */
						uid->m.pos.y,	/* y location */
						uid->m.pos.z	/* z location, 0 for 2D */
				);
			break;
		case MV_UID_ABS_POS:
			(*handler) (context, "MV_UID_ABS_POS - %s x=%d y=%d z=%d\n", uid_buttons (uid->m.pos.btns, buttons, sizeof (buttons)), uid->m.pos.x,	/* x location */
						uid->m.pos.y,	/* y location */
						uid->m.pos.z	/* z location, 0 for 2D */
				);
			break;
		case MV_UID_KBD:
			(*handler) (context, "MV_UID_KBD - code=0x%04x modifiers=0x%02x mode=0x%02x\n", uid->m.kbd.code,	/* keycode or scancode        */
