...
static int rtems_shell_main_msdos_format (int argc, char *argv[])
{
		...
		if (argv[arg][0] == '-')
		{
			switch (argv[arg][1])
			{
				...
				case 's':
					...
					{
						fprintf (stderr,
								 "error: sectors per cluster count.\n");
						return 1;
					}
	...
	if (rqdata.info_level)
	{
		...
		printf (" %-20s: %s\n", "skip_alignment",
				(0 == rqdata.skip_alignment) ? "false" : "true");
	}
