...
{
	printf ("Number of %s blocks:                    %12" PRId32 "\n"
			"Largest %s block:                       %12" PRId32 "\n"
			"Total bytes %s:                         %12" PRId32 "\n",
			c, h->number, c, h->largest, c, h->total);
}

void rtems_shell_print_heap_stats (const Heap_Statistics * s)
{
	printf ("Size of the allocatable area in bytes:    %12" PRIuPTR "\n"
			...
			s->lifetime_freed,
			s->searches, s->allocs, s->failed_allocs, s->frees, s->resizes);
}
