...
static int rtems_trace_buffering_wrong_number_of_args (void)
{
	printf ("error: wrong number of arguments\n");
	return 1;
}

static int rtems_trace_buffering_no_trace_buffer_code (void)
{
	printf
...
static void rtems_trace_buffering_banner (const char *label)
{
	printf ("RTEMS Trace Bufferring: %s\n", label);
}

static void rtems_trace_buffering_print_timestamp (uint64_t uptime)
{
	uint32_t hours;
	uint32_t minutes;
	uint32_t seconds;
...
static int rtems_trace_buffering_shell_status (int argc, char *argv[])
{
...
static int rtems_trace_buffering_shell_funcs (int argc, char *argv[])
{
...
static int rtems_trace_buffering_shell_start (int argc, char *argv[])
{
	if (argc != 1)
		return rtems_trace_buffering_wrong_number_of_args ();
...
static int rtems_trace_buffering_shell_stop (int argc, char *argv[])
{
	if (argc != 1)
		return rtems_trace_buffering_wrong_number_of_args ();
...
static int rtems_trace_buffering_shell_resume (int argc, char *argv[])
{
	if (argc != 1)
		return rtems_trace_buffering_wrong_number_of_args ();
...
static int rtems_trace_buffering_shell_trace (int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		...
		if (argc == 3)
		{
			...
			else
			{
				...
				{
					printf ("error: end record out of range (max %"
							PRIu32 ")\n", records);
				}
			}
		}
	...
	while ((r < records) && (count < end))
	{
		...
		const uint64_t when =
			(((uint64_t) trace_buffer[r + 4]) << 32) | trace_buffer[r + 5];
		...
		if (count > start)
		{
			const rtems_trace_sig *sig =
				rtems_trace_signatures (func_index);

			rtems_trace_buffering_print_timestamp (when);
			printf (" %10" PRIu32 " %c%08" PRIx32 " [%3" PRIu32 "/%3" PRIu32
					"] %c %s", (uint32_t) (when - last_sample),
					irq ? '*' : ' ', task_id, (task_status >> 8) & 0xff,
					task_status & 0xff, ret ? '<' : '>',
					rtems_trace_names (func_index));
			...
			if (sig->argc)
			{
				if (ret)
				{
					if (sig->args[0].size)
						printf (" => ");
					rtems_trace_buffering_print_arg (&sig->args[0],
													 argv);
				}
				else
				{
					...
					for (a = 1; a < sig->argc; ++a)
					{
						if (a > 1)
							printf (", ");
						rtems_trace_buffering_print_arg (&sig->args
														 [a], argv);
...
static int rtems_trace_buffering_shell_save (int argc, char *argv[])
{
	uint32_t *trace_buffer;
	uint32_t records;
	uint32_t traces;
...
static void rtems_trace_buffering_shell_usage (const char *arg)
{
...
static int rtems_shell_main_rtrace (int argc, char *argv[])
{
	int arg;
	size_t t;
