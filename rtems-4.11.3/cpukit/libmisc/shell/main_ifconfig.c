...
static int rtems_shell_main_ifconfig (int argc, char *argv[])
{
	...
	else
	{
		...
		else
		{
			...
			if (argc >= 3)
			{
				...
				{
					if (inet_pton (AF_INET, argv[2], &ipaddr.sin_addr) <
						0)
					{
	...
	while (argc > cur_idx)
	{
		...
		else if (strcmp (argv[cur_idx], "netmask") == 0)
		{
			...
			if (inet_pton (AF_INET, argv[cur_idx + 1], &netmask.sin_addr) <
				0)
			{
		...
		else if (strcmp (argv[cur_idx], "broadcast") == 0)
		{
			...
			if (inet_pton (AF_INET, argv[cur_idx + 1], &broadcast.sin_addr)
				< 0)
			{
		...
		else if (strcmp (argv[cur_idx], "pointopoint") == 0)
		{
			...
			if (inet_pton (AF_INET, argv[cur_idx + 1], &dstaddr.sin_addr) <
				0)
			{
