 ...
 */
static rtems_shell_env_t *rtems_shell_init_env (rtems_shell_env_t * shell_env_p)
{
 ...
 */
static void rtems_shell_env_free (void *ptr)
{
	rtems_shell_env_t *shell_env;
 ...
 */
static int rtems_shell_line_editor (char *cmds[],
									int count,
									int size,
									const char *prompt, FILE * in, FILE * out)
{
...
static bool rtems_shell_login (rtems_shell_env_t * env, FILE * in, FILE * out)
{
	...
	{
		if ((env->devname[5] != 'p') ||
			(env->devname[6] != 't') || (env->devname[7] != 'y'))
		{
...
void rtems_shell_print_env (rtems_shell_env_t * shell_env)
{
	if (!shell_env)
	{
	...
	printk ("shell_env=%p\n"
			...
			shell_env->exit_shell, shell_env->forever);
}
#endif
	...
	if (cmd_argv && cmds[0])
	{
		...
		do
		{
			...
			if (result)
			{
				...
				{
					result =
						rtems_shell_login (shell_env, stdin, stdout);
				}
				else
				{
			...
			if (result)
			{
				...
				for (;;)
				{
					...
					if (!strcmp (cmds[cmd], "bye")
						|| !strcmp (cmds[cmd], "exit"))
					{
					...
					{
						shell_env->errorlevel =
							rtems_shell_execute_cmd (argv[0], argc,
													 argv);
					}
...
/* ----------------------------------------------- */
static rtems_status_code rtems_shell_run (const char *task_name,
										...
										rtems_shell_login_check_t login_check)
{
	...
	if (task_name && strlen (task_name) >= 4)
		name =
			rtems_build_name (task_name[0], task_name[1], task_name[2],
							task_name[3]);
	else
									...
									rtems_shell_login_check_t login_check)
{
	...
	return rtems_shell_run (task_name,	/* task_name */
...
rtems_status_code rtems_shell_script (const char *task_name,
									...
									const char *input,
									const char *output,
									bool output_append, bool wait, bool echo)
{
	...
	sc = rtems_shell_run (task_name,	/* task_name */
						task_stacksize,	/* task_stacksize */
						task_priority,	/* task_priority */
						NULL,	/* devname */
						0,	/* forever */
