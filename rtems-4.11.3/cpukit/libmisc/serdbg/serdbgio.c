...
#define FD_STORE_CNT 3
	 ...
	 */
	if (!err_occurred && (dev_name != NULL) && (dev_name[0] != '\0'))
	{
		do
		{
		...
		while (!err_occurred && (serdbg_fd < 3));
	}
	/*
	 ...
	 * ... get fd settings
	 */
	if (!err_occurred && (0 != tcgetattr (serdbg_fd, &act_termios)))
	{
		err_occurred = true;
	}
	if (!err_occurred)
	{
		...
		act_termios.c_oflag &= ~OPOST;

		act_termios.c_lflag &= ~(ECHO | ECHONL | ICANON | ISIG | IEXTEN);
...
\*=========================================================================*/
{
	 ...
	 */
	if ((serdbg_tty != NULL) && (serdbg_tty->device.write != NULL))
	{
		/*
...
\*=========================================================================*/
{
	 ...
	 * check, whether debug serial port is available
	 */
	if ((serdbg_tty != NULL) && (serdbg_tty->device.pollRead != NULL))
	{
		do
		{
			/*
