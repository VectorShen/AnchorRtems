...
static void _termios_printk_null_char (char c __attribute__ ((unused)))
{
	return;
}
...
\*=========================================================================*/
{
		...
		(termios_printk_tty->device.write != NULL))
	{
		...
		if (c == '\n')
		{
			termios_printk_tty->device.write (termios_printk_tty->minor,
											&cr, 1);
		}
...
\*=========================================================================*/
{
		...
		(termios_printk_tty->device.pollRead != NULL))
	{
		do
		{
			 ...
			 */
			c = termios_printk_tty->device.pollRead (termios_printk_tty->
													 minor);
		}
		while (c < 0);
	}
...
\*=========================================================================*/
{
	 ...
	 * ... get fd settings
	 */
	if (!err_occurred && (0 != tcgetattr (termios_printk_fd, &act_termios)))
	{
		err_occurred = true;
	}
	if (!err_occurred)
