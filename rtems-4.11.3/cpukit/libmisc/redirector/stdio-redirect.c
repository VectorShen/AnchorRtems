...
static bool rtems_stdio_redirect_lock (rtems_stdio_redirect * sr)
{
...
static bool rtems_stdio_redirect_unlock (rtems_stdio_redirect * sr)
{
...
static bool
rtems_stdio_redirect_write (rtems_stdio_redirect * sr, const char *buf,
							ssize_t len)
{
												 ...
												 handler)
{
	...
	if (buffer_size)
	{
		...
		{
			fprintf (stderr,
					 "error: stdio-redirect: no memory for buffer\n");
			free (sr->input);
			free (sr);
			return NULL;
		}
	...
	sc = rtems_task_create (name,
							...
							RTEMS_LOCAL | RTEMS_NO_FLOATING_POINT, &sr->reader);
	if (sc != RTEMS_SUCCESSFUL)
	{
	...
	if (sr->buffer)
	{
		...
		if (sr->full)
		{
			if (length < sr->buffer_size)
			{
				...
				{
					rem = length - sr->in;
					memcpy (buffer, sr->buffer + sr->buffer_size - rem,
							rem);
				}
