...
rtems_id telnetd_dflt_spawn (const char *name,
							 unsigned priority,
							 unsigned stackSize,
							 void (*fn) (void *), void *fnarg);

/***********************************************************/
static rtems_id telnetd_task_id = RTEMS_ID_NONE;

rtems_id (*telnetd_spawn_task) (const char *,
								unsigned,
								unsigned,
								void (*)(void *), void *) = telnetd_dflt_spawn;

static char *grab_a_Connection (int des_socket,
								uni_sa * srv, char *peername, int sz)
{
	char *rval = 0;
	...
	syslog (LOG_DAEMON | LOG_INFO,
			"telnetd: accepted connection from %s on %s", peername, rval);
#endif
...
static void release_a_Connection (char *devname, char *peername, FILE ** pstd,
								  int n)
{

#ifdef PARANOIA
	syslog (LOG_DAEMON | LOG_INFO,
			"telnetd: releasing connection from %s on %s", peername, devname);
#endif
...
static void spawned_shell (void *arg);
		  ...
		  if (rtems_telnetd_config.keep_stdio)
			{
				...
				if (rtems_telnetd_config.login_check != NULL)
				  {
					  start = rtems_shell_login_prompt (stdin,
														stderr,
														device_name,
			...
			{
				devname =
					grab_a_Connection (des_socket, &srv, peername,
									   sizeof (peername));
				...
				telnetd_task_id = telnetd_spawn_task (devname,
													  rtems_telnetd_config.
													  priority,
													  rtems_telnetd_config.
													  stack_size, spawned_shell,
													  arg);
				if (telnetd_task_id == RTEMS_ID_NONE)
				  {
					  ...
					  if (telnetd_spawn_task != telnetd_dflt_spawn)
						{
							fprintf (stderr,
									 "Telnetd: Unable to spawn child task\n");
						}
					  ...
					  if (!(dummy = fopen (devname, "r+")))
						  perror
							  ("Unable to dummy open the pty, losing a slot :-(");
...
rtems_status_code rtems_telnetd_initialize (void)
{
	...
	/* Spawn task */
	telnetd_task_id = telnetd_spawn_task ("TNTD",
										  rtems_telnetd_config.priority,
										  rtems_telnetd_config.stack_size,
										  rtems_task_telnetd, 0);
	if (telnetd_task_id == RTEMS_ID_NONE)
	  {
		  return RTEMS_IO_ERROR;
	  }
...
/* utility wrapper */
static void spawned_shell (void *targ)
{
...
static rtems_task wrap_delete (rtems_task_argument arg)
{
...
rtems_id
telnetd_dflt_spawn (const char *name, unsigned int priority,
					unsigned int stackSize, void (*fn) (void *), void *fnarg)
{
	rtems_status_code sc;
	...
	if ((sc = rtems_task_create (rtems_build_name (nm[0], nm[1], nm[2], nm[3]),
		...
		|| (sc =
			rtems_task_start (task_id, wrap_delete, (rtems_task_argument) pwa)))
	  {
		  free (pwa);
 ...
 */
#ifdef stdin
static __inline__ FILE *_stdin (void)
{
	return stdin;
}
