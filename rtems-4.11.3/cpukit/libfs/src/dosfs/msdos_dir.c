			 ...
			 i += MSDOS_DIRECTORY_ENTRY_STRUCT_SIZE)
		{
				...
				MSDOS_ATTR_LFN)
			{
				...
				if (lfn_start == FAT_FILE_SHORT_NAME)
				{
					...
					memset (tmp_dirent.d_name, 0,
							sizeof (tmp_dirent.d_name));
				}
				else
				...
				lfn_len += msdos_get_utf16_string_from_long_entry (entry,
																	 &lfn_buf
																	 [offset_lfn],
																	 buf_size
																	 -
			...
			else
			{
				...
				if (lfn_start != FAT_FILE_SHORT_NAME)
				{
						...
						lfn_checksum == msdos_lfn_checksum (entry))
					{
						...
						eno =
							(*convert_handler->
							 utf16_to_utf8) (converter, lfn_buf,
											 lfn_len,
											 (uint8_t *) & tmp_dirent.
											 d_name[0], &len);
						if (eno == 0)
				...
				if (lfn_start == FAT_FILE_SHORT_NAME)
				{
					 ...
					 */
					tmp_dirent.d_namlen = msdos_format_dirent_with_dot (sfn_buf, entry);	/* src text */
					eno =
						(*convert_handler->codepage_to_utf8) (converter,
															sfn_buf,
															tmp_dirent.
															d_namlen,
															(uint8_t
				...
				if (cmpltd >= 0)
				{
					memcpy (buffer + cmpltd, &tmp_dirent,
 ...
 */
int
msdos_dir_stat (const rtems_filesystem_location_info_t * loc, struct stat *buf)
{
	rtems_status_code sc = RTEMS_SUCCESSFUL;
