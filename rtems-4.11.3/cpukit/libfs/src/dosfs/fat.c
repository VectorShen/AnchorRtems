...
static int _fat_block_release (fat_fs_info_t * fs_info);

static inline uint32_t
fat_cluster_num_to_block_num (const fat_fs_info_t * fs_info, uint32_t cln)
{
	uint32_t blk;
...
fat_buf_access (fat_fs_info_t * fs_info,
				const uint32_t sec_num, const int op_type, uint8_t ** sec_buf)
{
...
_fat_block_read (fat_fs_info_t * fs_info,
				 uint32_t start, uint32_t offset, uint32_t count, void *buff)
{
	int rc = RC_OK;
	ssize_t cmpltd = 0;
...
static ssize_t
fat_block_write (fat_fs_info_t * fs_info,
				 const uint32_t start_blk,
				 const uint32_t offset, const uint32_t count, const void *buf)
{
	int rc = RC_OK;
	uint32_t bytes_to_write =
		MIN (count, (fs_info->vol.bytes_per_block - offset));
	uint8_t *blk_buf;
				 ...
				 const uint32_t count, const uint8_t pattern)
{
	...
	uint32_t ofs_blk =
		offset - (blocks_in_offset << fs_info->vol.bytes_per_block_log2);
 ...
 */
int _fat_block_release (fat_fs_info_t * fs_info)
{
 ...
 */
static int fat_fat32_update_fsinfo_sector (fat_fs_info_t * fs_info)
{
 ...
 */
inline bool fat_ino_is_unique (fat_fs_info_t * fs_info, uint32_t ino)
{
