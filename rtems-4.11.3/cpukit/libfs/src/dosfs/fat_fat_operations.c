 ...
 */
int
fat_scan_fat_for_free_clusters (fat_fs_info_t * fs_info,
								uint32_t * chain,
								uint32_t count,
								uint32_t * cls_added,
	...
	*cls_added = 0;
	...
	while (*cls_added != count && i < data_cls_val)
	{
		...
		if (next_cln == FAT_GENFAT_FREE)
		{
			...
			{
				*chain = cl4find;
				rc = fat_set_fat_cluster (fs_info, cl4find,
											FAT_GENFAT_EOC);
				if (rc != RC_OK)
				{
					/*
			...
			if (zero_fill)
			{
				ssize_t bytes_written =
					fat_cluster_set (fs_info, cl4find, 0,
									 fs_info->vol.bpc, 0);
 ...
 */
int
fat_get_fat_cluster (fat_fs_info_t * fs_info, uint32_t cln, uint32_t * ret_val)
{
	int rc = RC_OK;
	uint8_t *sec_buf;
	uint32_t sec = 0;
...
int fat_set_fat_cluster (fat_fs_info_t * fs_info, uint32_t cln, uint32_t in_val)
{
	...
	switch (fs_info->vol.type)
	{
		case FAT_FAT12:
			if (FAT_CLUSTER_IS_ODD (cln))
			{
				...
				*(sec_buf + ofs) |= (uint8_t) (fat16_clv & 0x00F0);
				...
				if (ofs == (fs_info->vol.bps - 1))
				{
					rc = fat_buf_access (fs_info, sec + 1,
										 FAT_OP_TYPE_READ, &sec_buf);
					if (rc != RC_OK)
						return rc;
		...
		case FAT_FAT16:
			*((uint16_t *) (sec_buf + ofs)) = (uint16_t) (CT_LE_W (in_val));
			fat_buf_mark_modified (fs_info);
			break;
