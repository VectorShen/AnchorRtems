...
void msdos_date_unix2dos (unsigned int t, uint16_t * ddp, uint16_t * dtp)
{
	...
	if (lasttime != t)
	{
		...
		if (days != lastday)
		{
			 ...
			 */
			if (year > 1980)
				lastddate += (year - 1980) << MSDOS_DD_YEAR_SHIFT;
		}
	}
	*dtp = lastdtime;
	*ddp = lastddate;
...
msdos_filename_process_dot_names (const uint8_t * src_name,
								...
								uint8_t * dest_name, const size_t dest_size)
{
	 ...
	 */
	if (src_name[0] == UTF8_FULL_STOP && src_size == UTF8_FULL_STOP_SIZE)
	{
...
ssize_t
msdos_filename_utf8_to_long_name_for_compare (rtems_dosfs_convert_control *
											converter,
											...
											const size_t long_name_size)
{
	...
	returned_size = msdos_filename_process_dot_names (utf8_name,
													utf8_name_size,
													long_name,
													long_name_size);
	...
	if (returned_size == 0)
	{
		name_size = msdos_filename_delete_trailing_dots (&utf8_name[0],
														 utf8_name_size);
		if (name_size > 0)
		{
...
msdos_filename_utf8_to_long_name_for_save (rtems_dosfs_convert_control *
										 converter, const uint8_t * utf8_name,
										 const size_t utf8_name_size,
										 uint16_t * long_name,
										 const size_t long_name_size)
{
	...
	if (name_size > 0)
	{
		 ...
		 */
		eno = (*converter->handler->utf8_to_utf16) (converter,
													utf8_name,
													name_size,
													&long_name[0],
													&name_size_tmp);
		...
		{
			if (name_size_tmp <=
				(MSDOS_NAME_MAX_LNF_LEN * MSDOS_NAME_LFN_BYTES_PER_CHAR))
		...
		if (eno == 0)
		{
			...
			for (i = 0;
				 name_size
				 && (c =
					 msdos_get_valid_utf16_filename_character (long_name
															 [i])); ++i)
			{
				long_name[i] = c;
...
static void msdos_filename_remove_prepended_dots (const uint8_t ** name_utf8,
												size_t * name_size)
{
	while (*name_size >= UTF8_FULL_STOP_SIZE && **name_utf8 == UTF8_FULL_STOP)
	{
...
ssize_t
msdos_filename_utf8_to_short_name_for_compare (rtems_dosfs_convert_control *
											 converter,
											 const uint8_t * utf8_name,
											 const size_t utf8_name_size,
											 void *short_name,
											 const size_t short_name_size)
{
	...
	uint8_t name_normalized_buf[(MSDOS_SHORT_NAME_LEN + 1) *
	...
	if (returned_size == 0)
	{
		msdos_filename_remove_prepended_dots (&name_ptr, &name_size);
		if (name_size > 0)
		{
			/*
	...
	if (returned_size == 0)
	{
		...
		if (name_size > 0)
		{
			...
			for (i = 0; i < name_size; ++i)
				name_to_format_buf[i] =
					toupper ((unsigned char)(name_to_format_buf[i]));
			/*
			...
			if (name_size > 0)
			{
				...
				if (0x20 == *name_ptr)
					...
					dest_ptr[0] =
						msdos_get_valid_codepage_filename_character
						(*name_ptr);
				++name_ptr;
				++returned_size;
				--name_size;
