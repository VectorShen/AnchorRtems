...
fat_file_lseek (fat_fs_info_t * fs_info,
				fat_file_fd_t * fat_fd, uint32_t file_cln, uint32_t * disk_cln);
...
{
	return (FAT_FD_OF_ROOT_DIR (fat_fd))
		&& (volume_type & (FAT_FAT12 | FAT_FAT16));
}
...
fat_file_extend (fat_fs_info_t * fs_info,
				 fat_file_fd_t * fat_fd,
				 bool zero_fill, uint32_t new_length, uint32_t * a_length)
{
	int rc = RC_OK;
	uint32_t chain = 0;
 ...
 */
int
fat_file_ioctl (fat_fs_info_t * fs_info, fat_file_fd_t * fat_fd, int cmd, ...)
{
	int rc = RC_OK;
	uint32_t cur_cln = 0;
	uint32_t cl_start = 0;
