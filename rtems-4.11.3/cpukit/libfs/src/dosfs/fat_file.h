...
static inline bool FAT_FILE_HAS_META_DATA_CHANGED (const fat_file_fd_t *
												   fat_fd)
{
...
static inline uint32_t
	fat_construct_key (const fat_fs_info_t * fs_info, fat_pos_t * pos)
{
...
static inline void fat_file_set_file_size (fat_file_fd_t * fat_fd,
										   uint32_t s)
{
...
static inline void fat_file_set_ctime_mtime (fat_file_fd_t * fat_fd,
											 time_t t)
{
	fat_fd->ctime = t;
	fat_fd->mtime = t;
...
int fat_file_open (fat_fs_info_t * fs_info,
...
ssize_t
	fat_file_write (fat_fs_info_t * fs_info,
					fat_file_fd_t * fat_fd,
					uint32_t start, uint32_t count, const uint8_t * buf);

int fat_file_extend (fat_fs_info_t * fs_info,
					 fat_file_fd_t * fat_fd,
					 bool zero_fill,
					 uint32_t new_length, uint32_t * a_length);

int fat_file_truncate (fat_fs_info_t * fs_info,
					   fat_file_fd_t * fat_fd, uint32_t new_length);

int fat_file_ioctl (fat_fs_info_t * fs_info,
					fat_file_fd_t * fat_fd, int cmd, ...);

int fat_file_size (fat_fs_info_t * fs_info, fat_file_fd_t * fat_fd);

void fat_file_mark_removed (fat_fs_info_t * fs_info,
							fat_file_fd_t * fat_fd);

int fat_file_size (fat_fs_info_t * fs_info, fat_file_fd_t * fat_fd);

int fat_file_write_first_cluster_num (fat_fs_info_t * fs_info,
									  fat_file_fd_t * fat_fd);

int fat_file_write_file_size (fat_fs_info_t * fs_info,
							  fat_file_fd_t * fat_fd);

int fat_file_write_time_and_date (fat_fs_info_t * fs_info,
								  fat_file_fd_t * fat_fd);
