 ...
 *
 */
static msdos_name_type_t msdos_is_valid_name_char (const char ch)
{
	if (strchr (" +,;=[]", ch) != NULL)
 ...
 *
 */
static void msdos_short_name_hex (char *sfn, uint32_t num)
{
...
static msdos_name_type_t msdos_name_type (const char *name, int name_len)
{
...
#if MSDOS_NAME_TYPE_PRINT
				printf
					("MSDOS_NAME_TYPE: LONG[1]: is_dot:%d, at:%d cnt\n",
					 is_dot, dot_at, count);
#endif
				return MSDOS_NAME_LONG;
			}
...
#if MSDOS_NAME_TYPE_PRINT
				printf
					("MSDOS_NAME_TYPE: LONG[2]: is_dot:%d, at:%d cnt\n",
					 is_dot, dot_at, count);
#endif
				return MSDOS_NAME_LONG;
			}
...
msdos_long_to_short (rtems_dosfs_convert_control * converter,
					 const char *lfn, int lfn_len, char *sfn, int sfn_len)
{
	...
	eno = (*converter->handler->utf8_to_codepage) (converter,
												 (const uint8_t *)&lfn[0],
												 lfn_len,
												 buffer, &codepage_name_len);
	if (eno == EINVAL)
	{
		eno = 0;
	...
	{
		type = msdos_name_type (buffer, codepage_name_len);
	}

	if (type != MSDOS_NAME_INVALID)
	{
		short_filename_length =
			msdos_filename_utf8_to_short_name_for_save (converter,
														(const uint8_t *)lfn,
														lfn_len, sfn,
														short_filename_length);
				 ...
				 const char *name, int name_len)
{
	...
	if (((*MSDOS_DIR_ATTR (node_entry)) & MSDOS_ATTR_VOLUME_ID) ||
		((*MSDOS_DIR_ATTR (node_entry) & MSDOS_ATTR_LFN_MASK) ==
		 MSDOS_ATTR_LFN))
	...
	if (fat_fd->links_num == 1)
	{
		...
		fat_fd->mtime =
			msdos_date_dos2unix (CF_LE_W (date), CF_LE_W (time_val));
		...
		else
		{
			fat_fd->fat_file_size =
				CF_LE_L (*MSDOS_DIR_FILE_SIZE (node_entry));
 ...
 */
int
msdos_get_name_node (const rtems_filesystem_location_info_t * parent_loc,
					 bool create_node,
					 const char *name,
					 int name_len,
					 msdos_name_type_t name_type,
					 fat_dir_pos_t * dir_pos, char *name_dir_entry)
{
	...
	rc = msdos_find_name_in_fat_file (parent_loc->mt_entry, fat_fd,
									create_node, (const uint8_t *)name,
									name_len, name_type, dir_pos,
									name_dir_entry);
	...
	if (!create_node)
	{
		...
		if (rc == RC_OK)
		{
			if (strncmp (name, "..", 2) == 0)
			{
				...
				{
					rc = msdos_get_dotdot_dir_info_cluster_num_and_offset (parent_loc->mt_entry, dotdot_cln, dir_pos, name_dir_entry);
					if (rc != RC_OK)
						return rc;
				}
			}
 ...
 fat_dir_pos_t * dir_pos, char *dir_entry)
{
	...
	msdos_long_to_short (converter, ".", 1, dot_node, MSDOS_SHORT_NAME_LEN);
	rc = msdos_find_name_in_fat_file (mt_entry, fat_fd, false,
									(const uint8_t *)".", 1, MSDOS_NAME_SHORT,
									dir_pos, dot_node);
	...
	msdos_long_to_short (converter, "..", 2, dotdot_node, MSDOS_SHORT_NAME_LEN);
	rc = msdos_find_name_in_fat_file (mt_entry, fat_fd, false,
									(const uint8_t *)"..", 2,
									MSDOS_NAME_SHORT, dir_pos, dotdot_node);
 ...
 */
int
fat_file_write_time_and_date (fat_fs_info_t * fs_info, fat_file_fd_t * fat_fd)
{
	int rc = RC_OK;
	ssize_t ret;
	uint16_t time_val;
...
msdos_set_first_char4file_name (rtems_filesystem_mount_table_entry_t * mt_entry,
								fat_dir_pos_t * dir_pos, unsigned char fchar)
{
	ssize_t ret;
	msdos_fs_info_t *fs_info = mt_entry->fs_info;
	...
	{
		uint32_t sec =
			(fat_cluster_num_to_sector_num (&fs_info->fat, start.cln) +
			 (start.ofs >> fs_info->fat.vol.sec_log2));
		uint32_t byte = (start.ofs & (fs_info->fat.vol.bps - 1));

		ret =
			fat_sector_write (&fs_info->fat, sec,
								byte + MSDOS_FILE_NAME_OFFSET, 1, &fchar);
		if (ret < 0)
			return -1;
					...
					fat_file_fd_t * fat_fd, bool * ret_val)
{
	...
	*ret_val = false;
	...
	while ((ret =
			fat_file_read (&fs_info->fat, fat_fd, j * fs_info->fat.vol.bps,
						 fs_info->fat.vol.bps, fs_info->cl_buf)) != FAT_EOF)
	{
		...
		for (i = 0;
			 i < fs_info->fat.vol.bps; i += MSDOS_DIRECTORY_ENTRY_STRUCT_SIZE)
		{
			...
			if (((*MSDOS_DIR_ENTRY_TYPE (entry)) ==
				...
				(strncmp (MSDOS_DIR_NAME ((entry)),
						MSDOS_DOTDOT_NAME, MSDOS_SHORT_NAME_LEN) == 0))
				continue;
...
static int
msdos_on_entry_found (msdos_fs_info_t * fs_info,
					fat_file_fd_t * fat_fd,
					...
					fat_dir_pos_t * dir_pos,
					uint32_t dir_offset,
					const uint32_t dir_entry, const fat_pos_t * lfn_start)
{
	int rc = RC_OK;
#if MSDOS_FIND_PRINT
	...
	rc = fat_file_ioctl (&fs_info->fat,
						 fat_fd,
						 F_CLU_NUM, dir_offset * bts2rd, &dir_pos->sname.cln);
	if (rc == RC_OK)
	{
		...
		if (lfn_start->cln != FAT_FILE_SHORT_NAME)
		{
			rc = fat_file_ioctl (&fs_info->fat,
								 fat_fd,
								 F_CLU_NUM,
								 lfn_start->cln * bts2rd, &lfn_start->cln);
		}
		if (rc == RC_OK)
		{
...
static ssize_t
msdos_long_entry_to_utf8_name (rtems_dosfs_convert_control * converter,
							 const char *entry,
							 const bool is_first_entry,
							 uint8_t * entry_utf8_buf, const size_t buf_size)
{
	ssize_t retval = 0;
	int eno = 0;
	...
	retval = msdos_get_utf16_string_from_long_entry (entry,
													 entry_string,
													 sizeof (entry_string),
													 is_first_entry);
	...
	{
		bytes_in_buf = retval;
		eno = (*converter->handler->utf16_to_utf8) (converter,
													&entry_string[0],
													bytes_in_buf,
													&entry_utf8_buf[0],
													&bytes_in_utf8);
...
static ssize_t msdos_short_entry_to_utf8_name (rtems_dosfs_convert_control *
											 converter, const char *entry,
											 uint8_t * buf,
											 const size_t buf_size)
{
	...
	if (bytes_written > 0)
	{
		...
		eno = (*converter->handler->codepage_to_utf8) (converter,
														 char_buf,
														 bytes_written,
														 buf, &bytes_converted);
		if (eno == 0)
...
static ssize_t
msdos_compare_entry_against_filename (rtems_dosfs_convert_control * converter,
									const uint8_t * entry,
									...
									bool * is_matching)
{
	...
	eno = (*converter->handler->utf8_normalize_and_fold) (converter,
														&entry[0],
														entry_size,
														&entry_normalized[0],
	...
	if (eno == 0)
	{
		...
		printf ("MSFS:[6] entry_normalized:%s" "name:%s\n", entry, filename);
#endif
...
static void
msdos_prepare_for_next_entry (fat_pos_t * lfn_start,
							bool * entry_matched,
...
static int
msdos_find_file_in_directory (const uint8_t * filename_converted,
							...
							uint32_t * empty_entry_count)
{
	...
	while ((bytes_read =
			fat_file_read (&fs_info->fat, fat_fd, (dir_offset * bts2rd), bts2rd,
						 fs_info->cl_buf)) != FAT_EOF && rc == RC_OK)
	{
...
/*                    int   o;*/
					...
					printf ("MSFS:[4.2] lfn:%c entry:%i checksum:%i\n",
							lfn_start.cln ==
							FAT_FILE_SHORT_NAME ? 'f' : 't',
							*MSDOS_DIR_ENTRY_TYPE (entry) &
					...
					bytes_in_entry =
						msdos_long_entry_to_utf8_name (converter, entry,
													 is_first_lfn_entry,
					...
					if (bytes_in_entry > 0)
					{
						name_len_remaining =
							...
							(converter, &entry_utf8_normalized[0],
							 bytes_in_entry, &filename_converted[0],
							 name_len_remaining, &entry_matched);

						if (name_len_remaining < 0 || !entry_matched)
						{
							msdos_prepare_for_next_entry
								(&lfn_start, &entry_matched,
								 &name_len_remaining,
								 name_len_for_compare);
						}
					}
				...
				else
				{
					...
					if (entry_matched)
					{
						...
						else if (name_len_remaining == 0)
						{
							...
							rc = msdos_on_entry_found (fs_info,
													 fat_fd,
													 bts2rd,
													 name_dir_entry,
													 entry,
					...
					else if ((*MSDOS_DIR_ATTR (entry) &
							MSDOS_ATTR_VOLUME_ID) == 0)
					{
						bytes_in_entry = MSDOS_SHORT_NAME_LEN + 1;
						bytes_in_entry =
							msdos_short_entry_to_utf8_name (converter,
															MSDOS_DIR_NAME
															(entry),
															&entry_utf8_normalized
															[0],
						...
						if (bytes_in_entry > 0)
						{
								...
								&& name_len_remaining == 0)
							{
								...
								rc = msdos_on_entry_found
									(fs_info, fat_fd, bts2rd,
									 name_dir_entry, entry,
									 dir_pos, dir_offset,
									 dir_entry, &lfn_start);
							}
...
static int
msdos_get_pos (msdos_fs_info_t * fs_info,
			 fat_file_fd_t * fat_fd,
			 uint32_t bts2rd, uint32_t file_offset, fat_pos_t * pos)
{
...
static int
msdos_add_file (const char *name_converted,
				const msdos_name_type_t name_type,
				msdos_fs_info_t * fs_info,
				fat_file_fd_t * fat_fd,
				const uint32_t bts2rd,
				...
				fat_dir_pos_t * dir_pos,
				uint32_t empty_file_offset, const uint32_t empty_entry_count)
{
	int ret;
	...
	bytes_written = fat_file_write (&fs_info->fat, fat_fd,
									empty_file_offset, length, fs_info->cl_buf);
...
int
msdos_find_name_in_fat_file (rtems_filesystem_mount_table_entry_t * mt_entry,
							 fat_file_fd_t * fat_fd,
							 bool create_node,
							 ...
							 fat_dir_pos_t * dir_pos, char *name_dir_entry)
{
	...
	switch (name_type)
	{
		case MSDOS_NAME_SHORT:
			name_len_for_compare =
				msdos_filename_utf8_to_short_name_for_compare (converter,
																 name_utf8,
																 name_utf8_len,
																 buffer,
		...
		case MSDOS_NAME_LONG:
			name_len_for_save =
				msdos_filename_utf8_to_long_name_for_save (converter,
															 name_utf8,
															 name_utf8_len,
															 buffer,
															 buffer_size);
			if (name_len_for_save > 0)
			{
				...
				name_len_for_compare =
					msdos_filename_utf8_to_long_name_for_compare (converter,
																name_utf8,
																name_utf8_len,
																buffer,
																buffer_size);
	...
	if (retval == RC_OK)
	{
		...
		retval = msdos_find_file_in_directory (buffer,
												 name_len_for_compare,
												 name_len_for_save,
												 name_type,
												 fs_info,
	...
	if (retval == RC_OK && create_node)
	{
		switch (name_type)
		{
			...
			case MSDOS_NAME_LONG:
				...
				{
					lfn_entries =
						(name_len_for_save + MSDOS_LFN_ENTRY_SIZE -
						 1) / MSDOS_LFN_ENTRY_SIZE;
				}
				else
					retval = -1;
				break;
		...
		if (retval == RC_OK)
			retval = msdos_add_file (buffer,
									 ...
									 name_dir_entry,
									 dir_pos,
									 empty_file_offset, empty_entry_count);
	}
