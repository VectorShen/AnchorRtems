...
static devFS_node *devFS_search_node (const devFS_data * data,
									const char *path,
									size_t pathlen,
...
void devFS_eval_path (rtems_filesystem_eval_path_context_t * ctx)
{
	...
	devFS_node *node = devFS_search_node (devFS_get_data (currentloc),
	...
	else
	{
		if ((eval_flags & RTEMS_FS_MAKE) != 0)
		{
			if (free_node != NULL)
			{
				...
				rtems_filesystem_eval_path_set_token (ctx,
														rtems_filesystem_eval_path_get_path
														(ctx),
														rtems_filesystem_eval_path_get_pathlen
														(ctx));
