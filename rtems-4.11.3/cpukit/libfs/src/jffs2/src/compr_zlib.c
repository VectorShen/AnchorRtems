...
static rtems_jffs2_compressor_zlib_control
	*get_zlib_control (rtems_jffs2_compressor_control * super)
{
	return (rtems_jffs2_compressor_zlib_control *) super;
}

uint16_t rtems_jffs2_compressor_zlib_compress (rtems_jffs2_compressor_control *
											 super, unsigned char *data_in,
											 ...
											 uint32_t * dstlen)
{
	...
	{
		def_strm->avail_out =
			*dstlen - (def_strm->total_out + STREAM_END_SPACE);
		def_strm->avail_in =
			min ((unsigned)(*sourcelen - def_strm->total_in),
				 def_strm->avail_out);
