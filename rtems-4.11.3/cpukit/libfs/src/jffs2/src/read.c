					  ...
					  int ofs, int len)
{
	...
	ret =
		jffs2_flash_read (c, ref_offset (fd->raw), sizeof (*ri), &readlen,
						  (char *)ri);
	if (ret)
	  {
	...
	jffs2_dbg (1,
			   ...
			   je32_to_cpu (ri->dsize), je32_to_cpu (ri->csize),
	...
	D1 (if (ofs + len > je32_to_cpu (ri->dsize))
		...
		("jffs2_read_dnode() asked for %d bytes at %d from %d-byte node\n", len,
		 ofs, je32_to_cpu (ri->dsize)); ret = -EINVAL; goto out_ri;}
	) ;
	...
	if (ri->compr != JFFS2_COMPR_NONE)
	  {
		  ...
		  ret =
			  jffs2_decompress (c, f, ri->compr | (ri->usercompr << 8), readbuf,
	...
	while (offset < end)
	  {
		  ...
		  if (unlikely (!frag || frag->ofs > offset ||
		  ...
		  else
			{
				...
				ret =
					jffs2_read_dnode (c, f, frag->node, buf,
									  fragofs + frag->ofs - frag->node->ofs,
									  readlen);
