		  ...
		  while (c->nr_free_blocks + c->nr_erasing_blocks < blocksneeded)
			{
				 ...
				 */
				avail =
					c->free_size + c->dirty_size + c->erasing_size +
					c->unchecked_size;
	...
	if (list_empty (&c->free_list))
	  {
		  ...
		  if (!c->nr_erasing_blocks && !list_empty (&c->erasable_list))
			{
  ...
  restart:
	...
	*len = jeb->free_size - reserved_size;
	...
	jffs2_dbg (1, "%s(): Giving 0x%x bytes at 0x%x\n",
			   __func__, *len, jeb->offset + (c->sector_size - jeb->free_size));
	return 0;
}
							   ...
							   struct jffs2_raw_node_ref *ref)
{
	...
	if (ref_flags (ref) == REF_UNCHECKED)
	  {
		  D1 (if (unlikely (jeb->unchecked_size < freed_len))
			  ...
			  ("raw unchecked node of size 0x%08x freed from erase block %d at 0x%08x, but unchecked_size was already 0x%08x\n",
			   freed_len, blocknr, ref->flash_offset, jeb->used_size); BUG ();}
		  )
			  jffs2_dbg (1,
	...
	ret =
		jffs2_flash_read (c, ref_offset (ref), sizeof (n), &retlen, (char *)&n);
	if (ret)
	  {
	...
	ret =
		jffs2_flash_write (c, ref_offset (ref), sizeof (n), &retlen,
						   (char *)&n);
	if (ret)
	  {
...
int jffs2_thread_should_wake (struct jffs2_sb_info *c)
{
	...
	jffs2_dbg (1,
			   ...
			   __func__, c->nr_free_blocks, c->nr_erasing_blocks, c->dirty_size,
			   nr_very_dirty, ret ? "yes" : "no");
