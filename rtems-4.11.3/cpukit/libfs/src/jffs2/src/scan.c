	...
	for (i = 0; i < c->nr_blocks; i++)
	  {
		  ...
		  ret =
			  jffs2_scan_eraseblock (c, jeb,
									 buf_size ? flashbuf : (flashbuf +
															jeb->offset),
									 buf_size, s);
							 ...
							 struct jffs2_eraseblock *jeb)
{
	if ((jeb->used_size + jeb->unchecked_size) == PAD (c->cleanmarker_size)
		&& !jeb->dirty_size && (!jeb->first_node
								|| !ref_next (jeb->first_node)))
...
static int jffs2_scan_xattr_node (struct jffs2_sb_info *c,
								  struct jffs2_eraseblock *jeb,
...
static int jffs2_scan_xref_node (struct jffs2_sb_info *c,
								 struct jffs2_eraseblock *jeb,
								 ...
								 struct jffs2_summary *s)
{
	...
	jffs2_link_node_ref (c, jeb, ofs | REF_PRISTINE,
						 PAD (je32_to_cpu (rr->totlen)), (void *)ref);
	...
	if (jffs2_sum_active ())
	  {
					  ...
					  if (sumlen > buf_size)
						{
							...
							if (!sumptr)
								return -ENOMEM;
							memcpy (sumptr + sumlen - buf_len,
									buf + buf_size - buf_len, buf_len);
						}
					  if (buf_len < sumlen)
						{
		  ...
		  if (sumptr)
			{
				err =
					jffs2_sum_scan_sumnode (c, jeb, sumptr, sumlen,
											&pseudo_random);
		  ...
		  if (jffs2_cleanmarker_oob (c))
			{
				...
				switch (ret)
				  {
					  case 0:
						  return cleanmarkerfound ? BLK_STATE_CLEANMARKER :
							  BLK_STATE_ALLFF;
					  case 1:
						  return BLK_STATE_ALLDIRTY;
					  default:
	...
	while (ofs < jeb->offset + c->sector_size)
	  {
		  ...
		  if (*(uint32_t *) (&buf[ofs - buf_ofs]) == 0xffffffff)
			{
				...
				ofs += 4;
				scan_end =
					min_t (uint32_t, EMPTY_SCAN_SIZE (c->sector_size) / 8,
						   buf_len);
		  ...
		  if (je16_to_cpu (node->magic) != JFFS2_MAGIC_BITMASK)
			{
				...
				noisy_printk (&noise,
							  ...
							  __func__, JFFS2_MAGIC_BITMASK, ofs,
		  ...
		  crcnode.nodetype =
			  cpu_to_je16 (je16_to_cpu (node->nodetype) | JFFS2_NODE_ACCURATE);
		  ...
		  if (hdr_crc != je32_to_cpu (node->hdr_crc))
			{
				noisy_printk (&noise,
							  ...
							  __func__, ofs, je16_to_cpu (node->magic),
		  ...
		  switch (je16_to_cpu (node->nodetype))
			{
					...
					if (buf_ofs + buf_len <
						ofs + sizeof (struct jffs2_raw_inode))
					  {
						  ...
						  jffs2_dbg (1,
									 ...
									 sizeof (struct jffs2_raw_inode), buf_len,
									 ofs);
						  err = jffs2_fill_scan_buf (c, buf, ofs, buf_len);
						  if (err)
							  return err;
				...
				case JFFS2_NODETYPE_CLEANMARKER:
					...
					if (je32_to_cpu (node->totlen) != c->cleanmarker_size)
					  ...
					  {
						  jffs2_link_node_ref (c, jeb, ofs | REF_NORMAL,
											   c->cleanmarker_size, NULL);
