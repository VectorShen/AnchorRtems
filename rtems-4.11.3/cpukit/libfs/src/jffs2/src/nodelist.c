	...
	if (frag->node && (frag->ofs & (PAGE_CACHE_SIZE - 1)) == 0)
	  {
		  ...
		  frag->node->raw->flash_offset =
			  ref_offset (frag->node->raw) | REF_PRISTINE;
	  }
	return size;
}
		  ...
		  if (!this->node->frags)
			{
				...
				dbg_fragtree2
					("marking old node @0x%08x (0x%04x-0x%04x) obsolete\n",
					 ref_offset (this->node->raw), this->node->ofs,
		  ...
		  else
			{
				dbg_fragtree2
					("marking old node @0x%08x (0x%04x-0x%04x) REF_NORMAL. frags is %d\n",
					 ref_offset (this->node->raw), this->node->ofs,
					 this->node->ofs + this->node->size, this->node->frags);
	...
	if (this)
	  {
		  dbg_fragtree2 ("lookup gave frag 0x%04x-0x%04x; phys 0x%08x (*%p)\n",
						 ...
						 this->
						 node ? (ref_offset (this->node->raw)) : 0xffffffff,
						 this);
		  ...
		  if (this->ofs + this->size > newfrag->ofs + newfrag->size)
			{
				...
				newfrag2 =
					new_fragment (this->node, newfrag->ofs + newfrag->size,
								  this->ofs + this->size - newfrag->ofs -
								  newfrag->size);
	...
	else
	  {
		  ...
		  dbg_fragtree2
			  ("inserting newfrag (*%p),%d-%d in before 'this' (*%p),%d-%d\n",
			   newfrag, newfrag->ofs, newfrag->ofs + newfrag->size, this,
			   this->ofs, this->ofs + this->size);
		  ...
		  if (unlikely (ref != jeb->last_node))
			{
				pr_crit ("ref %p @0x%08x is not jeb->last_node (%p @0x%08x)\n",
						 ...
						 jeb->last_node ? ref_offset (jeb->last_node) : 0);
				BUG ();
			}
	...
	if (unlikely (ret != ref->__totlen))
	  {
		  ...
		  pr_crit
			  ("Totlen for ref at %p (0x%08x-0x%08x) miscalculated as 0x%x instead of %x\n",
			   ref, ref_offset (ref), ref_offset (ref) + ref->__totlen, ret,
			   ref->__totlen);
		  if (ref_next (ref))
			{
