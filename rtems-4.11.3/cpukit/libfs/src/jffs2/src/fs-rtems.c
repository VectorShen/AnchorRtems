							  ...
							  unsigned long offset)
{
	...
	ret =
		jffs2_reserve_space (c, sizeof (*ri), &alloc_len, ALLOC_NORMAL,
...
static void rtems_jffs2_do_lock (const struct super_block *sb)
{
	rtems_status_code sc =
		rtems_semaphore_obtain (sb->s_mutex, RTEMS_WAIT, RTEMS_NO_TIMEOUT);
...
static struct _inode *rtems_jffs2_get_inode_by_iop (const rtems_libio_t * iop)
{
...
rtems_jffs2_eval_token (rtems_filesystem_eval_path_context_t * ctx, void *arg,
...
static int rtems_jffs2_link (const rtems_filesystem_location_info_t * parentloc,
...
static bool rtems_jffs2_are_nodes_equal (const rtems_filesystem_location_info_t
										 * a,
...
static int rtems_jffs2_statvfs (const rtems_filesystem_location_info_t *
...
static int rtems_jffs2_symlink (const rtems_filesystem_location_info_t *
								parentloc, const char *name, size_t namelen,
								const char *target)
{
	struct _inode *dir_i = rtems_jffs2_get_inode_by_location (parentloc);
...
static ssize_t rtems_jffs2_readlink (const rtems_filesystem_location_info_t *
									 loc, char *buf, size_t bufsize)
{
	struct _inode *inode = rtems_jffs2_get_inode_by_location (loc);
	struct jffs2_inode_info *f = JFFS2_INODE_INFO (inode);
	const char *target = f->target;
...
static void jffs2_clear_inode (struct _inode *inode)
{
	...
	D1 (printk
		(KERN_DEBUG "jffs2_clear_inode(): ino #%lu mode %o\n", inode->i_ino,
		 inode->i_mode));
								...
								struct jffs2_raw_inode *ri)
{
	...
	D1 (printk
		(KERN_DEBUG "jffs2_new_inode(): dir_i %ld, mode 0x%x\n", dir_i->i_ino,
		 mode));
...
static int jffs2_read_inode (struct _inode *inode)
{
	...
	D1 (printk
		(KERN_DEBUG "jffs2_read_inode(): inode->i_ino == %lu\n", inode->i_ino));
