										 ...
										 struct jffs2_raw_node_ref *raw)
{
				...
				if (je32_to_cpu (node->i.data_crc) != crc)
				{
					pr_warn
						("Data CRC failed on REF_PRISTINE data node at 0x%08x: Read 0x%08x, calculated 0x%08x\n",
						 ref_offset (raw), je32_to_cpu (node->i.data_crc),
						 crc);
					goto bail;
				}
			}
				...
				if (je32_to_cpu (node->d.name_crc) != crc)
				{
					pr_warn
						("Name CRC failed on REF_PRISTINE dirent node at 0x%08x: Read 0x%08x, calculated 0x%08x\n",
						 ref_offset (raw), je32_to_cpu (node->d.name_crc),
						 crc);
					goto bail;
				}
			}
  ...
  retry:
	...
	if (ret || (retlen != rawlen))
	{
		...
		if (retlen)
		{
			jffs2_add_physical_node_ref (c, phys_ofs | REF_OBSOLETE, rawlen,
										 NULL);
		}
		else
		{
		...
		if (!retried)
		{
			...
			uint32_t dummy;
			struct jffs2_eraseblock *jeb =
				&c->blocks[phys_ofs / c->sector_size];
										 ...
										 struct jffs2_full_dnode *fn)
{
	...
	if (S_ISBLK (JFFS2_F_I_MODE (f)) || S_ISCHR (JFFS2_F_I_MODE (f)))
	{
#ifndef __rtems__
												...
												struct jffs2_full_dirent *fd)
{
	...
	if (!jffs2_can_mark_obsolete (c))
	{
		...
		for (raw = f->inocache->nodes; raw != (void *)f->inocache;
			 raw = raw->next_in_ino)
		{
			...
			ret =
				jffs2_flash_read (c, ref_offset (raw), rawlen, &retlen,
								(char *)rd);
			if (ret)
			{
				pr_warn
			...
			if (retlen != rawlen)
			{
				pr_warn
					("%s(): Short read (%zd not %u) reading header from obsolete node at %08x\n",
					 __func__, retlen, rawlen, ref_offset (raw));
				continue;
			}
			...
			jffs2_dbg (1,
					 ...
					 ref_offset (fd->raw), fd->name, ref_offset (raw),
					 je32_to_cpu (rd->ino));
			kfree (rd);
									 ...
									 uint32_t start, uint32_t end)
{
	...
	if (fn->frags > 1)
	{
		...
		ret =
			jffs2_flash_read (c, ref_offset (fn->raw), sizeof (ri), &readlen,
								(char *)&ri);
		...
		if (je16_to_cpu (ri.nodetype) != JFFS2_NODETYPE_INODE)
		{
			pr_warn
				("%s(): Node at 0x%08x had node type 0x%04x instead of JFFS2_NODETYPE_INODE(0x%04x)\n",
				 __func__, ref_offset (fn->raw), je16_to_cpu (ri.nodetype),
		...
		if (je32_to_cpu (ri.totlen) != sizeof (ri))
		{
			pr_warn
				("%s(): Node at 0x%08x had totlen 0x%x instead of expected 0x%zx\n",
				 __func__, ref_offset (fn->raw), je32_to_cpu (ri.totlen),
				 sizeof (ri));
			return -EIO;
		}
		...
		if (crc != je32_to_cpu (ri.node_crc))
		{
			pr_warn
				("%s: Node at 0x%08x had CRC 0x%08x which doesn't match calculated CRC 0x%08x\n",
				 __func__, ref_offset (fn->raw), je32_to_cpu (ri.node_crc),
				 crc);
	...
	D1 (if (unlikely (fn->frags <= 1))
		...
		("%s(): Replacing fn with %d frag(s) but new ver %d != highest_version %d of ino #%d\n",
		 __func__, fn->frags, je32_to_cpu (ri.version), f->highest_version,
		 je32_to_cpu (ri.ino));}
	) ;
										...
										uint32_t start, uint32_t end)
{
	...
	if (c->nr_free_blocks + c->nr_erasing_blocks > c->resv_blocks_gcmerge)
	{
		...
		D1 (BUG_ON
			(end >
			 frag_last (&f->fragtree)->ofs + frag_last (&f->fragtree)->size));
		BUG_ON (end < orig_end);
