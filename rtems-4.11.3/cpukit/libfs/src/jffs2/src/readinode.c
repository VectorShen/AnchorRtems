 ...
 *
 * Copyright 漏 2001-2007 Red Hat, Inc.
 *
							...
							struct jffs2_tmp_dnode_info *tn)
{
	...
	dbg_readinode
		("check node at %#08x, data length %u, partial CRC %#08x, correct CRC %#08x, data starts at %#08x, start checking from %#08x - %u bytes.\n",
		 ref_offset (ref), tn->csize, tn->partial_crc, tn->data_crc, ofs - len,
		 ofs, len);
...
/* Trivial function to remove the last node in the tree. Which by definition
   has no right-hand child 鈥� so can be removed just by making its left-hand
...
static struct jffs2_raw_node_ref *jffs2_first_valid_node (struct
														jffs2_raw_node_ref
														*ref)
{
							...
							struct jffs2_readinode_info *rii)
{
	...
	if (ref_flags (ref) == REF_UNCHECKED)
	{
		...
		if (jffs2_is_writebuffered (c) && csize != 0)
		{
			...
			dbg_readinode
				("Calculates CRC (%#08x) for %d bytes, csize %d\n",
	...
	dbg_readinode2
		("dnode @%08x: ver %u, offset %#04x, dsize %#04x, csize %#04x\n",
		 ref_offset (ref), je32_to_cpu (rd->version), je32_to_cpu (rd->offset),
		 je32_to_cpu (rd->dsize), csize);
								...
								struct jffs2_unknown_node *un)
{
	...
	un->nodetype =
		cpu_to_je16 (JFFS2_NODE_ACCURATE | je16_to_cpu (un->nodetype));
...
/* Get tmp_dnode_info and full_dirent for all non-obsolete nodes associated
   ...
   those which are completely obsoleted by newer ones. The na茂ve approach we
								...
								struct jffs2_readinode_info *rii)
{
	...
	while (valid_ref)
	{
		...
		if (je32_to_cpu (node->u.hdr_crc) !=
			crc32 (0, node, sizeof (node->u) - 4))
		{
			JFFS2_NOTICE
		...
		switch (je16_to_cpu (node->u.nodetype))
		{
			...
			case JFFS2_NODETYPE_DIRENT:
					...
					&& len < sizeof (struct jffs2_raw_dirent))
				{
					err =
						read_more (c, ref,
									 sizeof (struct jffs2_raw_dirent), &len,
									 buf);
					if (unlikely (err))
						goto free_out;
				}
			...
			case JFFS2_NODETYPE_INODE:
					...
					&& len < sizeof (struct jffs2_raw_inode))
				{
					err =
						read_more (c, ref,
									 sizeof (struct jffs2_raw_inode), &len,
									 buf);
					if (unlikely (err))
						goto free_out;
				}
										 ...
										 struct jffs2_raw_inode *latest_node)
{
	...
	if (unlikely (!rii.latest_ref))
	{
		...
		latest_node->mode =
			cpu_to_jemode (S_IFDIR | S_IRUGO | S_IWUSR | S_IXUGO);
	...
	ret =
		jffs2_flash_read (c, ref_offset (rii.latest_ref), sizeof (*latest_node),
						&retlen, (void *)latest_node);
	...
	switch (jemode_to_cpu (latest_node->mode) & S_IFMT)
	{
			...
			if (new_size != je32_to_cpu (latest_node->isize))
			{
				JFFS2_WARNING
					("Truncating ino #%u to %d bytes failed because it only had %d bytes to start with!\n",
					 f->inocache->ino, je32_to_cpu (latest_node->isize),
					 new_size);
		...
		case S_IFLNK:
			...
			if (f->inocache->state != INO_STATE_CHECKING)
			{
				...
				ret =
					jffs2_flash_read (c,
									ref_offset (rii.latest_ref) +
									sizeof (*latest_node), csize, &retlen,
									(char *)f->target);
