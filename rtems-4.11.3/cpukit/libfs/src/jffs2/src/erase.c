	...
	while (!list_empty (&c->erase_complete_list) ||
		   !list_empty (&c->erase_pending_list))
	  {
		  ...
		  if (!list_empty (&c->erase_complete_list))
			{
				jeb =
					list_entry (c->erase_complete_list.next,
			...
			{
				jeb =
					list_entry (c->erase_pending_list.next,
								struct jffs2_eraseblock, list);
														 ...
														 *jeb)
{
	...
	D2 (
		   ...
		   this = ic->nodes; printk (KERN_DEBUG); while (this)
		   {
		   pr_cont ("0x%08x(%d)->",
					ref_offset (this), ref_flags (this)); if (++i == 5)
		   {
		   printk (KERN_DEBUG); i = 0;}
									...
									uint32_t * bad_offset)
{
	...
	ret = mtd_point (c->mtd, jeb->offset, c->sector_size, &retlen, &ebuf, NULL);
	if (ret != -EOPNOTSUPP)
	  {
		  if (ret)
			{
