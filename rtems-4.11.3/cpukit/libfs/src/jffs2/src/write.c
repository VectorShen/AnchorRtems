	...
	if (je32_to_cpu (ri->totlen) != sizeof (*ri) + datalen)
	{
		pr_warn
			("%s(): ri->totlen (0x%08x) != sizeof(*ri) (0x%08zx) + datalen (0x%08x)\n",
			 __func__, je32_to_cpu (ri->totlen), sizeof (*ri), datalen);
	}
  ...
  retry:
		...
		&& (je32_to_cpu (ri->version) < f->highest_version))
	{
		...
		jffs2_dbg (1,
					 ...
					 __func__, je32_to_cpu (ri->version), f->highest_version);
		ri->version = cpu_to_je32 (++f->highest_version);
	...
	if (ret || (retlen != sizeof (*ri) + datalen))
	{
		...
		if (!retried && alloc_mode != ALLOC_NORETRY)
		{
			...
			{
				ret =
					jffs2_reserve_space_gc (c, sizeof (*ri) + datalen,
											&dummy,
			...
			else
			{
				...
				ret =
					jffs2_reserve_space (c, sizeof (*ri) + datalen,
										 &dummy, alloc_mode,
										 JFFS2_SUMMARY_INODE_SIZE);
				mutex_lock (&f->sem);
			}
	...
	fn->raw =
		jffs2_add_physical_node_ref (c, flash_ofs, PAD (sizeof (*ri) + datalen),
									 f->inocache);
	if (IS_ERR (fn->raw))
	{
											...
											uint32_t namelen, int alloc_mode)
{
	...
	jffs2_dbg (1,
			 ...
			 __func__, je32_to_cpu (rd->pino), name, name,
			 je32_to_cpu (rd->ino), je32_to_cpu (rd->name_crc));
  ...
  retry:
		...
		&& (je32_to_cpu (rd->version) < f->highest_version))
	{
		...
		jffs2_dbg (1,
					 ...
					 __func__, je32_to_cpu (rd->version), f->highest_version);
	...
	ret = jffs2_flash_writev (c, vecs, 2, flash_ofs, &retlen,
							(alloc_mode ==
							 ALLOC_GC) ? 0 : je32_to_cpu (rd->pino));
	if (ret || (retlen != sizeof (*rd) + namelen))
	{
		...
		if (!retried)
		{
			...
			{
				ret =
					jffs2_reserve_space_gc (c, sizeof (*rd) + namelen,
											&dummy,
	...
	/* Mark the space used */
	fd->raw =
		jffs2_add_physical_node_ref (c, flash_ofs | dirent_node_state (rd),
							 ...
							 uint32_t * retlen)
{
	...
	while (writelen)
	{
		...
		ri->hdr_crc =
			cpu_to_je32 (crc32
						 (0, ri, sizeof (struct jffs2_unknown_node) - 4));

		ri->ino = cpu_to_je32 (f->inocache->ino);
		ri->version = cpu_to_je32 (++f->highest_version);
		ri->isize =
			cpu_to_je32 (max (je32_to_cpu (ri->isize), offset + datalen));
...
int jffs2_do_unlink (struct jffs2_sb_info *c, struct jffs2_inode_info *dir_f,
					 ...
					 struct jffs2_inode_info *dead_f, uint32_t time)
{
	struct jffs2_raw_dirent *rd;
	struct jffs2_full_dirent *fd;
	uint32_t alloclen;
	...
	if (!jffs2_can_mark_obsolete (c))
	{
		...
		rd->hdr_crc =
			cpu_to_je32 (crc32
						 (0, rd, sizeof (struct jffs2_unknown_node) - 4));
	...
	else
	{
		...
		{
			if (fd->nhash == nhash &&
				!memcmp (fd->name, name, namelen) && !fd->name[namelen])
			{
