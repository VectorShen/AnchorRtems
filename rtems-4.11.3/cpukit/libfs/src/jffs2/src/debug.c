									...
									struct jffs2_eraseblock *jeb)
{
	...
	if (unlikely
		(c->used_size + c->dirty_size + c->free_size + c->erasing_size +
		 c->bad_size + c->wasted_size + c->unchecked_size != c->flash_size))
	{
		...
		if ((!ref_next (ref2)) != (ref2 == jeb->last_node))
		{
			JFFS2_ERROR
				("node_ref for node at %#08x (mem %p) has next at %#08x (mem %p), last_node is at %#08x (mem %p).\n",
				 ref_offset (ref2), ref2, ref_offset (ref_next (ref2)),
				 ref_next (ref2), ref_offset (jeb->last_node),
				 jeb->last_node);
			goto error;
		}
		ref2 = ref_next (ref2);
...
void __jffs2_dbg_dump_block_lists_nolock (struct jffs2_sb_info *c)
{
	...
	if (c->gcblock)
		printk (JFFS2_DBG
				...
				c->gcblock->offset, c->gcblock->used_size,
				c->gcblock->dirty_size, c->gcblock->wasted_size,
				c->gcblock->unchecked_size, c->gcblock->free_size);
	else
	...
	else
	{
		...
		list_for_each (this, &c->clean_list)
		{
			...
			if (!
				(jeb->used_size == 0 && jeb->dirty_size == 0
				 && jeb->wasted_size == 0))
			{
				printk (JFFS2_DBG
