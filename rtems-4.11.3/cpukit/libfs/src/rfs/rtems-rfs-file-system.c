...
static int rtems_rfs_fs_read_superblock (rtems_rfs_file_system * fs)
{
	...
	if (rc > 0)
	{
		if (rtems_rfs_trace (RTEMS_RFS_TRACE_OPEN))
			printf
				("rtems-rfs: read-superblock: handle open failed: %d: %s\n",
				 rc, strerror (rc));
		return rc;
	}
...
#define read_sb(_o) rtems_rfs_read_u32 (sb + (_o))
	...
	if (read_sb (RTEMS_RFS_SB_OFFSET_MAGIC) != RTEMS_RFS_SB_MAGIC)
	{
		if (rtems_rfs_trace (RTEMS_RFS_TRACE_OPEN))
			printf
				("rtems-rfs: read-superblock: invalid superblock, bad magic\n");
	...
	{
		if (rtems_rfs_trace (RTEMS_RFS_TRACE_OPEN))
			printf ("rtems-rfs: read-superblock: incompatible version: %08"
					PRIx32 " (%08" PRIx32 ")\n",
					read_sb (RTEMS_RFS_SB_OFFSET_VERSION),
	...
	{
		if (rtems_rfs_trace (RTEMS_RFS_TRACE_OPEN))
			printf ("rtems-rfs: read-superblock: inode size mismatch: fs:%"
					PRId32 " target:%" PRId32 "\n",
					read_sb (RTEMS_RFS_SB_OFFSET_VERSION),
					RTEMS_RFS_VERSION_MASK);
		rtems_rfs_buffer_handle_close (fs, &handle);
		return EIO;
	...
	if (!fs->groups)
	{
		...
		if (rtems_rfs_trace (RTEMS_RFS_TRACE_OPEN))
			printf
				("rtems-rfs: read-superblock: no memory for group table\n");
		return ENOMEM;
	}
	...
	for (group = 0; group < fs->group_count; group++)
	{
		rc = rtems_rfs_group_open (fs,
									 ...
									 fs->group_blocks,
									 fs->group_inodes, &fs->groups[group]);
		if (rc > 0)
		{
			int g;
			for (g = 0; g < group; g++)
			...
			if (rtems_rfs_trace (RTEMS_RFS_TRACE_OPEN))
				printf
					("rtems-rfs: read-superblock: no memory for group table%d: %s\n",
					 rc, strerror (rc));
			return rc;
		}
	}
