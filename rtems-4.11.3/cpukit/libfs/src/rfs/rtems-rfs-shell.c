...
rtems_rfs_shell_block (rtems_rfs_file_system * fs, int argc, char *argv[])
{
	...
	if (rc > 0)
	{
		...
		rtems_rfs_shell_unlock_rfs (fs);
		printf ("error: requesting buffer handle: block=%" PRIu32
				": (%d) %s\n", block, rc, strerror (rc));
		return 1;
	}
...
rtems_rfs_shell_inode (rtems_rfs_file_system * fs, int argc, char *argv[])
{
	...
	for (ino = start; ino <= end; ino++)
	{
		...
		{
			rtems_rfs_shell_unlock_rfs (fs);
			printf ("error: testing inode state: ino=%" PRIu32
					": (%d) %s\n", ino, rc, strerror (rc));
			return 1;
		}
		...
		if (show_all || allocated)
		{
			...
			{
				rtems_rfs_shell_unlock_rfs (fs);
				printf ("error: opening inode handle: ino=%" PRIu32
						": (%d) %s\n", ino, rc, strerror (rc));
				return 1;
			}
			...
			if (!error_check_only || error)
			{
				...
				else
				{
					...
					if (RTEMS_RFS_S_ISDIR (mode))
						...
						type = "LNK";
					printf
						("links=%03i mode=%04x (%s/%03o) bo=%04u bc=%04"
						 PRIu32 " b=[",
			...
			{
				rtems_rfs_shell_unlock_rfs (fs);
				printf ("error: closing inode handle: ino=%" PRIu32
						": (%d) %s\n", ino, rc, strerror (rc));
				return 1;
			}
		}
...
rtems_rfs_shell_dir (rtems_rfs_file_system * fs, int argc, char *argv[])
{
	...
	while (b < (rtems_rfs_fs_block_size (fs) - RTEMS_RFS_DIR_ENTRY_SIZE - 1))
	{
		...
		printf (" %5d: %04x inode=%-6" PRIu32 " hash=%08" PRIx32
				" name[%03u]=", entry, b, rtems_rfs_dir_entry_ino (data),
				rtems_rfs_dir_entry_hash (data), length);
...
rtems_rfs_shell_group (rtems_rfs_file_system * fs, int argc, char *argv[])
{
	...
	for (g = start; g <= end; g++)
	{
		...
		size_t inodes;
		blocks =
			group->size - rtems_rfs_bitmap_map_free (&group->block_bitmap);
		inodes =
			fs->group_inodes -
			rtems_rfs_bitmap_map_free (&group->inode_bitmap);
		printf (" %4d: base=%-7" PRIu32
				" size=%-6zu blocks=%-5zu (%3zu%%) inode=%-5zu (%3zu%%)\n", g,
				group->base, group->size, blocks,
				(blocks * 100) / group->size, inodes,
				(inodes * 100) / fs->group_inodes);
	}
...
int rtems_shell_debugrfs (int argc, char *argv[])
{
	...
	for (arg = 1; arg < argc; arg++)
	{
		...
		switch (argv[arg][1])
		{
			...
			case 'l':
				...
				for (t = 0;
					 t <
					 (sizeof (table) / sizeof (const rtems_rfs_shell_cmd));
					 t++)
	...
	else
	{
		...
		{
			for (t = 0;
				 t < (sizeof (table) / sizeof (const rtems_rfs_shell_cmd));
				 t++)
		...
		if (argv[arg][0] == '-')
		{
			switch (argv[arg][1])
			{
				...
				case 's':
					...
					{
						printf
							("error: block size needs an argument\n");
						return 1;
					}
				...
				case 'b':
					...
					{
						printf
							("error: group block count needs an argument\n");
						return 1;
					}
				...
				case 'i':
					...
					{
						printf
							("error: group inode count needs an argument\n");
						return 1;
					}
				...
				case 'o':
					...
					{
						printf
							("error: inode percentage overhead needs an argument\n");
						return 1;
					}
