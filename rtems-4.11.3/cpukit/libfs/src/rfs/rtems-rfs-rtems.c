...
{
	return rtems_filesystem_eval_path_check_access (ctx,
													eval_flags,
													rtems_rfs_inode_get_mode
													(inode),
													rtems_rfs_inode_get_uid
													(inode),
													rtems_rfs_inode_get_gid
													(inode));
}
...
static void
rtems_rfs_rtems_unlock_by_mt_entry (const rtems_filesystem_mount_table_entry_t *
									mt_entry)
{
	rtems_rfs_file_system *fs = mt_entry->fs_info;
...
static void rtems_rfs_rtems_follow_link (rtems_filesystem_eval_path_context_t *
										 ctx, rtems_rfs_file_system * fs,
										 rtems_rfs_ino ino)
{
...
rtems_rfs_rtems_eval_token (rtems_filesystem_eval_path_context_t * ctx,
							void *arg, const char *token, size_t tokenlen)
{
...
rtems_rfs_rtems_symlink (const rtems_filesystem_location_info_t * parent_loc,
						 ...
						 size_t node_name_len, const char *target)
{
						...
						mode_t mode)
{
	...
	if (rtems_rfs_rtems_trace (RTEMS_RFS_RTEMS_DEBUG_FCHMOD))
		printf ("rtems-rfs-rtems: fchmod: in: ino:%" PRId32 " mode:%06"
				PRIomode_t "\n", ino, mode);
					 ...
					 const rtems_filesystem_location_info_t * pathloc)
{
	...
	if (rtems_rfs_rtems_trace (RTEMS_RFS_RTEMS_DEBUG_RMNOD))
		printf ("rtems-rfs: rmnod: parent:%" PRId32 " doff:%" PRIu32 ", ino:%"
				PRId32 "\n", parent, doff, ino);
 ...
 */
static int
rtems_rfs_rtems_statvfs (const rtems_filesystem_location_info_t *
						 __restrict pathloc, struct statvfs *__restrict sb)
{
	rtems_rfs_file_system *fs = rtems_rfs_rtems_pathloc_dev (pathloc);
	size_t blocks;
	size_t inodes;
