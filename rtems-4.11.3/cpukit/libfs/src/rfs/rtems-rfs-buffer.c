								 ...
								 rtems_rfs_buffer_block block, bool read)
{
	...
	if (rtems_rfs_trace (RTEMS_RFS_TRACE_BUFFER_HANDLE_REQUEST))
		printf ("rtems-rfs: buffer-request: block=%" PRIu32 " bdbuf-%s=%" PRIu32
				" refs=%d\n", block, read ? "read" : "get",
				handle->buffer->block, handle->buffer->references);
								 ...
								 rtems_rfs_buffer_handle * handle)
{
	...
	if (rtems_rfs_buffer_handle_has_block (handle))
	{
		if (rtems_rfs_trace (RTEMS_RFS_TRACE_BUFFER_HANDLE_RELEASE))
			printf ("rtems-rfs: buffer-release: block=%" PRIu32
					" %s refs=%d %s\n", rtems_rfs_buffer_bnum (handle),
		...
		if (rtems_rfs_buffer_refs (handle) == 0)
		{
			rtems_chain_extract_unprotected (rtems_rfs_buffer_link
											 (handle));
			fs->buffers_count--;
			...
			else
			{
					 ...
					 fs->release_modified_count) >= fs->max_held_buffers)
				{
					...
					if (rtems_rfs_trace
						(RTEMS_RFS_TRACE_BUFFER_HANDLE_RELEASE))
						printf
							("rtems-rfs: buffer-release: local cache overflow:"
							 " %" PRIu32 "\n",
							 fs->release_count +
							 fs->release_modified_count);
					...
					if (fs->release_count > fs->release_modified_count)
					{
						buffer = (rtems_rfs_buffer *)
							rtems_chain_get_unprotected (&fs->
														 release);
						fs->release_count--;
						modified = false;
					}
				...
				{
					rtems_chain_append_unprotected (&fs->
													release_modified,
...
int rtems_rfs_buffer_close (rtems_rfs_file_system * fs)
{
	...
	if ((rc > 0) && rtems_rfs_trace (RTEMS_RFS_TRACE_BUFFER_CLOSE))
		printf
			("rtems-rfs: buffer-close: set media block size failed: %d: %s\n",
			 rc, strerror (rc));
...
int rtems_rfs_buffers_release (rtems_rfs_file_system * fs)
{
	...
	if (rtems_rfs_trace (RTEMS_RFS_TRACE_BUFFER_RELEASE))
		printf ("rtems-rfs: buffers-release: active:%" PRIu32 " "
				...
				fs->buffers_count, fs->release_count,
				fs->release_modified_count);
