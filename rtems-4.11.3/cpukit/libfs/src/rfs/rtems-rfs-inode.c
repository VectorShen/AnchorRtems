...
rtems_rfs_inode_alloc (rtems_rfs_file_system * fs,
					   rtems_rfs_bitmap_bit goal, rtems_rfs_ino * ino)
{
...
rtems_rfs_inode_create (rtems_rfs_file_system * fs,
						...
						uint16_t mode,
						uint16_t links,
						uid_t uid, gid_t gid, rtems_rfs_ino * ino)
{
	...
	if (RTEMS_RFS_S_ISDIR (mode))
		rtems_rfs_inode_set_links (&parent_inode,
								   rtems_rfs_inode_get_links (&parent_inode) +
								   1);
...
rtems_rfs_inode_initialise (rtems_rfs_inode_handle * handle,
							uint16_t links, uint16_t mode, uid_t uid, gid_t gid)
{
	int b;
