					...
					size_t size, size_t inodes, rtems_rfs_group * group)
{
	...
	if (base >= rtems_rfs_fs_blocks (fs))
	{
		if (rtems_rfs_trace (RTEMS_RFS_TRACE_GROUP_OPEN))
			printf
				("rtems-rfs: group-open: base outside file system range: %d: %s\n",
				 EIO, strerror (EIO));
		return EIO;
	}
	...
	if (rtems_rfs_trace (RTEMS_RFS_TRACE_GROUP_OPEN))
		printf ("rtems-rfs: group-open: base=%" PRId32
				", blocks=%zd inodes=%zd\n", base, size, inodes);
	...
	if (rc > 0)
	{
		...
		if (rtems_rfs_trace (RTEMS_RFS_TRACE_GROUP_OPEN))
			printf
				("rtems-rfs: group-open: could not open block bitmap: %d: %s\n",
				 rc, strerror (rc));
		return rc;
	}
	...
	if (rc > 0)
	{
		...
		if (rtems_rfs_trace (RTEMS_RFS_TRACE_GROUP_OPEN))
			printf
				("rtems-rfs: group-open: could not open inode bitmap: %d: %s\n",
				 rc, strerror (rc));
		return rc;
	}
...
rtems_rfs_group_bitmap_alloc (rtems_rfs_file_system * fs,
							...
							bool inode, rtems_rfs_bitmap_bit * result)
{
	int group_start;
	size_t size;
	...
	while (true)
	{
		...
		if (allocated)
		{
			...
			if (rtems_rfs_trace (RTEMS_RFS_TRACE_GROUP_BITMAPS))
				printf ("rtems-rfs: group-bitmap-alloc: %s allocated: %"
...
rtems_rfs_group_bitmap_free (rtems_rfs_file_system * fs,
							 bool inode, rtems_rfs_bitmap_bit no)
{
	rtems_rfs_bitmap_control *bitmap;
	unsigned int group;
	rtems_rfs_bitmap_bit bit;
...
rtems_rfs_group_bitmap_test (rtems_rfs_file_system * fs,
							 bool inode, rtems_rfs_bitmap_bit no, bool * state)
{
	rtems_rfs_bitmap_control *bitmap;
	unsigned int group;
	rtems_rfs_bitmap_bit bit;
