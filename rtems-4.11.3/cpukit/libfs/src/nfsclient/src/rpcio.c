...
static RpcUdpXact sockRcv (void);
...
static inline bool_t locked_marshal (RpcUdpServer s, XDR * xdrs)
{
	bool_t rval;
	MU_LOCK (s->authlock);
...
static inline bool_t locked_validate (RpcUdpServer s, struct opaque_auth *v)
{
	bool_t rval;
	MU_LOCK (s->authlock);
...
static inline bool_t locked_refresh (RpcUdpServer s)
{
	bool_t rval;
	MU_LOCK (s->authlock);
 ...
 */
enum clnt_stat
rpcUdpServerCreate (struct sockaddr_in *paddr,
					rpcprog_t prog,
					rpcvers_t vers, u_long uid, u_long gid, RpcUdpServer * psrv)
{
	RpcUdpServer rval;
	u_short port;
	...
	{
		fprintf (stderr, "RPCIO - error: unable to create RPC AUTH\n");
		return RPC_FAILED;
	}
	...
	if (!paddr->sin_port)
	{
		...
		pmap_err = rpcUdpCallRp (paddr,
								 PMAPPROG,
								 PMAPVERS,
								 PMAPPROC_GETPORT,
								 xdr_pmap,
								 &pmaparg, xdr_u_short, &port, uid, gid, 0);
...
RpcUdpXact rpcUdpXactCreate (u_long program, u_long version, u_long size)
{
	...
	rval =
		(RpcUdpXact) MY_CALLOC (1, sizeof (*rval) - sizeof (rval->obuf) + size);
	...
	if (rval)
	{
		...
		MU_LOCK (hlock);
		rval->obuf.xid =
			(xidHashSeed++ ^ ((uintptr_t) rval >> 10)) & XACT_HASH_MSK;
 ...
 */
enum clnt_stat
rpcUdpSend (RpcUdpXact xact,
			RpcUdpServer srvr,
			struct timeval *timeout,
			u_long proc,
			xdrproc_t xres, caddr_t pres, xdrproc_t xargs, caddr_t pargs, ...)
{
	register XDR *xdrs;
	unsigned long ms;
	va_list ap;
	...
	do
	{
...
#else
		xdrmem_create (&reply_xdrs, xact->ibuf->buf, xact->ibufsize,
						 XDR_DECODE);
#endif
		...
		if (refresh && locked_refresh (xact->server))
		{
			...
			ASSERT (RTEMS_SUCCESSFUL ==
					rtems_event_send (rpciod, RPCIOD_TX_EVENT));
		}
 ...
 * be more efficient
 */
static void rxWakeupCB (struct socket *sock, void *arg)
{
...
int rpcUdpInit (void)
{
	...
	if (ourSock < 0)
	{
		...
		if (ourSock >= 0)
		{
			...
			status =
				rtems_task_create (rtems_build_name ('R', 'P', 'C', 'd'),
								 rpciodPriority, RPCIOD_STACK,
								 RTEMS_DEFAULT_MODES,
								 /* fprintf saves/restores FP registers on PPC :-( */
								 RTEMS_DEFAULT_ATTRIBUTES |
								 RTEMS_FLOATING_POINT, &rpciod);
			assert (status == RTEMS_SUCCESSFUL);
			...
			if (rpciodCpuset == 0)
			{
				...
				rpciodCpusetSize =
					rtems_bsdnet_config.network_task_cpuset_size;
			}
			if (rpciodCpuset != 0)
				rtems_task_set_affinity (rpciod, rpciodCpusetSize,
										 rpciodCpuset);
#endif
			...
			wkup.sw_arg = &rpciod;
			assert (0 ==
					setsockopt (ourSock, SOL_SOCKET, SO_RCVWAKEUP, &wkup,
								sizeof (wkup)));
			status =
...
{
	rtems_semaphore_create (rtems_build_name ('R', 'P', 'C', 'f'),
							0, RTEMS_DEFAULT_ATTRIBUTES, 0, &fini);
 ...
 */
enum clnt_stat
rpcUdpClntCreate (struct sockaddr_in *psaddr,
				rpcprog_t prog,
				rpcvers_t vers, u_long uid, u_long gid, RpcUdpClnt * pclnt)
{
	RpcUdpXact x;
	RpcUdpServer s;
	enum clnt_stat err;
	...
	if (RPC_SUCCESS !=
		(err = rpcUdpServerCreate (psaddr, prog, vers, uid, gid, &s)))
		return err;
...
static void rpcUdpClntDestroy (RpcUdpClnt xact)
{
...
enum clnt_stat
rpcUdpClntCall (RpcUdpClnt xact,
				u_long proc,
				XdrProcT xargs,
				CaddrT pargs,
				XdrProcT xres, CaddrT pres, struct timeval *timeout)
{
	enum clnt_stat stat;

	if ((stat = rpcUdpSend (xact, xact->server, timeout, proc,
							xres, pres, xargs, pargs, 0)))
	{
...
enum clnt_stat rpcUdpCallRp (struct sockaddr_in *psrvr, u_long prog, u_long vers, u_long proc, XdrProcT xargs, CaddrT pargs, XdrProcT xres, CaddrT pres, u_long uid,	/* RPCIO_DEFAULT_ID picks default */
	...
	)
{
	...
	stat = rpcUdpClntCreate (psrvr, prog, vers, uid, gid, &clp);

	if (RPC_SUCCESS != stat)
		return stat;

	stat = rpcUdpClntCall (clp, proc, xargs, pargs, xres, pres, timeout);
...
static void nodeXtract (ListNode n)
{
	if (n->prev)
		n->prev->next = n->next;
	if (n->next)
...
static void nodeAppend (ListNode l, ListNode n)
{
	if ((n->next = l->next))
		n->next->prev = n;
	l->next = n;
...
static void rpcio_daemon (rtems_task_argument arg)
{
	...
	for (next_retrans = epoch;;)
	{
		...
		if (RTEMS_SUCCESSFUL !=
			(stat =
			 rtems_event_receive (RPCIOD_RX_EVENT | RPCIOD_TX_EVENT |
									RPCIOD_KILL_EVENT,
									RTEMS_WAIT | RTEMS_EVENT_ANY, next_retrans,
									&events)))
		{
		...
		if (events & RPCIOD_KILL_EVENT)
		{
			...
			{
				fprintf (stderr,
						 "RPCIO There are still transactions circulating; I refuse to go away\n");
		...
		if (RPCIOD_TX_EVENT & events)
		{
			...
			while (RTEMS_SUCCESSFUL == rtems_message_queue_receive (msgQ,
																	&xact,
																	&size,
																	RTEMS_NO_WAIT,
																	RTEMS_NO_TIMEOUT))
		...
		for (xact = (RpcUdpXact) listHead.next;
			 xact && xact->age <= now; xact = (RpcUdpXact) listHead.next)
		{
...
#if (DEBUG) & DEBUG_TIMEOUT
				fprintf (stderr,
						 "RPCIO XACT timed out; waking up requestor\n");
#endif
			...
			else
			{
				...
				if (len != SENDTO (ourSock,
									 ...
									 len,
									 0,
									 &srv->addr.sa, sizeof (srv->addr.sin)
#ifdef MBUF_TX
									 , xact, paranoia_free, paranoia_ref
#endif
					))
				{
		...
		for (; (xact = (RpcUdpXact) newList);)
		{
			...
			for (p = &listHead;
				 (n = p->next) && xact->age > ((RpcUdpXact) n)->age; p = n)
				/* nothing else to do */ ;
		...
		next_retrans = listHead.next ? ((RpcUdpXact) listHead.next)->age - now : epoch;	/* make sure we don't miss updating the epoch */
#if (DEBUG) & DEBUG_TIMEOUT
	...
	while (RTEMS_SUCCESSFUL == rtems_message_queue_receive (q,
															&xact,
															&size,
															RTEMS_NO_WAIT,
															RTEMS_NO_TIMEOUT))
...
RpcUdpXactPool
rpcUdpXactPoolCreate (rpcprog_t prog, rpcvers_t version,
					int xactsize, int poolsize)
{
	...
	ASSERT (rval);
	status = rtems_message_queue_create (rtems_build_name ('R', 'P', 'C', 'p'),
										 poolsize,
										 sizeof (RpcUdpXact),
										 RTEMS_DEFAULT_ATTRIBUTES, &rval->box);
	assert (status == RTEMS_SUCCESSFUL);
...
RpcUdpXact rpcUdpXactPoolGet (RpcUdpXactPool pool, XactPoolGetMode mode)
{
	...
	if (RTEMS_SUCCESSFUL != rtems_message_queue_receive (pool->box,
														 &xact,
														 &size,
														 XactGetWait == mode ?
														 RTEMS_WAIT :
														 ...
														 RTEMS_NO_TIMEOUT))
	{
		...
		xact = (XactGetCreate == mode) ?
			rpcUdpXactCreate (pool->prog, pool->version, pool->xactSize) : 0;
		if (xact)
			xact->pool = pool;
...
void rpcUdpXactPoolPut (RpcUdpXact xact)
{
	...
	if (RTEMS_SUCCESSFUL != rtems_message_queue_send (pool->box,
													&xact, sizeof (xact)))
		rpcUdpXactDestroy (xact);
}
...
ssize_t
recv_mbuf_from (int s, struct mbuf **ppm, long len, struct sockaddr *fromaddr,
				int *fromlen);
...
static RpcUdpXact sockRcv (void)
{
	...
	do
	{
		...
		len = recv_mbuf_from (ourSock,
								&ibuf, RPCIOD_RXBUFSZ, &fromAddr.sa, &fromLen);
#else
		if (!ibuf)
		...
		len = recvfrom (ourSock,
						ibuf->buf, RPCIOD_RXBUFSZ, 0, &fromAddr.sa, &fromLen);
#endif
		...
		if (!(xact = xactHashTbl[i]) || xact->obuf.xid != xid ||
			...
			xact->server->addr.sin.sin_port != fromAddr.sin.sin_port)
		{
			...
			if (xact)
			{
							 ...
							 || xact->obuf.xid == xid + 2 * XACT_HASHS))
				{
					...
					fprintf (stderr,
							 "RPCIO - FYI sockRcv(): dropping late/redundant retry answer\n");
#endif
				}
				else
				{
					...
					fprintf (stderr,
							 "xact: xid  0x%08" PRIx32 "  -- got 0x%08"
							 PRIx32 "\n", xact->obuf.xid, xid);
					fprintf (stderr,
							 "xact: addr 0x%08" PRIx32 "  -- got 0x%08"
							 PRIx32 "\n",
							 xact->server->addr.sin.sin_addr.s_addr,
							 fromAddr.sin.sin_addr.s_addr);
					fprintf (stderr,
							 "xact: port 0x%08x  -- got 0x%08x\n",
							 xact->server->addr.sin.sin_port,
							 fromAddr.sin.sin_port);
				}
			}
			...
			{
				fprintf (stderr,
						 "RPCIO WARNING sockRcv(): got xid 0x%08" PRIx32
