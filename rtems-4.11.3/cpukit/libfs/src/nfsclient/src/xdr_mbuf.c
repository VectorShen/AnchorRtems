...
static char *rcsid =
	"$FreeBSD: src/lib/libc/xdr/xdr_mem.c,v 1.8 1999/08/28 00:02:56 peter Exp $";
#endif
...
#ifdef TODO
static inline void *my_malloc (size_t i)
{
	return malloc (i);
}
 ...
 */
static inline void xdrmbuf_setup (XDR * xdrs, struct mbuf *m)
{
...
static struct mbuf *xdrmbuf_next (XDR * xdrs)
{
	struct mbuf *rval;
	MBPrivate mbp = (MBPrivate) xdrs->x_base;
...
static void xdrmbuf_destroy (XDR * xdrs)
{
	MBPrivate mbp = (MBPrivate) xdrs->x_base;
...
static u_int xdrmbuf_getpos (XDR * xdrs)
{
#if 1
	MBPrivate mbp = (MBPrivate) xdrs->x_base;
