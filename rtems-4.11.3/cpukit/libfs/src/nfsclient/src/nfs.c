...
#define NFS_GLOBAL_RELEASE(lock_context) \
    rtems_interrupt_lock_release(&nfs_global_lock, lock_context)
	 static inline char *nfs_dupname (const char *name, size_t namelen)
{
...
static bool_t xdr_readlinkres_strbuf (XDR * xdrs, readlinkres_strbuf * objp)
{
...
static bool_t xdr_dir_info_entry (XDR * xdrs, DirInfo di)
{
	union
	{
...
static bool_t xdr_dir_info (XDR * xdrs, DirInfo di)
{
	DirInfo dip;
 ...
 */
static bool_t xdr_serporidok (XDR * xdrs, serporidok * objp)
{
...
static bool_t xdr_serporid (XDR * xdrs, serporid * objp)
{
	if (!xdr_nfsstat (xdrs, &objp->status))
		return FALSE;
	switch (objp->status)
...
static inline TimeStamp nowSeconds (void)
{
	rtems_interval rval;
...
static ssize_t nfs_readlink_with_node (NfsNode node, char *buf, size_t len);
...
static int nfs_sattr (NfsNode node, sattr * arg, u_long mask);
...
{
0, 0, 0xffffffff, 0, 0, 0, NULL, NULL};
 ...
 */
static Nfs nfsCreate (RpcUdpServer server)
{
 ...
 */
static void nfsDestroy (Nfs nfs)
{
	register Nfs prev;
	if (!nfs)
		return;
 ...
 * 			are aged).
 */
static NfsNode nfsNodeCreate (Nfs nfs, fhandle * fh)
{
...
/* destroy a node */
static void nfsNodeDestroy (NfsNode node)
{
	rtems_interrupt_lock_context lock_context;
 ...
 * 			clone a node).
 */
static NfsNode nfsNodeClone (NfsNode node)
{
 ...
 */
STATIC int
nfscall (RpcUdpServer srvr,
		 int proc, xdrproc_t xargs, void *pargs, xdrproc_t xres, void *pres)
{
	RpcUdpXact xact;
	enum clnt_stat stat;
	RpcUdpXactPool pool;
...
buildIpAddr (u_long * puid, u_long * pgid,
			 char **pHost, struct sockaddr_in *psa, char **pPath)
{
	struct hostent *h;
	char host[64];
	char *chpt = *pPath;
...
mntcall (struct sockaddr_in *psrvr,
		 int proc,
		 xdrproc_t xargs,
		 void *pargs, xdrproc_t xres, void *pres, u_long uid, u_long gid)
{
#ifdef MOUNT_V1_PORT
	int retry;
#endif
...
#endif
		stat = rpcUdpCallRp (psrvr,
							 MOUNTPROG,
							 MOUNTVERS,
							 proc,
							 xargs,
							 pargs, xres, pres, uid, gid, MNTCALL_TIMEOUT);
	return stat;
}
...
static int nfs_search_in_directory (Nfs nfs,
									const NfsNode dir,
									char *part, NfsNode entry)
{
	int rv;
...
nfs_eval_part (rtems_filesystem_eval_path_context_t * ctx, char *part)
{
...
static rtems_filesystem_eval_path_generic_status
nfs_eval_token (rtems_filesystem_eval_path_context_t * ctx, void *arg,
...
static int nfs_link (const rtems_filesystem_location_info_t * parentloc,
					 ...
					 const char *name, size_t namelen)
{
	...
	memcpy (&SERP_ARGS (tNode).linkarg.to.dir,
			&SERP_FILE (pNode), sizeof (SERP_FILE (pNode)));
...
static void nfs_freenode (const rtems_filesystem_location_info_t * pathloc	/* IN */
	)
{
#if DEBUG & DEBUG_COUNT_NODES
...
STATIC void nfs_fsunmount_me (rtems_filesystem_mount_table_entry_t * mt_entry	/* in */
	)
{
	enum clnt_stat stat;
	struct sockaddr_in saddr;
...
static int nfs_symlink (const rtems_filesystem_location_info_t * parentloc,
						const char *name, size_t namelen, const char *target)
{
	...
	SERP_ARGS (node).symlinkarg.attributes.mode =
...
static ssize_t nfs_readlink (const rtems_filesystem_location_info_t * loc,
							 char *buf, size_t len)
{
	NfsNode node = loc->node_access;
...
static bool nfs_are_nodes_equal (const rtems_filesystem_location_info_t * a,
 ...
 */
static int nfs_dir_open (rtems_libio_t * iop,
						 const char *pathname, int oflag, mode_t mode)
{
	...
	/* rewind cookie */
	memset (&di->readdirargs.cookie, 0, sizeof (di->readdirargs.cookie));
...
static int nfs_file_close (rtems_libio_t * iop)
{
	return 0;
}

static int nfs_dir_close (rtems_libio_t * iop)
{
...
static ssize_t nfs_file_read_chunk (NfsNode node,
									uint32_t offset, void *buffer, size_t count)
{
	ssize_t rv;
	readres rr;
	Nfs nfs = node->nfs;
...
static ssize_t nfs_file_read (rtems_libio_t * iop, void *buffer, size_t count)
{
	ssize_t rv = 0;
	NfsNode node = iop->pathinfo.node_access;
...
static ssize_t nfs_dir_read (rtems_libio_t * iop, void *buffer, size_t count)
{
	ssize_t rv;
...
static off_t nfs_dir_lseek (rtems_libio_t * iop, off_t length, int whence)
{
 ...
 */
static int nfs_file_ftruncate (rtems_libio_t * iop, off_t length)
{
	sattr arg;
	 ...
	 */
	return nfs_sattr (iop->pathinfo.node_access, &arg, SATTR_SIZE);
}
 ...
 */
static
	rtems_device_driver nfs_initialize (rtems_device_major_number major,
...
static void resolve_path (rtems_task_argument arg)
{
								   ...
								   rtems_filesystem_location_info_t * loc)
{
	...
	status = rtems_semaphore_create (rtems_build_name ('r', 'e', 's', 's'),
									 0,
	...
	status = rtems_task_create (rtems_build_name ('r', 'e', 's', 's'),
								pri,
