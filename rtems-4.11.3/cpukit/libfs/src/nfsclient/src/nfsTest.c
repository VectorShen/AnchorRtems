 ...
 */
static rtems_task nfsTestReader (rtems_task_argument arg)
{
...
static rtems_id taskSpawn (char *filenm, int inst)
{
	rtems_status_code sc;
	rtems_id tid;

	sc = rtems_task_create (rtems_build_name ('n', 't', 't', '0' + inst),
							nfsTestReaderPri,
							1400,
							RTEMS_DEFAULT_MODES,
 ...
 *    ANY 'REAL' CODE.
 */
static int nfsTestReadBigbuf (char *buf, int off, int sz, int nrd)
{
	int i, rval = 0;
	struct nfsTestReq_ r;
	r.buf = buf;
