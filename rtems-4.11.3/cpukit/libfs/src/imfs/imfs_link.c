			   ...
			   const char *name, size_t namelen)
{
	 ...
	 */
	new_node = IMFS_create_node (parentloc,
								 &IMFS_node_control_hard_link,
								 sizeof (IMFS_link_t),
								 name,
								 namelen,
								 IMFS_STAT_FMT_HARD_LINK | (S_IRWXU | S_IRWXG |
															S_IRWXO), target);
...
static int IMFS_stat_hard_link (const rtems_filesystem_location_info_t * loc,
								struct stat *buf)
{
...
static IMFS_jnode_t *IMFS_node_initialize_hard_link (IMFS_jnode_t * node,
													 void *arg)
{
