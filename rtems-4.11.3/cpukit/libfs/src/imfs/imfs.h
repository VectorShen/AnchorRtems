...
*/
typedef IMFS_jnode_t *(*IMFS_node_control_initialize) (IMFS_jnode_t * node,
													   void *arg);
...
IMFS_jnode_t *IMFS_node_initialize_directory (IMFS_jnode_t * node,
											  void *arg);
...
static inline IMFS_memfile_t *IMFS_iop_to_memfile (const rtems_libio_t *
												   iop)
{
...
extern int IMFS_initialize (rtems_filesystem_mount_table_entry_t * mt_entry,
							const void *data);

extern int IMFS_initialize_support (rtems_filesystem_mount_table_entry_t *
									mt_entry, const void *data);

/**
* @brief Unmount this instance of IMFS.
*/
extern void IMFS_fsunmount (rtems_filesystem_mount_table_entry_t *
							mt_entry);
...
* @endcode
*/
extern int rtems_tarfs_load (const char *mountpoint,
							 uint8_t * tar_image, size_t tar_size);
...
*/
extern int IMFS_stat (const rtems_filesystem_location_info_t * loc,
					  struct stat *buf);

extern int IMFS_stat_file (const rtems_filesystem_location_info_t * loc,
						   struct stat *buf);

/**
* @brief IMFS evaluation node support.
*/
extern void IMFS_eval_path (rtems_filesystem_eval_path_context_t * ctx);
...
* to_loc.
*/
extern int IMFS_link (const rtems_filesystem_location_info_t * parentloc,
...
* call for the IMFS.
*/
extern int IMFS_chown (const rtems_filesystem_location_info_t * loc,
...
*/
extern int IMFS_mknod (const rtems_filesystem_location_info_t * parentloc,
					   const char *name,
					   size_t namelen, mode_t mode, dev_t dev);

extern IMFS_jnode_t *IMFS_initialize_node (IMFS_jnode_t * node,
										   const IMFS_node_control *
										   node_control, const char *name,
...
*/
extern IMFS_jnode_t *IMFS_create_node (const
									   rtems_filesystem_location_info_t *
									   parentloc,
									   const IMFS_node_control *
									   node_control, size_t node_size,
									   const char *name, size_t namelen,
									   mode_t mode, void *arg);

static inline bool IMFS_is_imfs_instance (const
										  rtems_filesystem_location_info_t *
										  loc)
{
...
* @endcode
*/
extern int IMFS_make_generic_node (const char *path,
								   mode_t mode,
...
* @brief Mount an IMFS.
*/
extern int IMFS_mount (rtems_filesystem_mount_table_entry_t * mt_entry	/* IN */
	);

/**
* @brief Unmount an IMFS.
*/
extern int IMFS_unmount (rtems_filesystem_mount_table_entry_t * mt_entry	/* IN */
	);
...
extern ssize_t IMFS_memfile_write (IMFS_memfile_t * memfile,
								   off_t start,
...
extern int device_open (rtems_libio_t * iop,	/* IN  */
						const char *pathname,	/* IN  */
						int oflag,	/* IN  */
						mode_t mode	/* IN  */
	);

extern int device_close (rtems_libio_t * iop	/* IN  */
	);

extern ssize_t device_read (rtems_libio_t * iop,	/* IN  */
							void *buffer,	/* IN  */
							size_t count	/* IN  */
	);

extern ssize_t device_write (rtems_libio_t * iop,	/* IN  */
							 const void *buffer,	/* IN  */
							 size_t count	/* IN  */
	);

extern int device_ioctl (rtems_libio_t * iop,
						 ioctl_command_t command, void *buffer);

extern int device_ftruncate (rtems_libio_t * iop,	/* IN  */
							 off_t length	/* IN  */
	);
...
* call for the IMFS.
*/
extern int IMFS_utime (const rtems_filesystem_location_info_t * loc,
					   time_t actime, time_t modtime);

/**
* @brief Change the IMFS file mode.
*/
extern int IMFS_fchmod (const rtems_filesystem_location_info_t * loc,
						mode_t mode);
...
* to_loc.
*/
extern int IMFS_symlink (const rtems_filesystem_location_info_t * parentloc,
						 const char *name,
...
*
*/
extern ssize_t IMFS_readlink (const rtems_filesystem_location_info_t * loc,
...
*/
extern int IMFS_rename (const rtems_filesystem_location_info_t *
						oldparentloc,
...
*/
extern int IMFS_rmnod (const rtems_filesystem_location_info_t * parentloc,
...
static inline void IMFS_Set_handlers (rtems_filesystem_location_info_t *
									  loc)
{
...
static inline void IMFS_add_to_directory (IMFS_jnode_t * dir_node,
...
static inline void *IMFS_generic_get_context_by_node (const IMFS_jnode_t *
													  node)
{
...
static inline void *IMFS_generic_get_context_by_location (const
														  rtems_filesystem_location_info_t
														  * loc)
{
...
static inline void *IMFS_generic_get_context_by_iop (const rtems_libio_t *
													 iop)
{
	return IMFS_generic_get_context_by_location (&iop->pathinfo);
}

static inline dev_t IMFS_generic_get_device_identifier_by_node (const
																IMFS_jnode_t
																* node)
{
