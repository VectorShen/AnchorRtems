...
static bool IMFS_eval_is_directory (rtems_filesystem_eval_path_context_t * ctx,
									void *arg)
{
...
static IMFS_jnode_t *IMFS_search_in_directory (IMFS_directory_t * dir,
											 const char *token,
											 size_t tokenlen)
{
...
IMFS_eval_token (rtems_filesystem_eval_path_context_t * ctx, void *arg,
				 const char *token, size_t tokenlen)
{
	...
	if (access_ok)
	{
		...
		if (entry != NULL)
		{
			...
			bool follow_hard_link =
				(eval_flags & RTEMS_FS_FOLLOW_HARD_LINK) != 0;
			...
			{
				const IMFS_sym_link_t *sym_link =
					(const IMFS_sym_link_t *)entry;
			...
			else
			{
				...
				{
					access_ok =
						rtems_filesystem_eval_path_check_access (ctx,
																 RTEMS_FS_PERMS_EXEC,
																 entry->
																 st_mode,
																 entry->
