...
static int task_pool_init (int count, rtems_task_priority priority)
{
	...
	sc = rtems_semaphore_create (rtems_build_name ('F', 'T', 'P', 'M'),
								 1,
								 RTEMS_DEFAULT_ATTRIBUTES
								 | RTEMS_BINARY_SEMAPHORE
								 | RTEMS_INHERIT_PRIORITY
								 | RTEMS_PRIORITY,
								 RTEMS_NO_PRIORITY, &task_pool.mutex);

	if (sc == RTEMS_SUCCESSFUL)
		sc = rtems_semaphore_create (rtems_build_name ('F', 'T', 'P', 'S'),
									 count,
									 RTEMS_DEFAULT_ATTRIBUTES,
									 RTEMS_NO_PRIORITY, &task_pool.sem);
	...
	for (i = 0; i < count; ++i)
	{
		...
		if (sc == RTEMS_SUCCESSFUL)
		{
			sc = rtems_task_start (info->tid, session,
								 (rtems_task_argument) info);
			if (sc != RTEMS_SUCCESSFUL)
				task_pool_done (i);
		}
		else
...
static FTPD_SessionInfo_t *task_pool_obtain (void)
{
	...
	rtems_status_code sc;
	sc = rtems_semaphore_obtain (task_pool.sem, RTEMS_NO_WAIT,
								 RTEMS_NO_TIMEOUT);
	if (sc == RTEMS_SUCCESSFUL)
	{
		rtems_semaphore_obtain (task_pool.mutex, RTEMS_WAIT,
								RTEMS_NO_TIMEOUT);
...
static int set_socket_timeout (int s, int seconds)
{
	...
	if (0 != setsockopt (s, SOL_SOCKET, SO_SNDTIMEO, &tv, len))
		syslog (LOG_ERR, "ftpd: Can't set send timeout on socket: %s.",
				serr ());
	else if (0 != setsockopt (s, SOL_SOCKET, SO_RCVTIMEO, &tv, len))
		syslog (LOG_ERR, "ftpd: Can't set receive timeout on socket: %s.",
				serr ());
	else
		res = 1;
	return res;
		...
		else
		{
			...
			else
			{
				struct sockaddr_in *data_dest =
					(info->use_default) ? &info->def_addr : &info->
					data_addr;
				if (0 >
					connect (s, (struct sockaddr *)data_dest,
							 sizeof (*data_dest)))
				{
					send_reply (info, 425,
...
static void close_stream (FTPD_SessionInfo_t * info)
{
	if (NULL != info->ctrl_fp)
	{
		...
		{
			syslog (LOG_ERR, "ftpd: Could not close control stream: %s",
					serr ());
		}
		else
			info->ctrl_socket = -1;
...
static void command_store (FTPD_SessionInfo_t * info, char const *filename)
{
	...
	else
	{
		...
		else if (info->xfer_mode == TYPE_A)
		{
			...
			int pended_cr = 0;
			while (res && rest == 0
				 && (n = recv (s, buf, FTPD_DATASIZE, 0)) > 0)
			{
				char const *e = buf;
				char const *b;
				int i;
	...
	if (wide)
	{
		...
		if (stat (buf, &stat_buf) == 0)
		{
			...
			len = snprintf (buf, FTPD_BUFSIZE,
							...
							(int)stat_buf.st_uid,
							(int)stat_buf.st_gid,
							(int)stat_buf.st_size, timeBuf, fname);
	...
	else
	{
		...
		else
		{
			...
			while (sc && (dp = readdir (dirp)) != NULL)
				sc = sc &&
					send_dirline (s, wide, curTime, fname, dp->d_name,
								dp->d_name, buf);
		}
	}
...
static void command_size (FTPD_SessionInfo_t * info, char const *fname)
{
	...
	if (info->xfer_mode != TYPE_I || 0 > stat (fname, &stbuf)
		|| stbuf.st_size < 0)
	{
...
static void command_port (FTPD_SessionInfo_t * info, char const *args)
{
	...
	n = sscanf (args, "%u,%u,%u,%u,%u,%u", a + 0, a + 1, a + 2, a + 3, a + 4,
				a + 5);
	if (NUM_FIELDS == n)
	{
		int i;
		...
		if (i == NUM_FIELDS)
		{
			...
			if (ip_info.u.ip == info->def_addr.sin_addr.s_addr)
			{
				...
				memset (info->data_addr.sin_zero, 0,
...
static void command_pasv (FTPD_SessionInfo_t * info)
{
	...
	else
	{
		...
		if (0 > bind (s, (struct sockaddr *)&addr, addrLen))
			...
			syslog (LOG_ERR, "ftpd: Error listening on PASV socket: %s",
					serr ());
		else if (set_socket_timeout (s, info->idle))
		{
			...
			snprintf (buf, FTPD_BUFSIZE,
					"Entering passive mode (%u,%u,%u,%u,%u,%u).", ip[0],
					ip[1], ip[2], ip[3], p[0], p[1]);
...
static void exec_command (FTPD_SessionInfo_t * info, char *cmd, char *args)
{
	...
	else if (!strcmp ("DELE", cmd))
	{
		...
		else if (strncpy (fname, args, 254) && unlink (fname) == 0)
		{
	...
	else if (!strcmp ("RMD", cmd))
	{
		...
		else if (strncpy (fname, args, 254) && rmdir (fname) == 0)
		{
...
static void session (rtems_task_argument arg)
{
	...
	while (1)
	{
		...
		rtems_event_receive (FTPD_RTEMS_EVENT, RTEMS_EVENT_ANY,
							 RTEMS_NO_TIMEOUT, &set);
 ...
 *   NONE
 */
static void daemon (rtems_task_argument args __attribute__ ((unused)))
{
	int s;
	socklen_t addrLen;
	struct sockaddr_in addr;
	...
	if (0 > bind (s, (struct sockaddr *)&addr, sizeof (addr)))
		...
		syslog (LOG_ERR, "ftpd: Error listening on control socket: %s",
				serr ());
	else
		while (1)
		{
			...
			else
			{
				...
				else
				{
					info->ctrl_socket = ss;
					if ((info->ctrl_fp =
						 fdopen (info->ctrl_socket, "r+")) == NULL)
					{
						syslog (LOG_ERR,
								"ftpd: fdopen() on socket failed: %s",
								serr ());
						close_stream (info);
						task_pool_release (info);
					}
					else
					{
						...
						info->def_addr = addr;
						if (0 >
							getsockname (ss, (struct sockaddr *)&addr,
										 &addrLen))
						{
							syslog (LOG_ERR,
									"ftpd: getsockname(): %s",
