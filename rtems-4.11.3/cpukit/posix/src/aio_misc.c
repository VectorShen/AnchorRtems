	...
	else
	{
		...
		else
		{
			...
			if (rtems_chain_is_empty (chain))
				...
				rtems_chain_insert (rtems_chain_previous (node),
									&r_chain->next_fd);
...
int rtems_aio_enqueue (rtems_aio_request * req)
{
	...
	else
	{
		...
		else
		{
			...
			r_chain =
				rtems_aio_search_fd (chain, req->aiocbp->aio_fildes, 1);
		...
		else
		{
			...
			if (rtems_chain_is_empty (chain))
			{
				...
				if (result == ETIMEDOUT)
				{
					...
					if (rtems_chain_is_empty
						(&aio_request_queue.idle_req))
					{
						AIO_printf
							("Chain is empty [IQ], wait for work\n");
					...
					node =
						rtems_chain_first (&aio_request_queue.idle_req);
					rtems_chain_extract (node);
