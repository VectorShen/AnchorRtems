					...
					void *(*start_routine) (void *), void *arg)
{
	...
	status =
		_CPU_set_Is_valid (the_attr->affinityset, the_attr->affinitysetsize);
	if (!status)
		return EINVAL;
#endif
#endif
	...
	status = _Thread_Initialize (&_POSIX_Threads_Information, the_thread, _Scheduler_Get (executing), the_attr->stackaddr, _POSIX_Threads_Ensure_minimum_stack (the_attr->stacksize), is_fp, core_priority, true,	/* preemptible */
								 budget_algorithm, budget_callout, 0,	/* isr level */
	 ...
	 */
	status = _Thread_Start (the_thread, THREAD_START_POINTER, start_routine, arg, 0,	/* unused */
							NULL);
