...
RTEMS_INLINE_ROUTINE void
	_POSIX_Message_queue_Free (POSIX_Message_queue_Control * the_mq)
{
...
RTEMS_INLINE_ROUTINE POSIX_Message_queue_Control
	*_POSIX_Message_queue_Get (Objects_Id id, Objects_Locations * location)
{
	return (POSIX_Message_queue_Control *)
...
*  by the Score.
*/
RTEMS_INLINE_ROUTINE unsigned int
	_POSIX_Message_queue_Priority_from_core (CORE_message_queue_Submit_types
											 priority)
{
...
*
*/
int _POSIX_Message_queue_Translate_core_message_queue_return_code (uint32_t
...
*/
RTEMS_INLINE_ROUTINE void
	_POSIX_Message_queue_Free_fd (POSIX_Message_queue_Control_fd *
								  the_mq_fd)
{
	_Objects_Free (&_POSIX_Message_queue_Information_fds,
				   &the_mq_fd->Object);
}
...
*/
RTEMS_INLINE_ROUTINE void
	_POSIX_Message_queue_Namespace_remove (POSIX_Message_queue_Control *
										   the_mq)
{
	_Objects_Namespace_remove (&_POSIX_Message_queue_Information,
							   &the_mq->Object);
}
...
*/
RTEMS_INLINE_ROUTINE POSIX_Message_queue_Control_fd
	*_POSIX_Message_queue_Get_fd (mqd_t id, Objects_Locations * location)
{
	return (POSIX_Message_queue_Control_fd *)
		_Objects_Get (&_POSIX_Message_queue_Information_fds,
					  (Objects_Id) id, location);
}

RTEMS_INLINE_ROUTINE POSIX_Message_queue_Control_fd
	*_POSIX_Message_queue_Get_fd_interrupt_disable (mqd_t id,
													Objects_Locations *
													location,
													ISR_lock_Context *
													lock_context)
{
	return (POSIX_Message_queue_Control_fd *)
		_Objects_Get_isr_disable (&_POSIX_Message_queue_Information_fds,
								  (Objects_Id) id, location, lock_context);
}
...
RTEMS_INLINE_ROUTINE int _POSIX_Message_queue_Name_to_id (const char *name,
														  ...
														  size_t * len)
{
	return _POSIX_Name_to_id (&_POSIX_Message_queue_Information, name, id,
							  len);
}
