...
void _Objects_MP_Handler_early_initialization (void)
{
	...
	if (node < 1 || node > maximum_nodes)
		_Terminate (INTERNAL_ERROR_CORE, true, INTERNAL_ERROR_INVALID_NODE);
...
void _Objects_MP_Handler_initialization (void)
{
	...
	_Chain_Initialize (&_Objects_MP_Inactive_global_objects,
...
void _Objects_MP_Open (Objects_Information * information, Objects_MP_Control * the_global_object, uint32_t the_name,	/* XXX -- wrong for variable */
					 Objects_Id the_id)
{
	the_global_object->Object.id = the_id;
	the_global_object->name = the_name;

	_Chain_Prepend (&information->global_table[_Objects_Get_node (the_id)],
					&the_global_object->Object.Node);

}

bool _Objects_MP_Allocate_and_open (Objects_Information * information, uint32_t the_name,	/* XXX -- wrong for variable */
									Objects_Id the_id, bool is_fatal_error)
{
	...
	if (_Objects_MP_Is_null_global_object (the_global_object))
	{
		...
		_Terminate (INTERNAL_ERROR_CORE,
					true, INTERNAL_ERROR_OUT_OF_GLOBAL_OBJECTS);
...
void _Objects_MP_Close (Objects_Information * information, Objects_Id the_id)
{
	...
	_Terminate (INTERNAL_ERROR_CORE, true, INTERNAL_ERROR_INVALID_GLOBAL_ID);
}

Objects_Name_or_id_lookup_errors
_Objects_MP_Global_name_search (Objects_Information * information,
								Objects_Name the_name, uint32_t nodes_to_search,
								Objects_Id * the_id)
{
	uint32_t low_node;
	uint32_t high_node;
	...
	{
		low_node = high_node = nodes_to_search;
	}
							...
							Objects_Control ** the_object)
{
	...
	if (node == 0 ||
		...
		node > _Objects_Maximum_nodes || information->global_table == NULL)
	{
