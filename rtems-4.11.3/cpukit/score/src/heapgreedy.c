								 ...
								 size_t block_count)
{
	...
	for (i = 0; i < block_count; ++i)
	{
		...
		if (next != NULL)
		{
			Heap_Block *next_block = _Heap_Block_of_alloc_area ((uintptr_t)
																next,
																heap->
																page_size);
	...
	{
		_Heap_Block_allocate (heap,
								current,
								_Heap_Alloc_area_of_block (current),
								_Heap_Block_size (current) -
													...
													allocatable_size)
{
	...
	{
		*allocatable_size =
			info.largest - HEAP_BLOCK_HEADER_SIZE + HEAP_ALLOC_BONUS;
	}
	else
	{
		*allocatable_size = 0;
