...
void _Thread_Handler_initialization (void)
{
...
#if defined(RTEMS_MULTIPROCESSING)
	uint32_t maximum_proxies = _Configuration_MP_table->maximum_proxies;
#endif

	if (rtems_configuration_get_stack_allocate_hook () == NULL ||
		rtems_configuration_get_stack_free_hook () == NULL)
		_Terminate (INTERNAL_ERROR_CORE, true, INTERNAL_ERROR_BAD_STACK_HOOK);
	 ...
	 */
	_Objects_Initialize_information (&_Thread_Internal_information, OBJECTS_INTERNAL_API, OBJECTS_INTERNAL_THREADS, _Thread_Get_maximum_internal_threads (), _Thread_Control_size,	/* size of this object's control block */
