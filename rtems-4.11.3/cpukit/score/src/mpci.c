...
RTEMS_STATIC_ASSERT (sizeof (MPCI_Internal_packet) <=
					 MP_PACKET_MINIMUM_PACKET_SIZE, MPCI_Internal_packet);
...
void _MPCI_Handler_initialization (uint32_t timeout_status)
{
	...
	if (_System_state_Is_multiprocessing && !users_mpci_table)
		_Terminate (INTERNAL_ERROR_CORE, true, INTERNAL_ERROR_NO_MPCI);
	...
	_MPCI_Register_packet_processor (MP_PACKET_MPCI_INTERNAL,
	...
	_CORE_semaphore_Initialize (&_MPCI_Semaphore, &attributes,	/* the_semaphore_attributes */
								0	/* initial_value */
		);
...
void _MPCI_Create_server (void)
{
	...
	_Thread_Initialize (&_Thread_Internal_information, _MPCI_Receive_server_tcb, _Scheduler_Get_by_CPU_index (_SMP_Get_current_processor ()), NULL,	/* allocate the stack */
						_Stack_Minimum () + CPU_MPCI_RECEIVE_SERVER_EXTRA_STACK + _Configuration_MP_table->extra_mpci_receive_server_stack, CPU_ALL_TASKS_ARE_FP, PRIORITY_PSEUDO_ISR, false,	/* no preempt */
						THREAD_CPU_BUDGET_ALGORITHM_NONE, NULL,	/* no budget algorithm callout */
...
MP_packet_Prefix *_MPCI_Get_packet (void)
{
	...
	if (the_packet == NULL)
		_Terminate (INTERNAL_ERROR_CORE, true, INTERNAL_ERROR_OUT_OF_PACKETS);
...
void _MPCI_Internal_packets_Process_packet (MP_packet_Prefix *
											the_packet_prefix)
{
	MPCI_Internal_packet *the_packet;
