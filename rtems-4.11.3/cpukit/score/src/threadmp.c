...
void _Thread_MP_Handler_initialization (uint32_t maximum_proxies)
{
	...
	_Chain_Initialize (&_Thread_MP_Inactive_proxies,
					   _Workspace_Allocate_or_fatal_error (maximum_proxies *
														   sizeof
...
Thread_Control *_Thread_MP_Allocate_proxy (States_Control the_state)
{
	...
	if (!_Thread_Is_null (the_thread))
	  {
		  ...
		  the_proxy->Object.id =
			  _MPCI_Receive_server_tcb->receive_packet->source_tid;
	...
	_Terminate (INTERNAL_ERROR_CORE, true, INTERNAL_ERROR_OUT_OF_PROXIES);
  ...
  restart:
	...
	for (proxy_node = _Chain_First (&_Thread_MP_Active_proxies);
		 !_Chain_Is_tail (&_Thread_MP_Active_proxies, proxy_node);)
	  {
		  ...
		  the_thread =
			  (Thread_Control *) _Addresses_Subtract_offset (proxy_node,
