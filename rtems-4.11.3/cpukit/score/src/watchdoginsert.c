...
static void _Watchdog_Insert_fixup (Watchdog_Header * header,
							...
							ISR_lock_Context * lock_context)
{
	if (the_watchdog->state == WATCHDOG_INACTIVE)
	{
		...
		while ((next =
				_Chain_Next (current)) != _Chain_Tail (&header->Watchdogs))
		{
			Watchdog_Control *next_watchdog;
			Watchdog_Interval delta_next;
			...
			if (delta < delta_next)
			{
				_Watchdog_Insert_fixup (header,
										the_watchdog,
										delta, next_watchdog, delta_next);
				break;
			}
