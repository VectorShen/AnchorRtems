...
void _SMP_Multicast_actions_process (void)
{
	...
	node =
		(SMP_Multicast_action *) _Chain_First (&_SMP_Multicast_action_context.
											   List);
				...
				if (CPU_COUNT_S (node->setsize, node->recipients) == 0)
				  {
					  ...
					  _Atomic_Store_ulong (&node->done, 1,
...
static void _SMP_Multicast_actions_try_process (void)
{
...
void _SMP_Multicast_action (const size_t setsize,
							const cpu_set_t * cpus,
							SMP_Multicast_action_handler handler, void *arg)
{
	uint32_t i;
	SMP_Multicast_action node;
	...
	_SMP_lock_ISR_disable_and_acquire (&_SMP_Multicast_action_context.Lock,
									   &lock_context);
	_Chain_Prepend_unprotected (&_SMP_Multicast_action_context.List,
								&node.Node);
	_SMP_lock_Release_and_ISR_enable (&_SMP_Multicast_action_context.Lock,
									  &lock_context);
