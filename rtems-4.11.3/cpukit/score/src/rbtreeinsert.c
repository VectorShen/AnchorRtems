...
RTEMS_STATIC_ASSERT (sizeof (RBTree_Compare_result) >= sizeof (intptr_t),
					 RBTree_Compare_result_intptr_t);

RTEMS_STATIC_ASSERT (sizeof (RBTree_Compare_result) >= sizeof (int32_t),
	...
	while (parent->color == RBT_RED)
	  {
		  ...
		  if (uncle != NULL && uncle->color == RBT_RED)
			...
			{					/* If uncle does not exist or is black */
				...
				_RBTree_Rotate (grandparent,
								_RBTree_Opposite_direction (parentdir));
...
RBTree_Node *_RBTree_Insert (RBTree_Control * the_rbtree,
							 ...
							 RBTree_Compare compare, bool is_unique)
{
				...
				if (!iter_node->child[dir])
				  {
					  ...
					  compare_result = (*compare) (the_node,
												   _RBTree_First (the_rbtree,
																  dir));

					  if ((dir == RBT_LEFT
						   && _RBTree_Is_lesser (compare_result))
						  || (dir == RBT_RIGHT
							  && !_RBTree_Is_lesser (compare_result)))
						{
