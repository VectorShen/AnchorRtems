...
void *_Workspace_Allocate (size_t size)
{
	...
	printk ("Workspace_Allocate(%d) from %p/%p -> %p\n",
			size,
			__builtin_return_address (0), __builtin_return_address (1), memory);
#endif
	return memory;
}
...
{
#if defined(DEBUG_WORKSPACE)
	printk ("Workspace_Free(%p) from %p/%p\n",
			block, __builtin_return_address (0), __builtin_return_address (1));
#endif
...
void *_Workspace_Allocate_or_fatal_error (size_t size)
{
	...
	if (memory == NULL)
		_Terminate (INTERNAL_ERROR_CORE,
					true, INTERNAL_ERROR_WORKSPACE_ALLOCATION);
