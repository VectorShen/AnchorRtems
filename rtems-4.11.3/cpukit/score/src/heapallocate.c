...
#else
static bool _Heap_Protection_free_delayed_blocks (Heap_Control * heap,
...
static void _Heap_Check_allocation (const Heap_Control * heap,
...
static uintptr_t _Heap_Check_block (const Heap_Control * heap,
									const Heap_Block * block,
									uintptr_t alloc_size,
									uintptr_t alignment, uintptr_t boundary)
{
	...
	do
	  {
				...
				if (block->size_and_flag > block_size_floor)
				  {
					  if (alignment == 0)
						...
						{
							alloc_begin = _Heap_Check_block (heap,
															 block,
															 alloc_size,
															 alignment,
															 boundary);
		  ...
		  search_again =
			  _Heap_Protection_free_delayed_blocks (heap, alloc_begin);
	  }
	while (search_again);
	...
	if (alloc_begin != 0)
	  {
		  ...
		  _Heap_Check_allocation (heap,
								  block,
								  alloc_begin, alloc_size, alignment, boundary);
