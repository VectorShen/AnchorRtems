...
static void _Heap_Walk_print_nothing (int source,
...
static bool _Heap_Walk_check_free_list (int source,
	...
	while (free_block != free_list_tail)
	  {
		  ...
		  if (!_Heap_Is_aligned
			  (_Heap_Alloc_area_of_block (free_block), page_size))
			{
				(*printer) (source,
							true,
		  ...
		  if (_Heap_Is_used (free_block))
			{
				(*printer) (source,
							true, "free block 0x%08x: is used\n", free_block);
...
static bool _Heap_Walk_check_control (int source,
									  Heap_Walk_printer printer,
									  Heap_Control * heap)
{
	uintptr_t const page_size = heap->page_size;
	...
	(*printer) (source,
				...
				first_block, last_block, first_free_block, last_free_block);
	...
	if (!_Addresses_Is_aligned ((void *)page_size))
	  {
		  (*printer) (source,
					  true, "page size %u not CPU aligned\n", page_size);
	...
	if (!_Heap_Is_aligned (_Heap_Alloc_area_of_block (first_block), page_size))
	  {
		  (*printer) (source,
					  true,
	...
	if (!_Heap_Is_prev_used (first_block))
	  {
		  (*printer) (source,
					  true, "first block: HEAP_PREV_BLOCK_USED is cleared\n");
	...
	if (_Heap_Block_at (last_block, _Heap_Block_size (last_block)) !=
		first_block)
	  {
		  (*printer) (source,
					  true, "last block: next block is not the first block\n");
...
static bool _Heap_Walk_check_free_block (int source,
										 Heap_Walk_printer printer,
										 Heap_Control * heap,
										 Heap_Block * block)
{
	...
	if (block_size != next_block->prev_size)
	  {
		  (*printer) (source,
					  ...
					  block, block_size, next_block->prev_size, next_block);
		  ...
		  if (!_Heap_Is_prev_used (next_block))
		  ...
		  else
			{
				(*printer) (source,
							...
							block, block_size, block->prev_size);
			}
