...
RTEMS_STATIC_ASSERT (MESSAGE_QUEUE_MP_PACKET_SIZE <=
...
void _Message_queue_MP_Send_process_packet (Message_queue_MP_Remote_operations
											operation,
...
rtems_status_code
_Message_queue_MP_Send_request_packet (Message_queue_MP_Remote_operations
									 operation, Objects_Id message_queue_id,
									 const void *buffer, size_t * size_p,
	...
	switch (operation)
	{
		...
		case MESSAGE_QUEUE_MP_GET_NUMBER_PENDING_REQUEST:
			...
			return (rtems_status_code)
				_MPCI_Send_request_packet (_Objects_Get_node
											 (message_queue_id),
											 &the_packet->Prefix,
		...
		case MESSAGE_QUEUE_MP_RECEIVE_REQUEST:
			...
			_Thread_Executing->Wait.return_argument_second.immutable_object =
				buffer;
											 ...
											 Thread_Control * the_thread)
{
	...
	switch (operation)
	{
		...
		case MESSAGE_QUEUE_MP_GET_NUMBER_PENDING_RESPONSE:
			...
			the_packet =
				(Message_queue_MP_Packet *) the_thread->receive_packet;
	...
	switch (the_packet->operation)
	{
		...
		case MESSAGE_QUEUE_MP_RECEIVE_REQUEST:
			...
			the_packet->Prefix.return_code =
				rtems_message_queue_receive (the_packet->Prefix.id,
											 the_packet->Buffer.buffer,
											 &the_packet->size,
			...
			if (the_packet->Prefix.return_code != RTEMS_PROXY_BLOCKING)
				_Message_queue_MP_Send_response_packet
					(MESSAGE_QUEUE_MP_RECEIVE_RESPONSE, the_packet->Prefix.id,
					 _Thread_Executing);
			break;
		...
		case MESSAGE_QUEUE_MP_RECEIVE_RESPONSE:
			...
			if (the_packet->Prefix.return_code == RTEMS_SUCCESSFUL)
			{
				*(size_t *) the_thread->Wait.return_argument =
				...
				_CORE_message_queue_Copy_buffer (the_packet->Buffer.buffer,
												 the_thread->Wait.
		...
		case MESSAGE_QUEUE_MP_SEND_REQUEST:
			...
			the_packet->Prefix.return_code =
				rtems_message_queue_send (the_packet->Prefix.id,
											the_packet->Buffer.buffer,
											the_packet->Buffer.size);

			_Message_queue_MP_Send_response_packet
				(MESSAGE_QUEUE_MP_SEND_RESPONSE, the_packet->Prefix.id,
				 _Thread_Executing);
			break;
		...
		case MESSAGE_QUEUE_MP_URGENT_REQUEST:
			...
			the_packet->Prefix.return_code =
				rtems_message_queue_urgent (the_packet->Prefix.id,
											the_packet->Buffer.buffer,
											the_packet->Buffer.size);

			_Message_queue_MP_Send_response_packet
				(MESSAGE_QUEUE_MP_URGENT_RESPONSE, the_packet->Prefix.id,
				 _Thread_Executing);
			break;
		...
		case MESSAGE_QUEUE_MP_BROADCAST_REQUEST:
			...
			the_packet->Prefix.return_code =
				rtems_message_queue_broadcast (the_packet->Prefix.id,
												 the_packet->Buffer.buffer,
												 the_packet->Buffer.size,
												 &the_packet->count);

			_Message_queue_MP_Send_response_packet
				(MESSAGE_QUEUE_MP_BROADCAST_RESPONSE, the_packet->Prefix.id,
				 _Thread_Executing);
			break;
		...
		case MESSAGE_QUEUE_MP_FLUSH_REQUEST:
			...
			the_packet->Prefix.return_code =
				rtems_message_queue_flush (the_packet->Prefix.id,
											 &the_packet->count);

			_Message_queue_MP_Send_response_packet
				(MESSAGE_QUEUE_MP_FLUSH_RESPONSE, the_packet->Prefix.id,
				 _Thread_Executing);
			break;
		...
		case MESSAGE_QUEUE_MP_GET_NUMBER_PENDING_REQUEST:
			...
			the_packet->Prefix.return_code =
				rtems_message_queue_get_number_pending (the_packet->Prefix.id,
														&the_packet->count);

			_Message_queue_MP_Send_response_packet
				(MESSAGE_QUEUE_MP_GET_NUMBER_PENDING_RESPONSE,
				 the_packet->Prefix.id, _Thread_Executing);
			break;
...
void _Message_queue_MP_Send_extract_proxy (void *argument)
{
	...
	_Message_queue_MP_Send_process_packet (MESSAGE_QUEUE_MP_EXTRACT_PROXY,
										 the_thread->Wait.id,
										 (rtems_name) 0,
