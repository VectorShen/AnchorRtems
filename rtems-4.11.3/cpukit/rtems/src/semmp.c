...
RTEMS_STATIC_ASSERT (sizeof (Semaphore_MP_Packet) <=
					 MP_PACKET_MINIMUM_PACKET_SIZE, Semaphore_MP_Packet);

void _Semaphore_MP_Send_process_packet (Semaphore_MP_Remote_operations
										operation, Objects_Id semaphore_id,
...
rtems_status_code
_Semaphore_MP_Send_request_packet (Semaphore_MP_Remote_operations operation,
								 ...
								 rtems_interval timeout)
{
	...
	switch (operation)
	{
		...
		case SEMAPHORE_MP_RELEASE_REQUEST:
			...
			return
				_MPCI_Send_request_packet (_Objects_Get_node (semaphore_id),
											 &the_packet->Prefix,
...
void _Semaphore_MP_Send_response_packet (Semaphore_MP_Remote_operations
										 operation, Objects_Id semaphore_id,
...
void _Semaphore_MP_Process_packet (rtems_packet_prefix * the_packet_prefix)
{
	...
	switch (the_packet->operation)
	{
		...
		case SEMAPHORE_MP_ANNOUNCE_CREATE:
			...
			ignored = _Objects_MP_Allocate_and_open (&_Semaphore_Information,
													 the_packet->name,
		...
		case SEMAPHORE_MP_RELEASE_REQUEST:
			...
			the_packet->Prefix.return_code =
				rtems_semaphore_release (the_packet->Prefix.id);

			_Semaphore_MP_Send_response_packet (SEMAPHORE_MP_RELEASE_RESPONSE,
												the_packet->Prefix.id,
												_Thread_Executing);
			break;
	}
...
void _Semaphore_MP_Send_object_was_deleted (Thread_Control * the_proxy)
{
	...
	_Semaphore_MP_Send_response_packet (SEMAPHORE_MP_OBTAIN_RESPONSE,
...
void _Semaphore_MP_Send_extract_proxy (void *argument)
{
	...
	_Semaphore_MP_Send_process_packet (SEMAPHORE_MP_EXTRACT_PROXY,
									 the_thread->Wait.id,
