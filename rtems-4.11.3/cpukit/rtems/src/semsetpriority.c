...
static rtems_status_code _Semaphore_Set_priority (Semaphore_Control *
												  the_semaphore,
	...
	if (_Attributes_Is_multiprocessor_resource_sharing (attribute_set))
	  {
		  ...
		  if (new_priority != RTEMS_CURRENT_PRIORITY)
			{
				_MRSP_Set_ceiling_priority (mrsp, scheduler_index,
											new_priority);
			}
												...
												old_priority)
{
	...
	switch (location)
	  {
		  case OBJECTS_LOCAL:
			  return _Semaphore_Set_priority (the_semaphore,
											  scheduler_id,
