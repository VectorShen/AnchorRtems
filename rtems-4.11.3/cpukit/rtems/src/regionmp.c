...
RTEMS_STATIC_ASSERT (sizeof (Region_MP_Packet) <= MP_PACKET_MINIMUM_PACKET_SIZE,
					 Region_MP_Packet);
...
rtems_status_code _Region_MP_Send_request_packet (Region_MP_Remote_operations
												operation,
												Objects_Id region_id,
												void *segment, intptr_t size,
												...
												rtems_interval timeout)
{
	...
	switch (operation)
	{
		...
		case REGION_MP_RETURN_SEGMENT_REQUEST:
			...
			return (rtems_status_code) _MPCI_Send_request_packet (_Objects_Get_node (region_id), &the_packet->Prefix, STATES_READY,	/* Not used */
																	RTEMS_TIMEOUT);
			break;
...
void _Region_MP_Process_packet (rtems_packet_prefix * the_packet_prefix)
{
	...
	switch (the_packet->operation)
	{
		...
		case REGION_MP_ANNOUNCE_CREATE:
			...
			ignored = _Objects_MP_Allocate_and_open (&_Region_Information,
													 the_packet->name,
		...
		case REGION_MP_GET_SEGMENT_REQUEST:
			...
			the_packet->Prefix.return_code =
				rtems_region_get_segment (the_packet->Prefix.id,
											the_packet->size,
											the_packet->option_set,
											the_packet->Prefix.timeout,
											&the_packet->segment);

			_Region_MP_Send_response_packet (REGION_MP_GET_SEGMENT_RESPONSE,
											 the_packet->Prefix.id,
											 _Thread_Executing);
			break;
		...
		case REGION_MP_RETURN_SEGMENT_REQUEST:
			...
			the_packet->Prefix.return_code =
				rtems_region_return_segment (the_packet->Prefix.id,
											 the_packet->segment);

			_Region_MP_Send_response_packet
				(REGION_MP_RETURN_SEGMENT_RESPONSE, the_packet->Prefix.id,
				 _Thread_Executing);
			break;
...
void _Region_MP_Send_extract_proxy (void *argument)
{
	...
	_Region_MP_Send_process_packet (REGION_MP_EXTRACT_PROXY,
									the_thread->Wait.id,
