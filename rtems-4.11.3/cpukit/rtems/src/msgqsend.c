											...
											const void *buffer, size_t size)
{
	...
	switch (location)
	{
		...
		case OBJECTS_LOCAL:
			status = _CORE_message_queue_Send (&the_message_queue->message_queue, buffer, size, id, MESSAGE_QUEUE_MP_HANDLER, false,	/* sender does not block */
												 0,	/* no timeout */
												 &lock_context);
...
#if defined(RTEMS_MULTIPROCESSING)
		case OBJECTS_REMOTE:
			return _Message_queue_MP_Send_request_packet (MESSAGE_QUEUE_MP_SEND_REQUEST, id, buffer, &size, 0,	/* option_set */
