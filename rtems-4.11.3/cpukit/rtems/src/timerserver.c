...
static void _Timer_server_Cancel_method (Timer_server_Control * ts,
										 Timer_Control * timer)
{
...
static void _Timer_server_Insert_timer (Timer_server_Watchdogs * watchdogs,
										Timer_Control * timer,
...
static void _Timer_server_Schedule_operation_method (Timer_server_Control * ts,
													 Timer_Control * timer)
{
	if (timer->the_class == TIMER_INTERVAL_ON_TASK)
	  {
...
static void _Timer_server_Initialize_watchdogs (Timer_server_Control * ts,
												rtems_id id,
												Timer_server_Watchdogs *
												watchdogs,
												Watchdog_Interval (*get_ticks)
												(void))
{
	Watchdog_Interval now;
											   ...
											   rtems_attribute attribute_set)
{
	 ...
	 */
	status = rtems_task_create (_Objects_Build_name ('T', 'I', 'M', 'E'),	/* "TIME" */
								_priority,	/* create with priority 1 since 0 is illegal */
								stack_size,	/* let user specify stack size */
								rtems_configuration_is_smp_enabled ()? RTEMS_DEFAULT_MODES :	/* no preempt is not supported for SMP */
	...
	_Timer_server_Initialize_watchdogs (ts,
										id,
