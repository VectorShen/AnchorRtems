...
static inline int gpio_nxp_pca9535_clear_and_set_output (int fd,
														 uint16_t clear,
														 uint16_t set)
{
...
static inline int gpio_nxp_pca9535_get_polarity_inversion (int fd,
														   uint16_t * val)
{
	return ioctl (fd, GPIO_NXP_PCA9535_GET_POL_INV, val);
}

static inline int gpio_nxp_pca9535_set_polarity_inversion (int fd,
														   uint16_t val)
{
	return ioctl (fd, GPIO_NXP_PCA9535_SET_POL_INV,
				  (void *)(uintptr_t) val);
}
