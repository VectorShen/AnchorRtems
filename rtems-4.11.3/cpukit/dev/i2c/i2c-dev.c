...
static ssize_t i2c_dev_write (rtems_libio_t * iop,
...
static int i2c_dev_ioctl (rtems_libio_t * iop,
...
static int i2c_dev_fstat (const rtems_filesystem_location_info_t * loc,
						struct stat *buf)
{
...
IMFS_GENERIC_INITIALIZER (&i2c_dev_handler,
...
int i2c_dev_register (i2c_dev * dev, const char *dev_path)
{
	...
	rv = IMFS_make_generic_node (dev_path,
								 S_IFCHR | S_IRWXU | S_IRWXG | S_IRWXO,
								 &i2c_dev_node_control, dev);
	if (rv != 0)
	{
		(*dev->destroy) (dev);
	}
...
static ssize_t i2c_dev_read_default (i2c_dev * dev,
									 void *buf, size_t n, off_t offset)
{
	(void)dev;
	(void)buf;
	(void)n;
...
static ssize_t i2c_dev_write_default (i2c_dev * dev,
...
static int i2c_dev_ioctl_default (i2c_dev * dev,
								ioctl_command_t command, void *arg)
{
	(void)dev;
	(void)command;
...
static int i2c_dev_do_init (i2c_dev * dev,
							const char *bus_path,
							uint16_t address, void (*destroy) (i2c_dev * dev))
{
	int rv;
...
i2c_dev *i2c_dev_alloc_and_init (size_t size,
								 const char *bus_path, uint16_t address)
{
	i2c_dev *dev = NULL;
	...
	if (size >= sizeof (*dev))
	{
		...
		if (dev != NULL)
		{
			...
			rv = i2c_dev_do_init (dev, bus_path, address,
