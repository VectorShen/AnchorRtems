...
static void eeprom_set_addr (const eeprom * dev,
							 uint32_t off,
...
static ssize_t eeprom_read (i2c_dev * base, void *buf, size_t n, off_t offset)
{
	...
	while (todo > 0)
	{
		...
		err =
			i2c_bus_transfer (dev->base.bus, &msgs[0],
...
static ssize_t eeprom_write (i2c_dev * base,
...
int i2c_dev_register_eeprom (const char *bus_path,
							 const char *dev_path,
