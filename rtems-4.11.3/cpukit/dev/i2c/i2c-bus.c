...
static ssize_t i2c_bus_write (rtems_libio_t * iop,
...
static int i2c_bus_ioctl (rtems_libio_t * iop,
						ioctl_command_t command, void *arg)
{
	...
	switch (command)
	{
		...
		case I2C_TIMEOUT:
			bus->timeout =
				RTEMS_MILLISECONDS_TO_TICKS (10 * (unsigned long)arg);
			err = 0;
			break;
		case I2C_SLAVE:
...
IMFS_GENERIC_INITIALIZER (&i2c_bus_handler,
...
int i2c_bus_register (i2c_bus * bus, const char *bus_path)
{
	...
	rv = IMFS_make_generic_node (bus_path,
								 S_IFCHR | S_IRWXU | S_IRWXG | S_IRWXO,
								 &i2c_bus_node_control, bus);
	if (rv != 0)
	{
		(*bus->destroy) (bus);
	}
...
static int i2c_bus_transfer_default (i2c_bus * bus,
									 i2c_msg * msgs, uint32_t msg_count)
{
	(void)bus;
	(void)msgs;
	(void)msg_count;
...
static int i2c_bus_do_init (i2c_bus * bus, void (*destroy) (i2c_bus * bus))
{
	...
	sc = rtems_semaphore_create (rtems_build_name ('I', '2', 'C', ' '),
								 1,
								 RTEMS_BINARY_SEMAPHORE | RTEMS_INHERIT_PRIORITY
								 | RTEMS_PRIORITY, 0, &bus->mutex);
