...
*/
typedef int (*rtems_block_device_ioctl) (rtems_disk_device * dd,
										 uint32_t req, void *argp);
...
static inline dev_t rtems_disk_get_device_identifier (const
													  rtems_disk_device *
													  dd)
{
	return dd->dev;
...
static inline rtems_device_major_number rtems_disk_get_major_number (const
																	 rtems_disk_device
																	 * dd)
{
...
static inline rtems_device_minor_number rtems_disk_get_minor_number (const
																	 rtems_disk_device
																	 * dd)
{
...
static inline void *rtems_disk_get_driver_data (const rtems_disk_device *
												dd)
{
	return dd->driver_data;
}

static inline uint32_t rtems_disk_get_media_block_size (const
														rtems_disk_device *
														dd)
{
...
static inline uint32_t rtems_disk_get_block_size (const rtems_disk_device *
												  dd)
{
	return dd->block_size;
}

static inline rtems_blkdev_bnum rtems_disk_get_block_begin (const
															rtems_disk_device
															* dd)
{
	return dd->start;
...
static inline rtems_blkdev_bnum rtems_disk_get_block_count (const
															rtems_disk_device
															* dd)
{
	return dd->size;
...
rtems_status_code rtems_disk_init_phys (rtems_disk_device * dd,
										uint32_t block_size,
										rtems_blkdev_bnum block_count,
										rtems_block_device_ioctl handler,
										void *driver_data);
...
/* Internal function, do not use */
rtems_status_code rtems_disk_init_log (rtems_disk_device * dd,
