...
\*=========================================================================*/
{
	...
	if (rc == RTEMS_SUCCESSFUL)
	{
		...
		for (pool_probe_size = 1;
			 pool_probe_size < (INT_MAX >> 1)
			 && (pool_rc == RTEMS_SUCCESSFUL); pool_probe_size <<= 1)
		{
...
\*=========================================================================*/
{
	...
	if (rc == RTEMS_SUCCESSFUL)
	{
		printf
			("------------------------------------------------------------------------------\n");
		printf (" pool #%03d: blksize=%5u nblks=%5u buf_mem=0x%08" PRIxPTR
				" bdbuf_mem=0x%08" PRIxPTR "\n", pool_idx, pool_ptr->blksize,
				pool_ptr->nblks, (intptr_t) pool_ptr->mallocd_bufs,
...
\*-------------------------------------------------------------------------*/
													...
													const show_bdbuf_filter_t *
													filter, bool * is_match)
/*-------------------------------------------------------------------------*\
| Return Value:                                                             |
|    rtems_status_code                                                      |
\*=========================================================================*/
	...
	if (rc == RTEMS_SUCCESSFUL)
	{
		...
		else
		{
			unmatch = ((filter->bdbuf_modified && !bdbuf_info->modified) ||
					 (filter->bdbuf_in_progress
						&& !bdbuf_info->in_progress)
					 || (filter->bdbuf_actual && !bdbuf_info->actual)
					 || (filter->bdbuf_used
						 && !(bdbuf_info->use_count > 0)));
...
\*=========================================================================*/
{
		...
		((selector->show_all) || (selector->show_node_chain)))
	{
		...
		else
		{
			printf ("%3s ",
					((bdbuf_info->in_chain ==
					bdbuf_chain_ident_free) ? "FRE" : (bdbuf_info->
														 in_chain ==
														 bdbuf_chain_ident_lru)
					 ? "LRU" : (bdbuf_info->in_chain ==
								bdbuf_chain_ident_mod) ? "MOD" : "???"));
		}
	}
		...
		else
		{
			printf ("%20s:%-10s ",
					((bdbuf_info->status == RTEMS_SUCCESSFUL)
					 ? "SUCCESSFUL" : rtems_status_text (bdbuf_info->
														 status)),
					((bdbuf_info->status ==
					RTEMS_SUCCESSFUL) ? "" : strerror (bdbuf_info->
														 error)));
		}
	}
	/*
...
\*=========================================================================*/
{
	 ...
	 */
	for (pool_idx = 0;
		 (rc == RTEMS_SUCCESSFUL) && (pool_idx < pool_cnt); pool_idx++)
	{
		if ((filter.pool_id < 0) || (filter.pool_id == pool_idx))
		{
			...
			if (rc == RTEMS_SUCCESSFUL)
			{
				...
				for (bdbuf_idx = 0;
					 ((rc == RTEMS_SUCCESSFUL) &&
						(bdbuf_idx < curr_pool->nblks)); bdbuf_idx++)
				{
					/*
					...
					if (rc == RTEMS_SUCCESSFUL)
					{
						rc = rtems_show_bdbuf_get_bdbuf_info
							(&(curr_pool->bdbufs[bdbuf_idx]),
							 bdbuf_idx, curr_pool, &bdbuf_info);
					}
					/*
					...
					if (rc == RTEMS_SUCCESSFUL)
					{
						rc = rtems_show_bdbuf_match_filter
							(&bdbuf_info, &filter, &bdbuf_matches);
					}
					/*
					...
					if (rc == RTEMS_SUCCESSFUL)
					{
						if (bdbuf_matches)
						{
							rc = rtems_show_bdbuf_print
								(&bdbuf_info, &selector, false);
							if ((rc == RTEMS_SUCCESSFUL)
								&& selector.show_sema)
							{
