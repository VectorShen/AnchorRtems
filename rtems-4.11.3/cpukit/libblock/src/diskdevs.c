...
RTEMS_INTERRUPT_LOCK_DEFINE (static, diskdevs_lock, "diskdevs")
	 static rtems_status_code disk_lock (void)
{
...
create_disk (dev_t dev,
			 const char *name,
			 rtems_disk_device ** dd_ptr, char **alloc_name_ptr)
{
										...
										void *driver_data, const char *name)
{
	...
	sc = rtems_disk_init_phys (dd,
							 block_size, block_count, handler, driver_data);
...
static void rtems_disk_cleanup (rtems_disk_device * disk_to_remove)
{
	...
	else
	{
		...
		{
			--physical_disk->uses;
			rtems_filesystem_split_dev_t (disk_to_remove->dev, major,
										minor);
			disktab[major].minor[minor] = NULL;
...
rtems_status_code rtems_disk_io_initialize (void)
{
	...
	diskdevs_protected = false;
	sc = rtems_semaphore_create (rtems_build_name ('D', 'D', 'E', 'V'),
								 1,
