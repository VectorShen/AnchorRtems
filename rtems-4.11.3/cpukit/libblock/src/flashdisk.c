...
rtems_fdisk_queue_status (rtems_flashdisk * fd,
						rtems_fdisk_segment_ctl * sc, char queues[5])
{
	queues[0] =
		rtems_fdisk_segment_queue_present (&fd->available, sc) ? 'A' : '-';
...
static bool rtems_fdisk_page_desc_erased (const rtems_fdisk_page_desc * pd)
{
	return ((pd->crc == 0xffff) &&
			(pd->flags == 0xffff) && (pd->block == 0xffffffff)) ? true : false;
}
 ...
 * no range checks.
 */
static const rtems_fdisk_segment_desc *rtems_fdisk_seg_descriptor (const
																 rtems_flashdisk
																 * fd,
																 uint32_t
																 device,
 ...
 */
static rtems_fdisk_segment_ctl *rtems_fdisk_seg_most_available (const
																rtems_fdisk_segment_ctl_queue
																* queue)
{
	rtems_fdisk_segment_ctl *sc = queue->head;
 ...
 */
#if 0
static bool rtems_fdisk_seg_pages_all_used (const rtems_fdisk_segment_ctl * sc)
{
...
rtems_fdisk_seg_write_page (rtems_flashdisk * fd,
							rtems_fdisk_segment_ctl * sc,
							uint32_t page, const void *buffer)
{
...
rtems_fdisk_seg_copy_page (rtems_flashdisk * fd,
						 ...
						 uint32_t src_page,
						 rtems_fdisk_segment_ctl * dst_sc, uint32_t dst_page)
{
	int ret;
#if RTEMS_FDISK_TRACE
...
rtems_fdisk_queue_segment (rtems_flashdisk * fd, rtems_fdisk_segment_ctl * sc)
{
	...
	rtems_fdisk_info (fd, " queue-seg:%02d-%03d: p=%d a=%d u=%d b=%d f=%s n=%s",
					...
					sc->pages_bad, sc->failed ? "FAILED" : "no",
					sc->next ? "set" : "null");
#endif
	...
	if (rtems_fdisk_seg_pages_available (sc) == 0)
	{
		if (sc->pages_active)
		{
			...
			if (seg)
				rtems_fdisk_segment_queue_insert_before (&fd->used, seg,
														 sc);
			else
...
rtems_fdisk_recycle_segment (rtems_flashdisk * fd,
							 ...
							 rtems_fdisk_segment_ctl * dsc, uint32_t * pages)
{
	int ret;
	uint32_t spage;
	uint32_t used = 0;
			...
			*dpd = *spd;
			...
			if (dst_pages == 0)
			{
				...
				if (!dsc)
				{
					...
					{
						rtems_fdisk_error
							("recycle: no available dst segment");
						ret = EIO;
					}
	...
	rtems_fdisk_printf (fd, "ssc end: %d-%d: p=%ld, a=%ld, u=%ld",
						ssc->device, ssc->segment, pages, active, used);
#endif
	if (ssc->pages_active != 0)
	{
...
static int rtems_fdisk_compact (rtems_flashdisk * fd)
{
	...
	while (fd->used.head)
	{
		...
		{
			rtems_fdisk_error
				("compacting: no available segments to compact too");
			return EIO;
		}
		...
		rtems_fdisk_printf (fd, " ssc scan: %d-%d: p=%ld, seg=%ld",
							ssc->device, ssc->segment, pages, segments);
#endif
...
static int rtems_fdisk_recover_block_mappings (rtems_flashdisk * fd)
{
	...
	{
		uint32_t segment;
		for (segment = 0; segment < fd->devices[device].segment_count;
			 segment++)
		{
			rtems_fdisk_segment_ctl *sc =
				&fd->devices[device].segments[segment];
...
#if RTEMS_FDISK_TRACE
			rtems_fdisk_info (fd, "recover-block-mappings:%02d-%03d",
							device, segment);
#endif

			sc->pages_desc =
				rtems_fdisk_page_desc_pages (sd, fd->block_size);
			sc->pages =
				rtems_fdisk_pages_in_segment (sd,
											fd->block_size) -
				sc->pages_desc;
			...
			if (!sc->page_descriptors)
				sc->page_descriptors =
					malloc (sc->pages_desc * fd->block_size);
			...
			for (page = 0; page < sc->pages; page++, pd++)
			{
				if (rtems_fdisk_page_desc_erased (pd))
				{
					...
					else
					{
						...
						rtems_fdisk_warning (fd,
											 ...
											 pd->block);
#endif
						rtems_fdisk_page_desc_set_flags (pd,
				...
				{
					if (rtems_fdisk_page_desc_flags_set
						(pd, RTEMS_FDISK_PAGE_USED))
					{
						sc->pages_used++;
					}
					else if (rtems_fdisk_page_desc_flags_set
							 (pd, RTEMS_FDISK_PAGE_ACTIVE))
					{
...
/**
   ...
   * a duplicate.
   */
							const rtems_fdisk_segment_ctl *bsc =
								fd->blocks[pd->block].segment;
							rtems_fdisk_error
								("duplicate block: %d-%d-%d: "
								 "duplicate: %d-%d-%d", bsc->device,
								 bsc->segment,
								 fd->blocks[pd->block].page, device,
								 segment, page);
							sc->pages_bad++;
						}
						else
 ...
 */
static bool
rtems_fdisk_read_block (rtems_flashdisk * fd, uint32_t block, uint8_t * buffer)
{
	...
	rtems_fdisk_info (fd,
					...
					sc->pages, sc->pages_active, sc->pages_used,
					sc->pages_bad, sc->next ? "set" : "null", pd->flags,
					pd->crc, pd->block);
#endif
						 ...
						 uint32_t block, const uint8_t * buffer)
{
	...
	for (page = 0; page < sc->pages; page++, pd++)
	{
		if (rtems_fdisk_page_desc_erased (pd))
		{
			 ...
			 */
			ret =
				rtems_fdisk_seg_write_page (fd, sc, page + sc->pages_desc,
											buffer);
			if (ret)
			{
#if RTEMS_FDISK_TRACE
				rtems_fdisk_info (fd,
									...
									"%s (%d)", sc->device, sc->segment,
									page, strerror (ret), ret);
#endif
			}
			else
			{
...
static int rtems_fdisk_print_status (rtems_flashdisk * fd)
{
	...
	rtems_fdisk_printf (fd,
						"Flash Disk Driver Status : %d.%d", fd->major,
						fd->minor);

	rtems_fdisk_printf (fd, "Block count\t%d", fd->block_count);
	rtems_fdisk_printf (fd, "Unavail blocks\t%d", fd->unavail_blocks);
	rtems_fdisk_printf (fd, "Starvation threshold\t%d",
	...
	for (device = 0; device < fd->device_count; device++)
	{
		...
		{
			rtems_fdisk_segment_ctl *sc =
				&fd->devices[device].segments[seg];
			uint32_t page;
			uint32_t erased = 0;
			uint32_t active = 0;
			uint32_t used = 0;
			...
			{
				if (rtems_fdisk_page_desc_erased
	...
	else
	{
		...
		switch (req)
		{
				...
				else
				{
					switch (r->req)
					{
						case RTEMS_BLKDEV_REQ_READ:
							errno =
								rtems_fdisk_read (&rtems_flashdisks
												[minor], r);
							break;

						case RTEMS_BLKDEV_REQ_WRITE:
							errno =
								rtems_fdisk_write (&rtems_flashdisks
												 [minor], r);
							break;
			...
			case RTEMS_FDISK_IOCTL_MONITORING:
				errno =
					rtems_fdisk_monitoring_data (&rtems_flashdisks[minor],
						...
						void *arg __attribute__ ((unused)))
{
	...
	for (minor = 0; minor < rtems_flashdisk_configuration_size; minor++, c++)
	{
		...
		fd->devices =
			calloc (c->device_count, sizeof (rtems_fdisk_device_ctl));
		if (!fd->devices)
			return RTEMS_NO_MEMORY;
		...
		for (device = 0; device < c->device_count; device++)
		{
			...
			segment_count =
				rtems_fdisk_count_segments (&c->devices[device]);
			...
			for (segment = 0; segment < c->devices[device].segment_count;
				 segment++)
			{
				const rtems_fdisk_segment_desc *sd;
				...
				for (seg_segment = 0; seg_segment < sd->count;
					 seg_segment++, sc++)
				{
					sc->descriptor = sd;
					sc->device = device;
