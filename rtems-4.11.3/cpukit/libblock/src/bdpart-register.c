...
static char *create_logical_disk_name (const char *disk_name, char **marker)
{
	...
	char *logical_disk_name =
		malloc (disk_name_size + RTEMS_BDPART_NUMBER_SIZE);
...
static rtems_status_code update_logical_disk_name (char *logical_disk_marker,
												 size_t i)
{
										 ...
										 size_t count)
{
	...
	logical_disk_name = create_logical_disk_name (disk_name,
												&logical_disk_marker);
	if (logical_disk_name == NULL)
	{
		esc = sc;
	...
	for (i = 0; i < count; ++i)
	{
		...
		/* Create logical disk */
		sc = rtems_blkdev_create_partition (logical_disk_name,
											disk_name,
