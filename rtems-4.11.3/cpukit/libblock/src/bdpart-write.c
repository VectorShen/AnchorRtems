...
static void rtems_bdpart_write_mbr_partition (uint8_t * data,
											uint32_t begin,
											uint32_t size,
...
static rtems_status_code rtems_bdpart_new_record (rtems_disk_device * dd,
									...
									size_t count)
{
	...
	/* Write disk ID */
	rtems_uint32_to_little_endian (format->mbr.disk_id,
								 block->buffer +
	...
	for (i = 0; i < ppc; ++i)
	{
		...
		rtems_bdpart_write_mbr_partition (data,
											p->begin,
											p->end - p->begin,
											rtems_bdpart_mbr_partition_type (p->
																			 type),
											(uint8_t) p->flags);
	...
	if (ppc != count)
	{
		...
		for (i = ppc; i < count; ++i)
		{
			...
			if (i > ppc)
			{
				...
				rtems_bdpart_write_mbr_partition (block->buffer +
