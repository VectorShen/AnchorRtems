...
static void media_server (rtems_task_argument arg __attribute__ ((unused)))
{
	...
	while (true)
	{
		...
		sc = rtems_chain_get_with_wait (&message_chain,
										EVENT,
										RTEMS_NO_TIMEOUT,
		...
		rtems_media_post_event (msg->event,
								msg->src, NULL, msg->worker, msg->worker_arg);
												 ...
												 rtems_attribute attributes)
{
	...
	if (server_id == RTEMS_ID_NONE)
	{
		...
		sc = rtems_task_create (rtems_build_name ('M', 'D', 'I', 'A'),
								priority,
								stack_size, modes, attributes, &server_id);
		if (sc != RTEMS_SUCCESSFUL)
		{
			goto error;
		}
												 ...
												 void *worker_arg)
{
	...
	if (msg != NULL)
	{
		...
		sc = rtems_chain_append_with_notification (&message_chain,
													 &msg->node,
													 server_id, EVENT);
		if (sc != RTEMS_SUCCESSFUL)
		{
