...
static listener_item *find_listener (rtems_media_listener listener,
									 void *listener_arg)
{
	...
	if (sc == RTEMS_SUCCESSFUL)
	{
		...
		if (item == NULL)
		{
			...
			if (item != NULL)
			{
				...
				rtems_chain_append_unprotected (&listener_item_chain,
												&item->node);
			}
			else
			{
...
static rtems_status_code notify (rtems_media_event event,
...
static media_item *get_media_item (const char *disk_path,
								 const char *mount_path)
{
	...
	while (!rtems_chain_is_tail (&media_item_chain, node))
	{
		...
		if ((disk_path == NULL || strcmp (disk_path, item->disk_path) == 0)
			&& (mount_path == NULL
...
static void create_item (media_item * parent,
						 const char *disk_path, const char *mount_path)
{
...
static void remove_partition (const char *partition_path)
{
	...
	if (item != NULL)
	{
		if (item->mount_path != NULL)
		{
			error (RTEMS_MEDIA_ERROR_PARTITION_DETACH_WITH_MOUNT,
				 partition_path, item->mount_path);
		}
		free_item (item);
	}
	else
	...
	if (item != NULL)
	{
		...
		while (!rtems_chain_is_tail (&media_item_chain, node))
		{
			...
			if (child->parent == item)
			{
				...
				{
					error (RTEMS_MEDIA_ERROR_MOUNT_POINT_ORPHAN,
						 child->mount_path, disk_path);
				}
				error (RTEMS_MEDIA_ERROR_PARTITION_ORPHAN,
						 child->disk_path, disk_path);
				free_item (child);
			}
		}
...
static void add_partition (const char *disk_path, const char *partition_path)
{
	...
	if (item != NULL)
	{
		error (RTEMS_MEDIA_ERROR_DISK_OR_PARTITION_EXISTS, partition_path,
				 NULL);
	...
	{
		error (RTEMS_MEDIA_ERROR_PARTITION_WITH_UNKNOWN_DISK,
...
static void add_mount_point (const char *disk_path, const char *mount_path)
{
	...
	if (item != NULL)
	{
		if (item->mount_path != NULL)
		{
			error (RTEMS_MEDIA_ERROR_MOUNT_POINT_EXISTS, item->mount_path,
				 NULL);
			free (item->mount_path);
		}
		item->mount_path = strdup (mount_path);
...
static rtems_status_code remember_event (rtems_media_event event,
										 rtems_media_state state,
										 const char *src, const char *dest)
{
	switch (event)
...
static rtems_status_code process_event (rtems_media_event event,
										const char *src,
										char **dest_ptr,
...
static rtems_status_code mount_worker (rtems_media_state state,
									 const char *src,
									 char **dest, void *worker_arg)
{
	int rv = 0;
	...
	if (state == RTEMS_MEDIA_STATE_READY)
	{
		...
		{
			mount_path =
				rtems_media_replace_prefix (RTEMS_MEDIA_MOUNT_BASE, src);
		}
		else
		{
		...
		rv = mount (src,
					...
					RTEMS_FILESYSTEM_READ_WRITE, &mount_options);
		if (rv != 0)
		{
			rmdir (mount_path);
			free (mount_path);
...
static rtems_status_code do_mount (const char *src,
								 char **dest_ptr,
								 rtems_media_worker worker, void *worker_arg)
{
	if (worker == NULL)
	{
		worker = mount_worker;
	...
	return process_event (RTEMS_MEDIA_EVENT_MOUNT,
						src, dest_ptr, worker, worker_arg);
}

static rtems_status_code do_partition_attach (const char *src,
											char **dest_ptr,
											rtems_media_worker worker,
											void *worker_arg)
{
	...
	if (worker != NULL)
	{
		sc = process_event (RTEMS_MEDIA_EVENT_PARTITION_ATTACH,
							src, &part_path, worker, worker_arg);
...
static rtems_status_code partition_attach_worker (rtems_media_state state,
												const char *src,
												char **dest, void *worker_arg)
{
	rtems_status_code sc = RTEMS_SUCCESSFUL;
	...
	if (state == RTEMS_MEDIA_STATE_READY)
	{
		...
		sc = rtems_blkdev_create_partition (part_path,
											src, part->begin, part->count);
		if (sc != RTEMS_SUCCESSFUL)
		{
			free (part_path);
...
static rtems_status_code attach_and_mount_partitions (const char *disk_path,
													const
													rtems_bdpart_partition *
													partitions, size_t count)
{
	rtems_status_code sc = RTEMS_SUCCESSFUL;
	size_t i = 0;
	...
	for (i = 0; i < count; ++i)
	{
		...
		sc = process_event (RTEMS_MEDIA_EVENT_PARTITION_ATTACH,
							disk_path,
							&part_path, partition_attach_worker, &part_desc);
...
static rtems_status_code partition_inquiry_worker (rtems_media_state state,
												 const char *src,
												 char **dest
...
static rtems_status_code do_partition_inquiry (const char *src,
											 char **dest_ptr,
											 rtems_media_worker worker,
											 void *worker_arg)
{
	...
	if (worker == NULL)
	{
		...
		sc = process_event (RTEMS_MEDIA_EVENT_PARTITION_INQUIRY,
							src, dest_ptr, partition_inquiry_worker, &pt);
...
static rtems_status_code do_disk_attach (const char *src,
										 char **dest_ptr,
										 rtems_media_worker worker,
										 void *worker_arg)
{
	...
	if (worker != NULL)
	{
		rsc = process_event (RTEMS_MEDIA_EVENT_DISK_ATTACH,
							 src, &disk_path, worker, worker_arg);
...
static rtems_status_code unmount_worker (rtems_media_state state,
										 const char *src,
...
static rtems_status_code do_unmount (const char *src,
									 char **dest_ptr,
									 rtems_media_worker worker,
									 void *worker_arg)
{
	...
	return process_event (RTEMS_MEDIA_EVENT_UNMOUNT,
						src, dest_ptr, worker, worker_arg);
}

static rtems_status_code disk_detach_worker (rtems_media_state state,
											 const char *src,
											 char **dest
											 __attribute__ ((unused)),
											 void *worker_arg
...
static rtems_status_code detach_item (rtems_media_event event,
									media_item * item)
{
	rtems_status_code sc = RTEMS_SUCCESSFUL;
	rtems_status_code rsc = RTEMS_SUCCESSFUL;
...
static rtems_status_code do_disk_detach (const char *src,
										 char **dest_ptr,
										 rtems_media_worker worker,
										 void *worker_arg)
{
	...
	return process_event (RTEMS_MEDIA_EVENT_DISK_DETACH,
						src, dest_ptr, worker, worker_arg);
}

static rtems_status_code do_partition_detach (const char *src,
											char **dest_ptr,
											rtems_media_worker worker,
											void *worker_arg)
{
	...
	return process_event (RTEMS_MEDIA_EVENT_PARTITION_DETACH,
						src, dest_ptr, worker, worker_arg);
}
...
rtems_status_code rtems_media_initialize (void)
{
	...
	{
		sc = rtems_semaphore_create (rtems_build_name ('M', 'D', 'I', 'A'),
									 1,
									 RTEMS_LOCAL | RTEMS_PRIORITY
									 | RTEMS_INHERIT_PRIORITY |
									 RTEMS_BINARY_SEMAPHORE, 0, &media_mutex);
		if (sc != RTEMS_SUCCESSFUL)
		{
			sc = RTEMS_NO_MEMORY;
		}
