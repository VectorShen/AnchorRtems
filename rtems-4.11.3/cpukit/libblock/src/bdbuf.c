...
typedef struct rtems_bdbuf_cache
{
	...
	rtems_chain_control swapout_free_workers;
											/**< The work threads for the swapout
                                             * task. */
...
void rtems_bdbuf_show_users (const char *where, rtems_bdbuf_buffer * bd)
{
	...
	printf ("bdbuf:users: %15s: [%" PRIu32 " (%s)] %td:%td = %" PRIu32 " %s\n",
			...
			bd - bdbuf_cache.bds,
			bd->group->users, bd->group->users > 8 ? "<<<<<<<" : "");
}
#else
...
#else
	return rtems_semaphore_create (name,
								 1, RTEMS_BDBUF_CACHE_LOCK_ATTRIBS, 0, lock);
#endif
}

static void rtems_bdbuf_lock_delete (rtems_bdbuf_lock_type * lock)
{
#if defined(RTEMS_BDBUF_USE_PTHREAD)
...
static void rtems_bdbuf_waiter_delete (rtems_bdbuf_waiters * waiter)
{
#if defined(RTEMS_BDBUF_USE_PTHREAD)
 ...
 */
static rtems_bdbuf_buffer *rtems_bdbuf_avl_search (rtems_bdbuf_buffer ** root,
 ...
 */
static int
rtems_bdbuf_avl_insert (rtems_bdbuf_buffer ** root, rtems_bdbuf_buffer * node)
{
...
{
	rtems_bdbuf_unlock (&bdbuf_cache.sync_lock, RTEMS_BDBUF_FATAL_SYNC_UNLOCK);
}
...
#if !defined(RTEMS_BDBUF_USE_PTHREAD)
static rtems_mode rtems_bdbuf_disable_preemption (void)
{
...
static void rtems_bdbuf_restore_preemption (rtems_mode prev_mode)
{
	rtems_status_code sc = RTEMS_SUCCESSFUL;
...
static void rtems_bdbuf_remove_from_tree_and_lru_list (rtems_bdbuf_buffer * bd)
{
	switch (bd->state)
	{
		...
		case RTEMS_BDBUF_STATE_CACHED:
			...
			break;
		default:
			rtems_bdbuf_fatal_with_state (bd->state,
											RTEMS_BDBUF_FATAL_STATE_10);
	}
 ...
 */
static rtems_bdbuf_buffer *rtems_bdbuf_group_realloc (rtems_bdbuf_group * group,
													size_t new_bds_per_group)
{
	...
	for (b = 0, bd = group->bdbuf;
		 b < group->bds_per_group; b++, bd += bufs_per_bd)
...
rtems_bdbuf_setup_empty_buffer (rtems_bdbuf_buffer * bd,
								rtems_disk_device * dd, rtems_blkdev_bnum block)
{
	bd->dd = dd;
	bd->block = block;
	bd->avl.left = NULL;
	...
	while (!rtems_chain_is_tail (&bdbuf_cache.lru, node))
	{
		...
		if (bd->waiters == 0)
		{
			if (bd->group->bds_per_group == dd->bds_per_group)
			{
				...
				empty_bd =
					rtems_bdbuf_group_realloc (bd->group,
											 dd->bds_per_group);
		}
...
static rtems_status_code
rtems_bdbuf_create_task (rtems_name name,
						 rtems_task_priority priority,
						 rtems_task_priority default_priority, rtems_id * id)
{
	rtems_status_code sc;
	...
	sc = rtems_task_create (name,
							...
							RTEMS_LOCAL | RTEMS_NO_FLOATING_POINT, id);
...
static void
rtems_bdbuf_transfer_done (rtems_blkdev_request * req,
...
static rtems_status_code rtems_bdbuf_swapout_workers_create (void)
{
		 ...
		 w++, worker_current += worker_size)
	{
		rtems_bdbuf_swapout_worker *worker =
			(rtems_bdbuf_swapout_worker *) worker_current;

		sc = rtems_bdbuf_create_task (rtems_build_name
										('B', 'D', 'o', 'a' + w),
		...
		if (sc == RTEMS_SUCCESSFUL)
		{
			rtems_bdbuf_swapout_transfer_init (&worker->transfer,
											 worker->id);

			rtems_chain_append_unprotected (&bdbuf_cache.
...
static rtems_status_code rtems_bdbuf_do_init (void)
{
	 ...
	 */
	bdbuf_cache.buffers =
		rtems_cache_aligned_malloc (bdbuf_cache.buffer_min_count *
		...
		switch (bd->state)
		{
			...
			case RTEMS_BDBUF_STATE_EMPTY:
				...
				else
				{
					 ...
					 */
					rtems_bdbuf_anonymous_wait (&bdbuf_cache.
												buffer_waiters);
					return false;
				}
			case RTEMS_BDBUF_STATE_ACCESS_CACHED:
...
static rtems_bdbuf_buffer *rtems_bdbuf_get_buffer_for_access (rtems_disk_device
															* dd,
															rtems_blkdev_bnum
															block)
{
...
rtems_bdbuf_get (rtems_disk_device * dd,
				 rtems_blkdev_bnum block, rtems_bdbuf_buffer ** bd_ptr)
{
	rtems_status_code sc = RTEMS_SUCCESSFUL;
	rtems_bdbuf_buffer *bd = NULL;
...
static void rtems_bdbuf_swapout_write (rtems_bdbuf_swapout_transfer * transfer)
{
	...
	if (!rtems_chain_is_empty (&transfer->bds))
	{
		...
		bool need_continuous_blocks =
			(dd->phys_dev->
			 capabilities & RTEMS_BLKDEV_CAP_MULTISECTOR_CONT) != 0;
		...
		while ((node = rtems_chain_get_unprotected (&transfer->bds)) != NULL)
		{
			...
			if (rtems_bdbuf_tracer)
				printf ("bdbuf:swapout write: bd:%" PRIu32 ", bufnum:%"
						PRIu32 " mode:%s\n", bd->block,
			...
			{
				rtems_chain_prepend_unprotected (&transfer->bds,
												 &bd->link);
				write = true;
			}
			else
			{
				...
				buf =
					&transfer->write_req.bufs[transfer->write_req.bufnum];
				transfer->write_req.bufnum++;
				buf->user = bd;
				buf->block = bd->block;
			...
			if (write)
			{
				rtems_bdbuf_execute_transfer_request (dd,
														&transfer->
														write_req, false);
		...
		{
			/* int result = */ dd->ioctl (dd->phys_dev,
										RTEMS_BLKDEV_REQ_SYNC, NULL);
	...
	if (!rtems_chain_is_empty (chain))
	{
		...
		while (!rtems_chain_is_tail (chain, node))
		{
			...
			if (bd->dd == *dd_ptr)
			{
				...
				{
					rtems_bdbuf_buffer *tbd =
						(rtems_bdbuf_buffer *) tnode;
...
static rtems_task rtems_bdbuf_swapout_worker_task (rtems_task_argument arg)
{
	...
	while (worker->enabled)
	{
		...
		rtems_chain_append_unprotected (&bdbuf_cache.swapout_free_workers,
										&worker->link);
...
static void rtems_bdbuf_swapout_workers_close (void)
{
	...
	{
		rtems_bdbuf_swapout_worker *worker =
			(rtems_bdbuf_swapout_worker *) node;
...
{
	rtems_bdbuf_swapout_transfer *transfer =
		(rtems_bdbuf_swapout_transfer *) arg;
		...
		if (cur->dd == dd)
		{
			switch (cur->state)
			{
				...
				case RTEMS_BDBUF_STATE_CACHED:
					...
					rtems_chain_append_unprotected (purge_list,
													&cur->link);
					break;
				case RTEMS_BDBUF_STATE_TRANSFER:
					rtems_bdbuf_set_state (cur,
	...
	if (block_size > 0)
	{
		...
		if (bds_per_group != 0)
		{
			...
			uint32_t media_blocks_per_block =
				block_size / dd->media_block_size;
			uint32_t one = 1;

			while ((one << block_to_media_block_shift) <
				 media_blocks_per_block)
			{
				++block_to_media_block_shift;
			}

			if ((dd->media_block_size << block_to_media_block_shift) !=
				block_size)
	...
	while (bdbuf_cache.read_ahead_enabled)
	{
		...
		while ((node = rtems_chain_get_unprotected (chain)) != NULL)
		{
			rtems_disk_device *dd =
				RTEMS_CONTAINER_OF (node, rtems_disk_device,
									read_ahead.node);
			...
			{
				rtems_bdbuf_buffer *bd =
					rtems_bdbuf_get_buffer_for_read_ahead (dd,
															 media_block);
				...
				if (bd != NULL)
				{
					...
					rtems_bdbuf_execute_read_request (dd, bd,
													transfer_count);
				}
			}
			else
