 ...
 */
static rtems_sparse_disk *sparse_disk_allocate (const uint32_t media_block_size,
												const rtems_blkdev_bnum
												blocks_with_buffer)
{
												 ...
												 const uint8_t fill_pattern)
{
	...
	sc = rtems_semaphore_create (rtems_build_name ('S', 'P', 'A', 'R'),
								 1,
								 RTEMS_PRIORITY | RTEMS_BINARY_SEMAPHORE |
								 RTEMS_INHERIT_PRIORITY, 0, &sd->mutex);
...
static rtems_sparse_disk_key *sparse_disk_get_new_block (rtems_sparse_disk *
														 sparse_disk,
														 const rtems_blkdev_bnum
														 block)
{
...
static int sparse_disk_read_block (const rtems_sparse_disk * sparse_disk,
...
static int sparse_disk_write_block (rtems_sparse_disk * sparse_disk,
									const rtems_blkdev_bnum block,
 ...
 * Read/write handling
 */
static int sparse_disk_read_write (rtems_sparse_disk * sparse_disk,
								 rtems_blkdev_request * req, const bool read)
{
	int rv = 0;
	uint32_t req_buffer;
	...
	sc = rtems_semaphore_obtain (sparse_disk->mutex, RTEMS_WAIT,
								 RTEMS_NO_TIMEOUT);
	if (sc != RTEMS_SUCCESSFUL)
	{
	...
	if (RTEMS_BLKIO_REQUEST == req)
	{
		...
		switch (r->req)
		{
			...
			case RTEMS_BLKDEV_REQ_WRITE:
				return sparse_disk_read_write (sd, r,
											 r->req ==
...
rtems_status_code rtems_sparse_disk_create_and_register (const char
														 *device_file_name,
														 uint32_t
														 media_block_size,
														 rtems_blkdev_bnum
														 ...
														 uint8_t fill_pattern)
{
	...
	rtems_sparse_disk *sparse_disk = sparse_disk_allocate (media_block_size,
														 blocks_with_buffer);

	if (sparse_disk != NULL)
	{
		sc = rtems_sparse_disk_register (device_file_name,
										 sparse_disk,
										 media_block_size,
										 blocks_with_buffer,
										 media_block_count,
											...
											sparse_disk_delete)
{
	...
	{
		sc = sparse_disk_initialize (sparse_disk,
									 media_block_size,
									 blocks_with_buffer,
									 sparse_disk_delete, fill_pattern);

		if (RTEMS_SUCCESSFUL == sc)
		{
			sc = rtems_blkdev_create (device_file_name,
									media_block_size,
									media_block_count,
									sparse_disk_ioctl, sparse_disk);
		}
	}
	else
	{
