...
bool rtems_bdpart_to_mbr_partition_type (const uuid_t type, uint8_t * mbr_type)
{
	*mbr_type = rtems_bdpart_mbr_partition_type (type);
	...
	return memcmp (type + 1,
				 RTEMS_BDPART_MBR_MASTER_TYPE + 1, sizeof (uuid_t) - 1) == 0;
}

/*
 * FIXME: This code should the deviceio interface and not the bdbug interface.
 */
rtems_status_code rtems_bdpart_get_disk_data (const char *disk_name,
											int *fd_ptr,
...
static rtems_status_code rtems_bdpart_read_mbr_partition (const uint8_t * data,
...
static rtems_status_code rtems_bdpart_read_record (rtems_disk_device * dd,
									 ...
									 size_t * count)
{
	...
	*count = 0;
	...
	format->mbr.disk_id =
		rtems_uint32_from_little_endian (block->buffer +
										 RTEMS_BDPART_MBR_OFFSET_DISK_ID);
	...
	while (ebr != 0)
	{
		...
		sc = rtems_bdpart_read_mbr_partition (block->buffer +
		...
		ebr =
			rtems_bdpart_next_ebr (block->buffer +
									 RTEMS_BDPART_MBR_OFFSET_TABLE_1);
		if (ebr != 0)
		{
