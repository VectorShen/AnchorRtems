...
static ssize_t rtems_blkdev_imfs_read (rtems_libio_t * iop,
...
static ssize_t rtems_blkdev_imfs_write (rtems_libio_t * iop,
...
static int rtems_blkdev_imfs_ioctl (rtems_libio_t * iop,
									uint32_t request, void *buffer)
{
	int rv = 0;
	...
	if (request != RTEMS_BLKIO_REQUEST)
	{
		rtems_blkdev_imfs_context *ctx =
			IMFS_generic_get_context_by_iop (iop);
		rtems_disk_device *dd = &ctx->dd;
...
static int rtems_blkdev_imfs_fstat (const rtems_filesystem_location_info_t *
									loc, struct stat *buf)
{
	rtems_blkdev_imfs_context *ctx = IMFS_generic_get_context_by_location (loc);
	rtems_disk_device *dd = &ctx->dd;
...
static IMFS_jnode_t *rtems_blkdev_imfs_initialize (IMFS_jnode_t * node,
												 void *arg)
{
	rtems_blkdev_imfs_context *ctx;
	rtems_disk_device *dd;
									 ...
									 void *driver_data)
{
	...
	if (ctx != NULL)
	{
		sc = rtems_disk_init_phys (&ctx->dd,
									 media_block_size,
		...
		if (sc == RTEMS_SUCCESSFUL)
		{
			int rv = IMFS_make_generic_node (device,
											 S_IFBLK | S_IRWXU | S_IRWXG |
											 S_IRWXO,
			...
			if (rv == 0)
			{
				...
				if (ctx != NULL)
				{
					...
					if (sc == RTEMS_SUCCESSFUL)
					{
						...
						rv = IMFS_make_generic_node (partition,
													 S_IFBLK | S_IRWXU
													 | S_IRWXG |
													 S_IRWXO,
													 &rtems_blkdev_imfs_control,
