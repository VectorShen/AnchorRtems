 ...
 * @return int The number of bytes written to the output.
 */
static int rtems_nvdisk_info (const rtems_nvdisk * nvd, const char *format, ...)
{
	int ret = 0;
	if (nvd->info_level >= 2)
	{
 ...
 */
static const rtems_nvdisk_device_desc *rtems_nvdisk_device_descriptor (const
																	 rtems_nvdisk
																	 * nvd,
																	 uint32_t
																	 device)
...
rtems_nvdisk_read_page (const rtems_nvdisk * nvd,
						uint32_t device, uint32_t page, void *buffer)
{
...
rtems_nvdisk_write_page (const rtems_nvdisk * nvd,
						 uint32_t device, uint32_t page, const void *buffer)
{
...
rtems_nvdisk_read_checksum (const rtems_nvdisk * nvd,
							uint32_t device, uint32_t page, uint16_t * cs)
{
	return rtems_nvdisk_device_read (nvd, device,
...
rtems_nvdisk_write_checksum (const rtems_nvdisk * nvd,
							 uint32_t device, uint32_t page, const uint16_t cs)
{
	return rtems_nvdisk_device_write (nvd, device,
									page * sizeof (uint16_t),
									&cs, sizeof (uint16_t));
 ...
 */
static rtems_nvdisk_device_ctl *rtems_nvdisk_get_device (rtems_nvdisk * nvd,
														 uint32_t block)
{
	uint32_t device;
 ...
 */
static uint32_t
rtems_nvdisk_get_page (rtems_nvdisk_device_ctl * dc, uint32_t block)
{
...
rtems_nvdisk_read_block (rtems_nvdisk * nvd, uint32_t block, uint8_t * buffer)
{
	...
	ret =
		rtems_nvdisk_read_page (nvd, dc->device, page + dc->pages_desc, buffer);
	...
	{
		rtems_nvdisk_error
			("read-block: crc failure: %d: buffer:%04x page:%04x", block, cs,
			 crc);
		return EIO;
	}
...
rtems_nvdisk_write_block (rtems_nvdisk * nvd,
						uint32_t block, const unsigned char *buffer)
{
	rtems_nvdisk_device_ctl *dc;
	uint32_t page;
	uint16_t cs;
...
#if RTEMS_NVDISK_TRACE
	rtems_nvdisk_info (nvd, " write-block:%d=>%02d-%03d", block, dc->device,
					 page);
#endif

	ret =
		rtems_nvdisk_write_page (nvd, dc->device, page + dc->pages_desc,
								 buffer);
...
static int rtems_nvdisk_erase_disk (rtems_nvdisk * nvd)
{
	...
	for (device = 0; device < nvd->device_count; device++)
	{
		...
		{
			int ret =
				rtems_nvdisk_write_checksum (nvd, dc->device, page, 0xffff);
			if (ret)
				return ret;
		}
	}
 ...
 */
static int rtems_nvdisk_ioctl (rtems_disk_device * dd, uint32_t req, void *argp)
{
			...
			case RTEMS_BLKIO_REQUEST:
				switch (r->req)
				{
					case RTEMS_BLKDEV_REQ_READ:
						errno =
							rtems_nvdisk_read (&rtems_nvdisks[minor], r);
						break;

					case RTEMS_BLKDEV_REQ_WRITE:
						errno =
							rtems_nvdisk_write (&rtems_nvdisks[minor], r);
						break;
						 ...
						 void *arg __attribute__ ((unused)))
{
	...
	for (minor = 0; minor < rtems_nvdisk_configuration_size; minor++, c++)
	{
		...
		nvd->devices =
			calloc (c->device_count, sizeof (rtems_nvdisk_device_ctl));
		if (!nvd->devices)
			return RTEMS_NO_MEMORY;
		...
		for (device = 0; device < c->device_count; device++)
		{
			...
			dc->device = device;
			dc->pages =
				rtems_nvdisk_pages_in_device (nvd, &c->devices[device]);
			dc->pages_desc =
				rtems_nvdisk_page_desc_pages (nvd, &c->devices[device]);
