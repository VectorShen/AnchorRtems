...
get_sector (int fd, uint32_t sector_num, rtems_sector_data_t ** sector)
{
	...
	s = (rtems_sector_data_t *) malloc (sizeof (rtems_sector_data_t) +
...
read_extended_partition (int fd, uint32_t start, rtems_part_desc_t * ext_part)
{
	...
	for (i = 0; i < RTEMS_IDE_PARTITION_MAX_SUB_PARTITION_NUMBER; i++)
	{
		...
		else
		{
			...
			new_part_desc->end =
				new_part_desc->start + new_part_desc->size - 1;
		}
		data += RTEMS_IDE_PARTITION_DESCRIPTOR_SIZE;
	}
...
static rtems_status_code read_mbr (int fd, rtems_disk_desc_t * disk_desc)
{
	...
	for (part_num = 0;
		 part_num < RTEMS_IDE_PARTITION_MAX_SUB_PARTITION_NUMBER; part_num++)
	{
