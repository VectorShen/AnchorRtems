...
static void rtems_bsdnet_initialize_sockaddr_in (struct sockaddr_in *addr)
{
 ...
 */
static int rtems_bsdnet_initialize (void)
{
	...
	rtems_set_udp_buffer_sizes (rtems_bsdnet_config.udp_tx_buf_size,
								rtems_bsdnet_config.udp_rx_buf_size);

	rtems_set_tcp_buffer_sizes (rtems_bsdnet_config.tcp_tx_buf_size,
...
void rtems_bsdnet_semaphore_release (void)
{
	...
	if (!the_networkSemaphore)
		rtems_panic
			("rtems-net: network sema obtain: network not initialised\n");
	_ISR_lock_ISR_disable (&lock_context);
	status = _CORE_mutex_Surrender (&the_networkSemaphore->Core_control.mutex,
									networkSemaphore, NULL, &lock_context);
...
static int rtems_bsdnet_sleep (rtems_event_set in, rtems_interval ticks)
{
	rtems_status_code sc;
	rtems_event_set out;
 ...
 * Task-start stub
 */
static void taskEntry (rtems_task_argument arg)
{
	struct newtask t;
 ...
 */
int rtems_bsdnet_rtrequest (int req,
							struct sockaddr *dst,
							struct sockaddr *gateway,
							struct sockaddr *netmask,
							int flags, struct rtentry **net_nrt)
{
	int error;
...
static int rtems_bsdnet_setup (void)
{
	...
	if (rtems_bsdnet_config.domainname)
		rtems_bsdnet_domain_name = strdup (rtems_bsdnet_config.domainname);
 ...
 */
int
rtems_bsdnet_parse_driver_name (const struct rtems_bsdnet_ifconfig *config,
								char **namep)
{
