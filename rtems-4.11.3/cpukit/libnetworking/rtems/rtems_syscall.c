 ...
 */
ssize_t sendmsg (int s, const struct msghdr * mp, int flags)
{
	int ret = -1;
	int error;
	struct uio auio;
	...
	{
		error =
			sockargstombuf (&to, mp->msg_name, mp->msg_namelen, MT_SONAME);
		if (error)
		{
			errno = error;
			rtems_bsdnet_semaphore_release ();
 ...
 */
ssize_t
sendto (int s, const void *buf, size_t buflen, int flags,
 ...
 */
ssize_t recvmsg (int s, struct msghdr * mp, int flags)
{
	int ret = -1;
	int error;
	struct uio auio;
 ...
 */
ssize_t
recvfrom (int s, void *buf, size_t buflen, int flags,
...
int getsockopt (int s, int level, int name, void *aval, int *avalsize)
{
	struct socket *so;
 ...
 ************************************************************************
 */
static int rtems_bsdnet_close (rtems_libio_t * iop)
{
	struct socket *so;
	int error;
...
static int
so_ioctl (rtems_libio_t * iop, struct socket *so, uint32_t command,
		void *buffer)
{
	switch (command)
	{
...
static int rtems_bsdnet_fcntl (rtems_libio_t * iop, int cmd)
{
	struct socket *so;
...
static int
rtems_bsdnet_fstat (const rtems_filesystem_location_info_t * loc,
					struct stat *sp)
{
	sp->st_mode = S_IFSOCK;
	return 0;
