 ...
 * Display an address
 */
static int showaddress (char *name, struct sockaddr *a)
{
	...
	for (ifp = ifnet; ifp; ifp = ifp->if_next)
	  {
					  ...
					  case AF_INET:
						  {
							  ...
							  if (ifp->if_flags & IFF_BROADCAST)
								  printed |=
									  showaddress ("Broadcast Address",
												   ifa->ifa_broadaddr);
							  if (ifp->if_flags & IFF_POINTOPOINT)
								  printed |=
									  showaddress ("Destination Address",
												   ifa->ifa_dstaddr);
							  printed |=
								  showaddress ("Net mask", ifa->ifa_netmask);
							  if (printed)
								  printf ("\n");
						  }
						  break;
		  ...
		  printf ("Send queue limit:%-4d length:%-4d Dropped:%-8d\n",
				  ...
				  ifp->if_snd.ifq_len, ifp->if_snd.ifq_drops);
