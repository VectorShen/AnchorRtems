...
void rtems_bsdnet_dhcp_failsafe_config (int network_fail_timeout_,
...
static int dhcp_if_down (const char *ifname)
{
	int16_t flags;
...
static int dhcp_if_up (const char *ifname)
{
	int16_t flags;
	if (rtems_bsdnet_ifconfig (ifname, SIOCGIFFLAGS, &flags) < 0)
	  {
...
static int set_static_address (struct rtems_bsdnet_ifconfig *ifp)
{
	int16_t flags;
	struct sockaddr_in address;
