 ...
 * Copy address
 */
static void copyAddress (void *to, void *from, int tolen)
{
	int ncopy;
 ...
 */
static int show_inet_route (struct radix_node *rn, void *vw)
{
 ...
 */
static char *link_ascii (struct sockaddr_dl *sdl, char *buf, int bufsize)
{
	char *cp;
	int i;
	int first = 1;
	...
	for (i = 0, r = d.routes; i < d.count; i++, r++)
	  {
		  ...
		  switch (r->un.sa.sa_family)
			{
				case AF_INET:
					addr =
						inet_ntop (AF_INET, &r->un.sin.sin_addr, buf,
								   sizeof buf);
					break;
		  ...
		  *cp = '\0';
		  printf ("%-10s%3d%9ld%7ld %.*s%d\n", flagbuf,
				  r->refcnt, r->pksent,
				  r->expire, (int)sizeof r->ifname, r->ifname, r->ifunit);
	  }
	free (d.routes);
}
