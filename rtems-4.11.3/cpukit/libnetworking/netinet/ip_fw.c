	...
	if (byname)
	  {
		  ...
		  if (ifu->fu_via_if.unit != -1 && ifp->if_unit != ifu->fu_via_if.unit)
			  return (0);
		  /* Check name */
		  ...
		  switch (rule->fw_reject_code)
			{
				...
				default:		/* Send an ICMP unreachable using code */
					icmp_error (*m, ICMP_UNREACH, rule->fw_reject_code, 0L, 0);
					*m = NULL;
					break;
			}
	  }
...
static struct ip_fw *check_ipfw_mbuf (struct mbuf *m)
{
	/* Check length */
	if (m->m_len != sizeof (struct ip_fw))
	  {
...
static struct ip_fw *check_ipfw_struct (struct ip_fw *frwl)
{
	...
	else if ((frwl->fw_flg & IP_FW_F_IN) && (frwl->fw_flg & IP_FW_F_OIFACE))
	  {
	...
	if ((frwl->fw_flg & IP_FW_F_FRAG) &&
		(frwl->fw_prot == IPPROTO_UDP || frwl->fw_prot == IPPROTO_TCP))
	  {
		  ...
		  if (frwl->fw_prot == IPPROTO_TCP && frwl->fw_tcpf != frwl->fw_tcpnf)
			{
...
static int ipfw_load (struct lkm_table *lkmtp, int cmd)
{
	int s = splnet ();
...
static int ipfw_unload (struct lkm_table *lkmtp, int cmd)
{
	int s = splnet ();
...
int ipfw_mod (struct lkm_table *lkmtp, int cmd, int ver)
{
