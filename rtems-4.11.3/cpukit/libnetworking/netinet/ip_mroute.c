...
extern int _ip_mrouter_get (int cmd, struct socket *so, struct mbuf **m);
extern int _ip_mrouter_set (int cmd, struct socket *so, struct mbuf *m);
...
static int X_ip_mforward (struct ip *ip, struct ifnet *ifp, struct mbuf *m,
						  struct ip_moptions *imo);
...
static int X_ip_mrouter_set (int cmd, struct socket *so, struct mbuf *m)
{
	...
	switch (cmd)
	  {
		  case MRT_INIT:
			  return ip_mrouter_init (so, m);
		  case MRT_DONE:
			  return ip_mrouter_done ();
		  case MRT_ADD_VIF:
			  return add_vif (mtod (m, struct vifctl *));
		  case MRT_DEL_VIF:
			  return del_vif (mtod (m, vifi_t *));
		  case MRT_ADD_MFC:
			  return add_mfc (mtod (m, struct mfcctl *));
		  case MRT_DEL_MFC:
			  return del_mfc (mtod (m, struct mfcctl *));
		  case MRT_ASSERT:
			  return set_assert (mtod (m, int *));
		  default:
			  return EOPNOTSUPP;
	  }
}
 ...
 */
static int get_assert (struct mbuf *m)
{
	int *i;
	...
	if (rt)
	  {
		  if (mrtdebug & DEBUG_MFC)
			  log (LOG_DEBUG, "add_mfc update o %x g %x p %x\n",
				   ...
				   ntohl (mfccp->mfcc_mcastgrp.s_addr), mfccp->mfcc_parent);
		  ...
		  for (mb_rt = mfctable[hash]; mb_rt; mb_rt = mb_rt->m_next)
			{
				...
				if ((rt->mfc_origin.s_addr == mfccp->mfcc_origin.s_addr) &&
					(rt->mfc_mcastgrp.s_addr == mfccp->mfcc_mcastgrp.s_addr))
				  {
					  ...
					  rt->mfc_byte_cnt = 0;
					  rt->mfc_wrong_if = 0;
					  rt->mfc_last_assert.tv_sec = rt->mfc_last_assert.tv_usec =
						  0;
					  if (rt->mfc_expire)
						  nexpire[hash]--;
					  rt->mfc_expire = 0;
	...
	if (s)
	  {
		  if (sbappendaddr (&s->so_rcv,
							(struct sockaddr *)src, mm, (struct mbuf *)0) != 0)
			{
				sorwakeup (s);
				return 0;
			}
	...
	else
	  {
		  ...
		  if (mb_rt == NULL)
		  ...
		  else
			{
				...
				for (rte_m = mb_rt, npkts = 0; rte_m->m_act;
					 rte_m = rte_m->m_act)
					npkts++;
 ...
 */
static void expire_upcalls (void *unused)
{
		  ...
		  for (mb_rt = *nptr; mb_rt != NULL; mb_rt = *nptr)
			{
				 ...
				 */
				if (mb_rt->m_act != NULL &&
					mfc->mfc_expire != 0 && --mfc->mfc_expire == 0)
				  {
...
static void phyint_send (struct ip *ip, struct vif *vifp, struct mbuf *m)
{
...
static void encap_send (struct ip *ip, struct vif *vifp, struct mbuf *m)
{
	register struct mbuf *mb_copy;
...
#endif
{
	 ...
	 */
	if (!IN_MULTICAST
		(ntohl (((struct ip *)((char *)ip + hlen))->ip_dst.s_addr)))
	  {
 ...
 */
static void tbf_queue (struct vif *vifp, struct mbuf *m)
{
...
static void tbf_reprocess_q (void *xvifp)
{
 ...
 */
static int tbf_dq_sel (struct vif *vifp, struct ip *ip)
{
	register int s = splnet ();
	register u_int p;
...
static void tbf_send_packet (struct vif *vifp, struct mbuf *m)
{
	struct ip_moptions imo;
	int error;
	...
	if (vifp->v_flags & VIFF_TUNNEL)
	...
	else
	  {
		   ...
		   */
		  error = ip_output (m, (struct mbuf *)0, &ro, IP_FORWARDING, &imo);
...
static int priority (struct vif *vifp, struct ip *ip)
{
	register int prio;
...
MOD_MISC ("ip_mroute_mod")
	 static int ip_mroute_mod_handle (struct lkm_table *lkmtp, int cmd)
{
	int i;
...
int ip_mroute_mod (struct lkm_table *lkmtp, int cmd, int ver)
{
