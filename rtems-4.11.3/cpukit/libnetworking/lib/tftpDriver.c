 ...
 */
static void releaseStream (tftpfs_info_t * fs, int s)
{
 ...
 * Map error message
 */
static int tftpErrno (struct tftpStream *tp)
{
 ...
 */
static int getPacket (struct tftpStream *tp, int retryCount)
{
	int len;
	struct timeval tv;
	...
	for (;;)
	{
		...
		len = recvfrom (tp->socket, &tp->pkbuf,
						sizeof tp->pkbuf, 0, &from.s, &fromlen);
		if (len < 0)
			break;
 ...
 */
static int sendAck (struct tftpStream *tp)
{
#ifdef RTEMS_TFTP_DRIVER_DEBUG
	if (rtems_tftp_driver_debug)
		printf ("TFTP: ACK %d\n", tp->blocknum);
	...
	if (sendto (tp->socket, (char *)&tp->pkbuf, sizeof tp->pkbuf.tftpACK, 0,
				(struct sockaddr *)&tp->farAddress, sizeof tp->farAddress) < 0)
		return errno;
	return 0;
}
 ...
 */
static int rtems_tftp_open_worker (rtems_libio_t * iop,
								 char *full_path_name, int oflag)
{
	tftpfs_info_t *fs;
	...
	if (s == fs->nStreams)
	{
		...
		np = realloc (fs->tftpStreams,
						++fs->nStreams * sizeof *fs->tftpStreams);
		if (np == NULL)
		{
			rtems_semaphore_release (fs->tftp_mutex);
			return ENOMEM;
	...
	for (;;)
	{
		...
		tp->myAddress.sin_port =
			htons (UDP_PORT_BASE + fs->nStreams * try + s);
		tp->myAddress.sin_addr.s_addr = htonl (INADDR_ANY);
		if (bind
			(tp->socket, (struct sockaddr *)&tp->myAddress,
			 sizeof tp->myAddress) >= 0)
			break;
		if (++retryCount == 10)
		{
			releaseStream (fs, s);
...
static int rtems_tftp_open (rtems_libio_t * iop,
							const char *new_name, int oflag, mode_t mode)
{
	tftpfs_info_t *fs;
	char *full_path_name;
	int err;
 ...
 */
static ssize_t rtems_tftp_read (rtems_libio_t * iop, void *buffer, size_t count)
{
	char *bp;
...
static ssize_t rtems_tftp_write (rtems_libio_t * iop,
 ...
 */
static int rtems_tftp_ftruncate (rtems_libio_t * iop __attribute__ ((unused)),
								 off_t count __attribute__ ((unused)))
{
	return 0;
}

static int rtems_tftp_fstat (const rtems_filesystem_location_info_t * loc,
							 struct stat *buf)
{
...
static void rtems_tftp_free_node_info (const rtems_filesystem_location_info_t *
									 loc)
{
	free (loc->node_access);
}

static bool rtems_tftp_are_nodes_equal (const rtems_filesystem_location_info_t *
										a,
