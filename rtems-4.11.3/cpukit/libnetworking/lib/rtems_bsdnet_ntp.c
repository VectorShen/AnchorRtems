...
static int processPacket (struct ntpPacketSmall *p, int state, void *unused)
{
	time_t tbuf;
	struct tm *lt;
	...
	tbuf =
		ntohl (p->transmit_timestamp.integer) - UNIX_BASE_TO_NTP_BASE -
		rtems_bsdnet_timeoffset;
	lt = gmtime (&tbuf);
	...
	rt.second = lt->tm_sec;
	rt.ticks =
		ntohl (p->transmit_timestamp.fraction) / (ULONG_MAX / ticks_per_second);
