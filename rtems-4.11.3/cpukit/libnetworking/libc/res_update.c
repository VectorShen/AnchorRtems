...
int res_update (ns_updrec * rrecp_in)
{
	...
	char name[MAXDNAME], zname[MAXDNAME], primary[MAXDNAME], mailaddr[MAXDNAME];
		...
		while (!done && dname)
		{
			if (qtype == T_SOA)
			{
				for (tmpzptr = zgrp_start;
					 tmpzptr && !seen_before; tmpzptr = tmpzptr->z_next)
				{
					if (strcasecmp (dname,
									tmpzptr->z_origin) == 0 &&
			...
			{
				for (tmpzptr = zgrp_start;
					 tmpzptr && !done; tmpzptr = tmpzptr->z_next)
			...
			if (qtype == T_SOA)
			{
				...
				else if (rcode == NOERROR && ancount == 1)
				{
					...
					if (strcasecmp (dname, zname) != 0 ||
						type != T_SOA || class != rrecp->r_class)
					{
						fprintf (stderr, "unknown answer\n");
						return (-1);
					}
			...
			for (i = 0; i < arcount; i++)
			{
				...
				if (type == T_A && dlen == INT32SZ && class == qclass)
				{
					for (j = 0; j < zptr->z_nscount; j++)
						if (strcasecmp (name, zptr->z_ns[j].nsname) ==
							0)
						{
	...
	for (zptr = zgrp_start; zptr; zptr = zptr->z_next)
	{
		...
		for (i = 0; i < zptr->z_nscount; i++)
		{
			if (strcasecmp (zptr->z_ns[i].nsname, zptr->z_soardata) == 0)
			{
				...
				if (i != 0)
				{
					strcpy (zptr->z_ns[i].nsname, zptr->z_ns[0].nsname);
		...
		_res.nscount = (zptr->z_nscount < MAXNS) ? zptr->z_nscount : MAXNS;
