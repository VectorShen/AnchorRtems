...
*/
extern int drvmgr_keys_get (struct drvmgr_dev *dev,
...
*/
extern struct drvmgr_key *drvmgr_key_get (struct drvmgr_key *keys,
										  char *key_name);
...
*                    Type.
*/
extern union drvmgr_key_value *drvmgr_key_val_get (struct drvmgr_key *keys,
												   char *key_name,
...
*/
extern union drvmgr_key_value *drvmgr_dev_key_get (struct drvmgr_dev *dev,
												   char *key_name,
												   enum drvmgr_kt key_type);
...
/*! Get parent bus */
RTEMS_INLINE_ROUTINE struct drvmgr_bus *drvmgr_get_parent (struct drvmgr_dev
														   *dev)
{
	if (dev)
		return dev->parent;
...
RTEMS_INLINE_ROUTINE struct drvmgr_drv *drvmgr_get_drv (struct drvmgr_dev
														*dev)
{
	if (dev)
		return dev->drv;
...
extern int
	drvmgr_for_each_dev (int (*func) (struct drvmgr_dev * dev, void *arg),
...
*/
extern int drvmgr_get_dev (struct drvmgr_drv *drv,
						   int minor, struct drvmgr_dev **pdev);
...
*/
extern int drvmgr_freq_get (struct drvmgr_dev *dev,
...
*/
extern int drvmgr_interrupt_register (struct drvmgr_dev *dev,
									  int index,
									  const char *info,
									  drvmgr_isr isr, void *arg);
...
*/
extern int drvmgr_interrupt_unregister (struct drvmgr_dev *dev,
										int index,
										drvmgr_isr isr, void *arg);
...
*/
extern unsigned int drvmgr_translate (struct drvmgr_dev *dev,
...
*  and src_address is stored in *dst_address.
*/
extern unsigned int drvmgr_translate_bus (struct drvmgr_bus *from,
...
*/
extern int drvmgr_translate_check (struct drvmgr_dev *dev,
								   unsigned int options,
								   void *src_address,
								   void **dst_address, unsigned int size);
...
extern int drvmgr_func_call (void *obj, int funcid, void *a, void *b,
							 void *c, void *d);
...
typedef int (*drvmgr_rmem_arg) (void *dest, const void *src, int n,
								void *a);
typedef int (*drvmgr_wmem_arg) (void *dest, const void *src, int n,
								void *a);
typedef int (*drvmgr_memset_arg) (void *dstadr, int c, size_t n, void *a);

/* Report an error to the parent bus of the device */
typedef void (*drvmgr_rw_err) (struct drvmgr_rw_arg * a,
							   struct drvmgr_bus * bus, int funcid,
							   void *adr);

/* Helper function for buses that implement the memset() over wmem() */
extern void drvmgr_rw_memset (void *dstadr,
							  int c,
							  size_t n, void *a, drvmgr_wmem_arg wmem);
...
*
*/
extern int drvmgr_for_each_listdev (struct drvmgr_list *devlist,
									unsigned int state_set_mask,
									unsigned int state_clr_mask,
									int (*func) (struct drvmgr_dev * dev,
												 void *arg), void *arg);
