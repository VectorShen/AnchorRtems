 ...
 */
static void drvmgr_insert_dev_into_drv (struct drvmgr_drv *drv,
										struct drvmgr_dev *dev)
{
 ...
 * The minor number is found by looking for a gap or at the end.
 */
static void drvmgr_insert_dev_into_bus (struct drvmgr_bus *bus,
										struct drvmgr_dev *dev)
{
	struct drvmgr_dev *curr, **pprevnext;
	int minor;
	...
	else
	{
		...
		else
		{
			...
			DBG ("Registered %s (DRV: %s) on %s\n",
				 dev->name, drv->name, bus ? bus->dev->name : "NO PARENT");
		}
	}
