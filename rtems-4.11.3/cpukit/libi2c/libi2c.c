...
static struct i2cbus
{
	...
	char *name;
} busses[MAX_NO_BUSSES] =
{
	{
NULL, RTEMS_ID_NONE, false, NULL}};
...
{
	int rval;
	DECL_CHECKED_BH (busno, bush, minor, +)lock_bus (busno);
...
int rtems_libi2c_ioctl (rtems_device_minor_number minor, int cmd, ...)
{
	...
	DECL_CHECKED_BH (busno, bush, minor, -)va_start (ap, cmd);
	args = va_arg (ap, void *);
...
do_s_rw (rtems_device_minor_number minor,
		 unsigned char *bytes, int nbytes, int rw)
{
	rtems_status_code sc;
	rtems_libi2c_bus_t *bush;
	int status;
