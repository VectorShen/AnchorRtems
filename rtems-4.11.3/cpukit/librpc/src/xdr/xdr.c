...
static char *rcsid =
	"$FreeBSD: src/lib/libc/xdr/xdr.c,v 1.9 1999/08/28 00:02:55 peter Exp $";
#endif
 ...
 */
bool_t xdr_union (XDR * xdrs, enum_t * dscmp,	/* enum to decide which arm to work on */
	...
	switch (xdrs->x_op)
	  {
		  case XDR_ENCODE:
			  return (xdr_opaque
					  (xdrs, (caddr_t) uint64_p, sizeof (u_int64_t)));
		  case XDR_DECODE:
			  if (!xdr_opaque (xdrs, (caddr_t) & x, sizeof x))
				{
					return (FALSE);
