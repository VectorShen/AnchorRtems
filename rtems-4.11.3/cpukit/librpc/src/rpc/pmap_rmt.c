...
static char *rcsid =
	"$FreeBSD: src/lib/libc/rpc/pmap_rmt.c,v 1.15 2000/01/27 23:06:40 jasone Exp $";
#endif
...
pmap_rmtcall (struct sockaddr_in *addr,
			...
			caddr_t argsp,
			xdrproc_t xdrres,
			caddr_t resp, struct timeval tout, u_long * port_ptr)
{
	int sock = -1;
	CLIENT *client;
...
static int getbroadcastnets (struct in_addr *addrs, int sock,	/* any valid socket will do */
							 char *buf
...
#else /* 4.2 BSD */
			addr = inet_makeaddr (inet_netof (sin->sin_addr), INADDR_ANY);
#endif
			for (n = i - 1; n >= 0; n--)
			{
