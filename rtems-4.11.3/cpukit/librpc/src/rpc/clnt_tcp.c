 ...
 */
CLIENT *clnttcp_create (struct sockaddr_in *raddr, u_long prog,	/* program number */
 ...
 */
static int readtcp (char *_ct, char *buf, int len)
{
...
static int writetcp (char *_ct, char *buf, int len)
{
	struct ct_data *ct = (struct ct_data *)_ct;
	int i, cnt;
