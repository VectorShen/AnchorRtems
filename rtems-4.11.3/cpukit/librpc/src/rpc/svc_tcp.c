...
static char *rcsid =
	"$FreeBSD: src/lib/libc/rpc/svc_tcp.c,v 1.18 2000/01/27 23:06:41 jasone Exp $";
#endif
 ...
 */
static int readtcp (char *_xprt, char *buf, int len)
{
 ...
 * Any error is fatal and the connection is closed.
 */
static int writetcp (char *_xprt, char *buf, int len)
{
	SVCXPRT *xprt = (SVCXPRT *) _xprt;
	register int i, cnt;
