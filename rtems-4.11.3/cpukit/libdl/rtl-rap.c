...
#define SYMNAME_BUFFER_SIZE (1024)
	...
	if (!symname_buffer)
	{
		rtems_rtl_set_error (ENOMEM,
							 "no memory for local symbol name buffer");
		return false;
	}
	...
	for (section = 0; section < RTEMS_RTL_RAP_SECS; ++section)
	{
		...
		targetsect =
			rtems_rtl_obj_find_section (obj, rap_sections[section].name);
		...
		for (r = 0; r < relocs; ++r)
		{
			...
			if ((info & (1 << 31)) == 0)
			{
				...
				symsect =
					rtems_rtl_obj_find_section_by_index (obj, info >> 8);
				if (!symsect)
				{
					free (symname_buffer);
					return false;
				...
				else
				{
					if (symname_size > (SYMNAME_BUFFER_SIZE - 1))
					{
						...
						rtems_rtl_set_error (EINVAL,
											 "reloc symbol too big");
						return false;
					}

					if (!rtems_rtl_obj_comp_read
						(rap->decomp, symname_buffer, symname_size))
					{
						free (symname_buffer);
						return false;
					}
...
rtems_rtl_rap_load_linkmap (rtems_rtl_rap_t * rap, rtems_rtl_obj_t * obj)
{
	...
	if (!rtems_rtl_obj_comp_read (rap->decomp,
								rap->strtable, rap->strtable_size))
	{
...
rtems_rtl_rap_load_symbols (rtems_rtl_rap_t * rap, rtems_rtl_obj_t * obj)
{
	...
	for (sym = 0, gsym = obj->global_table; sym < rap->symbols; ++sym)
	{
		...
		if (rtems_rtl_trace (RTEMS_RTL_TRACE_SYMBOL))
			printf
				("rtl: sym:load: data=0x%08lx name=0x%08lx value=0x%08lx\n",
				 data, name, value);
		...
		if (!symsect)
		{
			...
			obj->global_syms = 0;
			obj->global_size = 0;
			rtems_rtl_set_error (EINVAL, "section index not found: %lu",
								 data >> 16);
			return false;
		}
		...
		if (rtems_rtl_trace (RTEMS_RTL_TRACE_SYMBOL))
			printf
				("rtl: sym:add:%-2d name:%-20s bind:%-2d type:%-2d val:%8p sect:%d\n",
				 ...
				 (int)ELF_ST_TYPE (data & 0xffff), gsym->value,
				 (int)(data >> 16));
...
rtems_rtl_rap_parse_header (uint8_t * rhdr,
							...
							uint32_t * length,
							uint32_t * version,
							uint32_t * compression, uint32_t * checksum)
{
	char *sptr = (char *)rhdr;
	char *eptr;
	...
	*version = strtoul (sptr, &eptr, 10);
	...
	if ((sptr[0] == 'N') &&
		(sptr[1] == 'O') && (sptr[2] == 'N') && (sptr[3] == 'E'))
	{
		*compression = RTEMS_RTL_COMP_NONE;
		eptr = sptr + 4;
	}
	else if ((sptr[0] == 'L') &&
			 (sptr[1] == 'Z') && (sptr[2] == '7') && (sptr[3] == '7'))
	{
...
bool rtems_rtl_rap_file_load (rtems_rtl_obj_t * obj, int fd)
{
	...
	for (section = 0; section < RTEMS_RTL_RAP_SECS; ++section)
	{
		...
		if (!rtems_rtl_rap_read_uint32
			(rap.decomp, &rap.secs[section].alignment))
			return false;

		if (rtems_rtl_trace (RTEMS_RTL_TRACE_LOAD_SECT))
			printf ("rtl: rap: %s: size=%lu align=%lu\n",
					rap_sections[section].name,
					rap.secs[section].size, rap.secs[section].alignment);
		...
		if (!rtems_rtl_obj_add_section (obj,
										...
										0, 0, rap_sections[section].flags))
			return false;
	}
