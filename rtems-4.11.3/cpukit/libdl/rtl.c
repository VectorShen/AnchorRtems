...
rtems_rtl_obj_comp (rtems_rtl_obj_comp_t ** decomp,
					...
					int fd, int compression, off_t offset)
{
	if (!rtl)
	{
		*decomp = NULL;
...
rtems_rtl_data_t *rtems_rtl_lock (void)
{
	...
	sc = rtems_semaphore_obtain (rtl->lock, RTEMS_WAIT, RTEMS_NO_TIMEOUT);
	if (sc != RTEMS_SUCCESSFUL)
	{
		errno = EINVAL;
		return NULL;
...
void
rtems_rtl_base_sym_global_add (const unsigned char *esyms, unsigned int size)
{
