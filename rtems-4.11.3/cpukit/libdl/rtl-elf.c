...
static bool rtems_rtl_elf_machine_check (Elf_Ehdr * ehdr)
{
	...
	{
		ELFDEFNNAME (MACHDEP_ID_CASES) default:
			return false;
	}
	return true;
}
...
rtems_rtl_elf_find_symbol (rtems_rtl_obj_t * obj,
						 const Elf_Sym * sym,
						 const char *symname, Elf_Word * value)
{
	...
	if (ELF_ST_TYPE (sym->st_info) == STT_NOTYPE)
	{
		...
		rtems_rtl_obj_sym_t *symbol =
			rtems_rtl_symbol_obj_find (obj, symname);
		if (!symbol)
		{
			rtems_rtl_set_error (EINVAL, "global symbol not found: %s",
								 symname);
			return false;
		}
...
rtems_rtl_elf_relocator (rtems_rtl_obj_t * obj,
						 int fd, rtems_rtl_obj_sect_t * sect, void *data)
{
	...
	for (reloc = 0; reloc < (sect->size / reloc_size); ++reloc)
	{
		...
		if (relocate)
		{
			if (is_rela)
			{
				if (rtems_rtl_trace (RTEMS_RTL_TRACE_RELOC))
					printf
						("rtl: rela: sym:%s(%d)=%08lx type:%d off:%08lx addend:%d\n",
						 symname, (int)ELF_R_SYM (rela->r_info), symvalue,
						 (int)ELF_R_TYPE (rela->r_info), rela->r_offset,
						 (int)rela->r_addend);
				if (!rtems_rtl_elf_relocate_rela
					(obj, rela, targetsect, symname, sym.st_info,
					 symvalue))
					return false;
			}
			else
			{
				if (rtems_rtl_trace (RTEMS_RTL_TRACE_RELOC))
					printf
						("rtl: rel: sym:%s(%d)=%08lx type:%d off:%08lx\n",
						 symname, (int)ELF_R_SYM (rel->r_info), symvalue,
						 (int)ELF_R_TYPE (rel->r_info), rel->r_offset);
				if (!rtems_rtl_elf_relocate_rel
					(obj, rel, targetsect, symname, sym.st_info,
					 symvalue))
					return false;
			}
		}
	...
	if (is_rela)
	{
		...
		if (rtems_rtl_trace (RTEMS_RTL_TRACE_RELOC))
			printf ("rtl: rela: sym:%d type:%d off:%08lx addend:%d\n",
					...
					(int)rela.r_addend);
		if (!rtems_rtl_elf_relocate_rela
			(reloc->obj, &rela, sect, sym->name, sym->data, symvalue))
			return false;
	}
	else
	{
					 ...
					 int fd, rtems_rtl_obj_sect_t * sect, void *data)
{
	...
	for (sym = 0; sym < (sect->size / sizeof (Elf_Sym)); ++sym)
	{
		...
		if (!rtems_rtl_obj_cache_read
			(strings, fd, off, (void **)&name, &len))
			return false;
			 ...
			 (ELF_ST_TYPE (symbol.st_info) == STT_NOTYPE)))
		{
			...
			symsect =
				rtems_rtl_obj_find_section_by_index (obj, symbol.st_shndx);
			if (symsect)
			{
	...
	{
		obj->local_size =
			locals * sizeof (rtems_rtl_obj_sym_t) + local_string_space;
		obj->local_table =
			rtems_rtl_alloc_new (RTEMS_RTL_ALLOC_SYMBOL, obj->local_size,
								 true);
		if (!obj->local_table)
		{
			obj->local_size = 0;
	...
	{
		obj->global_size =
			globals * sizeof (rtems_rtl_obj_sym_t) + global_string_space;
		obj->global_table =
			rtems_rtl_alloc_new (RTEMS_RTL_ALLOC_SYMBOL, obj->global_size,
								 true);
		if (!obj->global_table)
		{
			if (locals)
			{
				rtems_rtl_alloc_del (RTEMS_RTL_ALLOC_SYMBOL,
									 obj->local_table);
				obj->local_size = 0;
				obj->local_syms = 0;
			}
...
rtems_rtl_elf_parse_sections (rtems_rtl_obj_t * obj, int fd, Elf_Ehdr * ehdr)
{
	...
	for (section = 0; section < ehdr->e_shnum; ++section)
	{
		...
		if (!rtems_rtl_obj_cache_read_byval
			(sects, fd, off, &shdr, sizeof (shdr)))
			return false;
...
static bool rtems_rtl_elf_load_linkmap (rtems_rtl_obj_t * obj)
{
	...
	for (mask = RTEMS_RTL_OBJ_SECT_TEXT;
		 mask <= RTEMS_RTL_OBJ_SECT_BSS; mask <<= 1)
	{
