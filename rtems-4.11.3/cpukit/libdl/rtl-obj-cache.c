...
rtems_rtl_obj_cache_read (rtems_rtl_obj_cache_t * cache,
						int fd, off_t offset, void **buffer, size_t * length)
{
	struct stat sb;

	if (rtems_rtl_trace (RTEMS_RTL_TRACE_CACHE))
		printf
			("rtl: cache: %2d: fd=%d offset=%d length=%d area=[%d,%d] cache=[%d,%d] size=%d\n",
			 fd, cache->fd, (int)offset, (int)*length, (int)offset,
			 (int)offset + *length, (int)cache->offset,
			 (int)cache->offset + cache->level, (int)cache->file_size);
	...
	if (cache->fd == fd)
	{
		if (offset > cache->file_size)
		{
			rtems_rtl_set_error (EINVAL,
								 "offset past end of file: offset=%i size=%i",
		...
		if (fd == cache->fd)
		{
				...
				(offset < (cache->offset + cache->level)))
			{
				...
				*buffer = cache->buffer + buffer_offset;
				 ...
				 */
				memmove (cache->buffer, cache->buffer + buffer_offset,
						 size);
				 ...
				 * Do not read past the end of the file.
				 */
				if ((offset + buffer_offset + buffer_read) >
					cache->file_size)
					buffer_read =
						cache->file_size - (offset + buffer_offset);
			}
		}

		if (rtems_rtl_trace (RTEMS_RTL_TRACE_CACHE))
			printf
				("rtl: cache: %2d: seek: offset=%d buffer_offset=%d read=%d cache=[%d,%d] dist=%d\n",
				 fd, (int)offset + buffer_offset, (int)buffer_offset,
				 (int)buffer_read, (int)offset, (int)offset + buffer_read,
