...
static bool rtems_rap_data_init (void)
{
	/*
	...
	if (!rap_.lock)
	{
		...
		if (!rap_.lock)
		{
			...
			sc = rtems_semaphore_obtain (lock, RTEMS_WAIT,
										 RTEMS_NO_TIMEOUT);
			if (sc != RTEMS_SUCCESSFUL)
			{
...
static rtems_rap_data_t *rtems_rap_lock (void)
{
	rtems_status_code sc;
	...
	sc = rtems_semaphore_obtain (rap_.lock, RTEMS_WAIT, RTEMS_NO_TIMEOUT);
	if (sc != RTEMS_SUCCESSFUL)
	{
		errno = EINVAL;
		return NULL;
...
static rtems_rap_app_t *rtems_rap_check_handle (void *handle)
{
	rtems_rap_app_t *app;
...
static rtems_rap_app_t *rtems_rap_app_alloc (void)
{
...
static void rtems_rap_app_free (rtems_rap_app_t * app)
{
	if (app->handle)
	{
		dlclose (app->handle);
...
static bool rtems_rap_match_name (rtems_rap_app_t * app, const char *name)
{
	const char *a;
