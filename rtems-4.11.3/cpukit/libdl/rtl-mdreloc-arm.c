...
uint32_t
rtems_rtl_elf_section_flags (const rtems_rtl_obj_t * obj, const Elf_Shdr * shdr)
{
	uint32_t flags = 0;
	...
	switch (ELF_R_TYPE (rel->r_info))
	{
		...
		case R_TYPE (JUMP24):	/* B/BL<cond> */
			...
			if (isThumb (symvalue))
			{
				...
				else
				{
					...
					{
						printf
							("JUMP24 is not suppored from arm to thumb\n");
						return false;
					}
				}
			}
		...
		case R_TYPE (MOVW_ABS_NC):
			...
			else
			{
				...
				if (((Elf_Sword) tmp > 0x7fff)
					|| ((Elf_Sword) tmp < -0x8000))
				{
			...
			*where =
				(insn & 0xfff0f000) | ((tmp & 0xf000) << 4) | (tmp & 0xfff);
		...
		case R_TYPE (TARGET2):	/* Equivalent to REL32 */
			...
			if (rtems_rtl_trace (RTEMS_RTL_TRACE_RELOC))
				printf
					("rtl: REL32/ABS32/GLOB_DAT/PREL31/TARGET2 %p @ %p in %s\n",
		...
		case R_TYPE (THM_MOVW_ABS_NC):
			...
			addend =
				((upper_insn & 0x000f) << 12) | ((upper_insn & 0x0400) << 1) |
		...
		case R_TYPE (THM_CALL):
			...
			tmp =
				(i1 << 23) | (i2 << 22) | ((upper_insn & 0x3ff) << 12) |
				((lower_insn & 0x7ff) << 1);
			...
			sign = (tmp >> 24) & 1;
			*(uint16_t *) where =
				(uint16_t) ((upper_insn & 0xf800) | (sign << 10) |
							((tmp >> 12) & 0x3ff));
		...
		case R_TYPE (THM_JUMP19):
			...
			tmp =
				((i2 << 19) | (i1 << 18) | ((upper_insn & 0x3f) << 12) |
				 ((lower_insn & 0x7ff) << 1));
			...
			*((uint16_t *) where + 1) =
				(lower_insn & 0xd000) | (i1 << 13) | (i2 << 11) | ((tmp >> 1)
																	 & 0x7ff);
		...
		default:
			...
			rtems_rtl_set_error (EINVAL,
								 ...
								 "in non-PLT relocations", sect->name,
								 (uint32_t) ELF_R_TYPE (rel->r_info));
			return false;
	}
