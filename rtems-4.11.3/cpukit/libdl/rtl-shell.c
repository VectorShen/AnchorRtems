 ...
 */
typedef int (*rtems_rtl_shell_handler_t) (rtems_rtl_data_t * rtl, int argc,
										char *argv[]);
 ...
 */
static int rtems_rtl_count_symbols (rtems_rtl_data_t * rtl)
{
	int count;
	int bucket;
...
rtems_rtl_shell_status (rtems_rtl_data_t * rtl, int argc, char *argv[])
{
	...
	rtems_rtl_chain_iterate (&rtl->objects,
							 rtems_rtl_obj_summary_iterator, &summary);
	/*
 ...
 */
static size_t rtems_rtl_delta_voids (void *higher, void *lower)
{
	char *ch = higher;
	char *cl = lower;
	return ch - cl;
 ...
 * Parse an argument.
 */
static bool rtems_rtl_parse_arg (const char *opt, int argc, char *argv[])
{
	int arg;
 ...
 */
static bool rtems_rtl_base_arg (int argc, char *argv[])
{
 ...
 */
static bool rtems_rtl_symbols_arg (int argc, char *argv[])
{
...
rtems_rtl_obj_printer (rtems_rtl_obj_print_t * print, rtems_rtl_obj_t * obj)
{
	...
	if (print->memory_map)
	{
		...
		printf ("%-*ctext base     : %p (%zi)\n", print->indent, ' ',
				obj->text_base, rtems_rtl_delta_voids (obj->const_base,
														 obj->text_base));
		printf ("%-*cconst base    : %p (%zi)\n", print->indent, ' ',
				obj->const_base, rtems_rtl_delta_voids (obj->data_base,
														obj->const_base));
		printf ("%-*cdata base     : %p (%zi)\n", print->indent, ' ',
				obj->data_base, rtems_rtl_delta_voids (obj->bss_base,
														 obj->data_base));
	...
	if (print->symbols)
	{
		...
		for (s = 0; s < obj->global_syms; ++s)
			printf ("%-*c%-*s = %p\n", print->indent + 2, ' ',
					max_len, obj->global_table[s].name,
					obj->global_table[s].value);
	}
	printf ("\n");
	return true;
}
 ...
 */
static bool
rtems_rtl_unresolved_printer (rtems_rtl_unresolv_rec_t * rec, void *data)
{
...
static int rtems_rtl_shell_list (rtems_rtl_data_t * rtl, int argc, char *argv[])
{
	...
	print.base = false;
	rtems_rtl_chain_iterate (&rtl->objects,
							 rtems_rtl_obj_print_iterator, &print);
	return 0;
}

static int rtems_rtl_shell_sym (rtems_rtl_data_t * rtl, int argc, char *argv[])
{
	rtems_rtl_obj_print_t print;
	print.rtl = rtl;
	print.indent = 1;
...
static void rtems_rtl_shell_usage (const char *arg)
{
...
int rtems_rtl_shell_command (int argc, char *argv[])
{
		...
		switch (argv[arg][1])
		{
			...
			case 'l':
				...
				for (t = 0;
					 t <
					 (sizeof (table) /
					sizeof (const rtems_rtl_shell_cmd_t)); ++t)
	...
	{
		for (t = 0;
			 t < (sizeof (table) / sizeof (const rtems_rtl_shell_cmd_t)); ++t)
		{
