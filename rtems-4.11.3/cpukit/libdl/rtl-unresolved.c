...
static size_t rtems_rtl_unresolved_name_recs (const char *name)
{
	...
	*rtems_rtl_unresolved_rec_next (rtems_rtl_unresolv_rec_t * rec)
{
	...
	switch (rec->type)
	{
		...
		case rtems_rtl_unresolved_name:
			 ...
			 */
			rec +=
				((rec->rec.name.length + sizeof (rtems_rtl_unresolv_name_t) -
...
rtems_rtl_unresolved_find_name (rtems_rtl_unresolved_t * unresolved,
								const char *name, bool update_refcount)
{
	size_t length = strlen (name);
	int index = 1;
	...
	{
		rtems_rtl_unresolv_block_t *block =
			(rtems_rtl_unresolv_block_t *) node;
		rtems_rtl_unresolv_rec_t *rec =
			rtems_rtl_unresolved_rec_first (block);
...
typedef struct rtems_rtl_unresolved_reloc_data_s
{
	...
	rtems_rtl_unresolv_rec_t *name_rec;
									/**< Name record. */
	rtems_rtl_obj_sym_t *sym;		/**< The symbol record. */
} rtems_rtl_unresolved_reloc_data_t;

static bool
rtems_rtl_unresolved_resolve_reloc (rtems_rtl_unresolv_rec_t * rec, void *data)
{
	...
	if (rec->type == rtems_rtl_unresolved_name)
	{
		...
		if (rd->sym)
		{
			...
			rtems_rtl_unresolved_interate
				(rtems_rtl_unresolved_resolve_reloc, rd);
...
rtems_rtl_unresolved_clean_block (rtems_rtl_unresolv_block_t * block,
								rtems_rtl_unresolv_rec_t * rec,
								size_t count, size_t recs_per_block)
{
		...
		while (!rtems_chain_is_head (&unresolved->blocks, node))
		{
			...
			while (!rtems_rtl_unresolved_rec_is_last (block, rec))
			{
				...
				if (rec->type == rtems_rtl_unresolved_name)
				{
					...
					{
						size_t name_recs =
							rtems_rtl_unresolved_name_recs (rec->rec.
															name.
															name);
						rtems_rtl_unresolved_clean_block (block, rec,
															name_recs,
															unresolved->
															block_recs);
						next = false;
...
rtems_rtl_unresolved_add (rtems_rtl_obj_t * obj,
						...
						const char *name,
						const uint16_t sect, const rtems_rtl_word_t * rel)
{
	...
	if (name_index < 0)
	{
		...
		memcpy ((void *)&rec->rec.name.name[0], name,
				rec->rec.name.length + 1);
