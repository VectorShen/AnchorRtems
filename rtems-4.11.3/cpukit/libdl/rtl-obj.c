...
static void rtems_rtl_obj_free_names (rtems_rtl_obj_t * obj)
{
...
bool rtems_rtl_obj_free (rtems_rtl_obj_t * obj)
{
	...
	rtems_rtl_alloc_module_del (&obj->text_base, &obj->const_base,
								&obj->eh_base, &obj->data_base, &obj->bss_base);
...
rtems_rtl_parse_name (const char *name,
					const char **aname, const char **oname, off_t * ooffset)
{
	...
	loname =
		rtems_rtl_alloc_new (RTEMS_RTL_ALLOC_OBJECT, colon - name + 1, true);
	if (!oname)
	{
	...
	if (colon != end)
	{
		...
		loname =
			rtems_rtl_alloc_new (RTEMS_RTL_ALLOC_OBJECT, at - colon + 1,
								 true);
		if (!loname)
		{
...
static bool rtems_rtl_obj_parse_name (rtems_rtl_obj_t * obj, const char *name)
{
 ...
 * alignment.
 */
static size_t rtems_rtl_sect_align (size_t offset, uint32_t alignment)
{
...
typedef struct
{
	...
	uint32_t alignment;
					/**< The alignment of the section type. */
...
rtems_rtl_obj_section_handler (uint32_t mask,
							 rtems_rtl_obj_t * obj,
							 int fd,
							 rtems_rtl_obj_sect_handler_t handler, void *data)
{
...
rtems_rtl_obj_add_section (rtems_rtl_obj_t * obj,
						 ...
						 off_t offset,
						 uint32_t alignment,
						 int link, int info, uint32_t flags)
{
	if (size > 0)
	{
		rtems_rtl_obj_sect_t *sect =
			rtems_rtl_alloc_new (RTEMS_RTL_ALLOC_OBJECT,
...
rtems_rtl_obj_sect_t *rtems_rtl_obj_find_section_by_index (const rtems_rtl_obj_t
														 * obj, int index)
{
	rtems_rtl_obj_sect_finder_t match;
	match.sect = NULL;
...
rtems_rtl_obj_sect_sync_handler (rtems_chain_node * node, void *data)
{
	...
	{
		old_end =
			(uintptr_t) sync_ctx->end_va & ~(sync_ctx->cache_line_size - 1);
		...
		{
			rtems_cache_instruction_sync_after_code_change (sync_ctx->
															start_va,
															sync_ctx->
															end_va -
															sync_ctx->
...
void rtems_rtl_obj_synchronize_cache (rtems_rtl_obj_t * obj)
{
	...
	rtems_rtl_chain_iterate (&obj->sections,
							 rtems_rtl_obj_sect_sync_handler, &sync_ctx);
...
rtems_rtl_obj_sections_linked_to_order (rtems_rtl_obj_t * obj,
										int section, uint32_t visited_mask)
{
	...
	while (!rtems_chain_is_tail (sections, node))
	{
		...
		if ((sect->size != 0) && ((sect->flags & mask) != 0))
		{
			if (sect->load_order == order)
			{
				if (!first)
					base_offset =
						rtems_rtl_sect_align (base_offset,
												sect->alignment);
				...
				if (rtems_rtl_trace (RTEMS_RTL_TRACE_LOAD_SECT))
					printf
						("rtl: loading:%2d: %s -> %8p (s:%zi f:%04lx a:%lu l:%02d)\n",
							 ...
							 rtems_rtl_obj_sect_handler_t handler, void *data)
{
	...
	text_size =
		rtems_rtl_obj_text_size (obj) + rtems_rtl_obj_const_alignment (obj);
	const_size =
		rtems_rtl_obj_const_size (obj) + rtems_rtl_obj_eh_alignment (obj);
	eh_size = rtems_rtl_obj_eh_size (obj) + rtems_rtl_obj_data_alignment (obj);
	data_size =
		rtems_rtl_obj_data_size (obj) + rtems_rtl_obj_bss_alignment (obj);
	...
	if (rtems_rtl_trace (RTEMS_RTL_TRACE_LOAD_SECT))
	{
		...
		printf ("rtl: load sect: data  - b:%p s:%zi a:%" PRIu32 "\n",
				obj->data_base, data_size,
				rtems_rtl_obj_data_alignment (obj));
	...
	if (!rtems_rtl_obj_sections_loader (RTEMS_RTL_OBJ_SECT_TEXT,
		...
		|| !rtems_rtl_obj_sections_loader (RTEMS_RTL_OBJ_SECT_CONST, obj, fd,
										 obj->const_base, handler, data)
		|| !rtems_rtl_obj_sections_loader (RTEMS_RTL_OBJ_SECT_EH, obj, fd,
										 obj->eh_base, handler, data)
		|| !rtems_rtl_obj_sections_loader (RTEMS_RTL_OBJ_SECT_DATA, obj, fd,
										 obj->data_base, handler, data)
		|| !rtems_rtl_obj_sections_loader (RTEMS_RTL_OBJ_SECT_BSS, obj, fd,
										 obj->bss_base, handler, data))
	{
		rtems_rtl_alloc_module_del (&obj->text_base, &obj->const_base,
									&obj->eh_base, &obj->data_base,
									&obj->bss_base);
		obj->exec_size = 0;
		return false;
	}
...
static void rtems_rtl_obj_run_cdtors (rtems_rtl_obj_t * obj, uint32_t mask)
{
	rtems_chain_node *node = rtems_chain_first (&obj->sections);
	while (!rtems_chain_is_tail (&obj->sections, node))
	{
 ...
 * object descriptor.
 */
static bool rtems_rtl_obj_archive_find (rtems_rtl_obj_t * obj, int fd)
{
...
#define RTEMS_RTL_AR_FHDR_SIZE  (60)
	...
	while (obj->ooffset < fsize)
	{
		...
		if (!rtems_rtl_seek_read
			(fd, obj->ooffset, RTEMS_RTL_AR_FHDR_SIZE, &header[0]))
		{
		...
		if (header[0] == '/')
		{
			...
			switch (header[1])
			{
				...
				case '9':
					 ...
					 */
					extended_off =
						rtems_rtl_scan_decimal (&header[1],
												RTEMS_RTL_AR_FNAME_SIZE);
...
#define RTEMS_RTL_MAX_FILE_SIZE (256)
						...
						if (!rtems_rtl_seek_read
							(fd, extended_file_names + extended_off,
		...
		{
			if (rtems_rtl_match_name
				(obj, (const char *)&header[RTEMS_RTL_AR_FNAME]))
			{
				obj->ooffset += RTEMS_RTL_AR_FHDR_SIZE;
				return true;
			}
...
static bool rtems_rtl_obj_file_load (rtems_rtl_obj_t * obj, int fd)
{
	int l;
...
static bool rtems_rtl_obj_file_unload (rtems_rtl_obj_t * obj)
{
