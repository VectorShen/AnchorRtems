...
/* BSP has decided Big Endian PCI Bus (non-standard) */
	...
	RTEMS_INLINE_ROUTINE void pci_st_le16 (volatile uint16_t * addr,
										   uint16_t val)
	{
		st_be16 (addr, val);
	}
	...
	RTEMS_INLINE_ROUTINE void pci_st_le32 (volatile uint32_t * addr,
										   uint32_t val)
	{
		st_be32 (addr, val);
	}
	...
	RTEMS_INLINE_ROUTINE void pci_st_be16 (volatile uint16_t * addr,
										   uint16_t val)
	{
		st_le16 (addr, val);
	}
	...
	RTEMS_INLINE_ROUTINE void pci_st_be32 (volatile uint32_t * addr,
										   uint32_t val)
	{
		st_le32 (addr, val);
	}
 ...
 */
	extern int pci_access_func (int wr, int size, void **func, int endian,
								int type);
