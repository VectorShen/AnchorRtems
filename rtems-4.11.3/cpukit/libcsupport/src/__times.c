...
clock_t _times (struct tms * ptms)
{
	 ...
	 */
	{
		...
		_Timestamp_Divide (&executing->cpu_time_used,
						 &per_tick, &ticks_of_executing, &fractional_ticks);
 ...
 */
clock_t _times_r (struct _reent * ptr __attribute__ ((unused)),
				struct tms * ptms)
{
	return _times (ptms);
}
