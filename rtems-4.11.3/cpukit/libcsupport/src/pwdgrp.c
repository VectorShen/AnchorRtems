...
static int scanInt (FILE * fp, int *val)
{
	...
	for (;;)
	{
		...
		if (!isdigit (c))
			return 0;
		d = c - '0';
		if ((i > (limit / 10)) || ((i == (limit / 10)) && (d > (limit % 10))))
			return 0;
		i = i * 10 + d;
	}
	if (sign == 0)
 ...
 */
int _libcsupport_scanpw (FILE * fp,
						 struct passwd *pwd, char *buffer, size_t bufsize)
{
	int pwuid, pwgid;
...
int _libcsupport_scangr (FILE * fp,
						 struct group *grp, char *buffer, size_t bufsize)
{
	int grgid;
	char *grmem, *cp;
	int memcount;
