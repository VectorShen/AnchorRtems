...
#ifdef DEBUG_GXX_WRAPPERS
	printk ("gxx_wrappers: create key=%x, dtor=%x, pkey=%x\n", key, dtor, pkey);
#endif
...
int rtems_gxx_setspecific (__gthread_key_t key, const void *ptr)
{
	...
	printk ("gxx_wrappers: setspecific key=%x, ptr=%x, id=%x\n",
			pkey, ptr, rtems_task_self ());
#endif

	if (eno != 0)
	{
		_Terminate (INTERNAL_ERROR_CORE,
					true, INTERNAL_ERROR_GXX_KEY_ADD_FAILED);
	}
...
void rtems_gxx_mutex_init (__gthread_mutex_t * mutex)
{
	...
	status = rtems_semaphore_create (rtems_build_name ('G', 'C', 'C', '2'),
									 1,
...
#endif
		_Terminate (INTERNAL_ERROR_CORE,
					true, INTERNAL_ERROR_GXX_MUTEX_INIT_FAILED);
	}
#ifdef DEBUG_GXX_WRAPPERS
...
int rtems_gxx_mutex_lock (__gthread_mutex_t * mutex)
{
	...
	status = rtems_semaphore_obtain (*(rtems_id *) mutex,
