 ...
 */
int _rename_r (struct _reent *ptr __attribute__ ((unused)),
			 const char *old, const char *new)
{
	...
	const rtems_filesystem_location_info_t *old_currentloc =
		rtems_filesystem_eval_path_start_with_parent (&old_ctx,
													old,
													old_eval_flags,
													&old_parentloc,
	...
	rv = rtems_filesystem_location_exists_in_same_instance_as (old_currentloc,
															 new_currentloc);
	if (rv == 0)
	{
		rv = (*new_currentloc->mt_entry->ops->rename_h) (&old_parentloc,
														 old_currentloc,
														 new_currentloc,
