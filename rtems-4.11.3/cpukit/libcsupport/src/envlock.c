...
static void __rtems_envlock_init (void)
{
	extern char **environ;
	...
	rc = rtems_semaphore_create (rtems_build_name ('E', 'N', 'V', 'S'),
								 1,
								 RTEMS_BINARY_SEMAPHORE | RTEMS_PRIORITY |
								 RTEMS_INHERIT_PRIORITY, 0, &envLock);
...
void __env_lock (struct _reent *r __attribute__ ((unused)))
{
	rtems_libio_lock ();
}

void __env_unlock (struct _reent *r __attribute__ ((unused)))
{
	rtems_libio_unlock ();
}
