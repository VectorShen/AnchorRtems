...
static rtems_termios_device_node
	*rtems_termios_find_device_node (rtems_device_major_number major,
...
rtems_status_code rtems_termios_device_install (const char *device_file,
												...
												handler,
												const rtems_termios_device_flow
												* flow,
												...
												context)
{
	...
	sc = rtems_semaphore_obtain (rtems_termios_ttyMutex, RTEMS_WAIT,
								 RTEMS_NO_TIMEOUT);
	...
	rtems_chain_append_unprotected (&rtems_termios_devices,
									&new_device_node->node);
...
static rtems_termios_tty *legacyContextToTTY (rtems_termios_device_context *
											ctx)
{
...
static bool
rtems_termios_callback_firstOpen (rtems_termios_tty * tty,
...
static void
rtems_termios_callback_lastClose (rtems_termios_tty * tty,
								rtems_termios_device_context * ctx,
								rtems_libio_open_close_args_t * args)
{
	(void)ctx;
...
static void
rtems_termios_callback_write (rtems_termios_device_context * ctx,
...
static bool
rtems_termios_callback_setAttributes (rtems_termios_device_context * ctx,
...
rtems_termios_destroy_tty (rtems_termios_tty * tty, void *arg, bool last_close)
{
	...
	if (tty->device_context == &tty->legacy_device_context)
		rtems_interrupt_lock_destroy (&tty->legacy_device_context.
									interrupt_lock);
												...
												* callbacks)
{
	...
	if (tty == NULL)
	{
		 ...
		 */
		sc = rtems_semaphore_create (rtems_build_name ('T', 'R', 'i', c),
									 1,
									 RTEMS_BINARY_SEMAPHORE |
									 RTEMS_INHERIT_PRIORITY | RTEMS_PRIORITY,
									 RTEMS_NO_PRIORITY, &tty->isem);
		if (sc != RTEMS_SUCCESSFUL)
			rtems_fatal_error_occurred (sc);
		sc = rtems_semaphore_create (rtems_build_name ('T', 'R', 'o', c),
									 1,
									 RTEMS_BINARY_SEMAPHORE |
									 RTEMS_INHERIT_PRIORITY | RTEMS_PRIORITY,
									 RTEMS_NO_PRIORITY, &tty->osem);
		if (sc != RTEMS_SUCCESSFUL)
			rtems_fatal_error_occurred (sc);
		sc = rtems_semaphore_create (rtems_build_name ('T', 'R', 'x', c),
									 0,
									 RTEMS_SIMPLE_BINARY_SEMAPHORE |
									 RTEMS_FIFO, RTEMS_NO_PRIORITY,
		...
		if (tty->device_context == NULL)
		{
			...
			rtems_termios_device_context_initialize (tty->device_context,
													 "Termios");
		}
		...
		if (tty->handler.mode == TERMIOS_TASK_DRIVEN)
		{
			sc = rtems_task_create (rtems_build_name ('T', 'x', 'T', c),
									TERMIOS_TXTASK_PRIO,
			...
			if (sc != RTEMS_SUCCESSFUL)
				rtems_fatal_error_occurred (sc);
			sc = rtems_task_create (rtems_build_name ('R', 'x', 'T', c),
									TERMIOS_RXTASK_PRIO,
	...
	if (!tty->refcount++)
	{
		if (tty->handler.first_open
			&& !(*tty->handler.first_open) (tty, tty->device_context,
											&tty->termios, args))
		{
		...
		if (tty->handler.mode == TERMIOS_TASK_DRIVEN)
		{
			sc = rtems_task_start (tty->rxTaskId, rtems_termios_rxdaemon,
								 (rtems_task_argument) tty);
			if (sc != RTEMS_SUCCESSFUL)
				rtems_fatal_error_occurred (sc);

			sc = rtems_task_start (tty->txTaskId, rtems_termios_txdaemon,
								 (rtems_task_argument) tty);
			if (sc != RTEMS_SUCCESSFUL)
				rtems_fatal_error_occurred (sc);
		}
...
rtems_status_code
rtems_termios_device_open (rtems_device_major_number major,
						 rtems_device_minor_number minor, void *arg)
{
	...
	tty =
		rtems_termios_open_tty (major, minor, arg, device_node->tty,
								device_node, NULL);
	if (tty == NULL)
	{
		rtems_semaphore_release (rtems_termios_ttyMutex);
 ...
 */
rtems_status_code
rtems_termios_open (rtems_device_major_number major,
					rtems_device_minor_number minor,
					void *arg, const rtems_termios_callbacks * callbacks)
{
	rtems_status_code sc;
	struct rtems_termios_tty *tty;
...
rtems_status_code rtems_termios_bufsize (size_t cbufsize,
										 size_t raw_input, size_t raw_output)
{
...
rtems_status_code rtems_termios_ioctl (void *arg)
{
	...
	switch (args->command)
	{
		...
		case RTEMS_IO_SET_ATTRIBUTES:
			...
			{
				sc = (*tty->handler.set_attributes) (tty->device_context,
													 &tty->
													 termios) ?
 ...
 */
static unsigned int
startXmit (struct rtems_termios_tty *tty,
		 unsigned int newTail, bool transmitting)
{
	...
	else
	{
		...
		(*tty->handler.write) (tty->device_context,
								 &tty->rawOutBuf.theBuf[newTail], nToSend);
	}
 ...
 */
void
rtems_termios_puts (const void *_buf, size_t len, struct rtems_termios_tty *tty)
{
	const char *buf = _buf;
 ...
 */
static void oproc (unsigned char c, struct rtems_termios_tty *tty)
{
	int i;
 ...
 */
static int iproc (unsigned char c, struct rtems_termios_tty *tty)
{
 ...
 */
static int siproc (unsigned char c, struct rtems_termios_tty *tty)
{
	int i;
	 ...
	 */
	if (tty->termios.
		c_lflag & (ECHO | ECHOE | ECHOK | ECHONL | ECHOPRT | ECHOCTL | ECHOKE))
	{
		rtems_status_code sc;
		sc = rtems_semaphore_obtain (tty->osem, RTEMS_WAIT, RTEMS_NO_TIMEOUT);
		if (sc != RTEMS_SUCCESSFUL)
 ...
 */
static void fillBufferPoll (struct rtems_termios_tty *tty)
{
	int n;
 ...
 */
static void fillBufferQueue (struct rtems_termios_tty *tty)
{
	rtems_termios_device_context *ctx = tty->device_context;
	...
	while (wait)
	{
				 ...
				 (tty->ccount < (CBUFSIZE - 1)))
		{
			...
			if (((tty->rawInBuf.Tail - newHead + tty->rawInBuf.Size)
				 % tty->rawInBuf.Size) < tty->lowwater)
			{
...
rtems_status_code rtems_termios_read (void *arg)
{
	...
	if (tty->cindex == tty->ccount)
	{
		...
		if (tty->handler.poll_read != NULL
			&& tty->handler.mode == TERMIOS_POLLED)
			fillBufferPoll (tty);
		else
			fillBufferQueue (tty);
	}
	...
	while (len--)
	{
		...
		else
		{
			...
			if (newTail != head)
			{
				 ...
				 * check to see if rcv wakeup callback was set
				 */
				if ((!tty->tty_rcvwakeup)
					&& (tty->tty_rcv.sw_pfn != NULL))
				{
					(*tty->tty_rcv.sw_pfn) (&tty->termios,
											tty->tty_rcv.sw_arg);
					tty->tty_rcvwakeup = 1;
				}
			}
...
static int rtems_termios_refill_transmitter (struct rtems_termios_tty *tty)
{
	...
	else
	{
		...
		if (newTail == tty->rawOutBuf.Head)
		{
			...
			{
				(*tty->tty_snd.sw_pfn) (&tty->termios,
										tty->tty_snd.sw_arg);
			}
		}
		else
...
static rtems_task rtems_termios_txdaemon (rtems_task_argument argument)
{
	...
	while (1)
	{
		...
		rtems_event_receive ((TERMIOS_TX_START_EVENT |
							 ...
							 RTEMS_EVENT_ANY | RTEMS_WAIT, RTEMS_NO_TIMEOUT,
							 &the_event);
