...
static bool contains_root_or_current_directory (const
...
int unmount (const char *path)
{
	...
	if (rtems_filesystem_location_is_instance_root (currentloc))
	{
		if (!contains_root_or_current_directory (mt_entry))
		{
			...
			if (rv == 0)
			{
				...
				rtems_filesystem_mt_entry_declare_lock_context
					(lock_context);
	...
	if (rv == 0)
	{
		rtems_status_code sc = rtems_event_transient_receive (RTEMS_WAIT,
																RTEMS_NO_TIMEOUT);
