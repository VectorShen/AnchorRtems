...
int unlink (const char *path)
{
	...
	int parent_eval_flags = RTEMS_FS_PERMS_WRITE
		| RTEMS_FS_PERMS_EXEC | RTEMS_FS_FOLLOW_LINK;
	const rtems_filesystem_location_info_t *currentloc =
		rtems_filesystem_eval_path_start_with_parent (&ctx,
													path,
													eval_flags,
													&parentloc,
													parent_eval_flags);
 ...
 */
int _unlink_r (struct _reent *ptr __attribute__ ((unused)), const char *path)
{
	return unlink (path);
}
#endif
