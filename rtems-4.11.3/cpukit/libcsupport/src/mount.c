...
static rtems_filesystem_mount_table_entry_t *alloc_mount_table_entry (const char
																	*source_or_null,
																	const char
																	*target_or_null,
																	const char
																	...
																	target_length_ptr)
{
	...
	if (mt_entry != NULL)
	{
		...
		rtems_chain_initialize (&mt_entry->location_chain,
								mt_fs_root, 1, sizeof (*mt_fs_root));
	}
...
static int
register_subordinate_file_system (rtems_filesystem_mount_table_entry_t *
								mt_entry, const char *target)
{
	int rv = 0;
	...
	if (!rtems_filesystem_location_is_instance_root (currentloc))
	{
		...
		mt_point_node =
			rtems_filesystem_location_transform_to_global (&targetloc);
		mt_entry->mt_point_node = mt_point_node;
		rv = (*mt_point_node->location.mt_entry->ops->mount_h) (mt_entry);
		if (rv == 0)
		{
			rtems_filesystem_mt_lock ();
			rtems_chain_append_unprotected (&rtems_filesystem_mount_table,
											&mt_entry->mt_node);
...
static int register_root_file_system (rtems_filesystem_mount_table_entry_t *
									mt_entry)
{
	int rv = 0;
	...
	if (rv == 0)
	{
		...
		rtems_filesystem_global_location_assign (&rtems_filesystem_root,
												 new_fs_root);
		rtems_filesystem_global_location_assign (&rtems_filesystem_current,
												 new_fs_current);
	}
		...
		|| options == RTEMS_FILESYSTEM_READ_WRITE)
	{
		...
		if (fsmount_me_h != NULL)
		{
			...
			rtems_filesystem_mount_table_entry_t *mt_entry =
				alloc_mount_table_entry (source,
										 target,
										 filesystemtype,
										 &target_length);
			...
			if (mt_entry != NULL)
			{
				...
				if (rv == 0)
				{
					if (target != NULL)
					{
						rv = register_subordinate_file_system
							(mt_entry, target);
					}
					else
					{
