...
static void set_startloc (rtems_filesystem_eval_path_context_t * ctx,
						rtems_filesystem_global_location_t *
						const *global_root_ptr,
						rtems_filesystem_global_location_t *
						const *global_current_ptr)
{
	if (ctx->pathlen > 0)
	{
		char c = ctx->path[0];
		...
		ctx->rootloc =
			rtems_filesystem_global_location_obtain (global_root_ptr);
		...
		if (rtems_filesystem_is_delimiter (c))
		{
			...
			--ctx->pathlen;
			ctx->startloc =
				rtems_filesystem_global_location_obtain (&ctx->rootloc);
		}
		else
		{
			ctx->startloc =
...
static void check_access (rtems_filesystem_eval_path_context_t * ctx,
						int eval_flags)
{
	...
	if ((eval_flags & RTEMS_FS_PERMS_WRITE) == 0 || mt_entry->writeable)
	{
		...
		if (rv == 0)
		{
			bool access_ok = rtems_filesystem_check_access (eval_flags,
															st.st_mode,
															st.st_uid,
															st.st_gid);
...
void rtems_filesystem_eval_path_continue (rtems_filesystem_eval_path_context_t *
										ctx)
{
	int eval_flags;
...
rtems_filesystem_location_info_t
	*rtems_filesystem_eval_path_start (rtems_filesystem_eval_path_context_t *
									 ctx, const char *path, int eval_flags)
{
	return rtems_filesystem_eval_path_start_with_root_and_current (ctx,
																 path,
																 strlen
																 (path),
																 eval_flags,
	 ...
	 int parent_eval_flags)
{
	...
	currentloc = rtems_filesystem_eval_path_start_with_root_and_current (ctx,
																		 parentpath,
																		 parentpathlen,
																		 parent_eval_flags,
																		 &rtems_filesystem_root,
...
void rtems_filesystem_eval_path_recursive (rtems_filesystem_eval_path_context_t
										 * ctx, const char *path,
										 size_t pathlen)
{
	if (pathlen > 0)
...
void rtems_filesystem_eval_path_cleanup (rtems_filesystem_eval_path_context_t *
										 ctx)
{
...
void
rtems_filesystem_eval_path_cleanup_with_parent
...
void rtems_filesystem_eval_path_restart (rtems_filesystem_eval_path_context_t *
										 ctx,
										 rtems_filesystem_global_location_t **
										 newstartloc_ptr)
{
	free_location (&ctx->currentloc);
	rtems_filesystem_instance_unlock (&ctx->startloc->location);
	rtems_filesystem_global_location_assign (&ctx->startloc,
											 rtems_filesystem_global_location_obtain
											 (newstartloc_ptr));
	rtems_filesystem_instance_lock (&ctx->startloc->location);
	rtems_filesystem_location_clone (&ctx->currentloc,
