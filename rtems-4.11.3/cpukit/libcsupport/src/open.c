...
static void create_regular_file (rtems_filesystem_eval_path_context_t * ctx,
								 mode_t mode)
{
	int rv = 0;
	...
	rv = rtems_filesystem_mknod (currentloc,
								 token, tokenlen, S_IFREG | mode, 0);
...
static int do_open (rtems_libio_t * iop,
					const char *path, int oflag, mode_t mode)
{
	...
	int eval_flags = RTEMS_FS_FOLLOW_LINK
		...
		| (make ? RTEMS_FS_MAKE : 0) | (exclusive ? RTEMS_FS_EXCLUSIVE : 0);
 ...
 */
int _open_r (struct _reent *ptr __attribute__ ((unused)),
			 const char *buf, int oflag, int mode)
{
