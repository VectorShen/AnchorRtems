...
int chroot (const char *path)
{
	 ...
	 */
	rtems_filesystem_eval_path_start_with_root_and_current (&ctx,
															path,
															strlen (path),
															eval_flags,
															&rtems_global_user_env.
															root_directory,
															&rtems_global_user_env.
															current_directory);
	...
	if (!rtems_filesystem_global_location_is_null (new_current_loc))
	{
		...
		mode_t type =
			rtems_filesystem_location_type (&new_root_loc->location);
		...
		if (S_ISDIR (type))
		{
			...
			if (sc == RTEMS_SUCCESSFUL)
			{
				rtems_filesystem_global_location_assign
					(&rtems_filesystem_root, new_root_loc);
				rtems_filesystem_global_location_assign
					(&rtems_filesystem_current, new_current_loc);
			}
			else
			{
		...
		{
			rtems_filesystem_location_error (&new_root_loc->location,
											 ENOTDIR);
			rv = -1;
		}
