...
*/
void rtems_filesystem_location_clone (rtems_filesystem_location_info_t *
									clone,
...
*/
void rtems_filesystem_location_free (rtems_filesystem_location_info_t *
									 loc);
...
{
	rtems_semaphore_obtain (rtems_libio_semaphore, RTEMS_WAIT,
							RTEMS_NO_TIMEOUT);
}
...
static inline void rtems_filesystem_instance_lock (const
...
static inline void rtems_filesystem_instance_unlock (const
													 rtems_filesystem_location_info_t
													 * loc)
{
	const rtems_filesystem_mount_table_entry_t *mt_entry = loc->mt_entry;
...
rtems_filesystem_location_info_t
	*rtems_filesystem_eval_path_start_with_parent
	(rtems_filesystem_eval_path_context_t * ctx, const char *path,
	 int eval_flags, rtems_filesystem_location_info_t * parentloc,
	 int parent_eval_flags);

rtems_filesystem_location_info_t
	*rtems_filesystem_eval_path_start_with_root_and_current
	(rtems_filesystem_eval_path_context_t * ctx, const char *path,
	 size_t pathlen, int eval_flags,
...
*/
typedef
	bool (*rtems_filesystem_eval_path_is_directory)
...
*/
rtems_filesystem_location_info_t
	*rtems_filesystem_location_copy (rtems_filesystem_location_info_t * dst,
									 const rtems_filesystem_location_info_t
									 * src);
...
{
	return rtems_filesystem_location_copy (loc,
										 &rtems_filesystem_global_location_null.
										 location);
}
...
void rtems_filesystem_global_location_release
	(rtems_filesystem_global_location_t * global_loc, bool deferred);

void rtems_filesystem_location_detach (rtems_filesystem_location_info_t *
									 detach);
...
static inline bool rtems_filesystem_location_is_null (const
													rtems_filesystem_location_info_t
													* loc)
{
...
static inline bool rtems_filesystem_global_location_is_null (const
															 rtems_filesystem_global_location_t
															 * global_loc)
{
...
static inline void rtems_filesystem_location_error (const
													rtems_filesystem_location_info_t
													* loc, int eno)
{
...
int rtems_filesystem_mknod (const rtems_filesystem_location_info_t *
							parentloc, const char *name, size_t namelen,
							mode_t mode, dev_t dev);

int rtems_filesystem_chdir (rtems_filesystem_location_info_t * loc);

int rtems_filesystem_chmod (const rtems_filesystem_location_info_t * loc,
							mode_t mode);

int rtems_filesystem_chown (const rtems_filesystem_location_info_t * loc,
							uid_t owner, gid_t group);

static inline bool
	rtems_filesystem_is_ready_for_unmount
...
static inline void
	rtems_filesystem_location_add_to_mt_entry
	(rtems_filesystem_location_info_t * loc)
{
	rtems_filesystem_mt_entry_declare_lock_context (lock_context);

	rtems_filesystem_mt_entry_lock (lock_context);
	rtems_chain_append_unprotected (&loc->mt_entry->location_chain,
									&loc->mt_entry_node);
...
static inline bool rtems_filesystem_location_is_instance_root (const
															 rtems_filesystem_location_info_t
															 * loc)
{
	const rtems_filesystem_mount_table_entry_t *mt_entry = loc->mt_entry;

	return (*mt_entry->ops->are_nodes_equal_h) (loc,
												&mt_entry->mt_fs_root->
												location);
}
...
static inline size_t
	rtems_filesystem_eval_path_get_pathlen
	(rtems_filesystem_eval_path_context_t * ctx)
{
	return ctx->pathlen;
}
...
static inline void
	rtems_filesystem_eval_path_clear_path
	(rtems_filesystem_eval_path_context_t * ctx)
{
	ctx->pathlen = 0;
}
...
static inline void
	...
	(rtems_filesystem_eval_path_context_t * ctx, const char *token,
	 size_t tokenlen)
{
	ctx->token = token;
...
static inline void
	rtems_filesystem_eval_path_clear_token
	(rtems_filesystem_eval_path_context_t * ctx)
{
	ctx->tokenlen = 0;
}
...
static inline void
	rtems_filesystem_eval_path_put_back_token
	(rtems_filesystem_eval_path_context_t * ctx)
{
...
static inline void
	rtems_filesystem_eval_path_get_next_token
	(rtems_filesystem_eval_path_context_t * ctx, const char **token,
	 size_t * tokenlen)
{
...
static inline bool rtems_filesystem_eval_path_has_path (const
...
static inline bool rtems_filesystem_eval_path_has_token (const
														 rtems_filesystem_eval_path_context_t
														 * ctx)
{
...
static inline int rtems_filesystem_eval_path_get_flags (const
														rtems_filesystem_eval_path_context_t
														* ctx)
{
	return ctx->flags;
...
static inline void
	...
	(rtems_filesystem_eval_path_context_t * ctx, int flags)
{
	ctx->flags = flags;
}

static inline void
	rtems_filesystem_eval_path_clear_and_set_flags
	(rtems_filesystem_eval_path_context_t * ctx, int clear, int set)
{
	int flags = ctx->flags;
...
static inline void
	rtems_filesystem_eval_path_extract_currentloc
	(rtems_filesystem_eval_path_context_t * ctx,
	 rtems_filesystem_location_info_t * get)
{
	rtems_filesystem_location_copy_and_detach (get, &ctx->currentloc);
}
...
*/
int rtems_filesystem_location_exists_in_same_instance_as (const
														rtems_filesystem_location_info_t
														* a,
														const
														rtems_filesystem_location_info_t
...
bool rtems_filesystem_eval_path_check_access
	(rtems_filesystem_eval_path_context_t * ctx, int eval_flags,
	 mode_t node_mode, uid_t node_uid, gid_t node_gid);
...
static inline bool rtems_filesystem_is_current_directory (const char *token,
														size_t tokenlen)
{
	return tokenlen == 1 && token[0] == '.';
}

static inline bool rtems_filesystem_is_parent_directory (const char *token,
														 size_t tokenlen)
{
	return tokenlen == 2 && token[0] == '.' && token[1] == '.';
}

static inline ssize_t rtems_libio_iovec_eval (int fd,
...
*/
static inline mode_t rtems_filesystem_location_type (const
													 rtems_filesystem_location_info_t
													 * loc)
{
	struct stat st;
