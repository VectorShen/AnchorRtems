...
static rtems_rbtree_compare_result chunk_compare (const rtems_rbtree_node * a,
...
static void add_to_chain (rtems_chain_control * chain,
						  rtems_rbheap_chunk * chunk)
{
	rtems_chain_prepend_unprotected (chain, &chunk->chain_node);
}

static void insert_into_tree (rtems_rbtree_control * tree,
							  rtems_rbheap_chunk * chunk)
{
...
static rtems_rbheap_chunk *search_free_chunk (rtems_chain_control * free_chain,
											  size_t size)
{
...
void *rtems_rbheap_allocate (rtems_rbheap_control * control, size_t size)
{
	...
	if (size > 0 && size <= aligned_size)
	  {
		  rtems_rbheap_chunk *free_chunk =
			  search_free_chunk (free_chain, aligned_size);
					  ...
					  if (new_chunk != NULL)
						{
							...
							new_chunk->begin =
								free_chunk->begin + new_free_size;
...
static rtems_rbheap_chunk *find (rtems_rbtree_control * chunk_tree,
								 uintptr_t key)
{
	rtems_rbheap_chunk chunk = {.begin = key };

	return
		rtems_rbheap_chunk_of_node (rtems_rbtree_find
									(chunk_tree, &chunk.tree_node,
									 chunk_compare, true));
}

static rtems_rbheap_chunk *get_next (const rtems_rbheap_chunk * chunk,
									 RBTree_Direction dir)
{
	return rtems_rbheap_chunk_of_node (_RBTree_Next (&chunk->tree_node, dir));
}

static void check_and_merge (rtems_rbheap_control * control,
							 rtems_rbheap_chunk * a,
							 rtems_rbheap_chunk * b, rtems_rbheap_chunk * c)
{
...
rtems_status_code rtems_rbheap_free (rtems_rbheap_control * control, void *ptr)
{
	...
	if (ptr != NULL)
	  {
		  rtems_rbheap_chunk *chunk =
			  find (&control->chunk_tree, (uintptr_t) ptr);
...
void rtems_rbheap_extend_descriptors_with_malloc (rtems_rbheap_control *
												  control)
{
