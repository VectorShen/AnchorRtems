...
static void report_per_cpu (context * ctx,
							const rtems_profiling_per_cpu * per_cpu)
{
	...
	indent (ctx, 2);
	rv = (*printf_func) (printf_arg,
						 "<ThreadDispatchDisabledCount>%" PRIu64
						 "</ThreadDispatchDisabledCount>\n",
						 per_cpu->thread_dispatch_disabled_count);
	update_retval (ctx, rv);

	indent (ctx, 2);
	rv = (*printf_func) (printf_arg,
						 "<MaxInterruptDelay unit=\"ns\">%" PRIu32
						 "</MaxInterruptDelay>\n",
	...
	indent (ctx, 2);
	rv = (*printf_func) (printf_arg,
						 "<TotalInterruptTime unit=\"ns\">%" PRIu64
						 "</TotalInterruptTime>\n",
	...
	indent (ctx, 1);
	rv = (*printf_func) (printf_arg, "</PerCPUProfilingReport>\n");
	update_retval (ctx, rv);
}

static void report_smp_lock (context * ctx,
							 const rtems_profiling_smp_lock * smp_lock)
{
	...
	indent (ctx, 2);
	rv = (*printf_func) (printf_arg,
						 "<MaxAcquireTime unit=\"ns\">%" PRIu32
						 "</MaxAcquireTime>\n", smp_lock->max_acquire_time);
	update_retval (ctx, rv);

	indent (ctx, 2);
	rv = (*printf_func) (printf_arg,
						 "<MaxSectionTime unit=\"ns\">%" PRIu32
	...
	indent (ctx, 2);
	rv = (*printf_func) (printf_arg,
						 "<TotalAcquireTime unit=\"ns\">%" PRIu64
						 "</TotalAcquireTime>\n", smp_lock->total_acquire_time);
	update_retval (ctx, rv);

	indent (ctx, 2);
	rv = (*printf_func) (printf_arg,
						 "<TotalSectionTime unit=\"ns\">%" PRIu64
