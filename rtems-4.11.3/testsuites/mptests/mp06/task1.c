...
rtems_task Test_task (rtems_task_argument argument)
{
	...
	do
	  {
		  status = rtems_task_ident (Task_name[remote_node],
									 RTEMS_SEARCH_ALL_NODES, &remote_tid);
	  }
	...
	status = rtems_timer_fire_after (Timer_id[1],
	...
	for (;;)
	  {
		  ...
		  if (Multiprocessing_configuration.node == 1)
			{
				status =
					rtems_event_send (remote_tid, event_for_this_iteration);
			...
			{
				status = rtems_event_receive (event_for_this_iteration,
	...
	if (Multiprocessing_configuration.node == 2)
	  {
		  ...
		  (void)rtems_event_receive (RTEMS_EVENT_16, RTEMS_NO_WAIT, 0,
									 &event_out);

		  puts ("Waiting for RTEMS_EVENT_16");
		  status = rtems_event_receive (RTEMS_EVENT_16,
										RTEMS_DEFAULT_OPTIONS,
