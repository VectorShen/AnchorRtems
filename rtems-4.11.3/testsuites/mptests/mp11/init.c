	...
	if (Multiprocessing_configuration.node == 1)
	  {
		  ...
		  status = rtems_message_queue_create (Queue_name[1],
											   3, 16, RTEMS_GLOBAL, &junk_id);
		  fatal_directive_status (status,
								  RTEMS_TOO_MANY, "rtems_message_queue_create");
		  ...
		  status = rtems_semaphore_create (Semaphore_name[1],
										   1,
										   RTEMS_GLOBAL,
										   RTEMS_NO_PRIORITY, &junk_id);
		  fatal_directive_status (status, RTEMS_TOO_MANY,
								  "rtems_semaphore_create");
		  puts ("rtems_semaphore_create correctly returned RTEMS_TOO_MANY");

		  puts ("Attempting to create Partition (Global)");
		  status = rtems_partition_create (1,
										   (uint8_t *) my_partition,
										   128, 64, RTEMS_GLOBAL, &junk_id);
		  fatal_directive_status (status, RTEMS_TOO_MANY,
