 ...
 */
static void test_disk_params (const int file_descriptor,
 ...
 */
static void test_writing (const int file_descriptor,
						  const uint32_t block_size,
	...
	for (block_count = 0; block_count < blocks_allocated; block_count++)
	  {
		  ...
		  for (byte_count = 0;
			   byte_count < (block_size / sizeof (byte_count)); byte_count++)
			{
 ...
 */
static void test_reading (const int file_descriptor,
						  const uint32_t block_size,
						  const rtems_blkdev_bnum blocks_allocated,
							...
							const uint8_t fill_pattern)
{
	...
	test_disk_params (file_descriptor,
					  block_size, media_block_size, block_number);
 ...
 */
static void test_static_key_table (const sparse_disk_container * disk_container,
								   const rtems_blkdev_bnum blocks_allocated,
								   const uint32_t block_size)
{
	unsigned int i;
 ...
 */
static void test_static_pattern (const unsigned int pattern_size,
...
static void test_with_whitebox (const char *device_name)
{
	...
	sc = rtems_sparse_disk_register ("/dev/sda1",
	...
	test_static_key_table (&disk_container,
						   STATIC_ALLOCATED_BLOCK_COUNT, STATIC_BLOCK_SIZE);
	...
	test_disk_params (file_descriptor,
					  STATIC_BLOCK_SIZE,
					  STATIC_BLOCK_SIZE, STATIC_SIMULATED_BLOCK_COUNT);

	test_writing (file_descriptor,
				  STATIC_BLOCK_SIZE, STATIC_ALLOCATED_BLOCK_COUNT);

	test_reading (file_descriptor,
				  STATIC_BLOCK_SIZE,
				  STATIC_ALLOCATED_BLOCK_COUNT, fill_pattern);
	...
	for (block_count = 0;
		 block_count < STATIC_ALLOCATED_BLOCK_COUNT; block_count++)
	  {
		  for (byte_count = 0;
			   byte_count < (STATIC_BLOCK_SIZE / sizeof (byte_count));
			   byte_count++)
			{
				rv = memcmp (&disk_container.
							 data[byte_count * sizeof (byte_count)],
							 &byte_count, sizeof (byte_count));
				rtems_test_assert (0 == rv);
			}
	  }
...
static void test (void)
{
	...
	blocks_allocated = 8;
	sc = rtems_sparse_disk_create_and_register ("/dev/sda1",
												block_size,
												blocks_allocated,
												block_number, fill_pattern);
	rtems_test_assert (RTEMS_SUCCESSFUL == sc);

	/* Test reading and writing with sector size 512 and 8 such sectors
	 * allocated. Block size will default to 512 */
	test_device_io (device_name,
					block_size,
					block_size, block_number, blocks_allocated, fill_pattern);
	...
	test_device_io (device_name,
					...
					block_size, block_number, 1, fill_pattern);
