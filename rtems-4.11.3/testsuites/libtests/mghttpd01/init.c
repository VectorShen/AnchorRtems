...
{
	int cbacktest =
		strncmp (mg_get_request_info (conn)->uri, CBACKTEST_URI,
				 sizeof (CBACKTEST_URI));
	if (cbacktest == 0)
	  {
...
{
	if (data_len == strlen (WSTEST_REQ)
		&& strncmp (data, WSTEST_REQ, data_len) == 0)
	  {
		  mg_websocket_write (connection, WEBSOCKET_OPCODE_TEXT, WSTEST_RESP,
							  strlen (WSTEST_RESP));
...
static void test_mg_index_html (void)
{
	...
	brv =
		httpc_send_request (&httpc_ctx, "GET /index.html", buffer, BUFFERSIZE);
	rtems_test_assert (brv);
...
static void test_mg_callback (void)
{
	...
	rtems_test_assert (brv);
	brv =
		httpc_send_request (&httpc_ctx, "GET " CBACKTEST_URI, buffer,
							BUFFERSIZE);
	rtems_test_assert (brv);
	brv = httpc_close_connection (&httpc_ctx);
	rtems_test_assert (brv);
