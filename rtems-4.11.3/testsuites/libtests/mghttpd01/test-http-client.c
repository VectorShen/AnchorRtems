...
static ssize_t httpc_read_full (const httpc_context * ctx,
								void *response, size_t responsesize);

void httpc_init_context (httpc_context * ctx)
{
	ctx->socket = -1;
	ctx->fd = NULL;
}

bool httpc_open_connection (httpc_context * ctx,
...
bool httpc_close_connection (httpc_context * ctx)
{
...
bool httpc_send_request (const httpc_context * ctx,
						 const char *request, char *response, int responsesize)
{
...
bool httpc_ws_open_connection (const httpc_context * ctx)
{
	rtems_test_assert (ctx != NULL);
	rtems_test_assert (ctx->socket >= 0);
...
bool httpc_ws_send_request (const httpc_context * ctx,
							const char *request,
							char *response, int responsesize)
{
	 ...
	 * chunks
	 */
	if (strlen (request) > ws_header_size)
	  {
		  return false;
	  }
	...
	while (pos < (unsigned char *)response + responsesize)
	  {
		  ssize_t size =
			  read (ctx->socket, pos,
					(unsigned char *)response + responsesize - pos);
		  if (size == -1)
			{
				return -1;
			}
