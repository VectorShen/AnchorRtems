...
static void test_heap_init_with_page_size (uintptr_t page_size)
{
	...
	_Heap_Initialize (&TestHeap, TestHeapMemory, sizeof (TestHeapMemory),
					  page_size);
}
...
static void test_check_control (void)
{
	...
	TestHeap.first_block =
		(Heap_Block *) ((char *)TestHeap.first_block + CPU_ALIGNMENT);
	test_call_heap_walk (false);
...
static void test_check_free_list (void)
{
	...
	first_free_block->next =
		(Heap_Block *) ((uintptr_t) first_free_block->next + CPU_ALIGNMENT);
...
static void test_main_loop (void)
{
	...
	block->size_and_flag =
		(2 * _Heap_Block_size (block)) | HEAP_PREV_BLOCK_USED;
	test_call_heap_walk (false);
	...
	block = _Heap_Block_of_alloc_area ((uintptr_t) p1, TestHeap.page_size);
	block->size_and_flag =
		(3 * _Heap_Block_size (block) / 2) | HEAP_PREV_BLOCK_USED;
	test_call_heap_walk (false);
	...
	block->size_and_flag =
		(_Heap_Block_size (block) +
		 _Heap_Block_size (next_block)) | HEAP_PREV_BLOCK_USED;
	test_call_heap_walk (true);
}
