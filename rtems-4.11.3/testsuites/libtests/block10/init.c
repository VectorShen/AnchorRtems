...
static rtems_status_code disk_register (uint32_t block_size,
...
static rtems_task Init (rtems_task_argument argument)
{
	...
	sc = rtems_task_create (rtems_build_name ('P', 'U', 'R', 'G'),
							PRIORITY_HIGH,
							0,
							RTEMS_DEFAULT_MODES,
							RTEMS_DEFAULT_ATTRIBUTES, &task_id_purger);
	ASSERT_SC (sc);
	...
	sc = rtems_task_create (rtems_build_name ('W', 'A', 'I', 'T'),
							PRIORITY_HIGH,
							0,
							RTEMS_DEFAULT_MODES,
							RTEMS_DEFAULT_ATTRIBUTES, &task_id_waiter);
	ASSERT_SC (sc);
				...
				for (i_w = 0; i_w < WAITER_COUNT; ++i_w)
				  {
					  ...
					  check_access (access_table[i_ac], release_table[i_rel],
									waiter_table[i_w]);
				  }
			}
	  }
