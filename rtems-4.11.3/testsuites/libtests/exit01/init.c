...
static void fatal_extension (rtems_fatal_source source,
							 bool is_internal, rtems_fatal_code error)
{
	if (source == RTEMS_FATAL_SOURCE_EXIT
		&& !is_internal && error == EXIT_STATUS && counter == 3)
	  {
		  rtems_test_endk ();
	  }
}
...
static void Init (rtems_task_argument arg)
{
	...
	sc = rtems_task_create (rtems_build_name ('E', 'X', 'I', 'T'),
