...
static void test_termios_baud2index (void)
{
	...
	for (i = 0; baud_table[i].constant != INVALID_CONSTANT; i++)
	  {
		  printf ("termios_baud_to_index(B%" PRIdrtems_termios_baud_t
				  ") - OK\n", baud_table[i].baud);
...
static void test_termios_baud2number (void)
{
	...
	for (i = 0; baud_table[i].constant != INVALID_CONSTANT; i++)
	  {
		  printf ("termios_baud_to_number(B%" PRIdrtems_termios_baud_t
				  ") - OK\n", baud_table[i].baud);
		  number = rtems_termios_baud_to_number (baud_table[i].constant);
		  if (number != baud_table[i].baud)
			{
				printf ("ERROR - returned %" PRIdrtems_termios_baud_t
						" should be %" PRIdrtems_termios_baud_t "\n",
						number, baud_table[i].baud);
				rtems_test_exit (0);
			}
	  }
}
...
static void test_termios_cfoutspeed (void)
{
	...
	for (i = 0; baud_table[i].constant != INVALID_CONSTANT; i++)
	  {
		  memset (&term, '\0', sizeof (term));
		  printf ("cfsetospeed(B%" PRIdrtems_termios_baud_t ") - OK\n",
				  baud_table[i].baud);
		  sc = cfsetospeed (&term, baud_table[i].constant);
		  rtems_test_assert (!sc);
		  printf ("cfgetospeed(B%" PRIdrtems_termios_baud_t ") - OK\n",
				  baud_table[i].baud);
...
static void test_termios_cfinspeed (void)
{
	...
	for (i = 0; baud_table[i].constant != INVALID_CONSTANT; i++)
	  {
		  memset (&term, '\0', sizeof (term));
		  printf ("cfsetispeed(B%" PRIdrtems_termios_baud_t ") - OK\n",
				  baud_table[i].baud);
		  sc = cfsetispeed (&term, baud_table[i].constant);
		  rtems_test_assert (!sc);

		  printf ("cfgetispeed(B%" PRIdrtems_termios_baud_t ") - OK\n",
				  baud_table[i].baud);
...
static void test_termios_cfsetspeed (void)
{
	...
	for (i = 0; baud_table[i].constant != INVALID_CONSTANT; i++)
	  {
		  memset (&term, '\0', sizeof (term));
		  printf ("cfsetspeed(B%" PRIdrtems_termios_baud_t ") - OK\n",
				  baud_table[i].baud);
		  status = cfsetspeed (&term, baud_table[i].constant);
		  rtems_test_assert (!status);

		  printf ("cfgetspeed(B%" PRIdrtems_termios_baud_t
...
static void test_termios_cfmakeraw (void)
{
	...
	rtems_test_assert (~
					   (term.
						c_iflag & (IMAXBEL | IGNBRK | BRKINT | PARMRK | ISTRIP |
...
test_early_device_install_remove (rtems_device_major_number major,
...
static void test_device_install_remove (void)
{
	...
	sc = rtems_termios_device_install (NULL,
									   major, minor, &handler, NULL, NULL);
	...
	sc = rtems_termios_device_install (&dev[0],
									   major, minor, &handler, NULL, NULL);
	rtems_test_assert (sc == RTEMS_SUCCESSFUL);
...
static bool first_open_error (rtems_termios_tty * tty,
							  rtems_termios_device_context * base,
							  struct termios *term,
							  rtems_libio_open_close_args_t * args)
{
...
static void test_first_open_error (void)
{
	...
	sc = rtems_termios_device_install (&dev[0],
									   major, minor, &handler, NULL, &ctx.base);
	rtems_test_assert (sc == RTEMS_SUCCESSFUL);
...
static bool set_attributes_error (rtems_termios_device_context * base,
	...
	for (i = 0; i < n; ++i)
	  {
		  ...
		  rtems_test_assert ((term.c_cflag & cbaud_mask) ==
							 baud_to_cflag_table[i].cflag);
	  }
}
