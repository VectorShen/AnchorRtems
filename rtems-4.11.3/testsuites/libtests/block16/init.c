...
static void test (void)
{
	...
	sc = rtems_blkdev_create (device,
							  media_block_size,
							  media_block_count, ramdisk_ioctl, rd);
	ASSERT_SC (sc);
	...
	for (i = 0; i < media_block_count; i += 2)
	  {
		  ...
		  rtems_test_assert (media_bd[i]->buffer[0] ==
							 buf[i] + media_block_count);
	  }
	...
	for (i = 0; i < media_block_count; i += 2)
	  {
		  rtems_test_assert (media_bd[i]->state ==
