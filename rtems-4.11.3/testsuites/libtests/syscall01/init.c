...
static void Init (rtems_task_argument arg)
{
	...
	sc = rtems_task_create (rtems_build_name ('C', 'L', 'O', 'S'),
							2,
							RTEMS_MINIMUM_STACK_SIZE,
							RTEMS_DEFAULT_MODES,
							RTEMS_DEFAULT_ATTRIBUTES, &ctx->close_task);
	rtems_test_assert (sc == RTEMS_SUCCESSFUL);

	sc = rtems_task_start (ctx->close_task,
						   close_task, (rtems_task_argument) ctx);
	rtems_test_assert (sc == RTEMS_SUCCESSFUL);
...
static rtems_device_driver open_driver_initialize (rtems_device_major_number
												   major,
...
static rtems_device_driver open_driver_open (rtems_device_major_number major,
