...
static void test_utf8proc_iterate (void)
{
	...
	for (index = 0; index < length_simple_string; ++index)
	  {
		  bytes_read = utf8proc_iterate (&utf8_str_simple_ptr[index],
										 length_simple_string - index,
										 &unicode_char);
		  rtems_test_assert (bytes_read == 1);
		  rtems_test_assert ((uint8_t) unicode_char ==
...
static void test_utf8proc_decompose_char (void)
{
	...
	for (unicode_char = 0x0000; unicode_char <= 0x10FFFF; ++unicode_char)
	  {
		  chars_written = utf8proc_decompose_char (unicode_char,
												   unicode_char_decomposed,
												   sizeof
												   (unicode_char_decomposed) /
												   sizeof
												   (unicode_char_decomposed[0]),
												   UTF8PROC_STABLE |
												   UTF8PROC_DECOMPOSE, 0);
...
static void test_utf8proc_decompose (void)
{
	...
	chars_written = utf8proc_decompose (string_simple_utf8,
	...
	for (index = 0; index < sizeof (string_simple); ++index)
	  {
		  rtems_test_assert (string_simple_utf8[index] ==
							 (uint8_t) string_decomposed[index]);
	  }
}
...
static void test_utf8proc_reencode (void)
{
	...
	chars_written = utf8proc_reencode (&string_decomposed[0],
									   chars_written,
									   UTF8PROC_NULLTERM | UTF8PROC_STABLE |
									   UTF8PROC_DECOMPOSE);
	rtems_test_assert (chars_written == strlen (string_simple));
...
static void test_utf8proc_map (void)
{
	...
	chars_written = utf8proc_map (string_simple_utf8,
								  sizeof (string_simple),
								  &dest,
								  UTF8PROC_NULLTERM | UTF8PROC_STABLE |
								  UTF8PROC_DECOMPOSE);
