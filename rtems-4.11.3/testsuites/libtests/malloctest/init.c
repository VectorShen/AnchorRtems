...
static void test_realloc (void)
{
	...
	p3 = realloc (p1, 64);
	if (p3 == p1 || p3 == NULL)
		printf ("realloc - failed non-in place: realloc(%p,%d) = %p\n", p1, 64,
				p3);
	free (p3);
	free (p2);
...
static void test_heap_init (uintptr_t page_size)
{
	...
	rv = _Heap_Initialize (&TestHeap, TestHeapMemory, TEST_HEAP_SIZE,
						   page_size);
	rtems_test_assert (rv > 0);
}

static void test_check_alloc (void *alloc_begin_ptr,
							  void *expected_alloc_begin_ptr,
							  uintptr_t alloc_size,
							  uintptr_t alignment, uintptr_t boundary)
{
	...
	if (expected_alloc_begin_ptr != NULL)
	  {
		  ...
		  uintptr_t const alloc_area_begin =
			  _Heap_Align_down (alloc_begin, page_size);
		  uintptr_t const alloc_area_offset = alloc_begin - alloc_area_begin;
#if UNUSED
		  uintptr_t const alloc_area_size = alloc_area_offset + alloc_size;
#endif
		  Heap_Block *block =
			  _Heap_Block_of_alloc_area (alloc_area_begin, page_size);
		  ...
		  rtems_test_assert (_Heap_Is_aligned
							 (block_begin + HEAP_BLOCK_HEADER_SIZE, page_size));
		  ...
		  if (boundary != 0)
			{
				uintptr_t boundary_line =
					_Heap_Align_down (alloc_end, boundary);

				rtems_test_assert (alloc_size <= boundary);
				rtems_test_assert (boundary_line <= alloc_begin
								   || alloc_end <= boundary_line);
			}
	  }

	rtems_test_assert (page_size < CPU_ALIGNMENT
					   || _Heap_Walk (&TestHeap, 0, false));
}

static void test_check_alloc_simple (void *alloc_begin_ptr,
									 uintptr_t alloc_size,
									 uintptr_t alignment, uintptr_t boundary)
{
	test_check_alloc (alloc_begin_ptr,
					  alloc_begin_ptr, alloc_size, alignment, boundary);
}

static void *test_alloc (uintptr_t alloc_size,
						 uintptr_t alignment,
						 uintptr_t boundary, void *expected_alloc_begin_ptr)
{
	void *alloc_begin_ptr = _Heap_Allocate_aligned_with_boundary (&TestHeap,
																  alloc_size,
																  alignment,
																  boundary);
	...
	test_check_alloc (alloc_begin_ptr,
					  ...
					  alloc_size, alignment, boundary);

	return alloc_begin_ptr;
}

static void *test_alloc_simple (uintptr_t alloc_size,
								uintptr_t alignment, uintptr_t boundary)
{
	void *alloc_begin_ptr = _Heap_Allocate_aligned_with_boundary (&TestHeap,
																  alloc_size,
...
static void *test_init_and_alloc (uintptr_t alloc_size,
								  uintptr_t alignment,
								  uintptr_t boundary,
								  void *expected_alloc_begin_ptr)
{
	...
	return test_alloc (alloc_size,
					   alignment, boundary, expected_alloc_begin_ptr);
}

static void *test_init_and_alloc_simple (uintptr_t alloc_size,
										 uintptr_t alignment,
										 uintptr_t boundary)
{
	test_heap_init (TEST_DEFAULT_PAGE_SIZE);
...
static void test_heap_do_initialize (uintptr_t area_size,
									 uintptr_t page_size,
...
static void test_heap_initialize (void)
{
	...
	test_heap_do_initialize (TEST_HEAP_SIZE,
							 (uintptr_t) (-2 * CPU_ALIGNMENT), false);
}
...
static void test_heap_allocate (void)
{
	...
	first_page_begin =
		((uintptr_t) TestHeap.first_block) + HEAP_BLOCK_HEADER_SIZE;
	alignment = first_page_begin + sizeof (uintptr_t);
	p1 = test_alloc (alloc_size, alignment, boundary, NULL);
	...
	boundary = 0;
	p1 = test_alloc (alloc_size, alignment, boundary,
...
static void test_block_alloc (int free_variant,
							  int alloc_variant,
							  uintptr_t alloc_begin, uintptr_t alloc_size)
{
	...
	pages_per_default_block =
		_Heap_Block_size (_Heap_Block_of_alloc_area
						  ((uintptr_t) p1,
	...
	_Heap_Block_allocate (&TestHeap,
						  _Heap_Block_of_alloc_area ((uintptr_t) p2,
													 test_page_size ()),
			  ...
			  switch (alloc_variant)
				{
					...
					case 2:
						...
						exp_free_pages =
							exp_free_pages + pages_per_default_block - 1;
						exp_free_blocks = 3;
						break;
					case 3:
						/* allocate block at the end */
						exp_free_pages =
							exp_free_pages + pages_per_default_block - 2;
						exp_free_blocks = 2;
						break;
					default:
			  ...
			  switch (alloc_variant)
				{
					case 1:
						/* allocate block full space */
						exp_free_pages =
							exp_free_pages - pages_per_default_block;
						exp_free_blocks = 1;
						break;
					case 2:
						/* allocate block in the middle */
	...
	rtems_test_assert (TestHeap.stats.free_size ==
					   exp_free_pages * TestHeap.page_size);
...
static void test_simple_resize_block (void *alloc_pointer,
									  ...
									  Heap_Resize_status expected_status)
{
	...
	Heap_Resize_status status = _Heap_Resize_block (&TestHeap,
													alloc_pointer,
													new_alloc_size,
													&old_size,
													&new_size);
...
rtems_task Init (rtems_task_argument argument)
{
	...
	if (p1)
	  {
		  printf
			  ("ERROR on attempt to calloc SIZE_MAX block expected failure.");
		  free (p1);
	  }
	...
	status = rtems_task_create (Task_name[2],
								1,
								TASK_STACK_SIZE,
								RTEMS_DEFAULT_MODES,
								RTEMS_DEFAULT_ATTRIBUTES | RTEMS_FLOATING_POINT,
	...
	status = rtems_task_create (Task_name[3],
								1,
								TASK_STACK_SIZE,
								RTEMS_DEFAULT_MODES,
								RTEMS_DEFAULT_ATTRIBUTES | RTEMS_FLOATING_POINT,
	...
	status = rtems_task_create (Task_name[4],
								1,
								TASK_STACK_SIZE,
								RTEMS_DEFAULT_MODES,
								RTEMS_DEFAULT_ATTRIBUTES | RTEMS_FLOATING_POINT,
	...
	status = rtems_task_create (Task_name[5],
								1,
								TASK_STACK_SIZE,
								RTEMS_DEFAULT_MODES,
								RTEMS_DEFAULT_ATTRIBUTES | RTEMS_FLOATING_POINT,
