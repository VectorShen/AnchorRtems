...
void Task1::body (rtems_task_argument argument)
{
	...
	EndTask end_task ("ENDT", (rtems_task_priority) 1,
					  RTEMS_MINIMUM_STACK_SIZE * 6);
	end_task.start (0);
...
void Task1::screen2 (void)
{
	...
	printf ("%s - connected object wake after 0 secs - ",
			task_1.name_string ());
	task_1.wake_after (0);
	printf ("%s\n", task_1.last_status_string ());

	printf ("%s - connected object wake after 500 msecs - ",
			task_1.name_string ());
	task_1.wake_after (500000);
	printf ("%s\n", task_1.last_status_string ());

	printf ("%s - connected object wake after 5 secs - ",
			task_1.name_string ());
	...
	printf ("%s - connected object set priority to 512 - ",
			task_1.name_string ());
	task_1.set_priority (512);
	printf ("%s\n", task_1.last_status_string ());

	printf ("%s - connected object set priority to 25 - ",
			task_1.name_string ());
	task_1.set_priority (25);
	printf ("%s\n", task_1.last_status_string ());

	printf ("%s - connected object set priority to original - ",
			task_1.name_string ());
	...
	printf ("%s - self object set priority to original - ",
			task_2.name_string ());
	...
	printf ("%s - connected object get with bad notepad number - ",
			task_1.name_string ());
	task_1.get_note (100, current_note);
	printf ("%s, note is %" PRIi32 "\n", task_1.last_status_string (),
			current_note);

	printf ("%s - connected object set note to 0xDEADBEEF - ",
			task_1.name_string ());
	...
	printf ("%s - connected object set note to original value - ",
			task_1.name_string ());
	...
	printf ("%s - self object get with bad notepad number - ",
			task_2.name_string ());
	task_2.get_note (100, current_note);
	printf ("%s, note is %" PRIi32 "\n", task_2.last_status_string (),
			current_note);

	printf ("%s - self object set note to 0xDEADBEEF - ",
			task_2.name_string ());
	...
	printf ("%s - self object set note to original value - ",
			task_2.name_string ());
...
void Task1::screen3 (void)
{
	...
	printf ("%s,\n\t mode is 0x%08" PRIXrtems_mode ", ",
			task_mode.last_status_string (), current_mode);
	...
	printf ("%s,\n\t mode is 0x%08" PRIXrtems_mode ", ",
			task_mode.last_status_string (), mode);
	print_mode (mode, RTEMS_PREEMPT_MASK);
	printf ("\n");

	printf ("%s - set preemption state to RTEMS_PREEMPT - ",
			self.name_string ());
	...
	printf ("%s - set preemption state to RTEMS_NO_PREEMPT - ",
			self.name_string ());
	...
	printf ("%s - set timeslice state to RTEMS_TIMESLICE - ",
			self.name_string ());
	...
	printf ("%s - set timeslice state to RTEMS_NO_TIMESLICE - ",
			self.name_string ());
	...
	printf ("%s - set interrupt level to original level - ",
			self.name_string ());
...
void Task1::screen4 (void)
{
	...
	printf ("%s - send event signal 0 using the task object reference - ",
			name_string ());
...
void Task1::screen6 (void)
{
	...
	printf ("%s - construct message queue 1 with no memory error - ",
			name_string ());
	...
	if (size == (strlen (u2) + 5))
	  {
		  if ((strncmp (in, task_3_1.name_string (), 4) == 0) &&
			...
			{
				printf
					("%s - message u2 received incorrectly, message='%s', size=%zu\n",
					 name_string (), in, size);
			}
	  }
	else
	...
	if (size == (strlen (u1) + 5))
	  {
		  if ((strncmp (in, task_3_2.name_string (), 4) == 0) &&
			...
			{
				printf
					("%s - message u1 received incorrectly, message='%s', size=%zu\n",
					 name_string (), in, size);
			}
	  }
	else
	...
	printf ("%s - receive message b1 on mq_2 from %s...\n",
			name_string (), task_3_1.name_string ());
	fflush (stdout);
	mq_2.receive (in, size, 5000000);
	printf ("%s - %s\n", name_string (), mq_2.last_status_string ());
	...
	if (size == (strlen (b1) + 5))
	  {
		  if ((strncmp (in, task_3_2.name_string (), 4) == 0) &&
			...
			{
				printf
					("%s - message b1 received incorrectly, message='%s', size=%zu\n",
					 name_string (), in, size);
			}
	  }
	else
...
void Task1::print_mode (rtems_mode mode, rtems_mode mask)
{
	...
	if (mask & RTEMS_ASR_MASK)
		printf ("RTEMS_%sASR ", task_mode.asr_set (mode) ? "" : "NO_");
...
EndTask::EndTask (const char *name,
				  ...
				  const uint32_t stack_size):rtemsTask (name, initial_priority,
														stack_size,
														RTEMS_NO_PREEMPT)
{
}
