	...
	if (req == RTEMS_BLKIO_REQUEST)
	  {
		  ...
		  rtems_test_assert (breq->bufnum ==
							 expected_request_bufnums[request_index]);
		  ++request_index;
		  ...
		  for (i = 0; i < breq->bufnum; ++i)
			{
				...
				for (j = 0; j < BLOCK_SIZE / MEDIA_BLOCK_SIZE; ++j)
				  {
					  rtems_test_assert (expected_write_blocks[write_index] ==
										 sg->block + j);
					  ++write_index;
				  }
			}
...
static void test (void)
{
	...
	sc = rtems_disk_create_phys (dev,
								 MEDIA_BLOCK_SIZE,
								 BLOCK_COUNT, test_disk_ioctl, NULL, NULL);
	rtems_test_assert (sc == RTEMS_SUCCESSFUL);
