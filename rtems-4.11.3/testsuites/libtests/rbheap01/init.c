...
static void extend_descriptors (rtems_rbheap_control * control)
{
	...
	rtems_rbheap_set_extend_descriptors (control,
										 rtems_rbheap_extend_descriptors_never);

	rtems_chain_initialize (chain, chunks, PAGE_COUNT, sizeof (chunks[0]));
}
...
static bool chunk_visitor (const RBTree_Node * node,
						   RBTree_Direction dir, void *visitor_arg)
{
...
static void test_init_begin_greater_than_end (void)
{
	...
	sc = rtems_rbheap_initialize (&control,
								  (void *)PAGE_SIZE,
								  (uintptr_t) - PAGE_SIZE,
								  PAGE_SIZE, extend_descriptors, NULL);
...
static void test_init_empty_descriptors (void)
{
	...
	sc = rtems_rbheap_initialize (&control,
								  ...
								  PAGE_SIZE,
								  PAGE_SIZE,
								  rtems_rbheap_extend_descriptors_never, NULL);
	rtems_test_assert (sc == RTEMS_NO_MEMORY);
}

static void test_chunk_tree (const rtems_rbheap_control * control,
							 ...
							 size_t chunk_count)
{
	...
	rtems_test_assert (rtems_chain_node_count_unprotected
					   (&control->spare_descriptor_chain) ==
					   PAGE_COUNT - chunk_count);

	_RBTree_Iterate (&control->chunk_tree, RBT_RIGHT, chunk_visitor, &context);
}
