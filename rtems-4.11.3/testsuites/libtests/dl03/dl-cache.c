...
static void dl_cache_create_file (void)
{
	...
	rtems_test_assert ((fd = open (filename,
								   ...
								   S_IRUSR | S_IWUSR)) >= 0);
	rtems_test_assert (write (fd, contents, CACHE_BUFFER_SIZE) ==
					   CACHE_BUFFER_SIZE);
...
static off_t dl_cache_buffer_offset (rtems_rtl_obj_cache_t * cache,
									 void *buffer)
{
...
int dl_cache_test (void)
{
	...
	printf ("cache read:  in: offset=%d length=%d\n", (int)offset_in,
			(int)length);
	rtems_test_assert (rtems_rtl_obj_cache_read
					   (&cache, fd, offset_in, &buffer, &length) == true);
