...
struct test_device
{
	int (*transfer) (i2c_bus * bus,
					 i2c_msg * msgs, uint32_t msg_count, test_device * dev);
};
...
static int test_simple_read_write_transfer (i2c_bus * bus,
											i2c_msg * msgs,
											uint32_t msg_count,
											test_device * base)
{
...
static int test_gpio_nxp_pca9535_transfer (i2c_bus * bus,
										   i2c_msg * msgs,
										   uint32_t msg_count,
										   test_device * base)
{
...
static int test_eeprom_transfer (i2c_bus * bus,
								 i2c_msg * msgs,
								 uint32_t msg_count, test_device * base)
{
...
static int test_switch_nxp_pca9548a_transfer (i2c_bus * bus,
											  i2c_msg * msgs,
											  uint32_t msg_count,
											  test_device * base)
{
	test_device_switch_nxp_pca9548a *dev =
		(test_device_switch_nxp_pca9548a *) base;
	uint32_t i;
...
static void test_simple_read_write (test_bus * bus, int fd)
{
	...
	rtems_test_assert (memcmp
					   (&bus->simple_read_write.buf[0], &zero[0],
						sizeof (buf)) == 0);
	...
	rtems_test_assert (memcmp
					   (&bus->simple_read_write.buf[0], &abc[0],
						sizeof (buf)) == 0);
...
static void test_gpio_nxp_pca9535 (void)
{
	...
	rv = i2c_dev_register_gpio_nxp_pca9535 (&bus_path[0],
...
static void test_eeprom (test_bus * bus)
{
	...
	rv = i2c_dev_register_eeprom (&bus_path[0],
								  &eeprom_path[0],
								  DEVICE_EEPROM, 1, 8, sizeof (out), 0);
	rtems_test_assert (rv == 0);
...
static void test_switch_nxp_pca9548a (void)
{
	...
	rv = i2c_dev_register_switch_nxp_pca9548a (&bus_path[0],
