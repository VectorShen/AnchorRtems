 ...
 */
static int bdbuf_test_printf (const char *format, ...)
{
	int ret = 0;
	va_list args;
	...
	switch (bdd->driver_action)
	  {
		  ...
		  case BDBUF_DISK_BLOCKS_INORDER:
			  bdbuf_test_printf
				  ("disk ioctl: multi-block order check: count = %d\n",
				   req->bufnum);
			  for (b = 0; b < (req->bufnum - 1); b++)
				  if (req->bufs[b].block >= req->bufs[b + 1].block)
					  bdbuf_test_printf
						  ("disk ioctl: out of order: index:%d (%d >= %d\n", b,
						   req->bufs[b].block, req->bufs[b + 1].block);
			  break;
	...
	for (minor = 0; minor < BDBUF_DISKS; minor++)
	  {
		  ...
		  sc = rtems_disk_create_phys (rtems_filesystem_make_dev_t
									   (major, minor), bdd->block_size,
									   bdd->block_count, bdbuf_disk_ioctl, bdd,
									   name);
		  if (sc != RTEMS_SUCCESSFUL)
			{
						 ...
						 rtems_task_entry entry_point)
{
	...
	bdbuf_test_printf ("creating task: %s: priority: %d: ", tc->name, priority);
		  ...
		  for (i = 0; (i < 5) && passed; i++)
			{
				...
				if (sc != RTEMS_SUCCESSFUL)
				...
				else
				  {
					  ...
					  sc = rtems_bdbuf_release (bd);
					  bdbuf_test_printf ("%s: rtems_bdbuf_release[%d]: ",
										 tc->name, i);
					  if (!bdbuf_test_print_sc (sc, true))
						{
							passed = false;
...
{
	return rtems_bdbuf_configuration.size /
		rtems_bdbuf_configuration.buffer_min;
}
		  ...
		  if (passed)
			{
				for (i = 1; (i < (num / 2)) && passed; i++)
				  {
					  bdbuf_test_printf
						  ("%s: rtems_bdbuf_release_modified[%d]: "
						   "unblocks task 1\n", tc->name, i);
					  bd = (rtems_bdbuf_buffer *) rtems_chain_get (&buffers);
					  sc = rtems_bdbuf_release_modified (bd);
					  bdbuf_test_printf
						  ("%s: rtems_bdbuf_release_modified[%d]: ", tc->name,
						   i);
					  passed = bdbuf_test_print_sc (sc, true);
					  if (!passed)
						  break;
				...
				if (passed)
				  {
					  passed =
						  bdbuf_disk_driver_watch_wait (tc, BDBUF_SECONDS (5));
							...
							for (i = 0; (i < (num / 2)) && passed; i++)
							  {
								  bdbuf_test_printf
									  ("%s: rtems_bdbuf_release_modified[%d]: ",
									   tc->name, i + (num / 2));
								  bd = (rtems_bdbuf_buffer *)
									  rtems_chain_get (&buffers);
								  passed =
									  bdbuf_test_print_sc
									  (rtems_bdbuf_release_modified (bd), true);
								  if (!passed)
									  break;
							  }

							passed =
								bdbuf_disk_driver_watch_wait (tc,
															  BDBUF_SECONDS
															  (5));
...
bdbuf_tests_ranged_get_release (bdbuf_task_control * tc,
								bool wake_master, int lower, int upper)
{
	rtems_status_code sc;
	bool passed;
	int i;
...
static void bdbuf_tester (void)
{
	...
	for (t = 0; t < BDBUF_TEST_TASKS; t++)
	  {
		  bdbuf_task_control_init (t, &tasks[t], rtems_task_self (), major, dd);
