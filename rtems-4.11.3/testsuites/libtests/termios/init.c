...
void print_c_iflag (struct termios *tp)
{
	...
	print_32bits (tp->c_iflag, sizeof (c_iflag_bits) / sizeof (char *),
				  c_iflag_bits);
	printf ("\n");
}
...
void print_c_lflag (struct termios *tp)
{
	...
	print_32bits (tp->c_lflag, sizeof (c_lflag_bits) / sizeof (char *),
				  c_lflag_bits);
	printf ("\n");
}
...
unsigned long get_stop_bits (void)
{
	...
	while (1)
	  {
		  printf
			  ("Enter the numerical value for the new number of stop bits\n");
...
unsigned long get_data_bits (void)
{
	...
	while (1)
	  {
		  printf
			  ("Enter the numerical value for the new number of data bits\n");
				...
				switch (c = getchar ())
				  {
					  case '1':
						  ...
						  test_termios = orig_termios;
						  if (tcsetattr
							  (fileno (stdin), TCSADRAIN, &test_termios) < 0)
							{
