...
rtems_timer_service_routine Rx_ISR (rtems_id ignored_id, void *ignored_address);
rtems_timer_service_routine Tx_ISR (rtems_id ignored_id, void *ignored_address);
ssize_t termios_test_driver_write_helper (int port,
...
ssize_t termios_test_driver_write_helper (int port, const char *buf, size_t len)
{
	if (len > 0)
	  {
 ...
 */
rtems_device_driver termios_test_driver_initialize (rtems_device_major_number
													major,
													...
													minor, void *arg)
{
	...
	status =
		rtems_timer_create (rtems_build_name ('T', 'M', 'R', 'X'), &Rx_Timer);
	if (status)
		rtems_fatal_error_occurred (1);

	status =
		rtems_timer_create (rtems_build_name ('T', 'M', 'T', 'X'), &Tx_Timer);
	if (status)
		rtems_fatal_error_occurred (1);
...
rtems_device_driver termios_test_driver_open (rtems_device_major_number major,
...
rtems_device_driver termios_test_driver_close (rtems_device_major_number major,
											   rtems_device_minor_number minor,
											   void *arg)
{
...
rtems_device_driver termios_test_driver_read (rtems_device_major_number major,
											  rtems_device_minor_number minor,
											  void *arg)
{
...
rtems_device_driver termios_test_driver_write (rtems_device_major_number major,
											   rtems_device_minor_number minor,
											   void *arg)
{
...
rtems_device_driver termios_test_driver_control (rtems_device_major_number
												 major,
												 rtems_device_minor_number
												 minor, void *arg)
{
