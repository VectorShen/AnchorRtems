...
void test_errors (void)
{
	...
	puts ("rtems_bsp_cmdline_get_param_raw - bsp_boot_cmdline=NULL - returns NULL");
	p = rtems_bsp_cmdline_get_param_raw ("name");
	rtems_test_assert (p == NULL);

	bsp_boot_cmdline = "edit";
	puts ("rtems_bsp_cmdline_get_param - bsp_boot_cmdline = edit name = "
		  "edit -no error");
...
void test_search (bool null_expected, const char *cmdline, const char *param)
{
	...
	printf ("\n"
			...
			"  Command Line : (%s)\n",
			param, ((null_expected) ? " - Expect NULL" : ""), cmdline);
	...
	else
	  {
		  ...
		  printf
			  ("rtems_bsp_cmdline_get_param_rhs(%s) returned (%s) value=(%s)\n",
			   param, ((*p == '\0') ? "ZERO_LENGTH_STRING" : p),
