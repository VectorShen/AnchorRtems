...
static rtems_bdbuf_buffer *get (enum get_type type, enum blk_kind kind)
{
	...
	rtems_status_code (*get_bd) (rtems_disk_device *,
								 rtems_blkdev_bnum,
								 rtems_bdbuf_buffer **) = NULL;
...
static void execute_test (unsigned i)
{
	...
	print (1,
		   ...
		   blk_kind_desc[high_blk], rel_type_desc[high_rel]);
	...
	rtems_test_assert (sc == RTEMS_SUCCESSFUL && bd->dd == dd_b
					   && bd->block == 0);
...
static void disk_register (uint32_t block_size,
						   ...
						   rtems_disk_device ** dd_ptr)
{
	...
	sc = rtems_disk_create_phys (dev,
								 block_size,
								 block_count, disk_ioctl, NULL, NULL);
	ASSERT_SC (sc);
...
static rtems_task Init (rtems_task_argument argument)
{
	...
	sc = rtems_task_create (rtems_build_name (' ', 'L', 'O', 'W'),
							PRIORITY_LOW,
							0,
							RTEMS_DEFAULT_MODES,
							RTEMS_DEFAULT_ATTRIBUTES, &task_id_low);
	ASSERT_SC (sc);

	sc = rtems_task_start (task_id_low, task_low, 0);
	ASSERT_SC (sc);

	sc = rtems_task_create (rtems_build_name ('H', 'I', 'G', 'H'),
							PRIORITY_HIGH,
							0,
							RTEMS_DEFAULT_MODES,
							RTEMS_DEFAULT_ATTRIBUTES, &task_id_high);
	ASSERT_SC (sc);

	sc = rtems_task_start (task_id_high, task_high, 0);
	ASSERT_SC (sc);

	sc = rtems_task_create (rtems_build_name ('R', 'E', 'S', 'U'),
							PRIORITY_RESUME,
							0,
							RTEMS_DEFAULT_MODES,
							RTEMS_DEFAULT_ATTRIBUTES, &task_id_resume);
	ASSERT_SC (sc);
				...
				for (low_get = GET; low_get <= READ; ++low_get)
				  {
					  for (high_get = GET; high_get <= READ; ++high_get)
						{
							for (trig_blk = BLK_A0; trig_blk <= BLK_B0;
								 ++trig_blk)
							  {
								  for (low_blk = BLK_A0; low_blk <= BLK_B0;
									   ++low_blk)
									{
										for (high_blk = BLK_A0;
											 high_blk <= BLK_B0; ++high_blk)
										  {
											  for (trig_rel = REL;
												   trig_rel <= SYNC; ++trig_rel)
												{
													for (low_rel = REL;
														 low_rel <= SYNC;
														 ++low_rel)
