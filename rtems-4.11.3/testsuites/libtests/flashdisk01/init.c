...
static int test_rfs_mount_handler (const char *disk_path,
								   const char *mount_path, void *arg)
{
	return mount_and_make_target_path (disk_path,
									   mount_path,
...
static void test (void)
{
	...
	rv = mount (device,
				mnt,
				RTEMS_FILESYSTEM_TYPE_RFS, RTEMS_FILESYSTEM_READ_WRITE, data);
	rtems_test_assert (rv == 0);
...
static rtems_device_driver flashdisk_initialize (rtems_device_major_number
												 major,
...
static uint8_t *get_data_pointer (const rtems_fdisk_segment_desc * sd,
...
static int flashdisk_read (const rtems_fdisk_segment_desc * sd,
						   uint32_t device,
						   uint32_t segment,
...
static int flashdisk_write (const rtems_fdisk_segment_desc * sd,
							uint32_t device,
							uint32_t segment,
...
static int flashdisk_blank (const rtems_fdisk_segment_desc * sd,
							uint32_t device,
...
static int flashdisk_verify (const rtems_fdisk_segment_desc * sd,
							 uint32_t device,
							 uint32_t segment,
							 uint32_t offset, const void *buffer, uint32_t size)
{
...
static int flashdisk_erase (const rtems_fdisk_segment_desc * sd,
							uint32_t device, uint32_t segment)
{
	int eno = 0;
...
static int flashdisk_erase_device (const rtems_fdisk_device_desc * sd,
								   uint32_t device)
{
	int eno = 0;
