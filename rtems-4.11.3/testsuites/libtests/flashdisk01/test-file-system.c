...
static test_state do_format (unsigned index,
							 fs_state * fs,
...
static test_state do_init_root (unsigned index, fs_state * fs,
...
static test_state remove_node (unsigned index, fs_state * fs, const char *path,
							   bool is_dir)
{
	test_state state = ERROR;
	int rv = 0;
...
static test_state do_sleep (unsigned index, test_state state)
{
	...
	rtems_interval interval =
		ms / rtems_configuration_get_milliseconds_per_tick ();
...
static test_state do_mount (unsigned index,
							fs_state * fs,
							const char *disk_path,
							const char *mount_path,
							const test_file_system_handler * handler,
...
void test_file_system_with_handler (unsigned index,
									const char *disk_path,
									const char *mount_path,
									const test_file_system_handler * handler,
									void *handler_arg)
		  ...
		  switch (state)
			{
				...
				case FORMAT:
					state =
						do_format (index, fs, disk_path, handler, handler_arg);
					break;
				case INIT:
					state = MOUNT;
					break;
				case INIT_ROOT:
					...
					break;
				case MOUNT:
					state = do_mount (index,
									  fs,
									  disk_path,
									  mount_path, handler, handler_arg);
					break;
				default:
					assert (false);
					break;
