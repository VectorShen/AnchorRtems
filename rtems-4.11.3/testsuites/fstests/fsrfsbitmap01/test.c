...
rtems_rfs_bitmap_ut_test_range (rtems_rfs_bitmap_control * control,
								int test,
								bool set, rtems_rfs_bitmap_bit bit, size_t size)
{
	unsigned int count;
	bool result;
	for (count = 0; count < size; count++)
	  {
		  ...
		  if (rc > 0)
			{
				printf (" %2d. Test bit %" PRId32 " in range (%" PRId32
						",%ld] is %s: ", test, bit + count, bit, bit + size - 1,
						!set ? "set" : "clear");
				printf ("FAIL (%s)\n", strerror (rc));
				return false;
			}
...
rtems_rfs_bitmap_ut_alloc_seq_test (rtems_rfs_bitmap_control * control,
									int test,
									rtems_rfs_bitmap_bit bit, size_t size)
{
	bool state;
	int i;
	int rc;
	...
	for (i = 0, bit = 0; i < size; i++)
	  {
		  ...
		  if (!result)
			{
				printf (" %2d. Find bit with seed = %" PRId32 ": %s: bit = %"
...
static void rtems_rfs_bitmap_ut_test_bitmap (size_t size)
{
	...
	printf
		("  4. Find bit (size - 1) with seed = (size - 1) (%zd): %s (%s): bit = %"
	...
	rc = rtems_rfs_bitmap_map_alloc (&control, size / 2, &result, &bit);
	result = result && (bit == 0);
	printf (" 17. Find bit with seed = (size / 2) (%zd): %s (%s): bit = %"
			PRId32 "\n", size / 2, result ? "pass" : "FAIL", strerror (rc),
			bit);
	rtems_rfs_exit_on_error (rc, !result, &control, buffer.buffer);
	...
	rc = rtems_rfs_bitmap_map_alloc (&control, size / 2, &result, &bit);
	result = result && (bit == (size - 1));
	printf (" 19. Find bit with seed = (size / 2) (%zd): %s (%s): bit = %"
			PRId32 "\n", size / 2, result ? "pass" : "FAIL", strerror (rc),
			bit);
	rtems_rfs_exit_on_error (rc, !result, &control, buffer.buffer);
	...
	result = result && (bit == 0);
	printf (" 21. Find bit with seed = ((size / 2) - 1) (%zd): %s (%s): bit = %"
			PRId32 "\n", (size / 2) - 1, result ? "pass" : "FAIL",
			strerror (rc), bit);
	rtems_rfs_exit_on_error (rc, !result, &control, buffer.buffer);
	...
	rc = rtems_rfs_bitmap_map_alloc (&control, (size / 2) - 1, &result, &bit);
	result = result && (bit == (size - 1));
	printf (" 23. Find bit with seed = ((size / 2) - 1) (%zd): %s (%s): bit = %"
			PRId32 "\n", (size / 2) - 1, result ? "pass" : "FAIL",
			strerror (rc), bit);
	rtems_rfs_exit_on_error (rc, !result, &control, buffer.buffer);
