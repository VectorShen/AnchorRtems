...
static int handler_open (rtems_libio_t * iop,
...
static ssize_t handler_write (rtems_libio_t * iop,
...
static int handler_ioctl (rtems_libio_t * iop, uint32_t request, void *buffer)
{
	test_state *state = IMFS_generic_get_context_by_iop (iop);
...
static int handler_fstat (const rtems_filesystem_location_info_t * loc,
						  struct stat *buf)
{
...
static ssize_t handler_readv (rtems_libio_t * iop,
...
static ssize_t handler_writev (rtems_libio_t * iop,
							   const struct iovec *iov,
							   int iovcnt, ssize_t total)
{
	test_state *state = IMFS_generic_get_context_by_iop (iop);
...
static void test_imfs_make_generic_node (void)
{
	...
	rv = IMFS_make_generic_node (path,
...
static void test_imfs_make_generic_node_errors (void)
{
	...
	rv = IMFS_make_generic_node (path,
								 S_IFCHR | S_IRWXU | S_IRWXG | S_IRWXO,
								 &node_initialization_error_control, NULL);
	rtems_test_assert (rv == -1);
	rtems_test_assert (errno == EIO);
	rtems_test_assert (rtems_resource_snapshot_check (&before));

	errno = 0;
	rv = IMFS_make_generic_node ("/nil/nada",
								 S_IFCHR | S_IRWXU | S_IRWXG | S_IRWXO,
								 &node_control, NULL);
	rtems_test_assert (rv == -1);
