...
static void rtems_test_assert_equal_to_null_loc (const
												 ...
												 * local_loc)
{
	...
	rtems_test_assert (null_loc->location.node_access ==
					   local_loc->node_access);
	rtems_test_assert (null_loc->location.node_access_2 ==
					   local_loc->node_access_2);
...
static void test_initial_values (void)
{
	...
	rtems_test_assert (rtems_chain_previous (loc_node) ==
					   rtems_chain_head (loc_chain));
	rtems_test_assert (rtems_chain_next (loc_node) ==
					   rtems_chain_tail (loc_chain));
	...
	for (i = 0; i < n; ++i)
	  {
		  ...
		  bool ok = rtems_filesystem_check_access (cac->flags,
												   cac->object_mode,
												   cac->object_uid,
												   cac->object_gid);
...
static void test_check_access (void)
{
	...
	check_access (&check_access_euid_0_cases[0],
				  RTEMS_ARRAY_SIZE (check_access_euid_0_cases));

	uenv->euid = 3;

	check_access (&check_access_egid_0_cases[0],
	...
	check_access (&check_access_other_cases[0],
