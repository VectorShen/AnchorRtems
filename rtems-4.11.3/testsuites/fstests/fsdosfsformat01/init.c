...
static void test_disk_params (const char *dev_name,
...
static void test_create_file (const char *mount_dir,
							  uint32_t file_idx, bool expect_ok)
{
	char file_name[MAX_PATH_LENGTH + 1];
	int fd;

	snprintf (file_name,
			  MAX_PATH_LENGTH, "%s/file%" PRIu32 ".txt", mount_dir, file_idx);
	fd = open (file_name,
			   O_RDWR | O_CREAT | O_TRUNC,
			   S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
...
static void test_file_creation (const char *dev_name,
								const char *mount_dir,
	...
	for (file_idx = 0; file_idx < number_of_files; ++file_idx)
	  {
		  snprintf (file_name,
					MAX_PATH_LENGTH,
					"%s/file%" PRIu32 ".txt", mount_dir, file_idx);
		  rv = unlink (file_name);
		  rtems_test_assert (0 == rv);
	  }
...
static void test (void)
{
	...
	/* FAT12 */
	/* For 1.44 MB disks */
	sc = rtems_sparse_disk_create_and_register (dev_name,
												SECTOR_SIZE, 64, 2880, 0);
