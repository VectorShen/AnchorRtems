...
static const char
	DIRECTORY_NAMES_INVALID[NUMBER_OF_DIRECTORIES_INVALID]
	[MAX_NAME_LENGTH_INVALID] = {
...
static const char NAMES_MULTIBYTE[NUMBER_OF_NAMES_MULTIBYTE][MAX_NAME_LENGTH] = {
	...
	while ((ep = readdir (dp)) != NULL && rc == 0)
	  {
		  if (0 != strcmp (".", ep->d_name) && 0 != strcmp ("..", ep->d_name))
			{
				snprintf (p_buf, sizeof (p_buf), "%s/%s", directory_name,
						  ep->d_name);
						  ...
						  const rtems_dosfs_mount_options * mount_opts)
{
	...
	rc = mount (RAMDISK_PATH,
				MOUNT_DIR, "dosfs", RTEMS_FILESYSTEM_READ_WRITE, mount_opts);
	rtems_test_assert (rc == 0);
 ...
 */
static void test_creating_directories (const char *start_dir,
	...
	for (index = 0; index < NUMBER_OF_DIRECTORIES_INVALID; ++index)
	  {
		  snprintf (dirname,
					sizeof (dirname),
					"%s/%s", MOUNT_DIR, DIRECTORY_NAMES_INVALID[index]);
		  rc = mkdir (dirname, S_IRWXU | S_IRWXG | S_IRWXO);
 ...
 */
static void test_creating_duplicate_directories (const char *start_dir,
	...
	for (index_dir = 0; index_dir < number_of_duplicates; ++index_dir)
	  {
		  ...
		  for (index_duplicate = 0;
			...
			{
				snprintf (dirname, sizeof (dirname), "%s/%s", start_dir,
						  duplicates[index_dir].
						  name_duplicates[index_duplicate]);
				errno = 0;
				rc = mkdir (dirname, S_IRWXU | S_IRWXG | S_IRWXO);
				rtems_test_assert (rc == -1);
 ...
 */
static void test_handling_files (const char *dirname,
	...
	for (index = 0; index < number_of_files; ++index)
	  {
		  snprintf (filename,
					sizeof (filename) - 1,
					"%s/%s", dirname, file_names + index * MAX_NAME_LENGTH);
		  fd = open (filename,
					 O_RDWR | O_CREAT,
 ...
 */
static void test_finding_files (const char *dirname,
								const char *file_names,
								const unsigned int number_of_files)
{
	int rc;
	...
	while (dp != NULL)
	  {
		  rtems_test_assert (0 ==
							 strcmp (file_names + index_file * MAX_NAME_LENGTH,
									 dp->d_name));

		  snprintf (filename,
					sizeof (filename) - 1,
					"%s/%s",
					dirname, file_names + index_file * MAX_NAME_LENGTH);
	...
	for (index_file = 0; index_file < number_of_files_duplicated; ++index_file)
	  {
		  snprintf (filename,
					sizeof (filename) - 1,
					"%s/%s", dirname, files_duplicated[index_file].name);
		  fd = open (filename,
					 O_RDWR | O_CREAT,
					 S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
		  rtems_test_assert (fd >= 0);
		  ...
		  for (index_duplicate = 0;
			   index_duplicate <
			   files_duplicated[index_file].number_of_duplicates;
			   ++index_duplicate)
			{
				snprintf (filename,
						  sizeof (filename) - 1,
 ...
 */
static void test_handling_directories (const char *start_dir,
									   ...
									   const unsigned int number_of_files)
{
	...
	for (index_directory = 0;
		 index_directory < number_of_directories; ++index_directory)
	  {
		  snprintf (dirname,
					sizeof (dirname) - 1,
					"%s/%s",
 ...
 */
static void test_finding_directories (const char *start_dir,
									  const char *directory_names,
									  const unsigned int number_of_directories,
									  const char *file_names,
									  const unsigned int number_of_files)
{
	...
	while (dp != NULL)
	  {
		  rtems_test_assert (0 ==
							 strcmp (directory_names +
									 index_directory * MAX_NAME_LENGTH,
									 dp->d_name));
...
#define                   BYTES_PER_ROW     8
	...
	printf ("/*\n"
			...
			include_guard, include_guard, image_name);
		  ...
		  for (index_row = 0; index_row < ROWS_PER_BLOCK; ++index_row)
			{
				printf ("  ");
				for (index_column = 0;
					 index_column < BYTES_PER_ROW; ++index_column)
				  {
...
static void compare_image (const char *mount_dir,
						   const char *dev,
						   const rtems_dosfs_mount_options * mount_opts)
{
#define MOUNT_DIR2 "/mnt2"
	...
	rc = mount_and_make_target_path (dev,
									 MOUNT_DIR2,
									 RTEMS_FILESYSTEM_TYPE_DOSFS,
									 RTEMS_FILESYSTEM_READ_WRITE, mount_opts);
	rtems_test_assert (rc == 0);
...
static void test_compatibility (void)
{
	...
	sc = rtems_disk_create_phys (dev,
								 BLOCK_SIZE,
								 BLOCK_COUNT,
								 ramdisk_ioctl, &disk_image, diskpath);
	rtems_test_assert (sc == RTEMS_SUCCESSFUL);

	rc = mount_and_make_target_path (diskpath,
									 MOUNT_DIR,
									 RTEMS_FILESYSTEM_TYPE_DOSFS,
									 RTEMS_FILESYSTEM_READ_WRITE, &mount_opts);
	rtems_test_assert (rc == 0);
	...
	while (dp != NULL)
	  {
		  ...
		  rtems_test_assert (0
							 == memcmp (&filenames[index_file][0],
										&dp->d_name[0], dp->d_namlen));
...
static void test (void)
{
	...
	test_creating_duplicate_directories (&start_dir[0],
	...
	test_duplicated_files (MOUNT_DIR,
						   FILES_DUPLICATES, NUMBER_OF_FILES_DUPLICATED);
	...
	test_creating_directories (&start_dir[0],
							   &DIRECTORY_NAMES[0][0], NUMBER_OF_DIRECTORIES);

	test_handling_directories (&start_dir[0],
							   &DIRECTORY_NAMES[0][0],
							   NUMBER_OF_DIRECTORIES,
							   &FILE_NAMES[0][0], NUMBER_OF_FILES);
	...
	test_finding_directories (&start_dir[0],
							  &DIRECTORY_NAMES[0][0],
							  NUMBER_OF_DIRECTORIES,
							  &FILE_NAMES[0][0], NUMBER_OF_FILES);
	unmount_and_close_device ();
	...
	rc = mount (RAMDISK_PATH,
				MOUNT_DIR, "dosfs", RTEMS_FILESYSTEM_READ_WRITE, NULL);
	rtems_test_assert (rc == 0);
...
rtems_ramdisk_config rtems_ramdisk_configuration[] = {
	...
	{.block_size = BLOCK_SIZE,.block_num = BLOCK_NUM,.location =
	 &IMAGE_BIN_LE_SINGLEBYTE[0]},
	{.block_size = BLOCK_SIZE,.block_num = BLOCK_NUM,.location =
	 &IMAGE_BIN_LE_MULTIBYTE[0]}
};

size_t rtems_ramdisk_configuration_size =
RTEMS_ARRAY_SIZE (rtems_ramdisk_configuration);
